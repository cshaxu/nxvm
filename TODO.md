...
| Storage | Bounded ATA PIO and FDD boot paths through declared ROM/device owners; core-owned 8237 DMA grants one unit at a time through the frozen FDC DMA2 binding | T270--T277 must move neutral media/controller mechanisms into core while retaining VM backing/topology; full FDC state machine, generic bus wait states, broad DMA behavior, extended IDE, and error/timing compatibility remain open. |
...
- [ ] **M5 second core/composition migration (`TODO(High)`, T270--T277).**
  Move only neutral multi-device media and optional MC146818/FDC/ATA controller
  mechanisms into core. VM retains PC/AT topology/defaults/NMI glue, backing
  objects, paths, mount/persistence, firmware, boot/media and retained UX.
  A core-only mantle-shape fixture proves configuration without implementing
  mantle. Host filesystem/path policy remains outside core/platform. A trusted external
