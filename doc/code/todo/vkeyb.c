...
void vkeybSetInput(t_nubit8 byte) {
    ...
    if (vkeyb.flagcmd) {
        ...
        switch (vkeyb.inbuf) {
        case 0xed:
            vkbcSetResponse(0xfa);                  /* write led info */
            vkeyb.flagcmd = 0x01;
            break;
        case 0xee:
            vkbcSetResponse(0xee);                            /* echo */
            break;
        case 0xf0:
            vkbcSetResponse(0xfa);            /* select scan code set */
            vkeyb.flagcmd = 0x01;
            break;
        case 0xf2:
            break;           /* NOTE: read keyboard id; not supported */
        case 0xf3:
            vkbcSetResponse(0xfa);       /* set repeat rate and delay */
            vkeyb.flagcmd = 0x01;
            break;
        case 0xf4:
            vkbcSetResponse(0xfa);   /* enable keyboard, clear buffer */
            memset(vkeyb.buffer, 0x00, VKEYB_SIZE_BUFFER);
            vkeyb.flagenable = 0x01;
            break;
        case 0xf5:
            vkbcSetResponse(0xfa);                /* default, disable */
            vkeybDefault();
            vkeyb.flagenable = 0x00;
            break;
        case 0xf6:
            vkbcSetResponse(0xfa);                     /* set default */
            vkeybDefault();
            vkeyb.flagenable = 0x01;
            break;
        case 0xf7:
            break;    /* NOTE: set repeat for all keys; not supported */
        case 0xf8:
            break;        /* NOTE: set mak/brk for all; not supported */
        case 0xf9:
            break;      /* NOTE: set make for all keys; not supported */
        case 0xfa:
            break;    /* NOTE: set rep/mak/brk for all; not supported */
        case 0xfb:
            break;       /* NOTE: set repeat for a key; not supported */
        case 0xfc:
            break;      /* NOTE: set mak/brk for a key; not supported */
        case 0xfd:
            break;      /* NOTE: set mak/brk for a key; not supported */
        case 0xfe:
            break;                     /* NOTE: resend; not supported */
        case 0xff:
            vkbcSetResponse(0xfa);                           /* reset */
            vkeybReset();
            break;
        default:
            vkbcSetResponse(0xfe);                 /* invalid command */
            break;
        }
    }
}
