...
static void CaseError(const char *str) {
    utilsPrint("The NXVM CPU has encountered an internal case error: %s.\n",str);
    deviceStop();
}
static void CalcCF() {
    switch (vcpuins.type) {
...
static void GetModRegRM(t_nubitcc regbit,t_nubitcc rmbit) {
    ...
    case 3:
        switch (RM) {
        case 0:
            if (rmbit == 8) vcpuins.rrm = GetRef(vcpu.al);
            else vcpuins.rrm = GetRef(vcpu.ax);
            break;
        case 1:
            if (rmbit == 8) vcpuins.rrm = GetRef(vcpu.cl);
            else vcpuins.rrm = GetRef(vcpu.cx);
            break;
        case 2:
            if (rmbit == 8) vcpuins.rrm = GetRef(vcpu.dl);
            else vcpuins.rrm = GetRef(vcpu.dx);
            break;
        case 3:
            if (rmbit == 8) vcpuins.rrm = GetRef(vcpu.bl);
            else vcpuins.rrm = GetRef(vcpu.bx);
            break;
        case 4:
            if (rmbit == 8) vcpuins.rrm = GetRef(vcpu.ah);
            else vcpuins.rrm = GetRef(vcpu.sp);
            break;
        case 5:
            if (rmbit == 8) vcpuins.rrm = GetRef(vcpu.ch);
            else vcpuins.rrm = GetRef(vcpu.bp);
            break;
        case 6:
            if (rmbit == 8) vcpuins.rrm = GetRef(vcpu.dh);
            else vcpuins.rrm = GetRef(vcpu.si);
            break;
        case 7:
            if (rmbit == 8) vcpuins.rrm = GetRef(vcpu.bh);
            else vcpuins.rrm = GetRef(vcpu.di);
            break;
        default:
    ...
    case 4:
        switch (REG) {
        case 0:
            vcpuins.rr = GetRef(vcpu.es.selector);
            break;
        case 1:
            vcpuins.rr = GetRef(vcpu.cs.selector);
            break;
        case 2:
            vcpuins.rr = GetRef(vcpu.ss.selector);
            break;
        case 3:
            vcpuins.rr = GetRef(vcpu.ds.selector);
            break;
        default:
    ...
    case 8:
        switch (REG) {
        case 0:
            vcpuins.rr = GetRef(vcpu.al);
            break;
        case 1:
            vcpuins.rr = GetRef(vcpu.cl);
            break;
        case 2:
            vcpuins.rr = GetRef(vcpu.dl);
            break;
        case 3:
            vcpuins.rr = GetRef(vcpu.bl);
            break;
        case 4:
            vcpuins.rr = GetRef(vcpu.ah);
            break;
        case 5:
            vcpuins.rr = GetRef(vcpu.ch);
            break;
        case 6:
            vcpuins.rr = GetRef(vcpu.dh);
            break;
        case 7:
            vcpuins.rr = GetRef(vcpu.bh);
            break;
        default:
    ...
    case 16:
        switch (REG) {
        case 0:
            vcpuins.rr = GetRef(vcpu.ax);
            break;
        case 1:
            vcpuins.rr = GetRef(vcpu.cx);
            break;
        case 2:
            vcpuins.rr = GetRef(vcpu.dx);
            break;
        case 3:
            vcpuins.rr = GetRef(vcpu.bx);
            break;
        case 4:
            vcpuins.rr = GetRef(vcpu.sp);
            break;
        case 5:
            vcpuins.rr = GetRef(vcpu.bp);
            break;
        case 6:
            vcpuins.rr = GetRef(vcpu.si);
            break;
        case 7:
            vcpuins.rr = GetRef(vcpu.di);
            break;
        default:
...
static void UndefinedOpcode() {
    utilsPrint("The NXVM CPU has encountered an illegal instruction.\n");
    utilsPrint("CS:%04X IP:%04X OP:%02X %02X %02X %02X\n",
