...
t_bool ecpuapiHasDiff()
{
	...
	if (!ecpu.flagignore) {
		...
		if ((ecpu.flags & ~VCPU_FLAG_IF) != (_flags & ~VCPU_FLAG_IF)) {
...
void ecpuapiPrintRegs()
{
	...
	vapiPrint("   ");
	if(ecpu.flags & VCPU_FLAG_OF) vapiPrint("OV ");
	else                           vapiPrint("NV ");
	if(ecpu.flags & VCPU_FLAG_DF) vapiPrint("DN ");
	else                           vapiPrint("UP ");
	if(ecpu.flags & VCPU_FLAG_IF) vapiPrint("EI ");
	else                           vapiPrint("DI ");
	if(ecpu.flags & VCPU_FLAG_SF) vapiPrint("NG ");
	else                           vapiPrint("PL ");
	if(ecpu.flags & VCPU_FLAG_ZF) vapiPrint("ZR ");
	else                           vapiPrint("NZ ");
	if(ecpu.flags & VCPU_FLAG_AF) vapiPrint("AC ");
	else                           vapiPrint("NA ");
	if(ecpu.flags & VCPU_FLAG_PF) vapiPrint("PE ");
	else                           vapiPrint("PO ");
	if(ecpu.flags & VCPU_FLAG_CF) vapiPrint("CY ");
	else                           vapiPrint("NC ");
	vapiPrint("\n");
}
