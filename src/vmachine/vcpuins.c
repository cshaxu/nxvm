...
todo _e_int3()
{
	...
	if (!_GetCR0_PE) {
		...
		_bb("CR0_PE(1)");
		_comment("_e_int::Protected: not implemented");
		_chk(_SetExcept_UD(0));
		_be;
	}
	_ce;
...
void JL()
{
	vcpu.eip++;
	GetImm(8);
	JCC((void *)vcpuins.rimm, (_GetEFLAGS_SF !=
		_GetEFLAGS_OF), 8);
}
void JNL()
{
...
void JLE()
{
	vcpu.eip++;
	GetImm(8);
	JCC((void *)vcpuins.rimm, (_GetEFLAGS_ZF ||
		(_GetEFLAGS_SF !=
		_GetEFLAGS_OF)), 8);
}
void JG()
...
void INS_F6()
{
	...
	i386(0xf6) {
		_adv;
		_chk(modrm = _s_read_cs(vcpu.eip, 1));
		switch (_GetModRM_REG(modrm)) {
		case 0: /* TEST_RM8_I8 */
			_bb("TEST_RM8_I8");
			_chk(_d_modrm(0, 1, 0));
