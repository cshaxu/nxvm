...
#define _newins_ if (1) {vapiPrint("NEW INSTRUCTION: %s\n",trace.callstack[trace.cid - 1].blockstack[0]);} else
...
tots _s_test_ss_push(t_nubit8 byte)
{
	...
	case 2: _bb("StackSize(2)");
		...
		_chk(_m_test_logical(&vcpu.ss, vcpu.sp - byte, byte, 1));
		_be;break;
	case 4: _bb("StackSize(4)");
		if (vcpu.esp && vcpu.esp < byte)
			_chk(_SetExcept_SS(0));
		_chk(_m_test_logical(&vcpu.ss, vcpu.esp - byte, byte, 1));
		_be;break;
...
/* port accessing unit */
/* kernel port accessing */
tots _kpa_test_iomap(t_nubit16 port, t_nubit8 byte)
{
	_cb("_kpa_test_iomap");
	_ce;
}
tots _kpa_test_mode(t_nubit16 port, t_nubit8 byte)
{
	_cb("_p_test");
	if (_GetCR0_PE && (_GetCPL > (t_nubit8)_GetEFLAGS_IOPL || _GetEFLAGS_VM)) {
		_bb("CR0_PE(1),(CPL>IOPL/EFLAGS_VM(1))");
		_chk(_kpa_test_iomap(port, byte));
		_be;
	}
	_ce;
}
/* regular port accessing */
static t_nubit32 _p_input(t_nubit16 port, t_nubit8 byte)
{
	t_nubit32 result = 0x00000000;
	_cb("_p_input");
	_chr(_kpa_test_mode(port, byte));
	ExecFun(vport.in[port]);
	switch (byte) {
...
tots _p_output(t_nubit16 port, t_nubit32 data, t_nubit8 byte)
{
	_cb("_p_output");
	_chk(_kpa_test_mode(port, byte));
	switch (byte) {
...
tots _d_modrm(t_nubit8 regbyte, t_nubit8 rmbyte, t_bool write)
{
	t_nsbit8 disp8 = 0x00;
...
static t_nubit64 _kec_pop(t_nubit8 byte)
{
	...
	switch (_GetStackSize) {
	case 2: _bb("StackSize(2)");
		_chr(result = _m_read_logical(&vcpu.ss, vcpu.sp, byte));
		vcpu.sp += byte;
		_be;break;
	case 4: _bb("StackSize(4)");
		_chr(result = _m_read_logical(&vcpu.ss, vcpu.esp, byte));
		vcpu.esp += byte;
		_be;break;
...
tots _ser_int_real(t_nubit8 intid)
{
	...
	_ClrEFLAGS_IF;
	_ClrEFLAGS_TF;
	_chk(_kec_push((t_vaddrcc)&vcpu.cs.selector, 2));
	_chk(_kec_push((t_vaddrcc)&vcpu.ip, 2));
	_chk(vector = (t_nubit32)_s_read_idt(intid));
	vcpu.eip = GetMax16(vector);
	vcpu.cs.selector = GetMax16(vector >> 16);
	_chk(_s_load_cs(vcpu.cs.selector));
	_ce;
}
tots _ser_iret_real(t_nubit8 byte)
{
	...
	case 2: _bb("byte(2)");
		...
		_chk(vcpu.eip = (t_nubit16)_kec_pop(2));
		_chk(vcpu.cs.selector = (t_nubit16)_kec_pop(2));
		_chk(_s_load_cs(vcpu.cs.selector));
		_chk(vcpu.flags = (t_nubit16)_kec_pop(2));
		_be;break;
	case 4: _bb("byte(4)");
		_chk(_s_test_ss_pop(12));
		_chk(vcpu.eip = (t_nubit32)_kec_pop(4));
		_chk(vcpu.cs.selector = (t_nubit16)_kec_pop(4));
		_chk(_s_load_cs(vcpu.cs.selector));
		_chk(tempEFLAGS = (t_nubit32)_kec_pop(4));
		_chk(vcpu.eflags = (tempEFLAGS & 0x00257fd5) | (vcpu.eflags & 0x001a0000));
		_be;break;
	default: _bb("byte");
		_chk(_SetExcept_CE(0));
		_be;break;}
...
todo _e_int_n(t_nubit8 intid)
{
	_cb("_e_int_n");
	if (!_GetCR0_PE) {
		_bb("CR0_PE(0)");
		_chk(_ser_int_real(intid));
		_be;
	} else {
		_bb("CR0_PE(1)");
...
done _kas_move_index(t_nubit8 bit, t_bool flagsi, t_bool flagdi)
{
	t_nubit8 step = 0x00;
	_cb("_kas_move_index");
	switch (bit) {
	case 8: step = 0x01;break;
	case 16:step = 0x02;break;
	case 32:step = 0x04;break;
	default:_bb("bit");
		_chk(_SetExcept_CE(0));
		_be;break;
	}
	...
	case 2:
		if(_GetEFLAGS_DF) {
			if (flagdi) vcpu.di -= step;
			if (flagsi) vcpu.si -= step;
		} else {
			if (flagdi) vcpu.di += step;
			if (flagsi) vcpu.si += step;
		}
	...
	case 4:
		if(_GetEFLAGS_DF) {
			if (flagdi) vcpu.edi -= step;
			if (flagsi) vcpu.esi -= step;
		} else {
			if (flagdi) vcpu.edi += step;
			if (flagsi) vcpu.esi += step;
		}
...
static void GetModRegRM(t_nubitcc regbit,t_nubitcc rmbit)
{
	...
	switch (regbit) {
	case 0: vcpuins.cr = vcpuins.rr = _GetModRM_REG(modrm);break;
	case 4:
		switch (_GetModRM_REG(modrm)) {
...
static void _a_or(void *dest, void *src, t_nubit8 bit)
{
	switch (bit) {
	case 8:
		vcpuins.bit = 8;
...
static void _a_sbb(void *dest, void *src, t_nubit8 bit)
{
	switch (bit) {
	case 8:
		vcpuins.bit = 8;
...
static void _a_sub(void *dest, void *src, t_nubit8 bit)
{
	switch (bit) {
	case 8:
		vcpuins.bit = 8;
...
static void _a_xor(void *dest, void *src, t_nubit8 bit)
{
	switch (bit) {
	case 8:
		vcpuins.bit = 8;
...
static void PUSH(void *src, t_nubit8 bit)
{
	...
	case 32:
		...
		vcpuins.bit = 32;
		vcpu.esp -= 4;
		vramVarDWord(vcpu.ss.selector,vcpu.esp) = data32;
		break;
...
static void NEG(void *dest, t_nubit8 bit)
{
	...
	case 8:
		vcpuins.bit = 8;
		_a_sub((void *)&zero,(void *)dest,8);
		d_nubit8(dest) = (t_nubit8)zero;
		break;
	case 16:
		vcpuins.bit = 16;
		_a_sub((void *)&zero,(void *)dest,16);
...
tots _p_ins(t_nubit8 bit)
{
	...
	switch (bit) {
	case 8: _bb("bit(8)");
		_comment("_p_ins::bit(8)");
		vcpuins.bit = 8;
		_chk(_m_test_logical(&vcpu.es, cedi, data, 1));
		_chk(data = (t_nubit8)_p_input(vcpu.dx, 1));
		_chk(_m_write_logical(&vcpu.es, cedi, data, 1));
		_chk(_kas_move_index(8, 1, 1));
		_be;break;
	case 16: _bb("bit(16)");
		_comment("_p_ins::bit(16)");
		vcpuins.bit = 16;
		_chk(_m_test_logical(&vcpu.es, cedi, data, 2));
		_chk(data = (t_nubit16)_p_input(vcpu.dx, 2));
		_chk(_m_write_logical(&vcpu.es, cedi, data, 2));
		_chk(_kas_move_index(16, 1, 1));
		_be;break;
	case 32: _bb("bit(32)");
		_comment("_p_ins::bit(32)");
		vcpuins.bit = 32;
		_chk(_m_test_logical(&vcpu.es, cedi, data, 4));
		_chk(data = (t_nubit32)_p_input(vcpu.dx, 4));
		_chk(_m_write_logical(&vcpu.es, cedi, data, 4));
		_chk(_kas_move_index(32, 1, 1));
		_be;break;
	default: _bb("bit");
		_chk(_SetExcept_CE(0));
		_be;break;}
...
tots _a_cmps(t_nubit8 bit)
{
	...
	switch (_GetAddressSize) {
	case 2:
		cesi = GetMax16(vcpu.si);
		cedi = GetMax16(vcpu.di);
		break;
	case 4:
		cesi = GetMax32(vcpu.esi);
		cedi = GetMax32(vcpu.edi);
		break;
	default:_impossible_;break;}
	switch (bit) {
	case 8: _bb("bit(8)");
		vcpuins.bit = 8;
		vcpuins.type = CMP8;
		vcpuins.opr1 = GetMax8(_m_read_logical(vcpuins.roverds, cesi, 1));
		vcpuins.opr2 = GetMax8(_m_read_logical(&vcpu.es, cedi, 1));
		vcpuins.result = GetMax8(vcpuins.opr1 - vcpuins.opr2);
		_chk(_kas_move_index(8, 1, 1));
		_chk(_kaf_set_flags(CMP_FLAG));
		_be;break;
	case 16: _bb("bit(16)");
		vcpuins.bit = 16;
		vcpuins.type = CMP16;
		vcpuins.opr1 = GetMax16(_m_read_logical(vcpuins.roverds, cesi, 2));
		vcpuins.opr2 = GetMax16(_m_read_logical(&vcpu.es, cedi, 2));
		vcpuins.result = GetMax16(vcpuins.opr1 - vcpuins.opr2);
		_chk(_kas_move_index(16, 1, 1));
		_chk(_kaf_set_flags(CMP_FLAG));
		_be;break;
	case 32: _bb("bit(32)");
		_comment("_a_cmps::bit(32)");
		vcpuins.bit = 32;
		vcpuins.type = CMP32;
		vcpuins.opr1 = GetMax32(_m_read_logical(vcpuins.roverds, cesi, 4));
		vcpuins.opr2 = GetMax32(_m_read_logical(&vcpu.es, cedi, 4));
		vcpuins.result = GetMax32(vcpuins.opr1 - vcpuins.opr2);
		_chk(_kas_move_index(32, 1, 1));
		_chk(_kaf_set_flags(CMP_FLAG));
		_be;break;
...
void OR_RM8_R8()
{
	vcpu.eip++;
	GetModRegRM(8,8);
	_a_or((void *)vcpuins.rrm,(void *)vcpuins.rr,8);
...
void OR_RM16_R16()
{
	vcpu.eip++;
	GetModRegRM(16,16);
	_a_or((void *)vcpuins.rrm,(void *)vcpuins.rr,16);
...
void OR_R8_RM8()
{
	vcpu.eip++;
	GetModRegRM(8,8);
	_a_or((void *)vcpuins.rr,(void *)vcpuins.rrm,8);
...
void OR_R16_RM16()
{
	vcpu.eip++;
	GetModRegRM(16,16);
	_a_or((void *)vcpuins.rr,(void *)vcpuins.rrm,16);
...
void OR_AL_I8()
{
	vcpu.eip++;
	GetImm(8);
	_a_or((void *)&vcpu.al,(void *)vcpuins.rimm,8);
...
void OR_AX_I16()
{
	vcpu.eip++;
	GetImm(16);
	_a_or((void *)&vcpu.ax,(void *)vcpuins.rimm,16);
...
void SBB_RM8_R8()
{
	vcpu.eip++;
	GetModRegRM(8,8);
	_a_sbb((void *)vcpuins.rrm,(void *)vcpuins.rr,8);
...
void SBB_RM16_R16()
{
	vcpu.eip++;
	GetModRegRM(16,16);
	_a_sbb((void *)vcpuins.rrm,(void *)vcpuins.rr,16);
...
void SBB_R8_RM8()
{
	vcpu.eip++;
	GetModRegRM(8,8);
	_a_sbb((void *)vcpuins.rr,(void *)vcpuins.rrm,8);
...
void SBB_R16_RM16()
{
	vcpu.eip++;
	GetModRegRM(16,16);
	_a_sbb((void *)vcpuins.rr,(void *)vcpuins.rrm,16);
...
void SBB_AL_I8()
{
	vcpu.eip++;
	GetImm(8);
	_a_sbb((void *)&vcpu.al,(void *)vcpuins.rimm,8);
...
void SBB_AX_I16()
{
	vcpu.eip++;
	GetImm(16);
	_a_sbb((void *)&vcpu.ax,(void *)vcpuins.rimm,16);
...
void SUB_RM8_R8()
{
	vcpu.eip++;
	GetModRegRM(8,8);
	_a_sub((void *)vcpuins.rrm,(void *)vcpuins.rr,8);
...
void SUB_RM16_R16()
{
	vcpu.eip++;
	GetModRegRM(16,16);
	_a_sub((void *)vcpuins.rrm,(void *)vcpuins.rr,16);
...
void SUB_R8_RM8()
{
	vcpu.eip++;
	GetModRegRM(8,8);
	_a_sub((void *)vcpuins.rr,(void *)vcpuins.rrm,8);
...
void SUB_R16_RM16()
{
	vcpu.eip++;
	GetModRegRM(16,16);
	_a_sub((void *)vcpuins.rr,(void *)vcpuins.rrm,16);
...
void SUB_AL_I8()
{
	vcpu.eip++;
	GetImm(8);
	_a_sub((void *)&vcpu.al,(void *)vcpuins.rimm,8);
...
void SUB_AX_I16()
{
	vcpu.eip++;
	GetImm(16);
	_a_sub((void *)&vcpu.ax,(void *)vcpuins.rimm,16);
...
void XOR_RM8_R8()
{
	vcpu.eip++;
	GetModRegRM(8,8);
	_a_xor((void *)vcpuins.rrm,(void *)vcpuins.rr,8);
...
void XOR_RM16_R16()
{
	vcpu.eip++;
	GetModRegRM(16,16);
	_a_xor((void *)vcpuins.rrm,(void *)vcpuins.rr,16);
...
void XOR_R8_RM8()
{
	vcpu.eip++;
	GetModRegRM(8,8);
	_a_xor((void *)vcpuins.rr,(void *)vcpuins.rrm,8);
...
void XOR_R16_RM16()
{
	vcpu.eip++;
	GetModRegRM(16,16);
	_a_xor((void *)vcpuins.rr,(void *)vcpuins.rrm,16);
...
void XOR_AL_I8()
{
	vcpu.eip++;
	GetImm(8);
	_a_xor((void *)&vcpu.al,(void *)vcpuins.rimm,8);
...
void XOR_AX_I16()
{
	vcpu.eip++;
	GetImm(16);
	_a_xor((void *)&vcpu.ax,(void *)vcpuins.rimm,16);
...
tots JO()
{
	...
	i386(0x70) {
		...
		vcpu.eip++;
		GetImm(8);
		JCC((void *)vcpuins.rimm, _GetEFLAGS_OF, 8);
	}
	_ce;
}
tots JNO()
...
done JZ()
{
	...
	i386(0x74) {
		...
		vcpu.eip++;
		GetImm(8);
		JCC((void *)vcpuins.rimm, _GetEFLAGS_ZF, 8);
	}
	_ce;
}
done JNZ()
...
done JS()
{
	...
	i386(0x78) {
		...
		vcpu.eip++;
		GetImm(8);
		JCC((void *)vcpuins.rimm, _GetEFLAGS_SF, 8);
	}
	_ce;
}
done JNS()
...
tots JP()
{
	...
	i386(0x7a) {
		...
		vcpu.eip++;
		GetImm(8);
		JCC((void *)vcpuins.rimm, _GetEFLAGS_PF, 8);
	}
	_ce;
}
tots JNP()
...
done JL()
{
	...
	i386(0x7c) {
		...
		vcpu.eip++;
		GetImm(8);
		JCC((void *)vcpuins.rimm, (_GetEFLAGS_SF != _GetEFLAGS_OF), 8);
	}
	_ce;
}
done JNL()
...
void INS_80()
{
	...
	i386 (0x80) {
		...
		case 1: /* OR_RM8_I8 */
			...
			_chk(_d_modrm(0, 1, 1));
			_chk(_d_imm(1));
			_chk(_a_or((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 2: /* ADC_RM8_I8 */
			_bb("ADC_RM8_I8");
			_chk(_d_modrm(0, 1, 1));
		...
		case 3: /* SBB_RM8_I8 */
			...
			_chk(_d_modrm(0, 1, 1));
			_chk(_d_imm(1));
			_chk(_a_sbb((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 4: /* AND_RM8_I8 */
			_bb("AND_RM8_I8");
			_chk(_d_modrm(0, 1, 1));
		...
		case 5: /* SUB_RM8_I8 */
			...
			_chk(_d_modrm(0, 1, 1));
			_chk(_d_imm(1));
			_chk(_a_sub((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 6: /* XOR_RM8_I8 */
			_bb("XOR_RM8_I8");
			_chk(_d_modrm(0, 1, 1));
			_chk(_d_imm(1));
			_chk(_a_xor((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 7: /* CMP_RM8_I8 */
			_bb("CMP_RM8_I8");
			_chk(_d_modrm(0, 1, 0));
		...
		case 1: _a_or((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8); break;
		case 2: _a_adc(vcpuins.rrm, vcpuins.rimm, 8); break;
		case 3: _a_sbb((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8); break;
		case 4: _a_and(vcpuins.rrm, vcpuins.rimm, 8); break;
		case 5: _a_sub((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8); break;
		case 6: _a_xor((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8); break;
...
void INS_81()
{
	...
	i386(0x81) {
		...
		case 1: /* OR_RM16_I8 */
			...
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(_d_imm(_GetOperandSize));
			_chk(_a_or((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 2: /* ADC_RM16_I8 */
			_bb("ADC_RM16_I8");
			_chk(_d_modrm(0, _GetOperandSize, 1));
		...
		case 3: /* SBB_RM16_I8 */
			...
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(_d_imm(_GetOperandSize));
			_chk(_a_sbb((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 4: /* AND_RM16_I8 */
			_bb("AND_RM16_I8");
			_chk(_d_modrm(0, _GetOperandSize, 1));
		...
		case 5: /* SUB_RM16_I8 */
			...
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(_d_imm(_GetOperandSize));
			_chk(_a_sub((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 6: /* XOR_RM16_I8 */
			_bb("XOR_RM16_I8");
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(_d_imm(_GetOperandSize));
			_chk(_a_xor((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 7: /* CMP_RM16_I8 */
			_bb("CMP_RM16_I8");
		...
		case 1: _a_or((void *)vcpuins.rrm, (void *)vcpuins.rimm, 16); break;
		case 2: _a_adc(vcpuins.rrm, vcpuins.rimm, 16); break;
		case 3: _a_sbb((void *)vcpuins.rrm, (void *)vcpuins.rimm, 16); break;
		case 4: _a_and(vcpuins.rrm, vcpuins.rimm, 16); break;
		case 5: _a_sub((void *)vcpuins.rrm, (void *)vcpuins.rimm, 16); break;
		case 6: _a_xor((void *)vcpuins.rrm, (void *)vcpuins.rimm, 16); break;
...
void INS_83()
{
	...
	i386(0x83) {
		...
		case 1: /* OR_RM16_I8 */
			...
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(_d_imm(1));
			_chk(_a_or((void *)vcpuins.rrm, (void *)vcpuins.rimm, bit));
			_be;break;
		case 2: /* ADC_RM16_I8 */
			_bb("ADC_RM16_I8");
			_chk(_d_modrm(0, _GetOperandSize, 1));
		...
		case 3: /* SBB_RM16_I8 */
			...
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(_d_imm(1));
			_chk(_a_sbb((void *)vcpuins.rrm, (void *)vcpuins.rimm, bit));
			_be;break;
		case 4: /* AND_RM16_I8 */
			_bb("AND_RM16_I8");
			_chk(_d_modrm(0, _GetOperandSize, 1));
		...
		case 5: /* SUB_RM16_I8 */
			...
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(_d_imm(1));
			_chk(_a_sub((void *)vcpuins.rrm, (void *)vcpuins.rimm, bit));
			_be;break;
		case 6: /* XOR_RM16_I8 */
			_bb("XOR_RM16_I8");
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(_d_imm(1));
			_chk(_a_xor((void *)vcpuins.rrm, (void *)vcpuins.rimm, bit));
			_be;break;
		case 7: /* CMP_RM16_I8 */
			_bb("CMP_RM16_I8");
			_chk(_d_modrm(0, _GetOperandSize, 0));
		...
		case 1: _a_or((void *)vcpuins.rrm, (void *)vcpuins.rimm, bit); break;
		case 2: _a_adc(vcpuins.rrm, vcpuins.rimm, bit); break;
		case 3: _a_sbb((void *)vcpuins.rrm, (void *)vcpuins.rimm, bit); break;
		case 4: _a_and(vcpuins.rrm, vcpuins.rimm, bit); break;
		case 5: _a_sub((void *)vcpuins.rrm, (void *)vcpuins.rimm, bit); break;
		case 6: _a_xor((void *)vcpuins.rrm, (void *)vcpuins.rimm, bit); break;
...
done CMPSB()
{
	...
	i386(0xa6) {
		...
		if(vcpuins.prefix_rep == PREFIX_REP_NONE) _a_cmps(8);
			...
			if (vcpu.cx && !(vcpuins.prefix_rep == PREFIX_REP_REPZ && !_GetEFLAGS_ZF) && !(vcpuins.prefix_rep == PREFIX_REP_REPZNZ && _GetEFLAGS_ZF))
...
void INS_C0()
{
	...
	i386(0xc0) {
		...
		case 0: /* ROL_RM8_I8 */
			_bb("ROL_RM8_I8");
			_chk(ROL((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 1: /* ROR_RM8_I8 */
			_bb("ROR_RM8_I8");
			_chk(ROR((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 2: /* RCL_RM8_I8 */
			_bb("RCL_RM8_I8");
			_chk(RCL((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 3: /* RCR_RM8_I8 */
			_bb("RCR_RM8_I8");
			_chk(RCR((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 4: /* SHL_RM8_I8 */
			_bb("SHL_RM8_I8");
			_chk(SHL((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 5: /* SHR_RM8_I8 */
			_bb("SHR_RM8_I8");
			_chk(SHR((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 6: /* SAL_RM8_I8 */
			_bb("SAL_RM8_I8");
			_chk(SAL((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		case 7: /* SAR_RM8_I8 */
			_bb("SAR_RM8_I8");
			_chk(SAR((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		default:_impossible_;break;}
	} else {
		vcpu.eip++;
...
void INS_C1()
{
	...
	i386(0xc1) {
		...
		case 0: /* ROL_RM16_I8 */
			_bb("ROL_RM16_I8");
			_chk(ROL((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 1: /* ROR_RM16_I8 */
			_bb("ROR_RM16_I8");
			_chk(ROR((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 2: /* RCL_RM16_I8 */
			_bb("RCL_RM16_I8");
			_chk(RCL((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 3: /* RCR_RM16_I8 */
			_bb("RCR_RM16_I8");
			_chk(RCR((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 4: /* SHL_RM16_I8 */
			_bb("SHL_RM16_I8");
			_chk(SHL((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 5: /* SHR_RM16_I8 */
			_bb("SHR_RM16_I8");
			_chk(SHR((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 6: /* SAL_RM16_I8 */
			_bb("SAL_RM16_I8");
			_chk(SAL((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 7: /* SAR_RM16_I8 */
			_bb("SAR_RM16_I8");
			_chk(SAR((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		default:_impossible_;break;}
	} else {
		vcpu.eip++;
...
tots ENTER()
{
	...
	i386(0xc8) {
		...
		if (level) {
			...
			_comment("ENTER: should I use vcpu.ss or overss in the for-loop?");
			_comment("ENTER: ");
			for (i = 0;i < level;++i) {
				_bb("for");
				switch (_GetOperandSize) {
...
todo INT_I8()
{
	...
	i386(0xcc) {
		...
		vcpu.eip++;
		GetImm(8);
		_e_int_n(d_nubit8(vcpuins.rimm));
	}
	_ce;
}
tots INTO()
{
	...
	i386(0xce) {
		...
		vcpu.eip++;
		if(_GetEFLAGS_OF) _e_int_n(0x04);
	}
	_ce;
}
todo IRET()
...
void INS_D0()
{
	...
	i386(0xd0) {
		...
		case 0: /* ROL_RM8 */
			_bb("ROL_RM8");
			_chk(ROL((void *)vcpuins.rrm, NULL, 8));
			_be;break;
		case 1: /* ROR_RM8 */
			_bb("ROR_RM8");
			_chk(ROR((void *)vcpuins.rrm, NULL, 8));
			_be;break;
		case 2: /* RCL_RM8 */
			_bb("RCL_RM8");
			_chk(RCL((void *)vcpuins.rrm, NULL, 8));
			_be;break;
		case 3: /* RCR_RM8 */
			_bb("RCR_RM8");
			_chk(RCR((void *)vcpuins.rrm, NULL, 8));
			_be;break;
		case 4: /* SHL_RM8 */
			_bb("SHL_RM8");
			_chk(SHL((void *)vcpuins.rrm, NULL, 8));
			_be;break;
		case 5: /* SHR_RM8 */
			_bb("SHR_RM8");
			_chk(SHR((void *)vcpuins.rrm, NULL, 8));
			_be;break;
		case 6: /* SAL_RM8 */
			_bb("SAL_RM8");
			_chk(SAL((void *)vcpuins.rrm, NULL, 8));
			_be;break;
		case 7: /* SAR_RM8 */
			_bb("SAR_RM8");
			_chk(SAR((void *)vcpuins.rrm, NULL, 8));
			_be;break;
		default:_impossible_;break;}
	} else {
		vcpu.eip++;
...
void INS_D1()
{
	...
	i386(0xd1) {
		...
		case 0: /* ROL_RM16 */
			_bb("ROL_RM16");
			_chk(ROL((void *)vcpuins.rrm, NULL, _GetOperandSize * 8));
			_be;break;
		case 1: /* ROR_RM16 */
			_bb("ROR_RM16");
			_chk(ROR((void *)vcpuins.rrm, NULL, _GetOperandSize * 8));
			_be;break;
		case 2: /* RCL_RM16 */
			_bb("RCL_RM16");
			_chk(RCL((void *)vcpuins.rrm, NULL, _GetOperandSize * 8));
			_be;break;
		case 3: /* RCR_RM16 */
			_bb("RCR_RM16");
			_chk(RCR((void *)vcpuins.rrm, NULL, _GetOperandSize * 8));
			_be;break;
		case 4: /* SHL_RM16 */
			_bb("SHL_RM16");
			_chk(SHL((void *)vcpuins.rrm, NULL, _GetOperandSize * 8));
			_be;break;
		case 5: /* SHR_RM16 */
			_bb("SHR_RM16");
			_chk(SHR((void *)vcpuins.rrm, NULL, _GetOperandSize * 8));
			_be;break;
		case 6: /* SAL_RM16 */
			_bb("SAL_RM16");
			_chk(SAL((void *)vcpuins.rrm, NULL, _GetOperandSize * 8));
			_be;break;
		case 7: /* SAR_RM16 */
			_bb("SAR_RM16");
			_chk(SAR((void *)vcpuins.rrm, NULL, _GetOperandSize * 8));
			_be;break;
		default:_impossible_;break;}
	} else {
		vcpu.eip++;
...
void INS_D2()
{
	...
	i386(0xd2) {
		...
		case 0: /* ROL_RM8_CL */
			_bb("ROL_RM8_CL");
			_chk(ROL((void *)vcpuins.rrm, (void *)&vcpu.cl, 8));
			_be;break;
		case 1: /* ROR_RM8_CL */
			_bb("ROR_RM8_CL");
			_chk(ROR((void *)vcpuins.rrm, (void *)&vcpu.cl, 8));
			_be;break;
		case 2: /* RCL_RM8_CL */
			_bb("RCL_RM8_CL");
			_chk(RCL((void *)vcpuins.rrm, (void *)&vcpu.cl, 8));
			_be;break;
		case 3: /* RCR_RM8_CL */
			_bb("RCR_RM8_CL");
			_chk(RCR((void *)vcpuins.rrm, (void *)&vcpu.cl, 8));
			_be;break;
		case 4: /* SHL_RM8_CL */
			_bb("SHL_RM8_CL");
			_chk(SHL((void *)vcpuins.rrm, (void *)&vcpu.cl, 8));
			_be;break;
		case 5: /* SHR_RM8_CL */
			_bb("SHR_RM8_CL");
			_chk(SHR((void *)vcpuins.rrm, (void *)&vcpu.cl, 8));
			_be;break;
		case 6: /* SAL_RM8_CL */
			_bb("SAL_RM8_CL");
			_chk(SAL((void *)vcpuins.rrm, (void *)&vcpu.cl, 8));
			_be;break;
		case 7: /* SAR_RM8_CL */
			_bb("SAR_RM8_CL");
			_chk(SAR((void *)vcpuins.rrm, (void *)&vcpu.cl, 8));
			_be;break;
		default:_impossible_;break;}
	} else {
		vcpu.eip++;
...
void INS_D3()
{
	...
	i386(0xd3) {
		...
		case 0: /* ROL_RM16_CL */
			_bb("ROL_RM16_CL");
			_chk(ROL((void *)vcpuins.rrm, (void *)&vcpu.cl, _GetOperandSize * 8));
			_be;break;
		case 1: /* ROR_RM16_CL */
			_bb("ROR_RM16_CL");
			_chk(ROR((void *)vcpuins.rrm, (void *)&vcpu.cl, _GetOperandSize * 8));
			_be;break;
		case 2: /* RCL_RM16_CL */
			_bb("RCL_RM16_CL");
			_chk(RCL((void *)vcpuins.rrm, (void *)&vcpu.cl, _GetOperandSize * 8));
			_be;break;
		case 3: /* RCR_RM16_CL */
			_bb("RCR_RM16_CL");
			_chk(RCR((void *)vcpuins.rrm, (void *)&vcpu.cl, _GetOperandSize * 8));
			_be;break;
		case 4: /* SHL_RM16_CL */
			_bb("SHL_RM16_CL");
			_chk(SHL((void *)vcpuins.rrm, (void *)&vcpu.cl, _GetOperandSize * 8));
			_be;break;
		case 5: /* SHR_RM16_CL */
			_bb("SHR_RM16_CL");
			_chk(SHR((void *)vcpuins.rrm, (void *)&vcpu.cl, _GetOperandSize * 8));
			_be;break;
		case 6: /* SAL_RM16_CL */
			_bb("SAL_RM16_CL");
			_chk(SAL((void *)vcpuins.rrm, (void *)&vcpu.cl, _GetOperandSize * 8));
			_be;break;
		case 7: /* SAR_RM16_CL */
			_bb("SAR_RM16_CL");
			_chk(SAR((void *)vcpuins.rrm, (void *)&vcpu.cl, _GetOperandSize * 8));
			_be;break;
		default:_impossible_;break;}
	} else {
		vcpu.eip++;
...
void LOOPNZ()
{
	...
	vcpu.cx--;
	if(vcpu.cx && !_GetEFLAGS_ZF) vcpu.eip += rel8;
}
void LOOPZ()
{
	t_nsbit8 rel8;
...
void INS_F6()
{
	...
	i386(0xf6) {
		...
		case 0: /* TEST_RM8_I8 */
			...
			_chk(_d_modrm(0, 1, 0));
			_chk(_d_imm(1));
			_chk(TEST((void *)vcpuins.rrm, (void *)vcpuins.rimm, 8));
			_be;break;
		...
		case 2: /* NOT_RM8 */
			_bb("NOT_RM8");
			_chk(_d_modrm(0, 1, 1));
			_chk(NOT((void *)vcpuins.rrm, 8));
			_be;break;
		case 3: /* NEG_RM8 */
			_bb("NEG_RM8");
			_chk(_d_modrm(0, 1, 1));
			_chk(NEG((void *)vcpuins.rrm, 8));
			_be;break;
		case 4: /* MUL_RM8 */
			_bb("MUL_RM8");
			_chk(_d_modrm(0, 1, 0));
			_chk(MUL((void *)vcpuins.rrm, 8));
			_be;break;
		case 5: /* IMUL_RM8 */
			_bb("IMUL_RM8");
			_chk(_d_modrm(0, 1, 0));
...
void INS_F7()
{
	...
	i386(0xf7) {
		...
		case 0: /* TEST_RM16_I16 */
			...
			_chk(_d_modrm(0, _GetOperandSize, 0));
			_chk(_d_imm(_GetOperandSize));
			_chk(TEST((void *)vcpuins.rrm, (void *)vcpuins.rimm, _GetOperandSize * 8));
			_be;break;
		case 1: /* UndefinedOpcode */
			_bb("ModRM_REG(1)");
			_chk(UndefinedOpcode());
		...
		case 2: /* NOT_RM16 */
			_bb("NOT_RM16");
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(NOT((void *)vcpuins.rrm, _GetOperandSize * 8));
			_be;break;
		case 3: /* NEG_RM16 */
			_bb("NEG_RM16");
			_chk(_d_modrm(0, _GetOperandSize, 1));
			_chk(NEG((void *)vcpuins.rrm, _GetOperandSize * 8));
			_be;break;
		case 4: /* MUL_RM16 */
			_bb("MUL_RM16");
			_chk(_d_modrm(0, _GetOperandSize, 0));
			_chk(MUL((void *)vcpuins.rrm, _GetOperandSize * 8));
			_be;break;
		case 5: /* IMUL_RM16 */
			_bb("IMUL_RM16");
			_chk(_d_modrm(0, _GetOperandSize, 0));
...
void vcpuinsInit()
{
	...
	vcpuins.table[0x76] = (t_faddrcc)JNA;
	...
	vcpuins.table[0x7e] = (t_faddrcc)JNG;
