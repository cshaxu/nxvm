...
done LODSB()
{
	...
	i386(0xac) {
		...
		vcpu.ip++;
		if(vcpuins.prefix_rep == PREFIX_REP_NONE) _m_lods(8);
		else {
			if (vcpu.cx) {
				_m_lods(8);
				vcpu.cx--;
...
done LODSW()
{
	...
	i386(0xad) {
		...
		if(vcpuins.prefix_rep == PREFIX_REP_NONE) {
			_bb("prefix_rep(PREFIX_REP_NONE)");
			_chk(_m_lods(_GetOperandSize * 8));
			_be;
		} else {
			_bb("prefix_rep(!PREFIX_REP_NONE)");
			if (vcpu.ecx) {
		...
		vcpu.ip++;
		if(vcpuins.prefix_rep == PREFIX_REP_NONE) _m_lods(16);
		else {
			if (vcpu.cx) {
				_m_lods(16);
				vcpu.cx--;
