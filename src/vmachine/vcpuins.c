...
static void OR(void *dest, void *src, t_nubit8 len)
{
	...
	MakeBit(vcpu.flags,VCPU_FLAG_OF,0);
	MakeBit(vcpu.flags,VCPU_FLAG_CF,0);
	MakeBit(vcpu.flags,VCPU_FLAG_AF,0);
	SetFlags(OR_FLAG);
}
...
static void IMUL(void *src, t_nubit8 len)
{
	...
	case 8:
		...
		if(vcpu.ax == vcpu.al) {
			...
			MakeBit(vcpu.flags,VCPU_FLAG_OF,1);
			MakeBit(vcpu.flags,VCPU_FLAG_CF,1);
		}
		break;
	case 16:
		vcpuins.bit = 16;
...
static void INT(t_nubit8 intid)
{
	...
	MakeBit(vcpu.flags,(VCPU_FLAG_IF | VCPU_FLAG_TF),0);
...
void DAA()
{
	...
	} else MakeBit(vcpu.flags,VCPU_FLAG_AF,0);
	if(((vcpu.al & 0xf0) > 0x90) || GetBit(vcpu.flags, VCPU_FLAG_CF)) {
		vcpu.al += 0x60;
		MakeBit(vcpu.flags,VCPU_FLAG_CF,1);
	} else MakeBit(vcpu.flags,VCPU_FLAG_CF,0);
...
void DAS()
{
	...
	if(((vcpu.al & 0x0f) > 0x09) || GetBit(vcpu.flags, VCPU_FLAG_AF)) {
		...
		MakeBit(vcpu.flags,VCPU_FLAG_AF,1);
	} else MakeBit(vcpu.flags,VCPU_FLAG_AF,0);
...
void LEA_R16_M16()
{
	vcpu.ip++;
	GetModRegRMEA();
	*(t_nubit16 *)vcpuins.r = vcpuins.rm&0xffff;
...
void CALL_PTR16_16()
{
	...
	vcpu.ip++;
	GetImm(16);
	newip = *(t_nubit16 *)vcpuins.imm;
	GetImm(16);
	newcs = *(t_nubit16 *)vcpuins.imm;
	PUSH((void *)&vcpu.cs,16);
	PUSH((void *)&vcpu.ip,16);
	vcpu.ip = newip;
	vcpu.cs = newcs;
...
void IN_AL_I8()
{
	vcpu.ip++;
	GetImm(8);
	ExecFun(vport.in[*(t_nubit8 *)vcpuins.imm]);
	vcpu.al = vcpu.iobyte;
...
void IN_AX_I8()
{
	vcpu.ip++;
	GetImm(8);
	ExecFun(vport.in[*(t_nubit8 *)vcpuins.imm+0]);
	vcpu.al = vcpu.iobyte;
	ExecFun(vport.in[*(t_nubit8 *)vcpuins.imm+1]);
	vcpu.ah = vcpu.iobyte;
...
void OUT_I8_AL()
{
	...
	GetImm(8);
	vcpu.iobyte = vcpu.al;
	ExecFun(vport.out[*(t_nubit8 *)vcpuins.imm]);
...
void OUT_I8_AX()
{
	...
	GetImm(8);
	vcpu.iobyte = vcpu.al;
	ExecFun(vport.out[*(t_nubit8 *)vcpuins.imm+0]);
	vcpu.iobyte = vcpu.ah;
	ExecFun(vport.out[*(t_nubit8 *)vcpuins.imm+1]);
...
{
	vcpu.ip++;
	MakeBit(vcpu.flags,VCPU_FLAG_IF,0);
	// _vapiPrintAddr(vcpu.cs,vcpu.ip);vapiPrint("  CLI\n");
}
void STI()
{
	vcpu.ip++;
	MakeBit(vcpu.flags,VCPU_FLAG_IF,1);
	// _vapiPrintAddr(vcpu.cs,vcpu.ip);vapiPrint("  STI\n");
}
void CLD()
{
	vcpu.ip++;
	MakeBit(vcpu.flags,VCPU_FLAG_DF,0);
	// _vapiPrintAddr(vcpu.cs,vcpu.ip);vapiPrint("  CLD\n");
}
void STD()
{
	vcpu.ip++;
	MakeBit(vcpu.flags,VCPU_FLAG_DF,1);
...
/* vcpuins bug fix #11
		...
		vcpu.cs = *(t_nubit16 *)(vcpuins.rm+2);
		break;
	case 4:	/* JMP_RM16 */
		vcpu.ip = *(t_nubit16 *)vcpuins.rm;
		break;
