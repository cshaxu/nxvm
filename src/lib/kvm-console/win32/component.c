...
static void kvm_console_receive_event(void *context,
    const lib_console_event *event)
{
    ...
    if (console == LIB_NULL || event == LIB_NULL ||
        ...
        (state = (kvm_console_win32_state *)console->worker_state) == LIB_NULL)
        return;
    if (event->kind == LIB_CONSOLE_EVENT_INPUT_RESET) {
        ...
        (void)kvm_keyboard_submit_record(&state->keyboard,
        ...
        if ((mouse->buttons & LIB_WIN32_RIGHTMOST_BUTTON_PRESSED) != 0u)
            input.data.mouse.buttons |= KVM_MOUSE_BUTTON_RIGHT;
