...
static const char app_command_help[] =
    ...
    "  J / Ctrl             B\n"
    "  K / Alt              A\n"
    "  Enter / Left|Right Shift Start / Select\n"
    "  Esc                 pause or resume\n";
    ...
    lib_bool *out_trapped)
{
    ...
    if (out_trapped == LIB_NULL) return LIB_FALSE;
    ...
    if (!app_command_debug_execute(context, request, sizeof(request), response,
            sizeof(response), &size) || size != 52u) return LIB_FALSE;
    ...
    lib_u16 *out_value)
{
    ...
    if (*cursor == '$') { base = 16u; ++cursor; }
    else if (cursor[0] == '0' && (cursor[1] == 'x' || cursor[1] == 'X')) {
        base = 16u;
        cursor += 2;
    }
    if (*cursor == '\0') return LIB_FALSE;
    for (;;) {
        if (*cursor >= '0' && *cursor <= '9') digit = (lib_u32)(*cursor - '0');
        else if (base == 16u && *cursor >= 'a' && *cursor <= 'f')
            digit = (lib_u32)(*cursor - 'a' + 10);
        else if (base == 16u && *cursor >= 'A' && *cursor <= 'F')
            digit = (lib_u32)(*cursor - 'A' + 10);
        else break;
        if (digit >= base || value > (65535u - digit) / base) return LIB_FALSE;
        value = value * base + digit;
        ++cursor;
    }
    if (cursor == text || (base == 16u && (cursor == text + 1u ||
        (text[0] == '0' && cursor == text + 2u)))) return LIB_FALSE;
    ...
    lib_size capacity)
{
    ...
    if (length == 0u || (quoted && *cursor != '"')) return LIB_FALSE;
    if (quoted) ++cursor;
    if (!app_command_end(cursor)) return LIB_FALSE;
    out_path[length] = '\0';
    return LIB_TRUE;
}
    ...
    common_session_command_result *out_result)
{
    if (pending == APP_COMMAND_SNAPSHOT_SAVED && was_paused) {
        app_command_result(out_result, status == LIB_STATUS_OK ?
        ...
        context->session.pending_snapshot = status == LIB_STATUS_OK ? pending :
            APP_COMMAND_SNAPSHOT_SAVE_FAILED;
        context->session.transition_pending = LIB_TRUE;
        context->session.prompt_due = LIB_FALSE;
        *out_result = (common_session_command_result) { 0 };
    } else app_command_result(out_result, pending == APP_COMMAND_SNAPSHOT_SAVED ?
    ...
    const char *text)
{
    ...
    if (target == LIB_NULL || target_capacity == 0u) return;
    if (length >= target_capacity) length = target_capacity - 1u;
    if (length != 0u) lib_memory_copy(target, text, length);
    target[length] = '\0';
...
static void app_command_finish_output(common_session_command_result *out_result)
{
    ...
    if (out_result == LIB_NULL || out_result->text[0] == '\0') return;
...
{
    if (left == LIB_NULL || right == LIB_NULL) return LIB_FALSE;
    while (*left != '\0' && *right != '\0') {
        ...
        if (left_char != right_char) return LIB_FALSE;
        ++left;
        ++right;
    }
...
static const char *app_command_after(const char *line, const char *prefix)
{
    ...
    if (line == LIB_NULL || lib_text_length(line) <= length) return LIB_NULL;
    for (index = 0u; index < length; ++index) {
        char left = line[index];
        char right = prefix[index];
        if (left >= 'A' && left <= 'Z') left = (char)(left + ('a' - 'A'));
        if (right >= 'A' && right <= 'Z') right = (char)(right + ('a' - 'A'));
        if (left != right) return LIB_NULL;
    }
    return line + length;
}
    ...
    common_session_command_result *out_result)
{
    ...
    if (!app_command_debug_execute(context, request, sizeof(request), response,
            sizeof(response), &size) || size != 16u + count) {
    ...
    common_session_command_result *out_result)
{
    ...
    if (!app_command_debug_execute(context, request, sizeof(request), response,
            sizeof(response), &size) || size != 16u + (lib_size)count * 3u) {
    ...
    common_session_command_result *out_result)
{
    ...
    if (app_command_end(arguments) || app_command_equal(arguments, "help")) {
        ...
        if (!app_command_debug_execute(context, request, sizeof(request), response,
                sizeof(response), &response_size) || response_size != 52u)
            app_command_result(out_result, "Soft reset failed.\n");
        else app_command_result(out_result, "Soft reset complete; machine paused.\n");
    } else {
    ...
    const char *line, common_session_command_result *out_result)
{
    ...
    if (app_command_end(line)) return;
    ...
    if (app_command_equal(line, "help")) {
        ...
        if (!app_command_parse_path(path == LIB_NULL ? "" : path, media_path, sizeof(media_path)))
            ...
            app_command_result(out_result, state == COMMON_SESSION_MACHINE_RUNNING ?
        ...
        else app_command_request(context, out_result, COMMON_SESSION_REQUEST_RESUME);
        ...
        } else app_command_result(out_result, "Machine is not running.\n");
    } else if (app_command_equal(line, "stop")) {
        if (state == COMMON_SESSION_MACHINE_RUNNING || state == COMMON_SESSION_MACHINE_PAUSED) {
            app_command_request(context, out_result, COMMON_SESSION_REQUEST_STOP);
        } else app_command_result(out_result, "Machine is already stopped.\n");
    ...
    common_session_machine_state completed, common_session_command_result *out_result)
{
    ...
    *out_result = (common_session_command_result) { 0 };
    ...
    if (context != LIB_NULL) context->session.transition_pending = LIB_FALSE;
    if (context != LIB_NULL && completed == COMMON_SESSION_MACHINE_RESET_COMPLETED) {
        if (context->run_after_reset) {
            context->run_after_reset = LIB_FALSE;
            app_command_request(context, out_result, COMMON_SESSION_REQUEST_RESUME);
        } else app_command_outcome(context, "Reset complete; machine paused.\n");
        ...
        if (context != LIB_NULL &&
            ...
            context->session.pending_snapshot = APP_COMMAND_SNAPSHOT_NONE;
            app_command_outcome(context, pending == APP_COMMAND_SNAPSHOT_SAVED ?
                "Machine saved and paused.\n" : pending == APP_COMMAND_SNAPSHOT_LOADED ?
        ...
        if (context != LIB_NULL && context->session.display == COMMON_SESSION_DISPLAY_WINDOW) {
            app_command_outcome(context, context->started_after_reset ?
                "Machine started.\n" : "Machine resumed.\n");
        }
        if (context != LIB_NULL) context->started_after_reset = LIB_FALSE;
