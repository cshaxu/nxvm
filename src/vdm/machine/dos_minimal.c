...
#define VDM_MACHINE_KEY_QUEUE_CAPACITY 16u

struct vdm_machine_dos_minimal {
    core_machine *machine;
    vdm_machine_text_snapshot snapshot;
    uint8_t key_queue[VDM_MACHINE_KEY_QUEUE_CAPACITY];
    uint8_t key_count;
};
...
static type_status vdm_machine_dos_minimal_port_read_handler(
    C_VOID *owner,
    uint16_t port,
    uint32_t *out_value)
{
...
static type_status vdm_machine_dos_minimal_port_write_handler(
    C_VOID *owner,
    uint16_t port,
    uint32_t value)
{
...
static type_status vdm_machine_dos_minimal_install_ports(
    vdm_machine_dos_minimal *session)
{
    ...
    uint8_t scan_code)
{
    ...
    if (session->key_count == VDM_MACHINE_KEY_QUEUE_CAPACITY) {
