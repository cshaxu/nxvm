    ...
    const vm_session_request *request)
{
    ...
    if (bridge->count == VM_SESSION_REQUEST_CAPACITY) {
        return TYPE_STATUS_NO_MEMORY;
    }

    index = (bridge->head + bridge->count) % VM_SESSION_REQUEST_CAPACITY;
    ...
    vm_session_request *out_request)
{
    ...
    *out_request = bridge->entries[bridge->head];
    bridge->head = (bridge->head + 1u) % VM_SESSION_REQUEST_CAPACITY;
    --bridge->count;
    return TYPE_STATUS_OK;
}
