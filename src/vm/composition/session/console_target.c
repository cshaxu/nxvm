...
static C_VOID vm_session_console_set_memory(C_VOID *context, STD_SIZE_T bytes) { vm_session *machine = (vm_session *)context; if (machine != STD_NULL) core_machine_memory_allocate_for(core_machine_executor_memory_borrow(machine->core_machine), bytes); }
