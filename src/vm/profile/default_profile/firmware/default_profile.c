    ...
    core_machine_firmware *firmware, vm_profile_default_firmware_plan *out_plan)
{
    ...
    if (firmware == STD_NULL || out_plan == STD_NULL || profile == STD_NULL) {
    ...
    out_plan->reset_segment = profile->rom.reset_segment;
    out_plan->reset_offset = profile->rom.reset_offset;
    ...
    core_machine *machine, C_INT boot_hdd)
{
    ...
    if (machine == STD_NULL || profile == STD_NULL) return TYPE_STATUS_INVALID_ARGUMENT;
    word = 0x03f8u;
    status = core_machine_memory_write(machine, 0x0400u, &word, sizeof(word));
    if (status != TYPE_STATUS_OK) return status;
    word = profile->cmos.base_memory_kib;
    status = core_machine_memory_write(machine, 0x0413u, &word, sizeof(word));
    if (status != TYPE_STATUS_OK) return status;
    value = boot_hdd ? profile->cmos.boot_hard_disk_drive :
        profile->cmos.boot_floppy_drive;
    ...
    if (status != TYPE_STATUS_OK) return status;
    return core_machine_memory_write(machine, profile->rom.physical_start,
        reset_stub, sizeof(reset_stub));
}
    ...
    vm_profile_default_firmware_cmos *cmos, C_INT boot_hdd)
{
    ...
    if (cmos == STD_NULL || profile == STD_NULL) return;
    cmos->equipment = profile->cmos.equipment;
    cmos->base_memory_kib = TYPE_MASK_UNSIGNED_8(profile->cmos.base_memory_kib);
    cmos->base_memory_kib_high = TYPE_MASK_UNSIGNED_8(
        profile->cmos.base_memory_kib >> 8);
    cmos->boot_drive = boot_hdd ? profile->cmos.boot_hard_disk_drive :
        profile->cmos.boot_floppy_drive;
}
