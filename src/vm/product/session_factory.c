    ...
    vm_machine_floppy_format *out_format)
{
    ...
    else if (!STD_STRCMP(value, "720k")) *out_format = VM_MACHINE_FLOPPY_FORMAT_720K;
    else if (!STD_STRCMP(value, "1200k")) *out_format = VM_MACHINE_FLOPPY_FORMAT_1200K;
    else if (!STD_STRCMP(value, "1440k")) *out_format = VM_MACHINE_FLOPPY_FORMAT_1440K;
    else return 0;
    return 1;
}
    ...
    vm_machine_profile_kind *out_profile)
{
    ...
    if (!STD_STRCMP(value, "default-pc-at")) {
        *out_profile = VM_MACHINE_PROFILE_DEFAULT_PC_AT;
    } else if (!STD_STRCMP(value, "ibm-5170-model-339")) {
        *out_profile = VM_MACHINE_PROFILE_IBM_5170_MODEL_339;
    } else if (!STD_STRCMP(value, "ibm-5160-model-268")) {
        *out_profile = VM_MACHINE_PROFILE_IBM_5160_MODEL_268;
    } else if (!STD_STRCMP(value, "compaq-deskpro-386-model-40")) {
        *out_profile = VM_MACHINE_PROFILE_COMPAQ_DESKPRO_386_MODEL_40;
    } else {
        return 0;
    }
    return 1;
    ...
    const vm_product_session_request *request, vm_machine_config *config)
{
    ...
    if (request->fixed_disk_count > 1u ||
        request->floppy_count > (config->profile_kind ==
            VM_MACHINE_PROFILE_COMPAQ_DESKPRO_386_MODEL_40 ? 2u : 1u)) {
        return TYPE_STATUS_INVALID_STATE;
    }
    if (config->profile_kind == VM_MACHINE_PROFILE_IBM_5160_MODEL_268 &&
    ...
    if (config->profile_kind == VM_MACHINE_PROFILE_IBM_5170_MODEL_339 &&
    ...
    if ((config->profile_kind == VM_MACHINE_PROFILE_DEFAULT_PC_AT &&
        ...
        (config->profile_kind == VM_MACHINE_PROFILE_IBM_5160_MODEL_268 &&
