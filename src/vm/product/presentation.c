    ...
    vm_product_presentation *presentation)
{
    ...
    if (presentation->console != STD_NULL) {
        (C_VOID)vm_product_console_host_release_guest(presentation->console_host,
            ux_console_get_console(presentation->console));
        ux_console_destroy(presentation->console);
        presentation->console = STD_NULL;
    }
    if (presentation->window != STD_NULL) {
        ux_window_destroy(presentation->window);
...
{
    ux_component_options component = {0};
    lib_status status;
    ...
    if (target == VM_PRODUCT_PRESENTATION_WINDOW) {
        ...
        status = ux_window_create(&presentation->window, &options);
    } else if (target == VM_PRODUCT_PRESENTATION_CONSOLE) {
        status = ux_console_create(&presentation->console, &component);
...
static C_INT vm_product_presentation_key_scan(ux_key key, type_unsigned_16 *scan)
{
    ...
    vm_product_presentation_event_sink event_sink, C_VOID *event_context)
{
    ...
    *out_presentation = STD_NULL;
    ...
    ux_hotkey_registry_initialize(&presentation->hotkeys);
    (C_VOID)ux_hotkey_registry_register(&presentation->hotkeys, 'P',
        UX_HOTKEY_MODIFIER_CONTROL | UX_HOTKEY_MODIFIER_ALT, "pause");
    (C_VOID)ux_hotkey_registry_register(&presentation->hotkeys, 'D',
        UX_HOTKEY_MODIFIER_CONTROL | UX_HOTKEY_MODIFIER_ALT, "cad");
    (C_VOID)ux_hotkey_registry_register(&presentation->hotkeys, 'F',
        UX_HOTKEY_MODIFIER_CONTROL | UX_HOTKEY_MODIFIER_ALT, "alt-enter");
    (C_VOID)ux_hotkey_registry_register(&presentation->hotkeys, 'M',
        UX_HOTKEY_MODIFIER_CONTROL | UX_HOTKEY_MODIFIER_ALT, "release-mouse");
    ...
    vm_product_presentation_target target)
{
    ...
    if (presentation->frame.valid) {
        return target == VM_PRODUCT_PRESENTATION_WINDOW ?
            (type_status)ux_window_publish_frame(presentation->window, &presentation->frame) :
            target == VM_PRODUCT_PRESENTATION_CONSOLE ?
            (type_status)ux_console_publish_frame(presentation->console, &presentation->frame) :
            TYPE_STATUS_OK;
    }
    return TYPE_STATUS_OK;
}
    ...
    vm_product_presentation *presentation, const C_CHAR *title)
{
    return presentation == STD_NULL || title == STD_NULL ? TYPE_STATUS_INVALID_ARGUMENT :
        ...
        (type_status)ux_window_set_title(presentation->window, title);
}
    ...
    vm_product_presentation *presentation, C_INT capturable)
{
    return presentation == STD_NULL ? TYPE_STATUS_INVALID_ARGUMENT :
        presentation->window == STD_NULL ? TYPE_STATUS_OK : (type_status)(capturable ?
            ux_window_unfreeze(presentation->window) : ux_window_freeze(presentation->window));
}
    ...
    vm_product_presentation *presentation)
{
    return presentation == STD_NULL ? TYPE_STATUS_INVALID_ARGUMENT :
        presentation->window == STD_NULL ? TYPE_STATUS_OK :
        (type_status)ux_window_release_mouse(presentation->window);
}
