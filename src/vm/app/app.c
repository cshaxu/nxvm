...
{ return app == STD_NULL || app->machine == STD_NULL ? TYPE_STATUS_INVALID_STATE : vm_machine_run_debugger(app->machine); }
type_status vm_app_record_start(vm_app *app, const C_CHAR *path)
{ return app == STD_NULL || app->machine == STD_NULL ? TYPE_STATUS_INVALID_STATE : vm_machine_record_start(app->machine, path); }
type_status vm_app_record_stop(vm_app *app)
{ return app == STD_NULL || app->machine == STD_NULL ? TYPE_STATUS_INVALID_STATE : vm_machine_record_stop(app->machine); }
