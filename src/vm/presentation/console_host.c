...
static C_VOID vm_presentation_console_host_event(C_VOID *opaque,
    const lib_console_event *event)
{
    vm_presentation_console_host *host = opaque;
    ...
    C_VOID *line_context, vm_presentation_console_line_sink line_sink)
{
    ...
    *out_host = STD_NULL;
    ...
    if (lib_console_create(&host->monitor) != LIB_STATUS_OK ||
        lib_console_set_event_sink(host->monitor, vm_presentation_console_host_event,
            host) != LIB_STATUS_OK ||
        host_console_broker_create(&host->broker, host->monitor,
            HOST_CONSOLE_COOKED_LINES) != LIB_STATUS_OK) {
        vm_presentation_console_host_destroy(host);
