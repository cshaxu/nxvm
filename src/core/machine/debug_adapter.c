    ...
    const x86_debug_request *request, x86_debug_response *out_result)
{
    ...
    if (request->operation == X86_DEBUG_READ_LINEAR || request->operation == X86_DEBUG_WRITE_LINEAR) {
        ...
        if ((request->operation == X86_DEBUG_READ_LINEAR ? core_machine_debug_read_linear(machine->core_machine, request->address, out_result->data, request->bytes) : core_machine_debug_write_linear(machine->core_machine, request->address, request->data, request->bytes)) != TYPE_STATUS_OK) return LIB_STATUS_INVALID_STATE;
    ...
    if (request->operation == X86_DEBUG_READ_REAL || request->operation == X86_DEBUG_WRITE_REAL) {
        if (request->bytes == 0u) return LIB_STATUS_INVALID_ARGUMENT;
        if ((request->operation == X86_DEBUG_READ_REAL ? core_machine_debug_read_real(machine->core_machine, request->segment, request->offset, out_result->data, request->bytes) : core_machine_debug_write_real(machine->core_machine, request->segment, request->offset, request->data, request->bytes)) != TYPE_STATUS_OK) return LIB_STATUS_INVALID_STATE;
        out_result->bytes = request->bytes; return LIB_STATUS_OK;
    }
    ...
    if (request->operation == X86_DEBUG_WRITE_PORT) return core_machine_debug_write_port(machine->core_machine, request->port, request->address) == TYPE_STATUS_OK ? LIB_STATUS_OK : LIB_STATUS_INVALID_STATE;
    ...
    if (request->operation == X86_DEBUG_GET_CODE_BASE) return core_machine_debug_get_code_base(machine->core_machine, &out_result->value) == TYPE_STATUS_OK ? LIB_STATUS_OK : LIB_STATUS_INVALID_STATE;
    ...
    if (request->operation == X86_DEBUG_SET_WATCH || request->operation == X86_DEBUG_CLEAR_WATCH) {
        ...
        return (request->operation == X86_DEBUG_SET_WATCH ? core_machine_debug_set_watchpoint(machine->core_machine, watch_kind, request->address) : core_machine_debug_clear_watchpoint(machine->core_machine, watch_kind)) == TYPE_STATUS_OK ? LIB_STATUS_OK : LIB_STATUS_INVALID_STATE;
    }
