...
void core_machine_memory_allocate_for(t_ram *ram, size_t newsize) {
    ...
    if (newsize) {
        ...
        if (ram->connect.pBase) {
            STD_FREE((void *) ram->connect.pBase);
        }
        ram->connect.pBase = (ntvdm64_type_virtual_address) STD_MALLOC(ram->connect.size);
        STD_MEMSET((void *) ram->connect.pBase, NTVDM64_TYPE_ZERO_8, ram->connect.size);
    }
}
...
{
    if (ram == NULL) return;
    STD_MEMCPY((void *) core_machine_memory_address(ram, physical),
        (void *) source, byte);
}
...
{
    if (ram == NULL) return;
    STD_MEMSET((void *)ram, NTVDM64_TYPE_ZERO_8, sizeof(*ram));
...
void core_machine_memory_reset(t_ram *ram)
{
    ...
    STD_MEMSET((void *)&ram->data, NTVDM64_TYPE_ZERO_8, sizeof(ram->data));
    STD_MEMSET((void *)ram->connect.pBase, NTVDM64_TYPE_ZERO_8, ram->connect.size);
}
    ...
    uint16_t offset, void *out_data, size_t size)
{
    ...
    STD_MEMCPY(out_data, (void *)(ram->connect.pBase + physical), size);
}
    ...
    uint16_t offset, const void *in_data, size_t size)
{
    ...
    physical = core_machine_memory_wrap_a20(ram,
        (NTVDM64_TYPE_MASK_UNSIGNED_16(segment) << 4) + NTVDM64_TYPE_MASK_UNSIGNED_16(offset));
    physical %= ram->connect.size;
    STD_MEMCPY((void *)(ram->connect.pBase + physical), (void *)in_data, size);
}
