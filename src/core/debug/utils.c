...
#define CORE_DEBUG_XASM_STATEMENT_CAPACITY \
    (CORE_DEBUG_XASM_MAX_STATEMENT_BYTES + 1u)

static type_status core_debug_validate_statement(const C_CHAR *statement,
    STD_SIZE_T statement_bytes, C_INT paragraph)
{
    STD_SIZE_T index;
    STD_SIZE_T line_bytes = 0u;

    if (statement == STD_NULL || statement_bytes == 0u) return TYPE_STATUS_INVALID_ARGUMENT;
    for (index = 0u; index < statement_bytes; ++index) {
        if (statement[index] == '\0') return TYPE_STATUS_INVALID_ARGUMENT;
        if (statement[index] == '\n' && paragraph) {
            if (line_bytes > CORE_DEBUG_XASM_MAX_STATEMENT_BYTES) return TYPE_STATUS_INVALID_ARGUMENT;
            line_bytes = 0u;
        } else if (++line_bytes > CORE_DEBUG_XASM_MAX_STATEMENT_BYTES) {
...
type_status core_debug_assemble(const C_CHAR *statement,
    STD_SIZE_T statement_bytes, type_unsigned_8 *code,
    STD_SIZE_T code_capacity, STD_SIZE_T *out_code_bytes, C_INT flag32)
{
    C_CHAR local_statement[CORE_DEBUG_XASM_STATEMENT_CAPACITY];
    type_unsigned_8 local_code[CORE_DEBUG_XASM_MAX_CODE_BYTES];
    ...
    status = core_debug_validate_statement(statement, statement_bytes, TYPE_FALSE);
...
type_status core_debug_assemble_paragraph(const C_CHAR *statement,
    STD_SIZE_T statement_bytes, type_unsigned_8 *code,
    STD_SIZE_T code_capacity, STD_SIZE_T *out_code_bytes, C_INT flag32)
{
    ...
    if (code == STD_NULL || out_code_bytes == STD_NULL || code_capacity == 0u ||
        ...
        (flag32 != TYPE_FALSE && flag32 != TYPE_TRUE)) return TYPE_STATUS_INVALID_ARGUMENT;
    status = core_debug_validate_statement(statement, statement_bytes, TYPE_TRUE);
    if (status != TYPE_STATUS_OK) return status;
...
type_status core_debug_disassemble(const type_unsigned_8 *code,
    ...
    STD_SIZE_T *out_statement_bytes, C_INT flag32)
{
    ...
    if (code == STD_NULL || statement == STD_NULL || out_statement_bytes == STD_NULL ||
        code_bytes < CORE_DEBUG_XASM_MAX_CODE_BYTES || statement_capacity == 0u ||
        (flag32 != TYPE_FALSE && flag32 != TYPE_TRUE)) return TYPE_STATUS_INVALID_ARGUMENT;
    decoded_bytes = dasm32(local_statement, (type_unsigned_8 *)code, flag32);
    statement_bytes = STD_STRLEN(local_statement);
    if (decoded_bytes == 0u || statement_bytes >= statement_capacity) return TYPE_STATUS_FAULT;
    ...
    STD_SIZE_T destination_capacity, const C_CHAR *source)
{
    ...
    if (destination_bytes >= destination_capacity ||
        source_bytes >= destination_capacity - destination_bytes) return TYPE_STATUS_INVALID_ARGUMENT;
