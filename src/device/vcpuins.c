...
#define UTILS_TRACE_ERROR  vcpuins.except
#define UTILS_TRACE_SETERR (_SetExcept_CE(0xffffffff))

static t_utils_trace UTILS_TRACE_VAR;

#define _newins_ /* if (1) {PRINTF("NEWINS: ");utilsTracePrintCall();} */
...
#ifndef GLOBAL_BOCHS
    _chr(_kma_read_ref(vramAddr(physical), rdata, byte));
#else
    _chr(vcpuapiReadPhysical(physical, rdata, byte));
#endif
    _ce;
}
...
#ifndef GLOBAL_BOCHS
    _chr(_kma_write_ref(vramAddr(physical), rdata, byte));
#else
    _chr(vcpuapiWritePhysical(physical, rdata, byte));
#endif
#endif
    _ce;
}
...
static void _kma_read_logical(t_cpu_sreg *rsreg, t_nubit32 offset, t_vaddrcc rdata, t_nubit8 byte, t_nubit8 vpl, t_bool force) {
    ...
    if (!force) {
        _bb("!force");
        vcpu.mem[vcpu.msize].flagwrite = 0;
        vcpu.mem[vcpu.msize].data = 0;
        _chr(_kma_read_ref(rdata, GetRef(vcpu.mem[vcpu.msize].data), byte));
        vcpu.mem[vcpu.msize].byte = byte;
        vcpu.mem[vcpu.msize].linear = linear;
        if (vcpuins.flagwr) {
        ...
        /*for (i = 0;i < vcpu.msize;++i) {
            if (vcpu.mem[i].flagwrite == vcpu.mem[vcpu.msize].flagwrite &&
                vcpu.mem[i].linear == vcpu.mem[vcpu.msize].linear) {
                _bb("mem(same)");
                _impossible_rz_;
                _ce;
            }
        ...
        vcpu.msize++;
        if (vcpu.msize == 0x20) _impossible_r_;
        _be;
    }
    _ce;
}
...
static void _kma_write_logical(t_cpu_sreg *rsreg, t_nubit32 offset, t_vaddrcc rdata, t_nubit8 byte, t_nubit8 vpl, t_bool force) {
    ...
    if (!force) {
        _bb("!force");
        vcpu.mem[vcpu.msize].flagwrite = 1;
        vcpu.mem[vcpu.msize].data = 0;
        _chr(_kma_read_ref(rdata, GetRef(vcpu.mem[vcpu.msize].data), byte));
        vcpu.mem[vcpu.msize].byte = byte;
        vcpu.mem[vcpu.msize].linear = linear;
...
static void _m_read_rm(t_nubit8 byte) {
    ...
    vcpuins.crm = 0;
    if (vcpuins.flagmem)
        _chr(_m_read_logical(vcpuins.mrm.rsreg, vcpuins.mrm.offset, GetRef(vcpuins.crm), byte));
    else
        _chr(_m_read_ref(vcpuins.rrm, GetRef(vcpuins.crm), byte));
    _ce;
}
static void _m_write_rm(t_nubit8 byte) {
    _cb("_m_write_rm");
    if (vcpuins.flagmem)
        _chr(_m_write_logical(vcpuins.mrm.rsreg, vcpuins.mrm.offset, GetRef(vcpuins.crm), byte));
    else
        _chr(_m_write_ref(vcpuins.rrm, GetRef(vcpuins.crm), byte));
    _ce;
}
...
static void _ksa_read_idt(t_nubit8 intid, t_vaddrcc rdata) {
    ...
    if (!_GetCR0_PE) {
        ...
        _chr(_kma_read_logical(&vcpu.idtr, (intid * 4), rdata, 4, 0x00, 0));
        _be;
    } else {
        _bb("CR0_PE(1)");
        if (GetMax16(intid * 8 + 7) > GetMax16(vcpu.idtr.limit)) _impossible_r_;
        _chr(_kma_read_logical(&vcpu.idtr, (intid * 8), rdata, 8, 0x00, 0));
        _be;
    }
    _ce;
}
static void _ksa_read_ldt(t_nubit16 selector, t_vaddrcc rdata) {
    ...
    _chr(_kma_read_logical(&vcpu.ldtr, _GetSelector_Offset(selector), rdata, 8, 0x00, 1));
    _ce;
}
static void _ksa_read_gdt(t_nubit16 selector, t_vaddrcc rdata) {
    ...
    _chr(_kma_read_logical(&vcpu.gdtr, _GetSelector_Offset(selector), rdata, 8, 0x00, 1));
    _ce;
}
...
static void _ksa_write_ldt(t_nubit16 selector, t_vaddrcc rdata) {
    ...
    if (GetMax16(_GetSelector_Offset(selector) + 7) > GetMax16(vcpu.ldtr.limit))
        _impossible_r_;
    _chr(_kma_write_logical(&vcpu.ldtr, _GetSelector_Offset(selector), rdata, 8, 0x00, 1));
    _ce;
}
static void _ksa_write_gdt(t_nubit16 selector, t_vaddrcc rdata) {
    ...
    if (GetMax16(_GetSelector_Offset(selector) + 7) > GetMax16(vcpu.gdtr.limit))
        _impossible_r_;
    _chr(_kma_write_logical(&vcpu.gdtr, _GetSelector_Offset(selector), rdata, 8, 0x00, 1));
    _ce;
}
...
static void _s_write_es(t_nubit32 offset, t_vaddrcc rdata, t_nubit8 byte) {
    _cb("_s_write_es");
    _chr(_m_write_logical(&vcpu.es, offset, rdata, byte));
    _ce;
}
...
static void _s_write_ss(t_nubit32 offset, t_vaddrcc rdata, t_nubit8 byte) {
    _cb("_s_write_ss");
    _chr(_m_write_logical(&vcpu.ss, offset, rdata, byte));
    _ce;
}
...
static void _s_test_tss(t_nubit32 offset, t_nubit8 byte) {
    _cb("_s_test_cs");
    _chr(_kma_test_logical(&vcpu.tr, offset, byte, 0, 0x00, 1));
    _ce;
}
static void _s_test_cs(t_nubit32 offset, t_nubit8 byte) {
    _cb("_s_test_cs");
    _chr(_kma_test_logical(&vcpu.cs, offset, byte, 0, 0x00, 1));
    _ce;
}
static void _s_test_ss_push(t_nubit8 byte) {
    ...
    case 2:
        ...
        _chr(_m_test_access(&vcpu.ss, GetMax16(vcpu.sp - byte), byte, 1));
        _be;
        break;
    case 4:
        _bb("StackSize(4)");
        if (vcpu.esp && vcpu.esp < byte)
            _chr(_SetExcept_SS(0));
        _chr(_m_test_access(&vcpu.ss, GetMax32(vcpu.esp - byte), byte, 1));
        _be;
        break;
    default:
        _impossible_r_;
...
static void _s_test_ss_pop(t_nubit8 byte) {
    ...
    case 2:
        _bb("StackSize(2)");
        _chr(_m_test_access(&vcpu.ss, vcpu.sp, byte, 0));
        _be;
        break;
    case 4:
        _bb("StackSize(4)");
        _chr(_m_test_access(&vcpu.ss, vcpu.esp, byte, 0));
        _be;
        break;
    default:
        _impossible_r_;
...
static void _s_load_gdtr(t_nubit32 base, t_nubit16 limit, t_nubit8 byte) {
    ...
    switch (byte) {
    case 2:
        vcpu.gdtr.base = GetMax24(base);
        break;
    case 4:
        vcpu.gdtr.base = GetMax32(base);
        break;
    default:
        _bb("byte");
...
static void _s_load_idtr(t_nubit32 base, t_nubit16 limit, t_nubit8 byte) {
    ...
    switch (byte) {
    case 2:
        vcpu.idtr.base = GetMax24(base);
        break;
    case 4:
        vcpu.idtr.base = GetMax32(base);
        break;
    default:
        _bb("byte");
        _chr(_SetExcept_CE(byte));
...
static void _s_load_ldtr(t_nubit16 selector) {
    ...
    _chr(_s_load_sreg(&vcpu.ldtr, selector));
    _ce;
}
static void _s_load_tr(t_nubit16 selector) {
    ...
    _chr(_s_load_sreg(&vcpu.tr, selector));
    _ce;
}
...
static void _s_load_cs(t_nubit16 newcs) {
    _cb("_s_load_cs");
    _chr(_s_load_sreg(&vcpu.cs, newcs));
    _ce;
}
static void _s_load_ss(t_nubit16 newss) {
    _cb("_s_load_ss");
    _chr(_s_load_sreg(&vcpu.ss, newss));
    _ce;
}
static void _s_load_ds(t_nubit16 newds) {
    _cb("_s_load_ds");
    _chr(_s_load_sreg(&vcpu.ds, newds));
    _ce;
}
static void _s_load_es(t_nubit16 newes) {
    _cb("_s_load_es");
    _chr(_s_load_sreg(&vcpu.es, newes));
    _ce;
}
static void _s_load_fs(t_nubit16 newfs) {
    _cb("_s_load_fs");
    _chr(_s_load_sreg(&vcpu.fs, newfs));
    _ce;
}
static void _s_load_gs(t_nubit16 newgs) {
    _cb("_s_load_gs");
    _chr(_s_load_sreg(&vcpu.gs, newgs));
    _ce;
}
static void _s_test_eip() {
    _cb("_s_test_eip");
    _chr(_s_test_cs(vcpu.eip, 0x01));
    _ce;
}
static void _s_test_esp() {
    ...
    switch (_GetStackSize) {
    case 2:
        cesp = GetMax16(vcpu.esp);
        break;
    case 4:
        cesp = GetMax32(vcpu.esp);
        break;
    default:
        _impossible_r_;
        break;
    }
    _chr(_m_test_logical(&vcpu.ss, cesp, 0x00, 0));
    _ce;
}
...
static void _p_input(t_nubit16 portid, t_vaddrcc rdata, t_nubit8 byte) {
    ...
    case 1:
        _bb("byte(1)");
        _chr(_m_write_ref(rdata, GetRef(vport.ioByte), 1));
        _be;
        break;
    case 2:
        _bb("byte(2)");
        _chr(_m_write_ref(rdata, GetRef(vport.ioWord), 2));
        _be;
        break;
    case 4:
        _bb("byte(4)");
        _chr(_m_write_ref(rdata, GetRef(vport.ioDWord), 4));
        _be;
        break;
    default:
        _bb("byte");
...
static void _p_output(t_nubit16 portid, t_vaddrcc rdata, t_nubit8 byte) {
    ...
    case 1:
        _bb("byte(1)");
        _chr(_m_read_ref(rdata, GetRef(vport.ioByte), 1));
        _be;
        break;
    case 2:
        _bb("byte(2)");
        _chr(_m_read_ref(rdata, GetRef(vport.ioWord), 2));
        _be;
        break;
    case 4:
        _bb("byte(4)");
        _chr(_m_read_ref(rdata, GetRef(vport.ioDWord), 4));
        _be;
        break;
    default:
        _bb("byte");
...
static void _kdf_code(t_vaddrcc rdata, t_nubit8 byte) {
    _cb("_kdf_code");
    _chr(_s_read_cs(vcpu.eip, rdata, byte));
    _chr(_kdf_skip(byte));
    _ce;
}
static void _kdf_modrm(t_nubit8 regbyte, t_nubit8 rmbyte) {
    ...
    case 2:
        ...
        case 0:
            ...
            case 0:
                vcpuins.mrm.offset = GetMax16(vcpu.bx + vcpu.si);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 1:
                vcpuins.mrm.offset = GetMax16(vcpu.bx + vcpu.di);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 2:
                vcpuins.mrm.offset = GetMax16(vcpu.bp + vcpu.si);
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 3:
                vcpuins.mrm.offset = GetMax16(vcpu.bp + vcpu.di);
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 4:
                vcpuins.mrm.offset = GetMax16(vcpu.si);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 5:
                vcpuins.mrm.offset = GetMax16(vcpu.di);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 6:
                _bb("ModRM_RM(6)");
                _chr(_kdf_code(GetRef(disp16), 2));
                vcpuins.mrm.offset = GetMax16(disp16);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                _be;
                break;
            case 7:
                vcpuins.mrm.offset = GetMax16(vcpu.bx);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            default:
                _impossible_r_;
        ...
        case 1:
            ...
            switch (_GetModRM_RM(modrm)) {
            case 0:
                vcpuins.mrm.offset = GetMax16(vcpu.bx + vcpu.si + disp8);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 1:
                vcpuins.mrm.offset = GetMax16(vcpu.bx + vcpu.di + disp8);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 2:
                vcpuins.mrm.offset = GetMax16(vcpu.bp + vcpu.si + disp8);
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 3:
                vcpuins.mrm.offset = GetMax16(vcpu.bp + vcpu.di + disp8);
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 4:
                vcpuins.mrm.offset = GetMax16(vcpu.si + disp8);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 5:
                vcpuins.mrm.offset = GetMax16(vcpu.di + disp8);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 6:
                vcpuins.mrm.offset = GetMax16(vcpu.bp + disp8);
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 7:
                vcpuins.mrm.offset = GetMax16(vcpu.bx + disp8);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            default:
                _impossible_r_;
        ...
        case 2:
            ...
            switch (_GetModRM_RM(modrm)) {
            case 0:
                vcpuins.mrm.offset = GetMax16(vcpu.bx + vcpu.si + disp16);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 1:
                vcpuins.mrm.offset = GetMax16(vcpu.bx + vcpu.di + disp16);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 2:
                vcpuins.mrm.offset = GetMax16(vcpu.bp + vcpu.si + disp16);
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 3:
                vcpuins.mrm.offset = GetMax16(vcpu.bp + vcpu.di + disp16);
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 4:
                vcpuins.mrm.offset = GetMax16(vcpu.si + disp16);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 5:
                vcpuins.mrm.offset = GetMax16(vcpu.di + disp16);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 6:
                vcpuins.mrm.offset = GetMax16(vcpu.bp + disp16);
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 7:
                vcpuins.mrm.offset = GetMax16(vcpu.bx + disp16);
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            default:
                _impossible_r_;
    ...
    case 4:
        ...
        case 0:
            ...
            switch (_GetModRM_RM(modrm)) {
            case 0:
                vcpuins.mrm.offset = vcpu.eax;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 1:
                vcpuins.mrm.offset = vcpu.ecx;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 2:
                vcpuins.mrm.offset = vcpu.edx;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 3:
                vcpuins.mrm.offset = vcpu.ebx;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 4:
                _bb("ModRM_RM(4)");
                ...
                case 0:
                    vcpuins.mrm.offset = vcpu.eax + sibindex;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 1:
                    vcpuins.mrm.offset = vcpu.ecx + sibindex;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 2:
                    vcpuins.mrm.offset = vcpu.edx + sibindex;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 3:
                    vcpuins.mrm.offset = vcpu.ebx + sibindex;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 4:
                    vcpuins.mrm.offset = vcpu.esp + sibindex;
                    vcpuins.mrm.rsreg = vcpuins.roverss;
                    break;
                case 5:
                    _bb("SIB_Base(5)");
                    _chr(_kdf_code(GetRef(disp32), 4));
                    vcpuins.mrm.offset = disp32 + sibindex;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    _be;
                    break;
                case 6:
                    vcpuins.mrm.offset = vcpu.esi + sibindex;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 7:
                    vcpuins.mrm.offset = vcpu.edi + sibindex;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                default:
                    _impossible_r_;
            ...
            case 5:
                _bb("ModRM_RM(5)");
                _chr(_kdf_code(GetRef(disp32), 4));
                vcpuins.mrm.offset = disp32;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                _be;
                break;
            case 6:
                vcpuins.mrm.offset = vcpu.esi;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 7:
                vcpuins.mrm.offset = vcpu.edi;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            default:
                _impossible_r_;
        ...
        case 1:
            ...
            switch (_GetModRM_RM(modrm)) {
            case 0:
                vcpuins.mrm.offset = vcpu.eax + disp8;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 1:
                vcpuins.mrm.offset = vcpu.ecx + disp8;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 2:
                vcpuins.mrm.offset = vcpu.edx + disp8;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 3:
                vcpuins.mrm.offset = vcpu.ebx + disp8;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 4:
                _bb("ModRM_RM(4)");
                switch (_GetSIB_Base(sib)) {
                case 0:
                    vcpuins.mrm.offset = vcpu.eax + sibindex + disp8;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 1:
                    vcpuins.mrm.offset = vcpu.ecx + sibindex + disp8;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 2:
                    vcpuins.mrm.offset = vcpu.edx + sibindex + disp8;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 3:
                    vcpuins.mrm.offset = vcpu.ebx + sibindex + disp8;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 4:
                    vcpuins.mrm.offset = vcpu.esp + sibindex + disp8;
                    vcpuins.mrm.rsreg = vcpuins.roverss;
                    break;
                case 5:
                    vcpuins.mrm.offset = vcpu.ebp + sibindex + disp8;
                    vcpuins.mrm.rsreg = vcpuins.roverss;
                    break;
                case 6:
                    vcpuins.mrm.offset = vcpu.esi + sibindex + disp8;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 7:
                    vcpuins.mrm.offset = vcpu.edi + sibindex + disp8;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                default:
                    _impossible_r_;
                ...
                break;
            case 5:
                vcpuins.mrm.offset = vcpu.ebp + disp8;
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 6:
                vcpuins.mrm.offset = vcpu.esi + disp8;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 7:
                vcpuins.mrm.offset = vcpu.edi + disp8;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            default:
                _impossible_r_;
        ...
        case 2:
            ...
            switch (_GetModRM_RM(modrm)) {
            case 0:
                vcpuins.mrm.offset = vcpu.eax + disp32;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 1:
                vcpuins.mrm.offset = vcpu.ecx + disp32;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 2:
                vcpuins.mrm.offset = vcpu.edx + disp32;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 3:
                vcpuins.mrm.offset = vcpu.ebx + disp32;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 4:
                _bb("ModRM_RM(4)");
                switch (_GetSIB_Base(sib)) {
                case 0:
                    vcpuins.mrm.offset = vcpu.eax + sibindex + disp32;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 1:
                    vcpuins.mrm.offset = vcpu.ecx + sibindex + disp32;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 2:
                    vcpuins.mrm.offset = vcpu.edx + sibindex + disp32;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 3:
                    vcpuins.mrm.offset = vcpu.ebx + sibindex + disp32;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 4:
                    vcpuins.mrm.offset = vcpu.esp + sibindex + disp32;
                    vcpuins.mrm.rsreg = vcpuins.roverss;
                    break;
                case 5:
                    vcpuins.mrm.offset = vcpu.ebp + sibindex + disp32;
                    vcpuins.mrm.rsreg = vcpuins.roverss;
                    break;
                case 6:
                    vcpuins.mrm.offset = vcpu.esi + sibindex + disp32;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                case 7:
                    vcpuins.mrm.offset = vcpu.edi + sibindex + disp32;
                    vcpuins.mrm.rsreg = vcpuins.roverds;
                    break;
                default:
                    _impossible_r_;
                ...
                break;
            case 5:
                vcpuins.mrm.offset = vcpu.ebp + disp32;
                vcpuins.mrm.rsreg = vcpuins.roverss;
                break;
            case 6:
                vcpuins.mrm.offset = vcpu.esi + disp32;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            case 7:
                vcpuins.mrm.offset = vcpu.edi + disp32;
                vcpuins.mrm.rsreg = vcpuins.roverds;
                break;
            default:
                _impossible_r_;
    ...
    if (_GetModRM_MOD(modrm) == 3) {
        ...
        case 1:
            switch (_GetModRM_RM(modrm)) {
            case 0:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.al);
                break;
            case 1:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.cl);
                break;
            case 2:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.dl);
                break;
            case 3:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.bl);
                break;
            case 4:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.ah);
                break;
            case 5:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.ch);
                break;
            case 6:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.dh);
                break;
            case 7:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.bh);
                break;
            default:
                _impossible_r_;
                break;
        ...
        case 2:
            switch (_GetModRM_RM(modrm)) {
            case 0:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.ax);
                break;
            case 1:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.cx);
                break;
            case 2:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.dx);
                break;
            case 3:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.bx);
                break;
            case 4:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.sp);
                break;
            case 5:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.bp);
                break;
            case 6:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.si);
                break;
            case 7:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.di);
                break;
            default:
                _impossible_r_;
                break;
        ...
        case 4:
            switch (_GetModRM_RM(modrm)) {
            case 0:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.eax);
                break;
            case 1:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.ecx);
                break;
            case 2:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.edx);
                break;
            case 3:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.ebx);
                break;
            case 4:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.esp);
                break;
            case 5:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.ebp);
                break;
            case 6:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.esi);
                break;
            case 7:
                vcpuins.rrm = (t_vaddrcc)(&vcpu.edi);
                break;
            default:
                _impossible_r_;
                break;
        ...
        _chr(_m_read_ref(vcpuins.rrm, GetRef(vcpuins.crm), rmbyte));
        _be;
    }
    if (!regbyte) {
        ...
        case 4:
            switch (_GetModRM_REG(modrm)) {
            case 0:
                vcpuins.rr = (t_vaddrcc)(&vcpu.eax);
                break;
            case 1:
                vcpuins.rr = (t_vaddrcc)(&vcpu.ecx);
                break;
            case 2:
                vcpuins.rr = (t_vaddrcc)(&vcpu.edx);
                break;
            case 3:
                vcpuins.rr = (t_vaddrcc)(&vcpu.ebx);
                break;
            case 4:
                vcpuins.rr = (t_vaddrcc)(&vcpu.esp);
                break;
            case 5:
                vcpuins.rr = (t_vaddrcc)(&vcpu.ebp);
                break;
            case 6:
                vcpuins.rr = (t_vaddrcc)(&vcpu.esi);
                break;
            case 7:
                vcpuins.rr = (t_vaddrcc)(&vcpu.edi);
                break;
            default:
                _impossible_r_;
                break;
        ...
        _chr(_m_read_ref(vcpuins.rr, GetRef(vcpuins.cr), regbyte));
    }
    _ce;
}
...
static void _d_imm(t_nubit8 byte) {
    _cb("_d_imm");
    vcpuins.cimm = 0;
    _chr(_d_code(GetRef(vcpuins.cimm), byte));
    _ce;
}
static void _d_moffs(t_nubit8 byte) {
    ...
    case 2:
        _bb("AddressSize(2)");
        _chr(_d_code(GetRef(vcpuins.mrm.offset), 2));
        _be;
        break;
    case 4:
        _bb("AddressSize(4)");
        _chr(_d_code(GetRef(vcpuins.mrm.offset), 4));
        _be;
        break;
    default:
        _impossible_r_;
...
static void _d_modrm_sreg(t_nubit8 rmbyte) {
    ...
    switch (vcpuins.cr) {
    case 0:
        vcpuins.rmovsreg = &vcpu.es;
        break;
    case 1:
        vcpuins.rmovsreg = &vcpu.cs;
        break;
    case 2:
        vcpuins.rmovsreg = &vcpu.ss;
        break;
    case 3:
        vcpuins.rmovsreg = &vcpu.ds;
        break;
    case 4:
        vcpuins.rmovsreg = &vcpu.fs;
        break;
    case 5:
        vcpuins.rmovsreg = &vcpu.gs;
        break;
    default:
        _bb("vcpuins.cr");
...
static void _kec_push(t_vaddrcc rdata, t_nubit8 byte) {
    ...
    switch (_GetStackSize) {
    case 2:
        cesp = GetMax16(vcpu.sp - byte);
        break;
    case 4:
        cesp = GetMax32(vcpu.esp - byte);
        break;
    default:
        _impossible_r_;
        break;
...
static void _kec_call_far(t_nubit16 newcs, t_nubit32 neweip, t_nubit8 byte) {
    ...
    case 2:
        ...
        _chr(_kec_push(GetRef(vcpu.ip), 2));
        _be;
        break;
    case 4:
        _bb("byte(4)");
        ...
        _chr(_kec_push(GetRef(vcpu.eip), 4));
        _be;
        break;
    default:
        _bb("bit");
...
static void _ser_int_real(t_nubit8 intid, t_nubit8 byte) {
    ...
    case 2:
        ...
        _chr(_kec_push(GetRef(vcpu.flags), 2));
        _ClrEFLAGS_IF;
        _ClrEFLAGS_TF;
        _chr(_kec_push(GetRef(oldcs), 2));
        _chr(_kec_push(GetRef(vcpu.ip), 2));
    ...
    case 4:
        ...
        _chr(_kec_push(GetRef(vcpu.eflags), 4));
        _ClrEFLAGS_IF;
        _ClrEFLAGS_TF;
        _chr(_kec_push(GetRef(oldcs), 4));
        _chr(_kec_push(GetRef(vcpu.eip), 4));
    ...
    if (_GetDesc_Type(gate_desc) == VCPU_DESC_SYS_TYPE_TASKGATE) {
        ...
        if (_IsDescCodeNonConform(cs_desc) &&
            ...
            if (!_GetEFLAGS_VM) {
                ...
                oldesp = vcpu.esp;
                _chr(_ksa_load_sreg(&vcpu.ss, newss));
                vcpu.esp = newesp;
                if (_IsDescSys32(gate_desc)) {
                    _bb("DescSys32(gate)");
                    xs_sel = vcpu.gs.selector;
                    _chr(_kec_push(GetRef(xs_sel), 4));
                    xs_sel = vcpu.fs.selector;
                    _chr(_kec_push(GetRef(xs_sel), 4));
                    xs_sel = vcpu.ds.selector;
                    _chr(_kec_push(GetRef(xs_sel), 4));
                    xs_sel = vcpu.es.selector;
                    _chr(_kec_push(GetRef(xs_sel), 4));
                    xs_sel = oldss;
                    _chr(_kec_push(GetRef(xs_sel), 4));
                    _chr(_kec_push(GetRef(oldesp), 4));
                    _chr(_kec_push(GetRef(oldeflags), 4));
                    xs_sel = vcpu.cs.selector;
                    _chr(_kec_push(GetRef(xs_sel), 4));
                    _chr(_kec_push(GetRef(vcpu.eip), 4));
                    ...
                    _chr(_ksa_load_sreg(&vcpu.cs, newcs));
                    switch (byte) {
                    case 2:
                        vcpu.eip = GetMax16(_GetDescGate_Offset(gate_desc));
                        break;
                    case 4:
                        vcpu.eip = GetMax32(_GetDescGate_Offset(gate_desc));
                        break;
                    default:
                        _bb("byte");
                        _chr(_SetExcept_CE(byte));
...
static void _kaf_calc_CF() {
    ...
    default:
        _bb("type");
        _chr(_SetExcept_CE(vcpuins.type));
        _be;
        break;
    }
    _ce;
...
static void _kaf_calc_SF() {
    ...
    switch (vcpuins.bit) {
    case 8:
        MakeBit(vcpu.eflags, VCPU_EFLAGS_SF, GetMSB8(vcpuins.result));
        break;
    case 16:
        MakeBit(vcpu.eflags, VCPU_EFLAGS_SF, GetMSB16(vcpuins.result));
        break;
    case 32:
        MakeBit(vcpu.eflags, VCPU_EFLAGS_SF, GetMSB32(vcpuins.result));
        break;
    default:
        _bb("bit");
        _chr(_SetExcept_CE(vcpuins.bit));
        _be;
        break;
    }
    _ce;
...
static void _p_ins(t_nubit8 byte) {
    ...
    case 1:
        _bb("byte(1)");
        _chr(_m_test_access(&vcpu.es, cedi, 1, 1));
        _chr(_p_input(vcpu.dx, GetRef(data), 1));
    ...
    case 2:
        _bb("byte(2)");
        _chr(_m_test_access(&vcpu.es, cedi, 2, 1));
        _chr(_p_input(vcpu.dx, GetRef(data), 2));
    ...
    case 4:
        _bb("byte(4)");
        _chr(_m_test_access(&vcpu.es, cedi, 4, 1));
        _chr(_p_input(vcpu.dx, GetRef(data), 4));
...
static void _p_outs(t_nubit8 byte) {
    ...
    case 1:
        _bb("byte(1)");
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(data), 1));
        _chr(_p_output(vcpu.dx, GetRef(data), 1));
        _chr(_kas_move_index(1, 1, 0));
        _be;
        break;
    case 2:
        _bb("byte(2)");
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(data), 2));
        _chr(_p_output(vcpu.dx, GetRef(data), 2));
        _chr(_kas_move_index(2, 1, 0));
        _be;
        break;
    case 4:
        _bb("byte(4)");
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(data), 4));
        _chr(_p_output(vcpu.dx, GetRef(data), 4));
...
static void _m_stos(t_nubit8 byte) {
    ...
    case 1:
        _bb("byte(1)");
        _chr(_s_write_es(cedi, GetRef(vcpu.al), 1));
        _chr(_kas_move_index(1, 0, 1));
        _be;
        break;
    case 2:
        _bb("byte(2)");
        _chr(_s_write_es(cedi, GetRef(vcpu.ax), 2));
        _chr(_kas_move_index(2, 0, 1));
        _be;
        break;
    case 4:
        _bb("byte(4)");
        _chr(_s_write_es(cedi, GetRef(vcpu.eax), 4));
        _chr(_kas_move_index(4, 0, 1));
        _be;
        break;
    default:
...
static void _m_lods(t_nubit8 byte) {
    ...
    case 1:
        _bb("byte(1)");
        vcpuins.bit = 8;
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(vcpu.al), 1));
        _chr(_kas_move_index(1, 1, 0));
    ...
    case 2:
        _bb("byte(2)");
        vcpuins.bit = 16;
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(vcpu.ax), 2));
        _chr(_kas_move_index(2, 1, 0));
        vcpuins.result = vcpu.ax;
        _be;
        break;
    case 4:
        _bb("byte(4)");
        vcpuins.bit = 32;
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(vcpu.eax), 4));
        _chr(_kas_move_index(4, 1, 0));
...
static void _a_cmps(t_nubit8 bit) {
    ...
    case 8:
        ...
        vcpuins.bit = 8;
        vcpuins.type = CMP8;
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(vcpuins.opr1), 1));
        _chr(_s_read_es(cedi, GetRef(vcpuins.opr2), 1));
    ...
    case 16:
        ...
        vcpuins.bit = 16;
        vcpuins.type = CMP16;
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(vcpuins.opr1), 2));
        _chr(_s_read_es(cedi, GetRef(vcpuins.opr2), 2));
    ...
    case 32:
        ...
        vcpuins.bit = 32;
        vcpuins.type = CMP32;
        _chr(_m_read_logical(vcpuins.roverds, cesi, GetRef(vcpuins.opr1), 4));
        _chr(_s_read_es(cedi, GetRef(vcpuins.opr2), 4));
...
static void ADD_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_add(vcpuins.crm, vcpuins.cr, 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(1));
    _ce;
}
static void ADD_RM32_R32() {
    ...
    i386(0x01) {
        ...
        _chr(_a_add(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
        vcpuins.crm = vcpuins.result;
        ...
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_add(vcpuins.crm, vcpuins.cr, 16));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(2));
    }
    _ce;
...
static void ADD_R8_RM8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_add(vcpuins.cr, vcpuins.crm, 8));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 1));
    _ce;
}
static void ADD_R32_RM32() {
    ...
    i386(0x03) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_add(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_add(vcpuins.cr, vcpuins.crm, 16));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 2));
    }
    _ce;
}
static void ADD_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_add(vcpu.al, vcpuins.cimm, 8));
...
static void ADD_EAX_I32() {
    ...
    i386(0x05) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_a_add(vcpu.ax, vcpuins.cimm, 16));
        ...
        case 4:
            _bb("OperandSize(4)");
            _chr(_d_imm(4));
            _chr(_a_add(vcpu.eax, vcpuins.cimm, 32));
...
static void OR_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_or(vcpuins.crm, vcpuins.cr, 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(1));
    _ce;
}
static void OR_RM32_R32() {
    ...
    i386(0x09) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_or(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(_GetOperandSize));
    }
    else {
        ...
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_or(vcpuins.crm, vcpuins.cr, 16));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(2));
    }
    _ce;
...
static void OR_R8_RM8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_or(vcpuins.cr, vcpuins.crm, 8));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 1));
    _ce;
}
static void OR_R32_RM32() {
    ...
    i386(0x0b) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_or(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_or(vcpuins.cr, vcpuins.crm, 16));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 2));
    }
    _ce;
}
static void OR_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_or(vcpu.al, vcpuins.cimm, 8));
    vcpu.al = GetMax8(vcpuins.result);
    _ce;
}
static void OR_EAX_I32() {
    ...
    i386(0x0d) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_a_or(vcpu.ax, vcpuins.cimm, 16));
            vcpu.ax = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_d_imm(4));
            _chr(_a_or(vcpu.eax, vcpuins.cimm, 32));
            vcpu.eax = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void INS_0F() {
    ...
    i386(0x0f) {
        _adv;
        _chr(_s_read_cs(vcpu.eip, GetRef(opcode), 1));
        _chr(ExecFun(table_0f[opcode]));
    }
    else
        POP_CS();
    _ce;
...
static void ADC_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_adc(vcpuins.crm, vcpuins.cr, 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(1));
    _ce;
}
static void ADC_RM32_R32() {
    ...
    i386(0x11) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_adc(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(_GetOperandSize));
    }
    else {
        ...
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_adc(vcpuins.crm, vcpuins.cr, 16));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(2));
    }
    _ce;
...
static void ADC_R8_RM8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_adc(vcpuins.cr, vcpuins.crm, 8));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 1));
    _ce;
}
static void ADC_R32_RM32() {
    ...
    i386(0x13) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_adc(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_adc(vcpuins.cr, vcpuins.crm, 16));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 2));
    }
    _ce;
}
static void ADC_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_adc(vcpu.al, vcpuins.cimm, 8));
    vcpu.al = GetMax8(vcpuins.result);
    _ce;
}
static void ADC_EAX_I32() {
    ...
    i386(0x15) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_a_adc(vcpu.ax, vcpuins.cimm, 16));
            vcpu.ax = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_d_imm(4));
            _chr(_a_adc(vcpu.eax, vcpuins.cimm, 32));
            vcpu.eax = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void SBB_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_sbb(vcpuins.crm, vcpuins.cr, 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(1));
    _ce;
}
static void SBB_RM32_R32() {
    ...
    i386(0x19) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_sbb(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(_GetOperandSize));
    }
    else {
        ...
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_sbb(vcpuins.crm, vcpuins.cr, 16));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(2));
    }
    _ce;
...
static void SBB_R8_RM8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_sbb(vcpuins.cr, vcpuins.crm, 8));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 1));
    _ce;
}
static void SBB_R32_RM32() {
    ...
    i386(0x1b) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_sbb(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_sbb(vcpuins.cr, vcpuins.crm, 16));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 2));
    }
    _ce;
}
static void SBB_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_sbb(vcpu.al, vcpuins.cimm, 8));
    vcpu.al = GetMax8(vcpuins.result);
    _ce;
}
static void SBB_EAX_I32() {
    ...
    i386(0x1d) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_a_sbb(vcpu.ax, vcpuins.cimm, 16));
            vcpu.ax = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_d_imm(4));
            _chr(_a_sbb(vcpu.eax, vcpuins.cimm, 32));
            vcpu.eax = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void AND_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_and(vcpuins.crm, vcpuins.cr, 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(1));
    _ce;
}
static void AND_RM32_R32() {
    ...
    i386(0x21) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_and(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(_GetOperandSize));
    }
    else {
        ...
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_and(vcpuins.crm, vcpuins.cr, 16));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(2));
    }
    _ce;
...
static void AND_R8_RM8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_and(vcpuins.cr, vcpuins.crm, 8));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 1));
    _ce;
}
static void AND_R32_RM32() {
    ...
    i386(0x23) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_and(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_and(vcpuins.cr, vcpuins.crm, 16));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 2));
    }
    _ce;
}
static void AND_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_and(vcpu.al, vcpuins.cimm, 8));
    vcpu.al = GetMax8(vcpuins.result);
    _ce;
}
static void AND_EAX_I32() {
    ...
    i386(0x25) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_a_and(vcpu.ax, vcpuins.cimm, 16));
            vcpu.ax = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_d_imm(4));
            _chr(_a_and(vcpu.eax, vcpuins.cimm, 32));
            vcpu.eax = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void SUB_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_sub(vcpuins.crm, vcpuins.cr, 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(1));
    _ce;
}
static void SUB_RM32_R32() {
    ...
    i386(0x29) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_sub(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(_GetOperandSize));
    }
    else {
        ...
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_sub(vcpuins.crm, vcpuins.cr, 16));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(2));
    }
    _ce;
...
static void SUB_R8_RM8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_sub(vcpuins.cr, vcpuins.crm, 8));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 1));
    _ce;
}
static void SUB_R32_RM32() {
    ...
    i386(0x2b) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_sub(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_sub(vcpuins.cr, vcpuins.crm, 16));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 2));
    }
    _ce;
}
static void SUB_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_sub(vcpu.al, vcpuins.cimm, 8));
    vcpu.al = GetMax8(vcpuins.result);
    _ce;
}
static void SUB_EAX_I32() {
    ...
    i386(0x2d) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_a_sub(vcpu.ax, vcpuins.cimm, 16));
            vcpu.ax = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_d_imm(4));
            _chr(_a_sub(vcpu.eax, vcpuins.cimm, 32));
            vcpu.eax = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void XOR_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_xor(vcpuins.crm, vcpuins.cr, 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(1));
    _ce;
}
static void XOR_RM32_R32() {
    ...
    i386(0x31) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_xor(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(_GetOperandSize));
    }
    else {
        ...
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_xor(vcpuins.crm, vcpuins.cr, 16));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(2));
    }
    _ce;
...
static void XOR_R8_RM8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_xor(vcpuins.cr, vcpuins.crm, 8));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 1));
    _ce;
}
static void XOR_R32_RM32() {
    ...
    i386(0x33) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_xor(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_xor(vcpuins.cr, vcpuins.crm, 16));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), 2));
    }
    _ce;
}
static void XOR_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_xor(vcpu.al, vcpuins.cimm, 8));
    vcpu.al = GetMax8(vcpuins.result);
    _ce;
}
static void XOR_EAX_I32() {
    ...
    i386(0x35) {
        ...
        case 2:
            _bb("OperxorSize(2)");
            _chr(_d_imm(2));
            _chr(_a_xor(vcpu.ax, vcpuins.cimm, 16));
            vcpu.ax = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperxorSize(4)");
            _chr(_d_imm(4));
            _chr(_a_xor(vcpu.eax, vcpuins.cimm, 32));
            vcpu.eax = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void CMP_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_cmp(vcpuins.crm, vcpuins.cr, 8));
    _ce;
}
static void CMP_RM32_R32() {
    ...
    i386(0x39) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_cmp(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_cmp(vcpuins.crm, vcpuins.cr, 16));
    }
    _ce;
}
static void CMP_R8_RM8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_cmp(vcpuins.cr, vcpuins.crm, 8));
    _ce;
}
static void CMP_R32_RM32() {
    ...
    i386(0x3b) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_cmp(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
    }
    else {
        vcpu.ip++;
        _chr(_d_modrm(2, 2));
        _chr(_m_read_rm(2));
        _chr(_a_cmp(vcpuins.cr, vcpuins.crm, 16));
    }
    _ce;
}
static void CMP_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_cmp(vcpu.al, vcpuins.cimm, 8));
    _ce;
}
static void CMP_EAX_I32() {
    ...
    i386(0x3d) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_a_cmp(vcpu.ax, vcpuins.cimm, 16));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_d_imm(4));
            _chr(_a_cmp(vcpu.eax, vcpuins.cimm, 32));
            _be;
            break;
        default:
            _impossible_r_;
...
static void INC_EAX() {
    ...
    i386(0x40) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_inc(vcpu.ax, 16));
            vcpu.ax = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_inc(vcpu.eax, 32));
            vcpu.eax = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void INC_ECX() {
    ...
    i386(0x41) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_inc(vcpu.cx, 16));
            vcpu.cx = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_inc(vcpu.ecx, 32));
...
static void INC_EDX() {
    ...
    i386(0x42) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_inc(vcpu.dx, 16));
            vcpu.dx = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_inc(vcpu.edx, 32));
...
static void INC_EBX() {
    ...
    i386(0x43) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_inc(vcpu.bx, 16));
            vcpu.bx = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_inc(vcpu.ebx, 32));
...
static void INC_ESP() {
    ...
    i386(0x44) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_inc(vcpu.sp, 16));
            vcpu.sp = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_inc(vcpu.esp, 32));
...
static void INC_EBP() {
    ...
    i386(0x45) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_inc(vcpu.bp, 16));
            vcpu.bp = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_inc(vcpu.ebp, 32));
...
static void INC_ESI() {
    ...
    i386(0x46) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_inc(vcpu.si, 16));
            vcpu.si = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_inc(vcpu.esi, 32));
...
static void INC_EDI() {
    ...
    i386(0x47) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_inc(vcpu.di, 16));
            vcpu.di = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_inc(vcpu.edi, 32));
...
static void DEC_EAX() {
    ...
    i386(0x48) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_dec(vcpu.ax, 16));
            vcpu.ax = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_dec(vcpu.eax, 32));
            vcpu.eax = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void DEC_ECX() {
    ...
    i386(0x49) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_dec(vcpu.cx, 16));
            vcpu.cx = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_dec(vcpu.ecx, 32));
            vcpu.ecx = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void DEC_EDX() {
    ...
    i386(0x4a) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_dec(vcpu.dx, 16));
            vcpu.dx = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_dec(vcpu.edx, 32));
            vcpu.edx = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void DEC_EBX() {
    ...
    i386(0x4b) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_dec(vcpu.bx, 16));
            vcpu.bx = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_dec(vcpu.ebx, 32));
            vcpu.ebx = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void DEC_ESP() {
    ...
    i386(0x4c) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_dec(vcpu.sp, 16));
            vcpu.sp = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_dec(vcpu.esp, 32));
            vcpu.esp = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void DEC_EBP() {
    ...
    i386(0x4d) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_dec(vcpu.bp, 16));
            vcpu.bp = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_dec(vcpu.ebp, 32));
            vcpu.ebp = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void DEC_ESI() {
    ...
    i386(0x4e) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_dec(vcpu.si, 16));
            vcpu.si = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_dec(vcpu.esi, 32));
            vcpu.esi = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void DEC_EDI() {
    ...
    i386(0x4f) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_a_dec(vcpu.di, 16));
            vcpu.di = GetMax16(vcpuins.result);
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_a_dec(vcpu.edi, 32));
            vcpu.edi = GetMax32(vcpuins.result);
            _be;
            break;
        default:
...
static void PUSH_EAX() {
    ...
    i386(0x50) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_push(GetRef(vcpu.ax), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_push(GetRef(vcpu.eax), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void PUSH_ECX() {
    ...
    i386(0x51) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_push(GetRef(vcpu.cx), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_push(GetRef(vcpu.ecx), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void PUSH_EDX() {
    ...
    i386(0x52) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_push(GetRef(vcpu.dx), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_push(GetRef(vcpu.edx), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void PUSH_EBX() {
    ...
    i386(0x53) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_push(GetRef(vcpu.bx), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_push(GetRef(vcpu.ebx), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void PUSH_ESP() {
    ...
    i386(0x54) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_push(GetRef(vcpu.sp), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_push(GetRef(vcpu.esp), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void PUSH_EBP() {
    ...
    i386(0x55) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_push(GetRef(vcpu.bp), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_push(GetRef(vcpu.ebp), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void PUSH_ESI() {
    ...
    i386(0x56) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_push(GetRef(vcpu.si), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_push(GetRef(vcpu.esi), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void PUSH_EDI() {
    ...
    i386(0x57) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_push(GetRef(vcpu.di), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_push(GetRef(vcpu.edi), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void POP_EAX() {
    ...
    i386(0x58) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_pop(GetRef(vcpu.ax), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_pop(GetRef(vcpu.eax), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void POP_ECX() {
    ...
    i386(0x59) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_pop(GetRef(vcpu.cx), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_pop(GetRef(vcpu.ecx), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void POP_EDX() {
    ...
    i386(0x5a) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_pop(GetRef(vcpu.dx), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_pop(GetRef(vcpu.edx), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void POP_EBX() {
    ...
    i386(0x5b) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_pop(GetRef(vcpu.bx), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_pop(GetRef(vcpu.ebx), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void POP_ESP() {
    ...
    i386(0x5c) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_pop(GetRef(vcpu.sp), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_pop(GetRef(vcpu.esp), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void POP_EBP() {
    ...
    i386(0x5d) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_pop(GetRef(vcpu.bp), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_pop(GetRef(vcpu.ebp), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void POP_ESI() {
    ...
    i386(0x5e) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_pop(GetRef(vcpu.si), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_pop(GetRef(vcpu.esi), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void POP_EDI() {
    ...
    i386(0x5f) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_e_pop(GetRef(vcpu.di), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_e_pop(GetRef(vcpu.edi), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void PUSH_I32() {
    ...
    i386(0x68) {
        ...
        _chr(_e_push(GetRef(vcpuins.cimm), _GetOperandSize));
    }
    else
        UndefinedOpcode();
    _ce;
...
static void IMUL_R32_RM32_I32() {
    ...
    i386(0x69) {
        ...
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_d_imm(_GetOperandSize));
        _chr(_a_imul3(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else
        UndefinedOpcode();
...
static void IMUL_R32_RM32_I8() {
    ...
    i386(0x6b) {
        ...
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_d_imm(1));
        _chr(_a_imul3(vcpuins.crm, vcpuins.cimm, ((_GetOperandSize * 8 + 8) >> 1)));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    }
    else
        UndefinedOpcode();
...
static void JO_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), 1, _GetEFLAGS_OF));
    _ce;
}
...
static void JC_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), 1, _GetEFLAGS_CF));
    _ce;
}
...
static void JZ_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), 1, _GetEFLAGS_ZF));
    _ce;
}
...
static void JNA_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), 1,
...
static void JS_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), 1, _GetEFLAGS_SF));
    _ce;
}
...
static void JP_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), 1, _GetEFLAGS_PF));
    _ce;
}
...
static void JL_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), 1, (_GetEFLAGS_SF != _GetEFLAGS_OF)));
    _ce;
}
...
static void INS_80() {
    ...
    case 0: /* ADD_RM8_I8 */
        _bb("ADD_RM8_I8");
        _chr(_a_add(vcpuins.crm, vcpuins.cimm, 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(1));
        _be;
        break;
    case 1: /* OR_RM8_I8 */
        _bb("OR_RM8_I8");
        _chr(_a_or(vcpuins.crm, vcpuins.cimm, 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(1));
        _be;
        break;
    case 2: /* ADC_RM8_I8 */
        _bb("ADC_RM8_I8");
        _chr(_a_adc(vcpuins.crm, vcpuins.cimm, 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(1));
        _be;
        break;
    case 3: /* SBB_RM8_I8 */
        _bb("SBB_RM8_I8");
        _newins_;
        _chr(_a_sbb(vcpuins.crm, vcpuins.cimm, 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(1));
        _be;
        break;
    case 4: /* AND_RM8_I8 */
        _bb("AND_RM8_I8");
        _chr(_a_and(vcpuins.crm, vcpuins.cimm, 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(1));
        _be;
        break;
    case 5: /* SUB_RM8_I8 */
        _bb("SUB_RM8_I8");
        _chr(_a_sub(vcpuins.crm, vcpuins.cimm, 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(1));
        _be;
        break;
    case 6: /* XOR_RM8_I8 */
        _bb("XOR_RM8_I8");
        _chr(_a_xor(vcpuins.crm, vcpuins.cimm, 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(1));
        _be;
        break;
    case 7: /* CMP_RM8_I8 */
        _bb("CMP_RM8_I8");
        _chr(_a_cmp(vcpuins.crm, vcpuins.cimm, 8));
        _be;
        break;
    default:
        _impossible_r_;
...
static void INS_81() {
    ...
    i386(0x81) {
        ...
        case 0: /* ADD_RM32_I32 */
            _bb("ADD_RM32_I32");
            _chr(_a_add(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 1: /* OR_RM32_I32 */
            _bb("OR_RM32_I32");
            _chr(_a_or(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 2: /* ADC_RM32_I32 */
            _bb("ADC_RM32_I32");
            _chr(_a_adc(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 3: /* SBB_RM32_I32 */
            _bb("SBB_RM32_I32");
            _newins_;
            _chr(_a_sbb(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 4: /* AND_RM32_I32 */
            _bb("AND_RM32_I32");
            _chr(_a_and(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 5: /* SUB_RM32_I32 */
            _bb("SUB_RM32_I32");
            _chr(_a_sub(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 6: /* XOR_RM32_I32 */
            _bb("XOR_RM32_I32");
            _chr(_a_xor(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 7: /* CMP_RM32_I32 */
            _bb("CMP_RM32_I32");
            _chr(_a_cmp(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            _be;
            break;
        default:
            _impossible_r_;
        ...
        case 0: /* ADD_RM16_I16 */
            _bb("ADD_RM16_I16");
            _chr(_a_add(vcpuins.crm, vcpuins.cimm, 2 * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(2));
            _be;
            break;
        case 1: /* OR_RM16_I16 */
            _bb("OR_RM16_I16");
            _chr(_a_or(vcpuins.crm, vcpuins.cimm, 2 * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(2));
            _be;
            break;
        case 2: /* ADC_RM16_I16 */
            _bb("ADC_RM16_I16");
            _chr(_a_adc(vcpuins.crm, vcpuins.cimm, 2 * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(2));
            _be;
            break;
        case 3: /* SBB_RM16_I16 */
            _bb("SBB_RM16_I16");
            _newins_;
            _chr(_a_sbb(vcpuins.crm, vcpuins.cimm, 2 * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(2));
            _be;
            break;
        case 4: /* AND_RM16_I16 */
            _bb("AND_RM16_I16");
            _chr(_a_and(vcpuins.crm, vcpuins.cimm, 2 * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(2));
            _be;
            break;
        case 5: /* SUB_RM16_I16 */
            _bb("SUB_RM16_I16");
            _chr(_a_sub(vcpuins.crm, vcpuins.cimm, 2 * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(2));
            _be;
            break;
        case 6: /* XOR_RM16_I16 */
            _bb("XOR_RM16_I16");
            _chr(_a_xor(vcpuins.crm, vcpuins.cimm, 2 * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(2));
            _be;
            break;
        case 7: /* CMP_RM16_I16 */
            _bb("CMP_RM16_I16");
            _chr(_a_cmp(vcpuins.crm, vcpuins.cimm, 2 * 8));
            _be;
            break;
        default:
            _impossible_r_;
...
static void INS_83() {
    ...
    i386(0x83) {
        ...
        case 0: /* ADD_RM32_I8 */
            _bb("ADD_RM32_I8");
            _chr(_a_add(vcpuins.crm, vcpuins.cimm, bit));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 1: /* OR_RM32_I8 */
            _bb("OR_RM32_I8");
            _chr(_a_or(vcpuins.crm, vcpuins.cimm, bit));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 2: /* ADC_RM32_I8 */
            _bb("ADC_RM32_I8");
            _chr(_a_adc(vcpuins.crm, vcpuins.cimm, bit));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 3: /* SBB_RM32_I8 */
            _bb("SBB_RM32_I8");
            _chr(_a_sbb(vcpuins.crm, vcpuins.cimm, bit));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 4: /* AND_RM32_I8 */
            _bb("AND_RM32_I8");
            _chr(_a_and(vcpuins.crm, vcpuins.cimm, bit));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 5: /* SUB_RM32_I8 */
            _bb("SUB_RM32_I8");
            _chr(_a_sub(vcpuins.crm, vcpuins.cimm, bit));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 6: /* XOR_RM32_I8 */
            _bb("XOR_RM32_I8");
            _chr(_a_xor(vcpuins.crm, vcpuins.cimm, bit));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 7: /* CMP_RM32_I8 */
            _bb("CMP_RM32_I8");
            _chr(_a_cmp(vcpuins.crm, vcpuins.cimm, bit));
            _be;
            break;
        default:
            _impossible_r_;
...
static void TEST_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_a_test(vcpuins.crm, vcpuins.cr, 8));
    _ce;
}
static void TEST_RM32_R32() {
    ...
    i386(0x85) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_a_test(vcpuins.crm, vcpuins.cr, _GetOperandSize * 8));
    }
    else {
        vcpu.ip++;
        _d_modrm(2, 2);
...
static void XCHG_RM8_R8() {
    ...
    _chr(_d_modrm(1, 1));
    _chr(_m_read_rm(1));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.crm), 1));
    vcpuins.crm = vcpuins.cr;
    _chr(_m_write_rm(1));
    _ce;
}
static void XCHG_RM32_R32() {
    ...
    i386(0x87) {
        ...
        _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
        _chr(_m_read_rm(_GetOperandSize));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.crm), _GetOperandSize));
        vcpuins.crm = vcpuins.cr;
        _chr(_m_write_rm(_GetOperandSize));
    }
    else {
        ...
        _d_modrm(2, 2);
        _chr(_m_read_rm(2));
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.crm), 2));
        vcpuins.crm = vcpuins.cr;
        _chr(_m_write_rm(2));
    }
    _ce;
...
static void LEA_R32_M32() {
    ...
    i386(0x8d) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.mrm.offset), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.mrm.offset), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void MOV_SREG_RM16() {
    ...
    i386(0x8e) {
        ...
        _chr(_m_read_rm(2));
        _chr(_s_load_sreg(vcpuins.rmovsreg, GetMax16(vcpuins.crm)));
...
static void INS_8F() {
    ...
    i386(0x8f) {
        ...
        case 0: /* POP_RM32 */
            _bb("POP_RM32");
            _chr(_e_pop(GetRef(vcpuins.crm), _GetOperandSize));
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 1:
        ...
        case 0: /* POP_RM16 */
            _bb("POP_RM16");
            _chr(_e_pop(GetRef(vcpuins.crm), 2));
            _chr(_m_write_rm(2));
            _be;
            break;
        case 1:
...
static void CWD() {
    ...
    i386(0x99) {
        ...
        switch (_GetOperandSize) {
        case 2:
            vcpu.dx = GetMSB16(vcpu.ax) ? 0xffff : 0x0000;
            break;
        case 4:
            vcpu.edx = GetMSB32(vcpu.eax) ? 0xffffffff : 0x00000000;
            break;
        default:
            _impossible_r_;
            break;
...
static void POPF() {
    ...
    i386(0x9d) {
        ...
        vcpu.ip++;
        _chr(_e_pop(GetRef(vcpu.flags), 2));
    }
    _ce;
}
static void SAHF() {
...
static void TEST_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_a_test(vcpu.al, vcpuins.cimm, 8));
    _ce;
}
static void TEST_EAX_I32() {
    ...
    i386(0xa9) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_a_test(vcpu.ax, vcpuins.cimm, 16));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _newins_;
            _chr(_d_imm(4));
            _chr(_a_test(vcpu.eax, vcpuins.cimm, 32));
            _be;
            break;
        default:
            _impossible_r_;
...
static void MOV_EAX_I32() {
    ...
    i386(0xb8) {
        ...
        case 2:
            ...
            break;
        case 4:
            vcpu.eax = GetMax32(vcpuins.cimm);
            break;
        default:
            _impossible_r_;
            break;
...
static void MOV_ECX_I32() {
    ...
    i386(0xb9) {
        ...
        case 2:
            ...
            break;
        case 4:
            vcpu.ecx = GetMax32(vcpuins.cimm);
            break;
        default:
            _impossible_r_;
            break;
...
static void MOV_EDX_I32() {
    ...
    i386(0xba) {
        ...
        case 2:
            ...
            break;
        case 4:
            vcpu.edx = GetMax32(vcpuins.cimm);
            break;
        default:
            _impossible_r_;
            break;
...
static void MOV_EBX_I32() {
    ...
    i386(0xbb) {
        ...
        case 2:
            ...
            break;
        case 4:
            vcpu.ebx = GetMax32(vcpuins.cimm);
            break;
        default:
            _impossible_r_;
            break;
...
static void MOV_ESP_I32() {
    ...
    i386(0xbc) {
        ...
        case 2:
            ...
            break;
        case 4:
            vcpu.esp = GetMax32(vcpuins.cimm);
            break;
        default:
            _impossible_r_;
            break;
...
static void MOV_EBP_I32() {
    ...
    i386(0xbd) {
        ...
        case 2:
            ...
            break;
        case 4:
            vcpu.ebp = GetMax32(vcpuins.cimm);
            break;
        default:
            _impossible_r_;
            break;
...
static void MOV_ESI_I32() {
    ...
    i386(0xbe) {
        ...
        case 2:
            ...
            break;
        case 4:
            vcpu.esi = GetMax32(vcpuins.cimm);
            break;
        default:
            _impossible_r_;
            break;
...
static void MOV_EDI_I32() {
    ...
    i386(0xbf) {
        ...
        case 2:
            ...
            break;
        case 4:
            vcpu.edi = GetMax32(vcpuins.cimm);
            break;
        default:
            _impossible_r_;
            break;
...
static void INS_C0() {
    ...
    i386(0xc0) {
        ...
        case 0: /* ROL_RM8_I8 */
            _bb("ROL_RM8_I8");
            _chr(_a_rol(GetMax8(vcpuins.crm), GetMax8(vcpuins.cimm), 8));
            _be;
            break;
        case 1: /* ROR_RM8_I8 */
            _bb("ROR_RM8_I8");
            _chr(_a_ror(GetMax8(vcpuins.crm), GetMax8(vcpuins.cimm), 8));
            _be;
            break;
        case 2: /* RCL_RM8_I8 */
            _bb("RCL_RM8_I8");
            _chr(_a_rcl(GetMax8(vcpuins.crm), GetMax8(vcpuins.cimm), 8));
            _be;
            break;
        case 3: /* RCR_RM8_I8 */
            _bb("RCR_RM8_I8");
            _chr(_a_rcr(GetMax8(vcpuins.crm), GetMax8(vcpuins.cimm), 8));
            _be;
            break;
        case 4: /* SHL_RM8_I8 */
            _bb("SHL_RM8_I8");
            _chr(_a_shl(GetMax8(vcpuins.crm), GetMax8(vcpuins.cimm), 8));
            _be;
            break;
        case 5: /* SHR_RM8_I8 */
            _bb("SHR_RM8_I8");
            _chr(_a_shr(GetMax8(vcpuins.crm), GetMax8(vcpuins.cimm), 8));
            _be;
            break;
        ...
        case 7: /* SAR_RM8_I8 */
            _bb("SAR_RM8_I8");
            _chr(_a_sar(GetMax8(vcpuins.crm), GetMax8(vcpuins.cimm), 8));
            _be;
            break;
        default:
            _impossible_r_;
...
static void INS_C1() {
    ...
    i386(0xc1) {
        ...
        case 0: /* ROL_RM32_I8 */
            _bb("ROL_RM32_I8");
            _chr(_a_rol(GetMax32(vcpuins.crm), GetMax8(vcpuins.cimm), _GetOperandSize * 8));
            _be;
            break;
        case 1: /* ROR_RM32_I8 */
            _bb("ROR_RM32_I8");
            _chr(_a_ror(GetMax32(vcpuins.crm), GetMax8(vcpuins.cimm), _GetOperandSize * 8));
            _be;
            break;
        case 2: /* RCL_RM32_I8 */
            _bb("RCL_RM32_I8");
            _chr(_a_rcl(GetMax32(vcpuins.crm), GetMax8(vcpuins.cimm), _GetOperandSize * 8));
            _be;
            break;
        case 3: /* RCR_RM32_I8 */
            _bb("RCR_RM32_I8");
            _chr(_a_rcr(GetMax32(vcpuins.crm), GetMax8(vcpuins.cimm), _GetOperandSize * 8));
            _be;
            break;
        case 4: /* SHL_RM32_I8 */
            _bb("SHL_RM32_I8");
            _chr(_a_shl(GetMax32(vcpuins.crm), GetMax8(vcpuins.cimm), _GetOperandSize * 8));
            _be;
            break;
        case 5: /* SHR_RM32_I8 */
            _bb("SHR_RM32_I8");
            _chr(_a_shr(GetMax32(vcpuins.crm), GetMax8(vcpuins.cimm), _GetOperandSize * 8));
            _be;
            break;
        case 6: /* UndefinedOpcode */
            _bb("vcpuins.cr(6)");
        ...
        case 7: /* SAR_RM32_I8 */
            _bb("SAR_RM32_I8");
            _chr(_a_sar(GetMax32(vcpuins.crm), GetMax8(vcpuins.cimm), _GetOperandSize * 8));
            _be;
            break;
        default:
            _impossible_r_;
...
static void RET_I16() {
    ...
    i386(0xc2) {
        _adv;
        _chr(_d_imm(2));
        _chr(_e_ret_near(GetMax16(vcpuins.cimm), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _d_imm(2);
...
static void LES_R32_M16_32() {
    ...
    i386(0xc4) {
        ...
        _chr(_e_load_far(&vcpu.es, vcpuins.rr, selector, offset, _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _d_modrm(2, 4);
        ...
        _chr(_m_read_rm(2));
        selector = GetMax16(vcpuins.crm);
        _chr(_e_load_far(&vcpu.es, vcpuins.rr, selector, offset, 2));
    }
    _ce;
}
static void LDS_R32_M16_32() {
    ...
    i386(0xc5) {
        ...
        _chr(_m_read_rm(2));
        selector = GetMax16(vcpuins.crm);
        _chr(_e_load_far(&vcpu.ds, vcpuins.rr, selector, offset, _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _d_modrm(2, 4);
        ...
        _chr(_m_read_rm(2));
        selector = GetMax16(vcpuins.crm);
        _chr(_e_load_far(&vcpu.ds, vcpuins.rr, selector, offset, 2));
    }
    _ce;
}
static void INS_C6() {
            ...
            for (i = 0; i < level; ++i) {
                ...
                case 2:
                    ...
                    case 2:
                        _bb("StackSize(2)");
                        vcpu.bp -= 2;
                        _chr(_s_read_ss(vcpu.bp, GetRef(data), 2));
                    ...
                    case 4:
                        _bb("StackSize(4)");
                        vcpu.ebp -= 2;
                        _chr(_s_read_ss(vcpu.ebp, GetRef(data), 2));
                        _chr(_e_push(GetRef(data), 2));
                        _be;
                        break;
                    default:
                ...
                case 4:
                    ...
                    case 2:
                        _bb("StackSize(2)");
                        vcpu.bp -= 4;
                        _chr(_s_read_ss(vcpu.bp, GetRef(data), 4));
                    ...
                    case 4:
                        _bb("StackSize(4)");
                        vcpu.ebp -= 4;
                        _chr(_s_read_ss(vcpu.ebp, GetRef(data), 4));
                        _chr(_e_push(GetRef(data), 4));
                        _be;
                        break;
                    default:
...
static void LEAVE() {
    ...
    i386(0xc9) {
        ...
        _chr(_m_test_logical(&vcpu.ss, vcpu.ebp, _GetOperandSize, 1));
        switch (_GetStackSize) {
        case 2:
            vcpu.sp = vcpu.bp;
            break;
...
static void RETF_I16() {
    ...
    i386(0xca) {
        _adv;
        _chr(_d_imm(2));
        _chr(_e_ret_far(GetMax16(vcpuins.cimm), _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _d_imm(2);
...
static void INT_I8() {
    ...
    i386(0xcc) {
        _adv;
        _chr(_d_imm(1));
        _chr(_e_int_n((t_nubit8)vcpuins.cimm, _GetOperandSize));
    }
    else {
        vcpu.ip++;
        _d_imm(1);
...
static void INS_D0() {
    ...
    case 0: /* ROL_RM8 */
        _bb("ROL_RM8");
        _chr(_a_rol(vcpuins.crm, 1, 8));
        _be;
        break;
    case 1: /* ROR_RM8 */
        _bb("ROR_RM8");
        _chr(_a_ror(vcpuins.crm, 1, 8));
        _be;
        break;
    case 2: /* RCL_RM8 */
        _bb("RCL_RM8");
        _chr(_a_rcl(vcpuins.crm, 1, 8));
        _be;
        break;
    case 3: /* RCR_RM8 */
        _bb("RCR_RM8");
        _chr(_a_rcr(vcpuins.crm, 1, 8));
        _be;
        break;
    case 4: /* SHL_RM8 */
        _bb("SHL_RM8");
        _chr(_a_shl(vcpuins.crm, 1, 8));
        _be;
        break;
    case 5: /* SHR_RM8 */
        _bb("SHR_RM8");
        _chr(_a_shr(vcpuins.crm, 1, 8));
        _be;
        break;
    case 6: /* UndefinedOpcode */
        _bb("vcpuins.cr(6)");
...
static void INS_D1() {
    ...
    i386(0xd1) {
        ...
        case 0: /* ROL_RM32 */
            _bb("ROL_RM32");
            _chr(_a_rol(vcpuins.crm, 1, _GetOperandSize * 8));
            _be;
            break;
        case 1: /* ROR_RM32 */
            _bb("ROR_RM32");
            _chr(_a_ror(vcpuins.crm, 1, _GetOperandSize * 8));
            _be;
            break;
        case 2: /* RCL_RM32 */
            _bb("RCL_RM32");
            _chr(_a_rcl(vcpuins.crm, 1, _GetOperandSize * 8));
            _be;
            break;
        case 3: /* RCR_RM32 */
            _bb("RCR_RM32");
            _chr(_a_rcr(vcpuins.crm, 1, _GetOperandSize * 8));
            _be;
            break;
        case 4: /* SHL_RM32 */
            _bb("SHL_RM32");
            _chr(_a_shl(vcpuins.crm, 1, _GetOperandSize * 8));
            _be;
            break;
        case 5: /* SHR_RM32 */
            _bb("SHR_RM32");
            _chr(_a_shr(vcpuins.crm, 1, _GetOperandSize * 8));
            _be;
            break;
        case 6: /* UndefinedOpcode */
            _bb("vcpuins.cr(6)");
        ...
        case 7: /* SAR_RM32 */
            _bb("SAR_RM32");
            _chr(_a_sar(vcpuins.crm, 1, _GetOperandSize * 8));
            _be;
            break;
        default:
            _impossible_r_;
        ...
        case 0: /* ROL_RM16 */
            _bb("ROL_RM16");
            _chr(_a_rol(vcpuins.crm, 1, 16));
            _be;
            break;
        case 1: /* ROR_RM16 */
            _bb("ROR_RM16");
            _chr(_a_ror(vcpuins.crm, 1, 16));
            _be;
            break;
        case 2: /* RCL_RM16 */
            _bb("RCL_RM16");
            _chr(_a_rcl(vcpuins.crm, 1, 16));
            _be;
            break;
        case 3: /* RCR_RM16 */
            _bb("RCR_RM16");
            _chr(_a_rcr(vcpuins.crm, 1, 16));
            _be;
            break;
        case 4: /* SHL_RM16 */
            _bb("SHL_RM16");
            _chr(_a_shl(vcpuins.crm, 1, 16));
            _be;
            break;
        case 5: /* SHR_RM16 */
            _bb("SHR_RM16");
            _chr(_a_shr(vcpuins.crm, 1, 16));
            _be;
            break;
        case 6: /* UndefinedOpcode */
            _bb("vcpuins.cr(6)");
...
static void INS_D2() {
    ...
    case 0: /* ROL_RM8_CL */
        _bb("ROL_RM8_CL");
        _chr(_a_rol(vcpuins.crm, vcpu.cl, 8));
        _be;
        break;
    case 1: /* ROR_RM8_CL */
        _bb("ROR_RM8_CL");
        _chr(_a_ror(vcpuins.crm, vcpu.cl, 8));
        _be;
        break;
    case 2: /* RCL_RM8_CL */
        _bb("RCL_RM8_CL");
        _chr(_a_rcl(vcpuins.crm, vcpu.cl, 8));
        _be;
        break;
    case 3: /* RCR_RM8_CL */
        _bb("RCR_RM8_CL");
        _chr(_a_rcr(vcpuins.crm, vcpu.cl, 8));
        _be;
        break;
    case 4: /* SHL_RM8_CL */
        _bb("SHL_RM8_CL");
        _chr(_a_shl(vcpuins.crm, vcpu.cl, 8));
        _be;
        break;
    case 5: /* SHR_RM8_CL */
        _bb("SHR_RM8_CL");
        _chr(_a_shr(vcpuins.crm, vcpu.cl, 8));
        _be;
        break;
    case 6: /* UndefinedOpcode */
        _bb("vcpuins.cr(6)");
    ...
    case 7: /* SAR_RM8_CL */
        _bb("SAR_RM8_CL");
        _chr(_a_sar(vcpuins.crm, vcpu.cl, 8));
        _be;
        break;
    default:
        _impossible_r_;
...
static void INS_D3() {
    ...
    i386(0xd3) {
        ...
        case 0: /* ROL_RM32_CL */
            _bb("ROL_RM32_CL");
            _chr(_a_rol(vcpuins.crm, vcpu.cl, _GetOperandSize * 8));
            _be;
            break;
        case 1: /* ROR_RM32_CL */
            _bb("ROR_RM32_CL");
            _chr(_a_ror(vcpuins.crm, vcpu.cl, _GetOperandSize * 8));
            _be;
            break;
        case 2: /* RCL_RM32_CL */
            _bb("RCL_RM32_CL");
            _chr(_a_rcl(vcpuins.crm, vcpu.cl, _GetOperandSize * 8));
            _be;
            break;
        case 3: /* RCR_RM32_CL */
            _bb("RCR_RM32_CL");
            _chr(_a_rcr(vcpuins.crm, vcpu.cl, _GetOperandSize * 8));
            _be;
            break;
        case 4: /* SHL_RM32_CL */
            _bb("SHL_RM32_CL");
            _chr(_a_shl(vcpuins.crm, vcpu.cl, _GetOperandSize * 8));
            _be;
            break;
        case 5: /* SHR_RM32_CL */
            _bb("SHR_RM32_CL");
            _chr(_a_shr(vcpuins.crm, vcpu.cl, _GetOperandSize * 8));
            _be;
            break;
        case 6: /* UndefinedOpcode */
            _bb("vcpuins.cr(6)");
        ...
        case 7: /* SAR_RM32_CL */
            _bb("SAR_RM32_CL");
            _chr(_a_sar(vcpuins.crm, vcpu.cl, _GetOperandSize * 8));
            _be;
            break;
        default:
            _impossible_r_;
        ...
        case 0: /* ROL_RM16_CL */
            _bb("ROL_RM16_CL");
            _chr(_a_rol(vcpuins.crm, vcpu.cl, 16));
            _be;
            break;
        case 1: /* ROR_RM16_CL */
            _bb("ROR_RM16_CL");
            _chr(_a_ror(vcpuins.crm, vcpu.cl, 16));
            _be;
            break;
        case 2: /* RCL_RM16_CL */
            _bb("RCL_RM16_CL");
            _chr(_a_rcl(vcpuins.crm, vcpu.cl, 16));
            _be;
            break;
        case 3: /* RCR_RM16_CL */
            _bb("RCR_RM16_CL");
            _chr(_a_rcr(vcpuins.crm, vcpu.cl, 16));
            _be;
            break;
        case 4: /* SHL_RM16_CL */
            _bb("SHL_RM16_CL");
            _chr(_a_shl(vcpuins.crm, vcpu.cl, 16));
            _be;
            break;
        case 5: /* SHR_RM16_CL */
            _bb("SHR_RM16_CL");
            _chr(_a_shr(vcpuins.crm, vcpu.cl, 16));
            _be;
            break;
        case 6: /* UndefinedOpcode */
            _bb("vcpuins.cr(6)");
        ...
        case 7: /* SAR_RM16_CL */
            _bb("SAR_RM16_CL");
            _chr(_a_sar(vcpuins.crm, vcpu.cl, 16));
            _be;
            break;
        default:
            _impossible_r_;
...
static void XLAT() {
    ...
    i386(0xd7) {
        ...
        case 2:
            _bb("AddressSize(2)");
            _chr(_m_read_logical(vcpuins.roverds, (vcpu.bx + vcpu.al), GetRef(vcpu.al), 1));
            _be;
            break;
        case 4:
            _bb("AddressSize(4)");
            _chr(_m_read_logical(vcpuins.roverds, (vcpu.ebx + vcpu.al), GetRef(vcpu.al), 1));
            _be;
            break;
        default:
            _impossible_r_;
...
static void LOOPNZ_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_loopcc((t_nsbit8)vcpuins.cimm, !_GetEFLAGS_ZF));
    _ce;
}
...
static void LOOP_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_loopcc((t_nsbit8)vcpuins.cimm, 1));
    _ce;
}
static void JCXZ_REL8() {
    ...
    i386(0xe3) {
        ...
        _chr(_e_jcc(GetMax32(vcpuins.cimm), 1, !cecx));
    }
    else {
        vcpu.ip++;
        _d_imm(1);
...
static void IN_AL_I8() {
    ...
    _chr(_d_imm(1));
    _chr(_p_input(GetMax8(vcpuins.cimm), GetRef(vcpu.al), 1));
    _ce;
}
static void IN_EAX_I8() {
    ...
    i386(0xe5) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_p_input(GetMax8(vcpuins.cimm), GetRef(vcpu.ax), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_p_input(GetMax8(vcpuins.cimm), GetRef(vcpu.eax), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void OUT_I8_AL() {
    ...
    _chr(_d_imm(1));
    _chr(_p_output(GetMax8(vcpuins.cimm), GetRef(vcpu.al), 1));
    _ce;
...
static void OUT_I8_EAX() {
    ...
    i386(0xe7) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_p_output(GetMax8(vcpuins.cimm), GetRef(vcpu.ax), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_p_output(GetMax8(vcpuins.cimm), GetRef(vcpu.eax), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void CALL_REL32() {
    ...
    i386(0xe8) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_d_imm(2));
            _chr(_e_call_near(GetMax16(vcpu.ip + (t_nsbit16)vcpuins.cimm), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _newins_;
            _chr(_d_imm(4));
            _chr(_e_call_near(GetMax32(vcpu.eip + (t_nsbit32)vcpuins.cimm), 4));
            _be;
            break;
        default:
            _bb("OperandSize");
...
static void JMP_REL32() {
    ...
    i386(0xe9) {
        _adv;
        _chr(_d_imm(_GetOperandSize));
        _chr(_e_jcc(GetMax32(vcpuins.cimm), _GetOperandSize, 1));
    }
    else {
        vcpu.ip++;
        _d_imm(2);
...
static void JMP_REL8() {
    ...
    _chr(_d_imm(1));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), 1, 1));
    _ce;
}
static void IN_AL_DX() {
    ...
    _chr(_p_input(vcpu.dx, GetRef(vcpu.al), 1));
    _ce;
}
static void IN_EAX_DX() {
    ...
    i386(0xed) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_p_input(vcpu.dx, GetRef(vcpu.ax), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_p_input(vcpu.dx, GetRef(vcpu.eax), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void OUT_DX_AL() {
    ...
    _chr(_p_output(vcpu.dx, GetRef(vcpu.al), 1));
    _ce;
}
static void OUT_DX_EAX() {
    ...
    i386(0xef) {
        ...
        case 2:
            _bb("OperandSize(2)");
            _chr(_p_output(vcpu.dx, GetRef(vcpu.ax), 2));
            _be;
            break;
        case 4:
            _bb("OperandSize(4)");
            _chr(_p_output(vcpu.dx, GetRef(vcpu.eax), 4));
            _be;
            break;
        default:
            _impossible_r_;
...
static void INS_F6() {
    ...
    case 0: /* TEST_RM8_I8 */
        _bb("TEST_RM8_I8");
        _chr(_d_imm(1));
        _chr(_a_test(vcpuins.crm, vcpuins.cimm, 8));
        _be;
        break;
    ...
    case 4: /* MUL_RM8 */
        _bb("MUL_RM8");
        _chr(_a_mul(vcpuins.crm, 8));
        _be;
        break;
    case 5: /* IMUL_RM8 */
        _bb("IMUL_RM8");
        _chr(_a_imul(vcpuins.crm, 8));
        _be;
        break;
    case 6: /* DIV_RM8 */
        _bb("DIV_RM8");
        _chr(_a_div(vcpuins.crm, 8));
        _be;
        break;
    case 7: /* IDIV_RM8 */
        _bb("IDIV_RM8");
        _newins_;
        _chr(_a_idiv(vcpuins.crm, 8));
        _be;
        break;
    default:
        _impossible_r_;
...
static void INS_F7() {
    ...
    i386(0xf7) {
        ...
        case 0: /* TEST_RM32_I32 */
            _bb("TEST_RM32_I32");
            _chr(_d_imm(_GetOperandSize));
            _chr(_a_test(vcpuins.crm, vcpuins.cimm, _GetOperandSize * 8));
            _be;
            break;
        case 1: /* UndefinedOpcode */
            _bb("ModRM_REG(1)");
        ...
        case 2: /* NOT_RM32 */
            _bb("NOT_RM32");
            _chr(_a_not(vcpuins.crm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 3: /* NEG_RM32 */
            _bb("NEG_RM32");
            _chr(_a_neg(vcpuins.crm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 4: /* MUL_RM32 */
            _bb("MUL_RM32");
            _chr(_a_mul(vcpuins.crm, _GetOperandSize * 8));
            _be;
            break;
        case 5: /* IMUL_RM32 */
            _bb("IMUL_RM32");
            _chr(_a_imul(vcpuins.crm, _GetOperandSize * 8));
            _be;
            break;
        case 6: /* DIV_RM32 */
            _bb("DIV_RM32");
            _chr(_a_div(vcpuins.crm, _GetOperandSize * 8));
            _be;
            break;
        case 7: /* IDIV_RM32 */
            _bb("IDIV_RM32");
            _chr(_a_idiv(vcpuins.crm, _GetOperandSize * 8));
            _be;
            break;
        default:
            _impossible_r_;
        ...
        case 0: /* TEST_RM16_I16 */
            _bb("TEST_RM16_I16");
            _chr(_d_imm(2));
            _chr(_a_test(vcpuins.crm, vcpuins.cimm, 16));
            _be;
            break;
        case 1: /* UndefinedOpcode */
            _bb("ModRM_REG(1)");
        ...
        case 4: /* MUL_RM16 */
            _bb("MUL_RM16");
            _chr(_a_mul(vcpuins.crm, 16));
            _be;
            break;
        case 5: /* IMUL_RM16 */
            _bb("IMUL_RM16");
            _chr(_a_imul(vcpuins.crm, 16));
            _be;
            break;
        case 6: /* DIV_RM16 */
            _bb("DIV_RM16");
            _chr(_a_div(vcpuins.crm, 16));
            _be;
            break;
        case 7: /* IDIV_RM16 */
            _bb("IDIV_RM16");
            _chr(_a_idiv(vcpuins.crm, 16));
            _be;
            break;
        default:
            _impossible_r_;
...
static void INS_FF() {
    ...
    i386(0xff) {
        _adv;
        _chr(_s_read_cs(vcpu.eip, GetRef(modrm), 1));
        switch (_GetModRM_REG(modrm)) {
        case 0: /* INC_RM32 */
            _bb("INC_RM32");
            _chr(_d_modrm(0, _GetOperandSize));
            _chr(_m_read_rm(_GetOperandSize));
            _chr(_a_inc(vcpuins.crm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        case 1: /* DEC_RM32 */
            ...
            _chr(_d_modrm(0, _GetOperandSize));
            _chr(_m_read_rm(_GetOperandSize));
            _chr(_a_dec(vcpuins.crm, _GetOperandSize * 8));
            vcpuins.crm = vcpuins.result;
            _chr(_m_write_rm(_GetOperandSize));
            _be;
            break;
        ...
        case 6: /* PUSH_RM32 */
            ...
            _chr(_d_modrm(0, _GetOperandSize));
            _chr(_m_read_rm(_GetOperandSize));
            _chr(_e_push(GetRef(vcpuins.crm), _GetOperandSize));
            _be;
            break;
        ...
        case 6: /* PUSH_RM16 */
            ...
            _chr(_d_modrm(0, 2));
            _chr(_m_read_rm(2));
            _chr(_e_push(GetRef(vcpuins.crm), 2));
            _be;
            break;
        case 7: /* UndefinedOpcode */
            _bb("ModRM_REG(7)");
...
static void _d_modrm_creg() {
    ...
    case 1:
        ...
        break;
    case 2:
        vcpuins.rr = (t_vaddrcc)&vcpu.cr2;
        vcpuins.cr = vcpu.cr2;
        break;
    case 3:
        vcpuins.rr = (t_vaddrcc)&vcpu.cr3;
        vcpuins.cr = vcpu.cr3;
        break;
    case 4:
        _bb("vcpuins.cr(4)");
...
static void _d_modrm_dreg() {
    ...
    switch (vcpuins.cr) {
    case 0:
        vcpuins.rr = (t_vaddrcc)&vcpu.dr0;
        vcpuins.cr = vcpu.dr0;
        break;
    case 1:
        vcpuins.rr = (t_vaddrcc)&vcpu.dr1;
        vcpuins.cr = vcpu.dr1;
        break;
    case 2:
        vcpuins.rr = (t_vaddrcc)&vcpu.dr2;
        vcpuins.cr = vcpu.dr2;
        break;
    case 3:
        vcpuins.rr = (t_vaddrcc)&vcpu.dr3;
        vcpuins.cr = vcpu.dr3;
        break;
    case 4:
        _bb("vcpuins.cr(4)");
    ...
    case 5:
        ...
        break;
    case 6:
        vcpuins.rr = (t_vaddrcc)&vcpu.dr6;
        vcpuins.cr = vcpu.dr6;
        break;
    case 7:
        vcpuins.rr = (t_vaddrcc)&vcpu.dr7;
        vcpuins.cr = vcpu.dr7;
        break;
    default:
        _impossible_r_;
...
static void _d_modrm_treg() {
    ...
    case 5:
        ...
        break;
    case 6:
        vcpuins.rr = (t_vaddrcc)&vcpu.tr6;
        vcpuins.cr = vcpu.tr6;
        break;
    case 7:
        vcpuins.rr = (t_vaddrcc)&vcpu.tr7;
        vcpuins.cr = vcpu.tr7;
        break;
    default:
        _impossible_r_;
...
static void _d_bit_rmimm(t_nubit8 regbyte, t_nubit8 rmbyte, t_bool write) {
    ...
    case 2:
        ...
        _chr(vcpuins.cimm = GetMax16((1 << bitoperand)));
        _be;
        break;
    case 4:
        _bb("rmbyte(4)");
        ...
        _chr(vcpuins.cimm = GetMax32((1 << bitoperand)));
        _be;
        break;
    default:
        _bb("rmbyte");
...
static void INS_0F_00() {
    ...
    if (_IsProtected) {
        ...
        case 0: /* SLDT_RM16 */
            ...
            _chr(_m_write_rm(vcpuins.flagmem ? 2 : _GetOperandSize));
            _be;
            break;
        case 1: /* STR_RM16 */
            _bb("STR_RM16");
        ...
        case 2: /* LLDT_RM16 */
            ...
            _chr(_d_modrm(0, 2));
            _chr(_m_read_rm(2));
            _chr(_s_load_ldtr(GetMax16(vcpuins.crm)));
            _be;
            break;
        case 3: /* LTR_RM16 */
            _bb("LTR_RM16");
            _chr(_d_modrm(0, 2));
            _chr(_m_read_rm(2));
            _chr(_s_load_tr(GetMax16(vcpuins.crm)));
            _be;
            break;
        case 4: /* VERR_RM16 */
            _bb("VERR_RM16");
            ...
            if (_s_check_selector(GetMax16(vcpuins.crm))) {
                ...
                _bb("selector(valid)");
                _chr(_s_read_xdt(GetMax16(vcpuins.crm), GetRef(descriptor)));
...
static void INS_0F_01() {
    ...
    case 0: /* SGDT_M32_16 */
        ...
        switch (_GetOperandSize) {
        case 2:
            vcpuins.crm = GetMax24(vcpu.gdtr.base);
            break;
        case 4:
            vcpuins.crm = GetMax32(vcpu.gdtr.base);
            break;
        default:
            _impossible_r_;
            break;
    ...
    case 1: /* SIDT_M32_16 */
        ...
        switch (_GetOperandSize) {
        case 2:
            vcpuins.crm = GetMax24(vcpu.idtr.base);
            break;
        case 4:
            vcpuins.crm = GetMax32(vcpu.idtr.base);
            break;
        default:
            _impossible_r_;
            break;
    ...
    case 2: /* LGDT_M32_16 */
        ...
        switch (_GetOperandSize) {
        case 2:
            base = GetMax24(vcpuins.crm);
            break;
        case 4:
            base = GetMax32(vcpuins.crm);
            break;
        default:
            _impossible_r_;
            break;
    ...
    case 6: /* LMSW_RM16 */
        ...
        _chr(_d_modrm(0, 2));
        _chr(_m_read_rm(2));
        _chr(_s_load_cr0_msw(GetMax16(vcpuins.crm)));
        _be;
        break;
    case 7:
        _bb("ModRM_REG(7)");
        ...
        if (_s_check_selector(selector)) {
            ...
            if (_GetEFLAGS_ZF) {
                ...
                case 2:
                    ...
                    _chr(_m_write_ref(vcpuins.rr, GetRef(descriptor), 2));
                    _be;
                    break;
                case 4:
                    _bb("OperandSize(4)");
                    descriptor = (GetMax32(descriptor >> 32) & 0x00ffff00);
                    _chr(_m_write_ref(vcpuins.rr, GetRef(descriptor), 4));
                    _be;
                    break;
                default:
                    _impossible_r_;
    ...
    if (_IsProtected) {
        ...
        if (_s_check_selector(selector)) {
            ...
            if (_GetEFLAGS_ZF) {
                ...
                case 2:
                    _bb("OperandSize(2)");
                    _chr(_m_write_ref(vcpuins.rr, GetRef(limit), 2));
                    _be;
                    break;
                case 4:
                    _bb("OperandSize(4)");
                    _chr(_m_write_ref(vcpuins.rr, GetRef(limit), 4));
                    _be;
                    break;
                default:
                    _impossible_r_;
...
static void MOV_R32_CR() {
    ...
    _chr(_d_modrm_creg());
    _chr(_m_write_ref(vcpuins.rrm, GetRef(vcpuins.cr), 4));
    _ce;
}
...
static void MOV_CR_R32() {
    ...
    _chr(_d_modrm_creg());
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.crm), 4));
...
static void JO_REL32() {
    ...
    _adv;
    _chr(_d_imm(_GetOperandSize));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), _GetOperandSize, _GetEFLAGS_OF));
    _ce;
}
...
static void JC_REL32() {
    ...
    _adv;
    _chr(_d_imm(_GetOperandSize));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), _GetOperandSize, _GetEFLAGS_CF));
    _ce;
}
...
static void JZ_REL32() {
    ...
    _adv;
    _chr(_d_imm(_GetOperandSize));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), _GetOperandSize, _GetEFLAGS_ZF));
    _ce;
}
...
static void JNA_REL32() {
    ...
    _adv;
    _chr(_d_imm(_GetOperandSize));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), _GetOperandSize,
                (_GetEFLAGS_CF || _GetEFLAGS_ZF)));
    _ce;
}
...
static void JS_REL32() {
    ...
    _adv;
    _chr(_d_imm(_GetOperandSize));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), _GetOperandSize, _GetEFLAGS_SF));
    _ce;
}
...
static void JP_REL32() {
    ...
    _adv;
    _chr(_d_imm(_GetOperandSize));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), _GetOperandSize, _GetEFLAGS_PF));
    _ce;
}
...
static void JL_REL32() {
    ...
    _adv;
    _chr(_d_imm(_GetOperandSize));
    _chr(_e_jcc(GetMax32(vcpuins.cimm), _GetOperandSize, (_GetEFLAGS_SF != _GetEFLAGS_OF)));
    _ce;
}
...
static void BT_RM32_R32() {
    ...
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_a_bt((t_nubit32)vcpuins.crm, (t_nubit32)vcpuins.cimm, _GetOperandSize * 8));
    _ce;
}
static void SHLD_RM32_R32_I8() {
    ...
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_d_imm(1));
    _chr(_a_shld(vcpuins.crm, vcpuins.cr, GetMax8(vcpuins.cimm), _GetOperandSize * 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(_GetOperandSize));
    _ce;
}
static void SHLD_RM32_R32_CL() {
    ...
    _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_a_shld(vcpuins.crm, vcpuins.cr, vcpu.cl, _GetOperandSize * 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(_GetOperandSize));
    _ce;
}
...
static void BTS_RM32_R32() {
    ...
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_a_bts((t_nubit32)vcpuins.crm, (t_nubit32)vcpuins.cimm, _GetOperandSize * 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(_GetOperandSize));
    _ce;
}
static void SHRD_RM32_R32_I8() {
    ...
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_d_imm(1));
    _chr(_a_shrd(vcpuins.crm, vcpuins.cr, GetMax8(vcpuins.cimm), _GetOperandSize * 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(_GetOperandSize));
    _ce;
}
static void SHRD_RM32_R32_CL() {
    ...
    _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_a_shrd(vcpuins.crm, vcpuins.cr, vcpu.cl, _GetOperandSize * 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(_GetOperandSize));
    _ce;
}
static void IMUL_R32_RM32() {
    ...
    _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_a_imul2(vcpuins.cr, vcpuins.crm, _GetOperandSize * 8));
    _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
    _ce;
}
static void LSS_R32_M16_32() {
    ...
    _chr(_m_read_rm(2));
    selector = GetMax16(vcpuins.crm);
    _chr(_e_load_far(&vcpu.ss, vcpuins.rr, selector, offset, _GetOperandSize));
    _ce;
}
static void BTR_RM32_R32() {
    ...
    _chr(_d_bit_rmimm(_GetOperandSize, _GetOperandSize, 1));
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_a_btr((t_nubit32)vcpuins.crm, (t_nubit32)vcpuins.cimm, _GetOperandSize * 8));
    vcpuins.crm = vcpuins.result;
    _chr(_m_write_rm(_GetOperandSize));
    _ce;
}
static void LFS_R32_M16_32() {
    ...
    _chr(_m_read_rm(2));
    selector = GetMax16(vcpuins.crm);
    _chr(_e_load_far(&vcpu.fs, vcpuins.rr, selector, offset, _GetOperandSize));
    _ce;
}
static void LGS_R32_M16_32() {
    ...
    _chr(_m_read_rm(2));
    selector = GetMax16(vcpuins.crm);
    _chr(_e_load_far(&vcpu.gs, vcpuins.rr, selector, offset, _GetOperandSize));
    _ce;
}
...
static void INS_0F_BA() {
    ...
    case 7: /* BTC_RM32_I8 */
        _bb("BTC_RM32_I8");
        _chr(_a_btc((t_nubit32)vcpuins.crm, (t_nubit32)vcpuins.cimm, _GetOperandSize * 8));
        vcpuins.crm = vcpuins.result;
        _chr(_m_write_rm(_GetOperandSize));
        _be;
        break;
...
static void BSF_R32_RM32() {
    ...
    _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_a_bscc(vcpuins.crm, _GetOperandSize * 8, 1));
    if (!_GetEFLAGS_ZF) {
        _bb("EFLAGS_ZF(0)");
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
        _be;
...
static void BSR_R32_RM32() {
    ...
    _chr(_d_modrm(_GetOperandSize, _GetOperandSize));
    _chr(_m_read_rm(_GetOperandSize));
    _chr(_a_bscc(vcpuins.crm, _GetOperandSize * 8, 0));
    if (!_GetEFLAGS_ZF) {
        _bb("EFLAGS_ZF(0)");
        _chr(_m_write_ref(vcpuins.rr, GetRef(vcpuins.result), _GetOperandSize));
        _be;
...
#if VCPUINS_TRACE == 1
    if (trace.callCount && !vcpuins.except) _SetExcept_CE(trace.cid);
...
static void QDX() {
    ...
    default: /* QDINT */
        _bb("QDINT");
        qdxExecInt(GetMax8(vcpuins.cimm));
        if (vcpuins.cimm < 0x80) {
            _chr(_s_read_ss(vcpu.sp + 4, GetRef(flags), 2));
            MakeBit(flags, VCPU_EFLAGS_ZF, _GetEFLAGS_ZF);
            MakeBit(flags, VCPU_EFLAGS_CF, _GetEFLAGS_CF);
            _chr(_s_write_ss(vcpu.sp + 4, GetRef(flags), 2));
        }
        _be;
        break;
    }
...
void vcpuinsInit() {
    MEMSET(&vcpuins, Zero8, sizeof(t_cpuins));
