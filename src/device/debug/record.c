...
void recordNow(const t_strptr fname) {
	vrecord.flagnow = 1;
	if (!fname) {
		utilsPrint("ERROR:\tinvalid file name.\n");
		return;
	}
...
void recordDump(const t_strptr fname) {
	...
	if (!vrecord.size) {
		utilsPrint("ERROR:\tno record to dump.\n");
		return;
	}
	...
	if (!vrecord.flagready) {
		...
		if (vrecord.flagnow) {
			if (!vrecord.fp) {
				...
				utilsPrint("Record dumping began.\n");
				vrecord.flagready = 1;
			}
		} else {
			if (vrecord.fp) fclose(vrecord.fp);
			vrecord.fp = NULL;
			utilsPrint("Record began.\n");
			vrecord.flagready = 1;
		}
	}
	if (vrecord.flagready) {
		...
		if (!device.flagRun) {
			...
			if (vrecord.flagnow && vrecord.fp) {
				fclose(vrecord.fp);
				utilsPrint("Record dumping ended.\n");
			} else {
				utilsPrint("Record ended.\n");
			}
			vrecord.flagrecord = 0;
			vrecord.flagnow = 0;
			vrecord.fp = NULL;
