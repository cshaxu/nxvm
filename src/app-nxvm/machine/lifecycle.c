...
static void vm_machine_execution_provider_reset(void *context)
{
...
static lib_bool vm_machine_driver_reset(void *context)
{
    ...
    return vm_machine_finish_reset(machine, status) == LIB_STATUS_OK;
}
...
lib_status vm_machine_initialize(vm_machine *machine) {
    lib_status status;
    if (machine == LIB_NULL) return LIB_STATUS_INVALID_ARGUMENT;
    if (machine->active) return LIB_STATUS_INVALID_STATE;
