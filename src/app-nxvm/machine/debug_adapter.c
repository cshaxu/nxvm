...
{
    switch (status) {
    case LIB_STATUS_OK: return LIB_STATUS_OK;
    case LIB_STATUS_INVALID_ARGUMENT: return LIB_STATUS_INVALID_ARGUMENT;
    case LIB_STATUS_INVALID_STATE: return LIB_STATUS_INVALID_STATE;
    case LIB_STATUS_UNSUPPORTED: return LIB_STATUS_UNSUPPORTED;
    case LIB_STATUS_NO_MEMORY: return LIB_STATUS_NO_MEMORY;
    case LIB_STATUS_INTERNAL_ERROR: return LIB_STATUS_IO_ERROR;
    }
    return LIB_STATUS_IO_ERROR;
}

static lib_status vm_machine_debug_map_watch(
    x86_debug_watch_kind source,
    core_machine_debug_watch_kind *out_target)
{
    if (out_target == LIB_NULL) return LIB_STATUS_INVALID_ARGUMENT;
    switch (source) {
    case X86_DEBUG_WATCH_READ: *out_target = CORE_MACHINE_DEBUG_WATCH_READ; return LIB_STATUS_OK;
    case X86_DEBUG_WATCH_WRITE: *out_target = CORE_MACHINE_DEBUG_WATCH_WRITE; return LIB_STATUS_OK;
    case X86_DEBUG_WATCH_EXECUTE: *out_target = CORE_MACHINE_DEBUG_WATCH_EXECUTE; return LIB_STATUS_OK;
    ...
    const x86_debug_request *request, x86_debug_response *out_result)
{
    ...
    if (request->operation == X86_DEBUG_SET_WATCH || request->operation == X86_DEBUG_CLEAR_WATCH) {
        if (vm_machine_debug_map_watch(request->watch_kind, &watch_kind) != LIB_STATUS_OK) return LIB_STATUS_INVALID_ARGUMENT;
    ...
    if (request->operation == X86_DEBUG_SET_EXECUTION_PLAN) {
        return vm_machine_debug_set_execution_plan(&machine->debug, request) ==
            LIB_STATUS_OK ? LIB_STATUS_OK : LIB_STATUS_INVALID_ARGUMENT;
    }
