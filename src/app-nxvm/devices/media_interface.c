...
static void core_machine_media_set_result(core_machine_media_result *out_result,
...
static lib_status core_machine_media_get_binding(
    const core_machine_media_registry *registry, core_machine_media_id id,
    ...
    core_machine_media_result *out_result)
{
    ...
    if (out_binding == LIB_NULL || out_result == LIB_NULL || registry == LIB_NULL ||
        !registry->frozen) return LIB_STATUS_INVALID_STATE;
...
static lib_status core_machine_media_get_sector_range(
    const core_machine_media_registry *registry, core_machine_media_id id,
    ...
    lib_u32 *out_byte_count, core_machine_media_result *out_result)
{
    ...
    if (status != LIB_STATUS_OK || *out_result != CORE_MACHINE_MEDIA_RESULT_OK)
        return status;
...
lib_status core_machine_media_registry_create(core_machine_media_registry **out_registry)
{
    ...
    if (out_registry == LIB_NULL) return LIB_STATUS_INVALID_ARGUMENT;
    *out_registry = LIB_NULL;
    registry = (core_machine_media_registry *)lib_allocate_zero(1u, sizeof(*registry));
    if (registry == LIB_NULL) return LIB_STATUS_NO_MEMORY;
    ...
    const core_machine_media_provider *provider)
{
    ...
    if (registry == LIB_NULL || provider == LIB_NULL || provider->query == LIB_NULL ||
        id == CORE_MACHINE_MEDIA_ID_INVALID) return LIB_STATUS_INVALID_ARGUMENT;
    if (registry->frozen) return LIB_STATUS_INVALID_STATE;
...
{
    if (registry == LIB_NULL) return LIB_STATUS_INVALID_ARGUMENT;
    if (registry->frozen) return LIB_STATUS_INVALID_STATE;
    ...
    core_machine_media_result *out_result)
{
    ...
    if (out_info == LIB_NULL || out_result == LIB_NULL) return LIB_STATUS_INVALID_ARGUMENT;
    lib_memory_set(out_info, 0u, sizeof(*out_info));
    status = core_machine_media_get_binding(registry, id, &binding, out_result);
    if (status != LIB_STATUS_OK) return status;
    if (binding->provider->query == LIB_NULL) return LIB_STATUS_UNSUPPORTED;
    ...
    core_machine_media_result *out_result)
{
    ...
    status = core_machine_media_get_binding(registry, id, &binding, out_result);
    if (status != LIB_STATUS_OK) return status;
    if (binding->provider->read_bytes == LIB_NULL) return LIB_STATUS_UNSUPPORTED;
    core_machine_media_set_result(out_result,
    ...
    lib_u32 byte_count, core_machine_media_result *out_result)
{
    ...
    status = core_machine_media_get_binding(registry, id, &binding, out_result);
    if (status != LIB_STATUS_OK) return status;
    if (binding->provider->write_bytes == LIB_NULL) return LIB_STATUS_UNSUPPORTED;
    core_machine_media_set_result(out_result,
    ...
    void *buffer, core_machine_media_result *out_result)
{
    lib_u64 offset;
    lib_u32 byte_count;
    lib_status status = core_machine_media_get_sector_range(registry, id,
    ...
    core_machine_media_id id, core_machine_media_result *out_result)
{
    ...
    if (out_result == LIB_NULL) return LIB_STATUS_INVALID_ARGUMENT;
    status = core_machine_media_query(registry, id, &info, out_result);
    ...
    core_machine_media_address_mark *out_mark, core_machine_media_result *out_result)
{
    ...
    if (out_mark == LIB_NULL) return LIB_STATUS_INVALID_ARGUMENT;
