...
int main(void)
{
    ...
    nxvm_baseline_cpu_probe_capture capture;
    int failed = 0;

    if (!nxvm_baseline_cpu_probe_begin()) {
        return 1;
    }
    failed |= !nxvm_baseline_cpu_probe_step(mov_ax, sizeof(mov_ax), &capture);
    failed |= !expect_capture(&capture, 0x00001234u, 3u, 0u);
    failed |= !nxvm_baseline_cpu_probe_step(add_ax, sizeof(add_ax), &capture);
    failed |= !expect_capture(&capture, 0x00001234u, 3u, 0u);
    failed |= !nxvm_baseline_cpu_probe_step(
    ...
    failed |= !nxvm_baseline_cpu_probe_step(invalid, sizeof(invalid), &capture);
