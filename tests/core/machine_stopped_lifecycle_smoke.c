    ...
    core_machine_lifecycle expected)
{
    ...
    return core_machine_get_lifecycle(machine, &actual) != NTVDM64_STATUS_OK ||
        actual != expected;
}
...
static C_INT expect_reset_vector(core_machine *machine)
{
    ...
    return core_machine_get_cpu_state(machine, &cpu) != NTVDM64_STATUS_OK ||
...
C_INT main(C_VOID)
{
    ...
    failed |= test_core_machine_create_executor(0u, &machine) != NTVDM64_STATUS_OK;
