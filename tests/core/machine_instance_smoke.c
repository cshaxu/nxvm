...
int main(void)
{
    ...
    result |= expect_status(core_machine_create(&config, &second),
                            NXVM_CORE_STATUS_OK);
    result |= expect_status(core_machine_reset(first), NXVM_CORE_STATUS_OK);
    result |= expect_status(core_machine_reset(second), NXVM_CORE_STATUS_OK);
    ...
    result |= expect_status(core_machine_memory_write(first, 0u, &value, 1u),
                            NXVM_CORE_STATUS_OK);
    result |= expect_status(core_machine_set_a20(first, 1), NXVM_CORE_STATUS_OK);
    value = 0x22u;
    ...
    result |= expect_status(core_machine_memory_read(first, 0x100000u, &value, 1u),
                            NXVM_CORE_STATUS_OK);
    result |= value != 0x22u;
    result |= expect_status(core_machine_set_a20(first, 0), NXVM_CORE_STATUS_OK);
    value = 0u;
    result |= expect_status(core_machine_memory_read(first, 0x100000u, &value, 1u),
                            NXVM_CORE_STATUS_OK);
    result |= value != 0x11u;
