...
int main(void) {
 ...
 if (core_machine_firmware_register_service(&f,&post) || core_machine_firmware_register_service(&f,&rom) || core_machine_firmware_register_service(&f,&in) || core_machine_firmware_service_at(&f,0u)!=&rom || core_machine_firmware_register_service(&f,&duplicate)!=NXVM_CORE_STATUS_UNSUPPORTED || core_machine_firmware_freeze(&f) || core_machine_firmware_register_service(&f,&post)!=NXVM_CORE_STATUS_INVALID_STATE) return 1;
