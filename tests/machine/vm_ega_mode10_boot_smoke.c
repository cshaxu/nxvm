...
static C_INT t285_mode10_write_fixture(C_CHAR path[MAX_PATH])
{
    static const type_unsigned_8 boot_code[] = {
        ...
        0xb0u, 0x01u, 0xa2u, 0x04u, 0x05u,     /* mov al,1; mov [0504h],al */
        /* Leave a bounded observation window before mode 03h. */
...
C_INT main(C_VOID)
{
    ...
    for (instruction = 0u; instruction < T285_MODE10_BOOT_BUDGET; ++instruction) {
        ...
        if (saw_mode10 && !saw_pixels &&
            core_machine_memory_read(session->core_machine, 0x0504u,
    ...
    if (saw_mode10 && saw_pixels && saw_text) {
        STD_PRINTF("M5:T285:S3:EGA-MODE10:BOOT:OK\n");
        return 0;
    }
    STD_FPRINTF(STD_STDERR,
        "M5:T285:S3:EGA-MODE10:BOOT:FAIL mode10=%04x text=%04x bda=%02x ready=%u frame=%d pixels=%u/%u/%u/%u palette=%06x saw=%d/%d/%d\n",
        mode10_query, text_query, bda_mode, graphics_ready, (C_INT)snapshot.kind,
        observed_pixels[0], observed_pixels[1], observed_pixels[2], observed_pixels[3],
        snapshot.palette_rgb[15], saw_mode10, saw_write_frame, saw_text);
    return 1;
}
