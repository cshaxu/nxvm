    ...
    type_unsigned_64 expected_ticks)
{
    ...
    if (vm_session_virtual_time_on_waiting(session, &waiting, &advanced) != TYPE_STATUS_OK ||
        !advanced || core_machine_get_elapsed_ticks(session->core_machine, &after) !=
            TYPE_STATUS_OK || after != before + expected_ticks) {
...
int main(void)
{
    ...
    failed |= verify_wait_speed(default_session, VM_SESSION_SPEED_TURBO,
        1u);
    failed |= verify_wait_speed(model_339_session, VM_SESSION_SPEED_TURBO,
        1u);
    vm_session_destroy(default_session);
    vm_session_destroy(model_339_session);
    if (failed) return 1;
    puts("M5:T459:S1:SESSION-SPEED-POLICY:OK");
    return 0;
}
