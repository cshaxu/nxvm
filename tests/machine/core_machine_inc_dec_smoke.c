...
static C_INT inc_dec_test_test_rm_reg_forms(C_VOID)
{
    ...
    static const uint8_t lengths[] = { 2u,2u,3u,4u,4u,5u };
    uint8_t form;

    for (form = 0u; form != sizeof(lengths); ++form) {
        const uint8_t bytes = form == 0u || form == 3u ? 1u :
        ...
        for (pass = 0u; pass != 2u; ++pass) {
            const uint32_t destination = mask;
            const uint32_t source = pass ? 0u : (bytes == 1u ? 0x80u :
                (bytes == 2u ? 0x8000u : 0x80000000u));
            const uint32_t initial_flags = VCPU_EFLAGS_CF | VCPU_EFLAGS_OF |
                VCPU_EFLAGS_AF | VCPU_EFLAGS_ZF | VCPU_EFLAGS_PF;
            const uint32_t expected_flags = pass ?
...
{
    static const uint8_t address_code[] = { 0x67u,0x66u,0x85u,0x16u };
    static const uint8_t rejected_prefix[] = { 0x66u,0x85u,0xd1u };
    static const uint8_t accepted_legacy[] = { 0x85u,0xd1u };
    const uint32_t initial_flags = VCPU_EFLAGS_CF | VCPU_EFLAGS_OF |
        VCPU_EFLAGS_AF | VCPU_EFLAGS_ZF | VCPU_EFLAGS_PF;
    inc_dec_machine state;
    t_cpu after;
...
static C_INT inc_dec_test_adc_forms(C_VOID)
{
    ...
    for (form = 0u; form != sizeof(lengths); ++form) {
        const uint8_t bytes = (form == 0u || form == 3u || form == 6u ||
        ...
        if (!failed) {
            ...
            state.machine->executor_cpu.data.eflags = VCPU_EFLAGS_CF;
            if (memory) {
                const uint32_t memory_value = memory_source ? 0u : destination;
                failed |= core_machine_memory_write(state.machine, INC_DEC_MEMORY,
...
{
    static const uint8_t address_code[] = { 0x67u,0x66u,0x11u,0x16u };
    static const uint8_t rejected[] = { 0x66u,0x11u,0xd1u };
    static const uint8_t legacy[] = { 0x11u,0xd1u };
    static const uint8_t fault_code[] = { 0x11u,0x16u,0x10u,0u };
    inc_dec_machine state;
    t_cpu after;
    core_machine_cpu_diagnostic diagnostic;
    core_machine_run_result result;
...
{
    static const uint8_t overflow[][6] = {
        { 0x1cu, 0u },
        { 0x1du, 0u, 0u },
        { 0x66u, 0x1du, 0u, 0u, 0u, 0u }
    };
    static const uint8_t overflow_lengths[] = { 2u, 3u, 6u };
    static const uint8_t sign_extended_immediate[][4] = {
        { 0x80u, 0xd9u, 0xffu },
        { 0x83u, 0xd9u, 0xffu },
        { 0x66u, 0x83u, 0xd9u, 0xffu }
    };
    static const uint8_t sign_extended_immediate_lengths[] = { 3u, 3u, 4u };
    static const uint8_t address_code[] = { 0x67u, 0x66u, 0x19u, 0x16u };
    static const uint8_t rejected[] = { 0x66u, 0x19u, 0xd1u };
    static const uint8_t legacy[] = { 0x19u, 0xd1u };
    static const uint8_t fault_code[] = { 0x19u, 0x16u, 0x10u, 0u };
    uint8_t form;
    uint8_t pass;

    for (form = 0u; form != sizeof(overflow_lengths); ++form) {
        const uint8_t bytes = form == 0u ? 1u : (form == 1u ? 2u : 4u);
        const uint32_t initial = bytes == 1u ? 0x11223380u :
            (bytes == 2u ? 0x11228000u : 0x80000000u);
        const uint32_t expected = bytes == 1u ? 0x1122337fu :
            (bytes == 2u ? 0x11227fffu : 0x7fffffffu);
        inc_dec_machine state;
        t_cpu after;
        core_machine_cpu_diagnostic diagnostic;
...
static C_INT inc_dec_test_or_rm_reg_forms(C_VOID)
{
    ...
    for (form = 0u; form != sizeof(lengths); ++form) {
        ...
        const uint32_t mask = bytes == 1u ? 0xffu :
            (bytes == 2u ? 0xffffu : 0xffffffffu);
        const uint32_t destination = bytes == 1u ? 0x11223380u :
            (bytes == 2u ? 0x11228000u : 0x80000000u);
        const uint32_t expected = destination | 1u;
        const uint32_t flags = VCPU_EFLAGS_SF |
        ...
        if (!failed) {
            ...
            if (memory) {
                const uint32_t value = memory_source ? 1u : destination;
                failed |= core_machine_memory_write(state.machine, INC_DEC_MEMORY,
                    &value, bytes) != TYPE_STATUS_OK;
            }
            failed |= !inc_dec_run(&state, forms[form], lengths[form], 0, &after,
...
static C_INT inc_dec_test_or_immediate_forms(C_VOID)
{
    ...
    for (form = 0u; form != sizeof(lengths); ++form) {
        ...
        const C_INT signed_immediate = form == 6u || form == 7u ||
            form == 11u || form == 12u;
        const uint32_t destination = signed_immediate ? 0u :
            (bytes == 1u ? 0x11223380u : (bytes == 2u ? 0x11228000u :
                0x80000000u));
        const uint32_t expected = signed_immediate ?
...
{
    static const uint8_t address_code[] = { 0x67u, 0x66u, 0x09u, 0x16u };
    static const uint8_t rejected[] = { 0x66u, 0x09u, 0xd1u };
    static const uint8_t legacy[] = { 0x09u, 0xd1u };
    static const uint8_t fault_code[] = { 0x09u, 0x16u, 0x10u, 0u };
    uint8_t pass;
    ...
    for (pass = 0u; pass != 2u; ++pass) {
        ...
        core_machine_cpu_diagnostic diagnostic;
        core_machine_run_result result;
        uint16_t before = 0x8000u;
        uint16_t observed = 0u;
        const uint32_t flags = VCPU_EFLAGS_AF | VCPU_EFLAGS_CF | VCPU_EFLAGS_OF;
        C_INT failed = !inc_dec_prepare_protected(0, pass == 0u, &state);
...
static C_INT inc_dec_test_and_forms(C_VOID)
{
    ...
    static const uint8_t lengths[] = { 2u, 2u, 3u, 2u, 2u, 3u, 4u, 4u,
        5u, 4u, 4u, 5u };
    uint8_t form;
    ...
    for (form = 0u; form != sizeof(lengths); ++form) {
        ...
        const uint32_t mask = bytes == 1u ? 0xffu :
            (bytes == 2u ? 0xffffu : 0xffffffffu);
        const uint32_t destination = bytes == 1u ? 0x11223381u :
            (bytes == 2u ? 0x11228081u : 0x80000081u);
        const uint32_t result = destination & mask;
        const uint32_t expected = (destination & ~mask) | result;
        inc_dec_machine state;
        t_cpu after;
        core_machine_cpu_diagnostic diagnostic;
        uint32_t observed = 0u;
        ...
        if (!failed) {
            ...
            state.machine->executor_cpu.data.eflags = VCPU_EFLAGS_AF |
                VCPU_EFLAGS_CF | VCPU_EFLAGS_OF;
            if (memory) {
                const uint32_t value = memory_source ? mask : destination;
                failed |= core_machine_memory_write(state.machine, INC_DEC_MEMORY,
                    &value, bytes) != TYPE_STATUS_OK;
            }
            failed |= !inc_dec_run(&state, forms[form], lengths[form], 0, &after,
...
static C_INT inc_dec_test_and_immediate(C_VOID)
{
    ...
    static const uint8_t lengths[] = { 2u, 3u, 6u, 3u, 4u, 7u, 3u, 4u,
        5u, 6u, 9u, 5u, 6u };
    uint8_t form;
...
{
    static const uint8_t address_code[] = { 0x67u, 0x66u, 0x21u, 0x16u };
    static const uint8_t rejected[] = { 0x66u, 0x21u, 0xd1u };
    static const uint8_t legacy[] = { 0x21u, 0xd1u };
    static const uint8_t fault_code[] = { 0x21u, 0x16u, 0x10u, 0u };
    uint8_t pass;
...
static C_INT inc_dec_test_sub_forms(C_VOID)
{
    ...
    static const uint8_t lengths[] = { 2u, 2u, 3u, 2u, 2u, 3u, 4u, 4u, 5u, 4u,
    ...
    for (form = 0u; form != sizeof(lengths); ++form) {
        ...
        if (!failed) {
            ...
            state.machine->executor_cpu.data.edx = register_destination || memory_source ?
                0u : 1u;
            if (memory) {
                const uint32_t value = memory_source ? 1u : 0u;
                failed |= core_machine_memory_write(state.machine, INC_DEC_MEMORY,
                    &value, bytes) != TYPE_STATUS_OK;
            }
            failed |= !inc_dec_run(&state, forms[form], lengths[form], 0, &after,
...
static C_INT inc_dec_test_sub_boundaries(C_VOID)
{
    ...
    static const uint8_t lengths[] = { 2u, 3u, 6u };
    static const uint8_t signext[][4] = {
        { 0x80u, 0xe9u, 0xffu }, { 0x83u, 0xe9u, 0xffu },
        { 0x66u, 0x83u, 0xe9u, 0xffu }
    };
    static const uint8_t signlengths[] = { 3u, 3u, 4u };
    uint8_t form;
...
{
    static const uint8_t address_code[] = { 0x67u, 0x66u, 0x29u, 0x16u };
    static const uint8_t rejected[] = { 0x66u, 0x29u, 0xd1u };
    static const uint8_t legacy[] = { 0x29u, 0xd1u };
    static const uint8_t fault_code[] = { 0x29u, 0x16u, 0x10u, 0u };
    uint8_t pass;
    {
        inc_dec_machine state;
        t_cpu after;
...
static C_INT inc_dec_test_xor_forms(C_VOID)
{
    ...
    static const uint8_t lengths[] = { 2u, 2u, 3u, 2u, 2u, 3u, 4u, 4u, 5u,
        4u, 4u, 5u };
    uint8_t form;
    ...
    for (form = 0u; form != sizeof(lengths); ++form) {
        ...
        const uint32_t mask = bytes == 1u ? 0xffu :
            (bytes == 2u ? 0xffffu : 0xffffffffu);
        const uint32_t value = bytes == 1u ? 0x11223381u :
            (bytes == 2u ? 0x11228081u : 0x80000081u);
        inc_dec_machine state;
        t_cpu after;
        core_machine_cpu_diagnostic diagnostic;
...
static C_INT inc_dec_test_xor_immediates(C_VOID)
{
    ...
    for (form = 0u; form != sizeof(lengths); ++form) {
        ...
        const C_INT memory = form >= 8u;
        const C_INT accumulator = form < 3u;
        const uint32_t value = bytes == 1u ? 0x112233ffu :
...
{
    static const uint8_t address_code[] = { 0x67u, 0x66u, 0x31u, 0x16u };
    static const uint8_t rejected[] = { 0x66u, 0x31u, 0xd1u };
    static const uint8_t legacy[] = { 0x31u, 0xd1u };
    static const uint8_t fault_code[] = { 0x31u, 0x16u, 0x10u, 0u };
    uint8_t pass;
    ...
    for (form = 0u; form != sizeof(lengths); ++form) {
        ...
        if (!failed) {
            ...
            state.machine->executor_cpu.data.edx = register_destination || memory_source ?
                destination : source;
            if (memory) {
                const uint32_t value = memory_source ? source : destination;
                failed |= core_machine_memory_write(state.machine, INC_DEC_MEMORY,
                    &value, bytes) != TYPE_STATUS_OK;
            }
            failed |= !inc_dec_run(&state, forms[form], lengths[form], 0, &after,
...
{
    static const uint8_t address_code[] = { 0x67u, 0x66u, 0x39u, 0x16u };
    static const uint8_t rejected[] = { 0x66u, 0x39u, 0xd1u };
    static const uint8_t legacy[] = { 0x39u, 0xd1u };
    static const uint8_t fault_codes[][4] = {
