...
typedef struct imul_provider { uint32_t reads; } imul_provider;
...
static C_INT imul_prepare(core_machine_cpu_profile p,imul_provider *provider,imul_machine *s)
{
 ...
 if(core_machine_create(&c,&s->machine)!=TYPE_STATUS_OK||(provider!=STD_NULL&&test_core_machine_fixture_register_memory_device_provider(s->machine,IMUL_PROVIDER_ADDRESS,4u,imul_read,imul_write,imul_query,provider)!=TYPE_STATUS_OK)||core_machine_bind_execution_provider(s->machine,&imul_execution,s)!=TYPE_STATUS_OK||core_machine_freeze_execution_providers(s->machine)!=TYPE_STATUS_OK||core_machine_reset(s->machine)!=TYPE_STATUS_OK){core_machine_destroy(s->machine);s->machine=STD_NULL;return 0;}return 1;
}
static C_INT imul_run(imul_machine *s,const uint8_t *code,STD_SIZE_T bytes,C_INT fault,t_cpu *out,core_machine_cpu_diagnostic *d)
...
{
 static const int32_t left[]={-2,0x7fff,-2,0x7fffffff};static const int32_t right[]={3,2,0x40000000,2};
 uint8_t width,memory,overflow;
 for(width=0u;width<2u;++width)for(memory=0u;memory<2u;++memory)for(overflow=0u;overflow<2u;++overflow){
  uint8_t code[6]={0};STD_SIZE_T n=0u;const int32_t l=left[width*2u+overflow],r=right[width*2u+overflow];const int64_t product=(int64_t)l*(int64_t)r;const uint32_t expected=(uint32_t)product;const C_INT ov=width?(product>INT32_MAX||product<INT32_MIN):(product>INT16_MAX||product<INT16_MIN);imul_machine s;t_cpu after;core_machine_cpu_diagnostic d;uint32_t read=0u;C_INT failed=!imul_prepare(CORE_MACHINE_CPU_PROFILE_80386,STD_NULL,&s);
...
{
 static const uint8_t code[]={0x0fu,0xafu,0x0eu,0x00u,0x50u};core_machine_cpu_profile ps[]={CORE_MACHINE_CPU_PROFILE_80186,CORE_MACHINE_CPU_PROFILE_80286};uint8_t i;
...
{
 static const uint8_t p[]={0x1f,0,0,3,0,0},g[]={0,0,0,0,0,0,0,0,0xff,0xff,0,0x20,0,0x9a,0,0,0x0f,0,0,0x30,0,0x92,0,0,0xff,0xff,0,0x40,0,0x92,0,0},b[]={0x0f,1,0x16,0,1,0xb8,1,0,0x0f,1,0xf0,0xb8,0x10,0,0x8e,0xd8,0x8e,0xc0,0xb8,0x18,0,0x8e,0xd0,0xbc,0,0x80,0xea,0,0,8,0},h[]={0xf4};core_machine_run_result r;
...
{
 static const uint8_t code[]={0x0f,0xafu,0x0eu,0x10u,0};imul_machine s;t_cpu a;core_machine_cpu_diagnostic d;core_machine_run_result r;const uint32_t flags=VCPU_EFLAGS_CF|VCPU_EFLAGS_OF;C_INT f=!imul_limit(&s);
