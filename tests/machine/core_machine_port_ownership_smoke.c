...
static C_INT core_machine_port_probe_prepare(core_machine **out_machine,
    ...
    core_machine_port_probe_state *fdc_state)
{
    ...
    failed |= out_machine == STD_NULL || port_state == STD_NULL ||
        fdc_state == STD_NULL;
    ...
    failed |= !failed && core_machine_configure_dma(machine, &dma_wiring,
        &dma_request) != TYPE_STATUS_OK;
    failed |= !failed && core_machine_install_port_provider(machine, 0x03f2u,
        0x03f2u, &read_provider, fdc_state) != TYPE_STATUS_OK;
    fdc_topology.dma_request = dma_request;
    failed |= !failed && core_machine_configure_fdc(machine, &fdc_topology) !=
...
C_INT main(C_VOID)
{
    ...
    core_machine_port_probe_state fdc_state = {0u, 0u, 0u};
    ...
    if (!failed) {
        failed |= core_machine_bus_write(machine, 0x00e0u, 0x33u) !=
            ...
            value != 0xa5u ||
            core_machine_bus_read(machine, 0x03f2u, &value) != TYPE_STATUS_OK ||
            value != 0x5au || fdc_state.reads != 1u ||
    ...
    if (failed) {
        STD_FPRINTF(STD_STDERR,
            "M5:T300:S1:PORT-OWNERSHIP:FAIL reads=%u writes=%u fdc_reads=%u\\n",
            port_state.reads, port_state.writes, fdc_state.reads);
        return 1;
    }
