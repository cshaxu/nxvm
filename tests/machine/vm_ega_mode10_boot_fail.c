#include "type.h"

#include <windows.h>

#include "core/machine/machine_interface.h"
#include "vm/composition/session/session_interface.h"
#include "vm/composition/session/session.h"

#define T284_MODE10_IMAGE_BYTES (1440u * 1024u)
#define T284_MODE10_BOOT_BUDGET 500000u

static uint8_t t284_mode10_image[T284_MODE10_IMAGE_BYTES];

static C_INT t284_mode10_write_fixture(C_CHAR path[MAX_PATH])
{
    static const uint8_t boot_code[] = {
        0x31u, 0xc0u, 0x8eu, 0xd8u,             /* xor ax,ax; mov ds,ax */
        0xb8u, 0x10u, 0x00u, 0xcdu, 0x10u,      /* mov ax,0010h; int 10h */
        0xb4u, 0x0fu, 0xcdu, 0x10u,             /* mov ah,0fh; int 10h */
        0xa3u, 0x00u, 0x05u,                    /* mov [0500h],ax */
        0xb8u, 0x00u, 0xa0u, 0x8eu, 0xc0u,      /* mov ax,a000h; mov es,ax */
        0xbau, 0xc4u, 0x03u, 0xb0u, 0x02u, 0xeeu, 0x42u,
        0xb0u, 0x01u, 0xeeu, 0x31u, 0xffu, 0xb0u, 0x80u, 0xaau,
        0xbau, 0xc4u, 0x03u, 0xb0u, 0x02u, 0xeeu, 0x42u,
        0xb0u, 0x02u, 0xeeu, 0xb0u, 0x40u, 0xaau,
        0xbau, 0xc4u, 0x03u, 0xb0u, 0x02u, 0xeeu, 0x42u,
        0xb0u, 0x04u, 0xeeu, 0xb0u, 0x20u, 0xaau,
        0xbau, 0xc4u, 0x03u, 0xb0u, 0x02u, 0xeeu, 0x42u,
        0xb0u, 0x08u, 0xeeu, 0xb0u, 0x10u, 0xaau,
        0xb8u, 0x03u, 0x00u, 0xcdu, 0x10u,      /* mov ax,0003h; int 10h */
        0xb4u, 0x0fu, 0xcdu, 0x10u,             /* mov ah,0fh; int 10h */
        0xa3u, 0x02u, 0x05u, 0xf4u, 0xebu, 0xfeu /* mov [0502h],ax; hlt; jmp $ */
    };
    STD_FILE *file;
    DWORD length;

    length = GetTempPathA(MAX_PATH, path);
    if (length == 0u || length >= MAX_PATH ||
        GetTempFileNameA(path, "n64", 0u, path) == 0u) return 0;
    STD_MEMSET(t284_mode10_image, 0, sizeof(t284_mode10_image));
    STD_MEMCPY(t284_mode10_image, boot_code, sizeof(boot_code));
    t284_mode10_image[510u] = 0x55u;
    t284_mode10_image[511u] = 0xaau;
    file = STD_FOPEN(path, "wb");
    if (file == STD_NULL) return 0;
    if (STD_FWRITE(t284_mode10_image, 1u, sizeof(t284_mode10_image), file) !=
        sizeof(t284_mode10_image)) {
        STD_FCLOSE(file);
        DeleteFileA(path);
        return 0;
    }
    STD_FCLOSE(file);
    return 1;
}

C_INT main(C_VOID)
{
    const vm_session_config config = {
        .fdd_image = "",
        .cpu_profile = CORE_MACHINE_CPU_PROFILE_8086,
        .fpu_profile = CORE_MACHINE_FPU_PROFILE_NONE
    };
    core_machine_run_budget budget = { 1u, 0u };
    core_machine_run_result result;
    core_machine_display_snapshot snapshot;
    vm_session *session = STD_NULL;
    C_CHAR path[MAX_PATH] = {0};
    uint16_t mode10_query = 0u;
    uint16_t text_query = 0u;
    uint8_t bda_mode = 0u;
    uint32_t instruction;
    C_INT saw_mode10 = 0;
    C_INT saw_text = 0;

    if (!t284_mode10_write_fixture(path)) goto done;
    {
        vm_session_config fixture_config = config;
        fixture_config.fdd_image = path;
        if (vm_session_create(&fixture_config, &session) != TYPE_STATUS_OK ||
            session == STD_NULL) goto done;
    }
    for (instruction = 0u; instruction < T284_MODE10_BOOT_BUDGET; ++instruction) {
        if (core_machine_run(session->core_machine, budget, &result) !=
                TYPE_STATUS_OK || result.reason == CORE_MACHINE_STOP_FAULT ||
            core_machine_capture_display_snapshot(session->core_machine,
                &snapshot) != TYPE_STATUS_OK) goto done;
        if (snapshot.pixel_width == 640u && snapshot.pixel_height == 350u &&
            core_machine_memory_read(session->core_machine, 0x0449u, &bda_mode,
                sizeof(bda_mode)) == TYPE_STATUS_OK && bda_mode == 0x10u &&
            core_machine_memory_read(session->core_machine, 0x0500u, &mode10_query,
                sizeof(mode10_query)) == TYPE_STATUS_OK && mode10_query == 0x5010u) {
            saw_mode10 = 1;
        }
        if (saw_mode10 && snapshot.kind == CORE_MACHINE_DISPLAY_KIND_TEXT &&
            core_machine_memory_read(session->core_machine, 0x0449u, &bda_mode,
                sizeof(bda_mode)) == TYPE_STATUS_OK && bda_mode == 0x03u &&
            core_machine_memory_read(session->core_machine, 0x0502u, &text_query,
                sizeof(text_query)) == TYPE_STATUS_OK && text_query == 0x5003u) {
            saw_text = 1;
            break;
        }
    }

done:
    if (session != STD_NULL && session->core_machine != STD_NULL) {
        (C_VOID)core_machine_memory_read(session->core_machine, 0x0449u, &bda_mode,
            sizeof(bda_mode));
        (C_VOID)core_machine_memory_read(session->core_machine, 0x0500u, &mode10_query,
            sizeof(mode10_query));
        (C_VOID)core_machine_memory_read(session->core_machine, 0x0502u, &text_query,
            sizeof(text_query));
    }
    vm_session_destroy(session);
    if (path[0] != '\0') DeleteFileA(path);
    if (saw_mode10 && saw_text) {
        STD_PRINTF("M5:T284:S2:EGA-MODE10:BOOT:READY\n");
        return 0;
    }
    STD_FPRINTF(STD_STDERR,
        "M5:T284:S2:EGA-MODE10-BOOT:EXPECTED-FAIL mode10=%04x text=%04x bda=%02x saw=%d/%d\n",
        mode10_query, text_query, bda_mode, saw_mode10, saw_text);
    return 1;
}
