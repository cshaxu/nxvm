...
static C_INT run_and_stop(vm_session *session)
{
    ...
    for (waited = 0u; waited < 5000u; ++waited) {
        if (vm_platform_run_handle_is_active(vm_session_fixture_platform_run_handle(session))) break;
    ...
    return !vm_platform_run_handle_is_active(vm_session_fixture_platform_run_handle(session)) &&
