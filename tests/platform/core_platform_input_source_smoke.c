...
C_INT main(C_VOID)
{
    ...
    core_platform_input_source second_source;
    core_platform_input_event event = {0};

    core_platform_input_source_initialize(&source, &sink, &first);
    core_platform_input_source_initialize(&second_source, &sink, &second);
    ...
    core_platform_input_source_stop(&second_source);
    ...
    if (core_platform_input_source_submit(&second_source, &event) !=
        TYPE_STATUS_INVALID_STATE || second.submissions != 1u) return 1;
    ...
    core_platform_input_source_stop(&source);
