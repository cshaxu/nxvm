...
static C_VOID vm_platform_input_smoke_receive_key_event(C_VOID *context,
    uint16_t scan_code, uint16_t virtual_key, C_INT pressed)
{
    (C_VOID)virtual_key;
    ((vm_platform_input_smoke_state *)context)->key_code = scan_code;
    ((vm_platform_input_smoke_state *)context)->pressed = pressed;
}
...
C_INT main(C_VOID)
{
    ...
    vm_platform_keyboard_sink sink = {
        vm_platform_input_smoke_receive_key_event
    };
    ...
    vm_platform_keyboard_transport_initialize(&second_transport, &sink,
                                              &second_state);
    vm_platform_keyboard_receive_key_event_for(&second_transport, 0x1234u,
        0x5678u, 0);
    if (second_state.key_code != 0x1234u || second_state.pressed) {
        return 1;
    }
