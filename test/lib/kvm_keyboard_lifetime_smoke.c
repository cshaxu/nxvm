...
static lib_win32_bool LIB_WIN32_WINAPI translate_unmapped(const lib_win32_msg *message)
{ lib_test_assert(message->wParam == LIB_WIN32_KEY_PACKET); ++translations; return LIB_WIN32_TRUE; }
#undef lib_win32_translate_message
#define lib_win32_translate_message translate_unmapped
static lib_win32_long_ptr LIB_WIN32_WINAPI context_pointer(lib_win32_hwnd window, lib_i32 index)
{ (void)window; (void)index; return (lib_win32_long_ptr)window_context; }
...
{
    lib_test_assert(left->count == right->count);
    for (lib_u32 i = 0; i < left->count; ++i) {
        const kvm_input_event *a = &left->events[i], *b = &right->events[i];
        lib_test_assert(a->source == b->source && a->source_identity == b->source_identity);
        lib_test_assert(a->type == b->type);
        if (a->type == KVM_EVENT_KEY) {
            lib_test_assert(a->data.key.key == b->data.key.key);
            lib_test_assert(a->data.key.scan_code == b->data.key.scan_code);
            lib_test_assert(a->data.key.flags == b->data.key.flags);
            lib_test_assert(a->data.key.modifiers == b->data.key.modifiers);
            lib_test_assert(a->data.key.pressed == b->data.key.pressed);
        } else {
            lib_test_assert(a->type == KVM_EVENT_TEXT);
            lib_test_assert(a->data.text.scalar == b->data.text.scalar);
        }
    }
}
static void permutations(void)
{
    ...
    for (lib_u32 b = 0; b < 6; ++b) {
        ...
        capture c = {0};
        lib_test_assert(kvm_hotkey_registry_register(&registry, triggers[t], 3, "action") == LIB_STATUS_OK);
        kvm_hotkey_matcher_initialize(&matcher, &registry);
        for (lib_u32 n = 0; n < 3; ++n) {
            lib_u32 k = orders[a][n];
            if (k < 2) mods |= 1u << k;
            lib_test_assert(submit(&matcher, &c, keys[k], scans[k], flags[k], mods, 1));
        }
        /* Repeat after modifiers arrive must not consume an earlier ordinary make. */
        for (lib_u32 n = 0; n < 16; ++n)
            lib_test_assert(submit(&matcher, &c, keys[2], scans[2], 0, mods, 1));
        lib_test_assert(matcher.held_count == 3);
        for (lib_u32 n = 0; n < 3; ++n) {
            lib_u32 k = orders[b][n];
            if (k < 2) mods &= ~(1u << k);
            lib_test_assert(submit(&matcher, &c, keys[k], scans[k], flags[k], mods, 0));
        }
        for (lib_u32 n = 0; n < c.count; ++n) {
            kvm_input_event *e = &c.events[n];
            if (e->type == KVM_EVENT_HOTKEY) { ++hotkeys; continue; }
            lib_test_assert(e->type == KVM_EVENT_KEY);
            lib_u32 k = e->data.key.key == keys[0] ? 0 : e->data.key.key == keys[1] ? 1 : 2;
            if (e->data.key.pressed) down[k] = 1;
            else { lib_test_assert(down[k]); down[k] = 0; }
        }
        lib_test_assert(!down[0] && !down[1] && !down[2] && matcher.held_count == 0);
        lib_test_assert(hotkeys == (orders[a][2] == 2 ? 17u : 0u));
...
static void failure_paths(void)
{
    ...
    kvm_hotkey_matcher_discard(&matcher);
    lib_test_assert(submit(&matcher, &c, KVM_KEY_CONTROL, 0x1d, 0, 1, 1));
    lib_test_assert(submit(&matcher, &c, KVM_KEY_ALT, 0x38, 0, 3, 1));
    c.reject_at = 2;
    lib_test_assert(!submit(&matcher, &c, 'A', 0x1e, 0, 3, 1));
    lib_test_assert(!submit(&matcher, &c, 'B', 0x30, 0, 3, 1));
...
static void adapter_equivalence(lib_u32 scan)
{
    ...
    lib_test_assert(kvm_component_initialize(&window.base, &options, no_join, no_dispose,
    ...
    lib_test_assert(kvm_component_initialize(&console.base, &options, no_join, no_dispose,
    ...
    lib_test_assert(translations == 0);
    ...
    win32_window_proc(handle, LIB_WIN32_WM_KEYDOWN, 'A', 0);
    win32_window_proc(handle, LIB_WIN32_WM_KEYDOWN, 'B', 0);
    win32_window_proc(handle, LIB_WIN32_WM_KEYUP, 'B', 0x300000);
    win32_window_proc(handle, LIB_WIN32_WM_KEYUP, 'A', 0x1e0000);
    ...
    lib_test_assert(w.count == 6 && c.count == 6 && translations == 0);
    win32_window_proc(handle, LIB_WIN32_WM_KEYDOWN, LIB_WIN32_KEY_PACKET, 0);
    lib_test_assert(translations == 1 && w.count == 6);
    translations = 0;
    same_input(&w, &c);
    w.count = w.attempts = c.count = c.attempts = 0;
    for (lib_u32 i = 0; i < 2; ++i) {
        lib_u32 unit = i ? 0xde00 : 0xd83d;
        win32_window_proc(handle, LIB_WIN32_WM_CHAR, unit, 0);
        console_record(&console, 0, 0, unit, 1, 0);
        console_record(&console, 0, 0, unit, 0, 0);
    }
    lib_test_assert(w.count == 1 && c.count == 1);
    lib_test_assert(w.events[0].type == KVM_EVENT_TEXT && w.events[0].data.text.scalar == 0x1f600);
    same_input(&w, &c);
    /* Adapters preserve counts; only the shared normalizer expands batches. */
    w.count = w.attempts = c.count = c.attempts = 0;
    win32_window_proc(handle, LIB_WIN32_WM_KEYDOWN, 'A', 0x1e0004);
    win32_window_proc(handle, LIB_WIN32_WM_KEYUP, 'A', 0x1e0001);
    console_record(&console, 'A', 0x1e, 'a', 1, 4);
    console_record(&console, 'A', 0x1e, 'a', 0, 0);
    lib_test_assert(w.count == 5 && c.count == 5);
    same_input(&w, &c);
    w.count = w.attempts = c.count = c.attempts = 0;
    win32_window_proc(handle, LIB_WIN32_WM_CHAR, 0x4e00, 3);
    console_record(&console, 0, 0, 0x4e00, 1, 3);
    lib_test_assert(w.count == 3 && c.count == 3);
    same_input(&w, &c);
    w.count = w.attempts = c.count = c.attempts = 0;
    win32_window_proc(handle, LIB_WIN32_WM_CHAR, 0x4e00, 1);
     ...
     * Expected scalars use supplementary characters, independent of keyboard layout. */
    {
        ...
        for (lib_u32 n = 0; n < sizeof(cases)/sizeof(cases[0]); ++n) {
            ...
            for (lib_u32 i = 0; i < cases[n].length; ++i) {
                win32_window_proc(handle, LIB_WIN32_WM_CHAR, cases[n].units[i], cases[n].repeats[i]);
            ...
            lib_test_assert(w.count == cases[n].expected && c.count == w.count);
            same_input(&w, &c);
            for (lib_u32 i = 0; i < w.count; ++i) {
                if (cases[n].scalar)
                    lib_test_assert(w.events[i].type == KVM_EVENT_TEXT &&
                        w.events[i].data.text.scalar == cases[n].scalar);
                else
                    lib_test_assert(w.events[i].type == KVM_EVENT_KEY &&
                        w.events[i].data.key.key == 'A' &&
                        w.events[i].data.key.pressed == (i % 2 == 0));
            }
            lib_test_assert(context.keyboard_normalizer.pending_high_surrogate == 0 &&
                console.keyboard.pending_high_surrogate == 0);
        }
    }
    /* A physical record cancels an unfinished text prefix on both inputs. */
    w.count = w.attempts = c.count = c.attempts = 0;
    win32_window_proc(handle, LIB_WIN32_WM_CHAR, 0xd83d, 2);
    console_record(&console, 0, 0, 0xd83d, 1, 2);
    win32_window_proc(handle, LIB_WIN32_WM_KEYDOWN, 'A', 0x1e0001);
    console_record(&console, 'A', 0x1e, 'a', 1, 1);
    win32_window_proc(handle, LIB_WIN32_WM_KEYUP, 'A', 0x1e0005);
    console_record(&console, 'A', 0x1e, 'a', 0, 5);
    win32_window_proc(handle, LIB_WIN32_WM_CHAR, 0xde00, 2);
    ...
    win32_window_proc(handle, LIB_WIN32_WM_CHAR, 0xdc00, 0);
    console_record(&console, 0, 0, 0xdc00, 1, 0);
    lib_test_assert(w.count == 1 && c.count == 1);
    w.reject_at = c.reject_at = 3;
    for (lib_u32 i = 0; i < 2; ++i) {
        lib_u32 unit = i ? 0xde00 : 0xd83d;
        win32_window_proc(handle, LIB_WIN32_WM_CHAR, unit, 4);
        console_record(&console, 0, 0, unit, 1, 4);
    }
    lib_test_assert(window.base.stopping && console.base.stopping);
    win32_window_proc(handle, LIB_WIN32_WM_KEYDOWN, 'B', 0);
    console_record(&console, 'B', 0, 'b', 1, 0);
    lib_test_assert(w.attempts == 3 && c.attempts == 3 && w.count == 2 && c.count == 2);
    lib_test_assert(kvm_component_destroy(&window.base) == LIB_STATUS_OK);
    lib_test_assert(kvm_component_destroy(&console.base) == LIB_STATUS_OK);
}
static void repeat_delivery_failure(void)
{
    ...
    win32_window_proc((lib_win32_hwnd)1, LIB_WIN32_WM_KEYDOWN, 'A', 0x1e0005);
    lib_test_assert(c.count == 2 && c.attempts == 3 && window.base.stopping);
    win32_window_proc((lib_win32_hwnd)1, LIB_WIN32_WM_KEYDOWN, 'B', 0x300001);
    lib_test_assert(c.attempts == 3);
    lib_test_assert(kvm_component_destroy(&window.base) == LIB_STATUS_OK);
}
...
static void synthesis_lifetimes(void)
{
    ...
    for (lib_u32 trigger = 0; trigger < 2; ++trigger) {
        ...
        for (lib_u32 i = 0; i < 4; ++i)
            if (i == 3 ? trigger : (mask & (1u << i))) {
                lib_test_assert(submit(&m, &c, keys[i], scans[i], flags[i], mask, 1));
                ++held;
            }
        ...
        for (lib_u32 j = first; j < c.count; ++j)
            if (c.events[j].type == KVM_EVENT_KEY && !c.events[j].data.key.pressed)
                for (lib_u32 i = 0; i < 4; ++i)
                    if (i == 3 ? trigger : (mask & (1u << i)))
                        lib_test_assert(c.events[j].data.key.key != keys[i]);
        for (lib_u32 i = 0; i < 4; ++i)
            if (i == 3 ? trigger : (mask & (1u << i)))
                lib_test_assert(submit(&m, &c, keys[i], scans[i], flags[i], 0, 0));
        lib_test_assert(m.held_count == 0);
    ...
    for (lib_u32 reject = 1; reject <= 4; ++reject) {
        ...
        lib_test_assert(!kvm_keyboard_submit_record(&n, &m, pair, match_normalized, &text));
        lib_test_assert(c.attempts == reject);
        kvm_hotkey_matcher_discard(&m);
    }
}
...
static void physical_identity(void)
{
    ...
    capture c = {0};
    kvm_hotkey_matcher_initialize(&m, LIB_NULL);
    lib_test_assert(submit(&m, &c, KVM_KEY_KEYPAD_7, 0x47, 0, 0, 1));
    lib_test_assert(submit(&m, &c, KVM_KEY_HOME, 0x47, 0, 0, 1));
    lib_test_assert(m.held_count == 1 && c.events[1].data.key.key == KVM_KEY_KEYPAD_7);
    lib_test_assert(submit(&m, &c, KVM_KEY_HOME, 0x47, 0, 0, 0));
    lib_test_assert(m.held_count == 0 && c.count == 3);
    lib_test_assert(c.events[2].data.key.key == KVM_KEY_KEYPAD_7 && !c.events[2].data.key.pressed);
    lib_test_assert(submit(&m, &c, KVM_KEY_HOME, 0x47, KVM_KEY_FLAG_EXTENDED, 0, 1));
    lib_test_assert(submit(&m, &c, KVM_KEY_KEYPAD_7, 0x47, 0, 0, 1));
    lib_test_assert(m.held_count == 2);
    lib_test_assert(submit(&m, &c, KVM_KEY_HOME, 0x47, 0, 0, 0));
    lib_test_assert(submit(&m, &c, KVM_KEY_HOME, 0x47, KVM_KEY_FLAG_EXTENDED, 0, 0));
    lib_test_assert(m.held_count == 0);
    lib_test_assert(submit(&m, &c, 'A', 0, 0, 0, 1));
    lib_test_assert(submit(&m, &c, 'B', 0, 0, 0, 1));
    lib_test_assert(submit(&m, &c, 'A', 0x1e, 0, 0, 1));
    lib_test_assert(m.held_count == 3);
    lib_test_assert(submit(&m, &c, 'A', 0, 0, 0, 0));
    lib_test_assert(submit(&m, &c, 'B', 0, 0, 0, 0));
    lib_test_assert(submit(&m, &c, 'A', 0x1e, 0, 0, 0));
    lib_test_assert(m.held_count == 0);
    kvm_hotkey_matcher_discard(&m);
}
