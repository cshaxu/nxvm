...
{ core_machine *machine = STD_NULL; core_machine_trace_provider trace; fixture state = { { 0 }, 0 }; C_INT failed = 0;
  ...
  failed |= core_machine_set_trace_provider(machine, &trace) != TYPE_STATUS_OK || core_machine_freeze_execution_providers(machine) != TYPE_STATUS_OK || core_machine_reset(machine) != TYPE_STATUS_OK || core_machine_advance_time(machine, 37u) != TYPE_STATUS_OK || state.count != 1 || state.event.value != 37u || state.event.elapsed_ticks != 0u;
