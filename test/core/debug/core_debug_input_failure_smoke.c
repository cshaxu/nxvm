...
static C_INT read_register(C_VOID *context, core_debug_register reg,
...
static C_INT write_register(C_VOID *context, core_debug_register reg,
...
{
    core_debugger *debugger = STD_NULL;
    ...
    if (input == STD_NULL || (text != STD_NULL && STD_FPUTS(text, input) < 0) ||
        ...
        core_debugger_create(&debugger) != TYPE_STATUS_OK) goto done;
    ...
    resume_calls = 0u;
    completion_waits = 0u;
    core_debugger_run(debugger, &target);
    ...
    clearerr(STD_STDIN);
    core_debugger_destroy(debugger);
