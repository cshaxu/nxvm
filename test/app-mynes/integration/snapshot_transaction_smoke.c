...
static lib_status put(void *o,const lib_u8 *b,lib_size n){snapshot_bytes*s=o;if(n>sizeof(s->bytes)-s->size)return LIB_STATUS_LIMIT_EXCEEDED;lib_memory_copy(s->bytes+s->size,b,n);s->size+=n;return LIB_STATUS_OK;}
static lib_status get(void *o,lib_u8*b,lib_size n){snapshot_bytes*s=o;if(n>s->size-s->cursor)return LIB_STATUS_IO_ERROR;lib_memory_copy(b,s->bytes+s->cursor,n);s->cursor+=n;return LIB_STATUS_OK;}
