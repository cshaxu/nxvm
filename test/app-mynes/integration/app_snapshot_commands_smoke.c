...
{ core_driver*d=0;common_machine*m=0;common_machine_driver cd;app_command_context app;common_session_command_result result;
 ...
 app_command_submit_line(&app,COMMON_SESSION_MACHINE_PAUSED,"save mynes-app-snapshot.mns",&result);assert(strcmp(result.text,"Machine saved and paused.\n\n")==0);wait_state(m,COMMON_MACHINE_PAUSED);assert(common_machine_resume(m));wait_state(m,COMMON_MACHINE_RUNNING);
 app_command_submit_line(&app,COMMON_SESSION_MACHINE_RUNNING,"save mynes-app-snapshot.mns",&result);assert(result.text[0]=='\0' && !result.arm_prompt);wait_state(m,COMMON_MACHINE_PAUSED);app_command_note_runtime(&app,COMMON_SESSION_MACHINE_RUNNING,COMMON_SESSION_MACHINE_PAUSED,&result);app_command_note_monitor_current(&app,LIB_TRUE,&result);assert(strcmp(result.text,"Machine saved and paused.\n\n")==0);assert(common_machine_stop(m));wait_state(m,COMMON_MACHINE_STOPPED);
 app_command_submit_line(&app,COMMON_SESSION_MACHINE_STOPPED,"load mynes-app-snapshot.mns",&result);assert(result.text[0]=='\0' && !result.arm_prompt);wait_state(m,COMMON_MACHINE_PAUSED);app_command_note_runtime(&app,COMMON_SESSION_MACHINE_STOPPED,COMMON_SESSION_MACHINE_PAUSED,&result);app_command_note_monitor_current(&app,LIB_TRUE,&result);assert(strcmp(result.text,"Machine loaded and paused.\n\n")==0);assert(common_machine_stop(m));wait_state(m,COMMON_MACHINE_STOPPED);
 app_command_submit_line(&app,COMMON_SESSION_MACHINE_STOPPED,"load missing.mns",&result);assert(strcmp(result.text,"Cannot load machine state.\n\n")==0);assert(common_machine_state_get(m)==COMMON_MACHINE_STOPPED);assert(common_machine_shutdown(m)==LIB_STATUS_OK);assert(common_machine_destroy(m)==LIB_STATUS_OK);assert(core_driver_destroy(d)==LIB_STATUS_OK);return 0; }
