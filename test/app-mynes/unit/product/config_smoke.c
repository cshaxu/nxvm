...
int main(void)
{
    ...
    const char valid[] = "; startup configuration\r\nrom = fixture.nes\r\ndisplay = console\r\n";
    const char window[] = "rom = fixture.nes\ndisplay = window\n";
    const char default_window[] = "rom = fixture.nes\n";
    const char invalid_display[] = "display = raster\n";
    const char retired_video[] = "video = window\n";
    const char invalid_key[] = "set = anything\n";
    const char quoted_with_comment[] =
        "ordinary heading\nrom = \"fixture game.nes\" ; local path\n"
        "display = console # presentation\n";

    assert(app_config_load_text(valid, sizeof(valid) - 1u, &config));
    assert(lib_c_strcmp(config.rom_path, "fixture.nes") == 0);
    assert(config.text_output);
    assert(app_config_load_text(window, sizeof(window) - 1u, &config));
    assert(lib_c_strcmp(config.rom_path, "fixture.nes") == 0 && !config.text_output);
    ...
    assert(app_config_load_text(quoted_with_comment, sizeof(quoted_with_comment) - 1u,
        &config));
    assert(lib_c_strcmp(config.rom_path, "fixture game.nes") == 0 && config.text_output);
    return 0;
}
