...
{ snapshot_bytes *s=opaque; if (count > sizeof(s->bytes)-s->size) return LIB_STATUS_LIMIT_EXCEEDED; memcpy(s->bytes+s->size,bytes,count); s->size+=count; return LIB_STATUS_OK; }
static lib_status get(void *opaque, lib_u8 *bytes, lib_size count)
{ snapshot_bytes *s=opaque; if (count > s->size-s->cursor) return LIB_STATUS_IO_ERROR; memcpy(bytes,s->bytes+s->cursor,count); s->cursor+=count; return LIB_STATUS_OK; }
...
{ memset(rom,0,16400u); rom[0]='N';rom[1]='E';rom[2]='S';rom[3]=0x1au;rom[4]=1u; rom[16]=0xa2u;rom[17]=0u;rom[18]=0xe8u;rom[19]=0x4cu;rom[20]=2u;rom[21]=0x80u;rom[16u+0x3ffcu]=0u;rom[16u+0x3ffdu]=0x80u; }
int main(void)
