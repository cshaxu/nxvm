...
int main(void)
{
    ...
    lib_test_assert(x86_debug_create(&command) == LIB_STATUS_OK);
    lib_test_assert(x86_debug_open(command, (common_machine *)command) == LIB_STATUS_OK);
    ...
    lib_test_assert(x86_debug_submit_line(command, "?", &result) == LIB_STATUS_NO_MEMORY);
    fail_allocation = LIB_FALSE;
    lib_test_assert(x86_debug_submit_line(command, "?", &result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_find_substring(result.text, "assemble") && lib_text_compare(result.prompt, "-") == 0);
    lib_test_assert(x86_debug_submit_line(command, "x?", &result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_find_substring(result.text, "XA [address]"));
    lib_test_assert(x86_debug_submit_line(command, "v", &result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_compare(result.text, "") == 0 && lib_text_compare(result.prompt, ":") == 0);
    lib_test_assert(x86_debug_submit_line(command, "Hello", &result) == LIB_STATUS_OK);
    ...
    x86_debug_close(command);
    lib_test_assert(x86_debug_submit_line(command, "?", &result) == LIB_STATUS_INVALID_ARGUMENT);
    lib_test_assert(x86_debug_open(command, (common_machine *)command) == LIB_STATUS_OK);
