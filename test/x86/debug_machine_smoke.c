...
static void extended_registers(x86_debug *debug, debug_fake *fake)
{
    ...
    for (index = 0; index < sizeof(commands) / sizeof(commands[0]); ++index) {
        ...
        lib_test_assert(x86_debug_submit_line(debug, commands[index], &result) == LIB_STATUS_OK);
        if (index >= 4u) {
            lib_test_assert(result.lifecycle_request == X86_DEBUG_LIFECYCLE_RESUME);
            lib_test_assert(x86_debug_observe_machine(debug, X86_DEBUG_MACHINE_PAUSED,
    ...
    lib_test_assert(x86_debug_submit_line(debug, "xr", &result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_find_substring(result.text,
        "EFL=00000002: vm rf nt IOPL=0 of df if tf sf zf af pf cf \n") != LIB_NULL);
    fake->real_reads = fake->linear_reads = 0u;
    lib_test_assert(x86_debug_submit_line(debug, "r", &result) == LIB_STATUS_OK);
    ...
    for (index = 0; index < 10u; ++index) {
        ...
        lib_test_assert(x86_debug_submit_line(debug, "89abcdef", &result) == LIB_STATUS_OK);
        lib_test_assert(fake->registers[index] == 0x89abcdefu);
    }
    lib_test_assert(x86_debug_submit_line(debug, "xsreg", &result) == LIB_STATUS_OK);
    ...
    const char *text, const char *prompt)
{
    ...
    lib_test_assert(result.lifecycle_request == X86_DEBUG_LIFECYCLE_NONE);
}
...
static void original_cli(x86_debug *debug, debug_fake *fake)
{
    ...
    lib_test_assert(fake->registers[X86_DEBUG_EAX] == 0x12345678u);
    transcript(debug, "rf", "", "NV UP DI PL NZ NA PO NC  -");
    transcript(debug, "cy", "", "-");
    lib_test_assert((fake->registers[X86_DEBUG_EFLAGS] & 1u) != 0u);
    ...
    lib_test_assert(x86_debug_submit_line(debug, "not_an_instruction", &result) == LIB_STATUS_OK);
    ...
    lib_test_assert(x86_debug_submit_line(debug, "xu 540 1", &result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_find_substring(result.text, "<ERROR>") == LIB_NULL);
    lib_test_assert(x86_debug_submit_line(debug, "xu", &result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_compare_n(result.text, "L0000054F", 9) == 0);
    lib_test_assert(x86_debug_submit_line(debug, "u 0:540 54e", &result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_find_substring(result.text, "<ERROR>") == LIB_NULL);
    ...
    lib_test_assert(x86_debug_submit_line(debug, "xd 0 1000", &result) == LIB_STATUS_OK);
    ...
    lib_test_assert(x86_debug_submit_line(debug, "xd", &result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_compare_n(result.text, "L00001000", 9) == 0);
    fake->memory_fixture = LIB_FALSE;
    lib_test_assert(x86_debug_submit_line(debug, "g", &result) == LIB_STATUS_OK);
    lib_test_assert(result.lifecycle_request == X86_DEBUG_LIFECYCLE_RESUME);
    lib_test_assert(x86_debug_observe_machine(debug, X86_DEBUG_MACHINE_PAUSED,
        LIB_STATUS_OK, &result) == LIB_STATUS_OK);
    ...
    for (lib_u32 linear = 0; linear < 2u; ++linear) {
        lib_test_assert(x86_debug_submit_line(debug, linear ? "xt 2" : "t 2", &result) == LIB_STATUS_OK);
...
int main(void)
{
    ...
    lib_test_assert(x86_debug_open(debug, machine) == LIB_STATUS_OK);
    lib_test_assert(fake.debug_calls == 0);
    lib_test_assert(x86_debug_submit_line(debug, "?", &debug_command_result) == LIB_STATUS_OK);
    lib_test_assert(debug_command_result.keep_active && fake.debug_calls == 0);
    lib_test_assert(x86_debug_submit_line(debug, "h 1 2", &debug_command_result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_find_substring(debug_command_result.text, "0003") != LIB_NULL && fake.debug_calls == 0);
    lib_test_assert(x86_debug_submit_line(debug, "r", &debug_command_result) == LIB_STATUS_OK);
    ...
    lib_test_assert(x86_debug_submit_line(debug, "d", &debug_command_result) == LIB_STATUS_OK);
    lib_test_assert(lib_text_find_substring(debug_command_result.text, "must be paused") != LIB_NULL);
    lib_test_assert(x86_debug_submit_line(debug, "q", &debug_command_result) == LIB_STATUS_OK);
    ...
    lib_test_assert(x86_debug_submit_line(debug, "?", &debug_command_result) ==
        LIB_STATUS_OK);
