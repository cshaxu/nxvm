    ...
    const x86_debug_request *request, x86_debug_response *result)
{
    ...
    *result = (x86_debug_response){0};
    if (request->operation == X86_DEBUG_READ_REGISTER) {
        ...
        lib_test_assert(request->bytes != 0u && request->bytes - 1u <= LIB_UINT32_MAX - address);
        lib_test_assert(address >= memory_base && address - memory_base < sizeof(memory));
        lib_test_assert(request->bytes <= sizeof(memory) - (address - memory_base));
        if (request->operation == X86_DEBUG_WRITE_LINEAR ||
            request->operation == X86_DEBUG_WRITE_REAL) {
            lib_test_assert(++writes <= sizeof(memory));
            lib_memory_copy(memory + address - memory_base, request->data, request->bytes);
        } else {
            lib_test_assert(++reads <= sizeof(memory));
...
{
    reads = writes = 0u;
    lib_test_assert(x86_debug_submit_line(debug, line, &result) == LIB_STATUS_OK);
    return result.text;
}
...
int main(void)
{
    ...
    lib_test_assert(x86_debug_create(&debug) == LIB_STATUS_OK);
    lib_test_assert(x86_debug_open(debug, (common_machine *)debug) == LIB_STATUS_OK);

    x86_debug_response response;
    short_response = LIB_TRUE;
    lib_test_assert(command_execute(debug, &(x86_debug_request){
    ...
    lib_test_assert(lib_memory_compare(memory, "AABCD", 5) == 0 && reads == 4 && writes == 4);
    lib_memory_copy(memory, "ABCDE", 5);
    submit("xm 1 0 4");
    lib_test_assert(lib_memory_compare(memory, "BCDEE", 5) == 0 && reads == 4 && writes == 4);
    submit("xm 0 0 4"); lib_test_assert(reads == 0 && writes == 0);
    submit("xm 0 10 0"); lib_test_assert(reads == 0 && writes == 0);
    submit("xm 0 10 4"); lib_test_assert(lib_memory_compare(memory, memory + 0x10, 4) == 0);

    lib_memory_set(memory, 0x90, sizeof(memory)); /* One-byte NOPs. */
    submit("xu 0 100"); lib_test_assert(reads == 256 && debug->unassemble_linear == 256);
    submit("xu 0 0"); lib_test_assert(reads == 0);
    submit("xu 0"); lib_test_assert(reads == 16);
    submit("xu"); lib_test_assert(reads == 10);

    /* Both debugger entry points stop on an overlong instruction, then recover. */
    lib_memory_set(memory, 0x66, 15);
    lib_test_assert(lib_text_find_substring(submit("u 0 l1"), "<ERROR>") != LIB_NULL && reads == 1);
    lib_test_assert(lib_text_find_substring(submit("xu 0 1"), "<ERROR>") != LIB_NULL && reads == 1);
    memory[14] = 0x90;
    lib_test_assert(lib_text_find_substring(submit("u 0 l1"), "NOP") != LIB_NULL);
    lib_test_assert(debug->unassemble_offset == 15);
    lib_test_assert(lib_text_find_substring(submit("xu 0 1"), "NOP") != LIB_NULL);
    lib_test_assert(debug->unassemble_linear == 15);
    ...
    for (lib_size i = 0; i < sizeof(invalid) / sizeof(invalid[0]); ++i) {
        ...
        lib_test_assert(reads == 0 && writes == 0);
    }
    memory_base = 0xfffff000u;
    lib_memory_set(memory, 0x90, sizeof(memory));
    submit("xu ffffffff 100");
    lib_test_assert(reads == 1 && debug->unassemble_linear == LIB_UINT32_MAX);
    ...
    submit("xa ffffffff");
    lib_test_assert(lib_text_find_substring(submit("mov eax,12345678"), "^ Error") != LIB_NULL && writes == 0);
    submit("xa ffffffff"); submit("nop");
    lib_test_assert(writes == 1 && debug->assemble_linear == LIB_UINT32_MAX);
    lib_test_assert(lib_text_compare(result.prompt, "-") == 0);
    ...
    for (lib_size i = 0; i < sizeof(partial) / sizeof(partial[0]); ++i) {
        ...
        lib_test_assert(lib_text_find_substring(submit(partial[i]), "^ Error") != LIB_NULL);
        lib_test_assert(writes == 0 && memory[0] == 0 && memory[1] == 0);
    }
    ...
    for (lib_size i = 0; i < sizeof(bad_ranges) / sizeof(bad_ranges[0]); ++i) {
        lib_test_assert(lib_text_find_substring(submit(bad_ranges[i]), "^ Error"));
        lib_test_assert(reads == 0 && writes == 0);
    }
    submit("E0:100 \"Ab C\" 21 'D''E' \"\"\"F\"");
    lib_test_assert(lib_text_compare(result.text, "") == 0);
    lib_test_assert(lib_memory_compare(memory + 0x100, "Ab C!D'E\"F", 10) == 0 && writes == 10);
    submit("xe 120 \"Ab C\" 21 'D''E' \"\"\"F\"");
    lib_test_assert(lib_memory_compare(memory + 0x100, memory + 0x120, 10) == 0);
    submit("f 200 l 5 'Ab'"); lib_test_assert(lib_memory_compare(memory + 0x200, "AbAbA", 5) == 0);
    submit("xf 220 5 'Ab'"); lib_test_assert(lib_memory_compare(memory + 0x220, "AbAbA", 5) == 0);
    submit("c 200 l5 220"); lib_test_assert(lib_text_compare(result.text, "") == 0 && reads == 10);
    submit("m 200 l5 201"); lib_test_assert(lib_memory_compare(memory + 0x200, "AAbAbA", 6) == 0);
    ...
    submit("f 0  ffff 90"); lib_test_assert(writes == 65536);
    submit("s 0 ffff 41"); lib_test_assert(reads == 65536);
    submit("f ffff l1 41"); lib_test_assert(writes == 1 && memory[65535] == 0x41);
    submit("e ffff 'AB'"); lib_test_assert(memory[65535] == 'A' && memory[0] == 'B');
    submit("d ffff"); lib_test_assert(reads == 1 && debug->dump_offset == 0);
    submit("d ffff l1"); lib_test_assert(reads == 1);
    submit("d ffff:10 l1"); lib_test_assert(reads == 1); /* No artificial 1 MiB clamp. */
    lib_memory_set(memory, 0x90, sizeof(memory));
    submit("u 0 l20"); lib_test_assert(debug->unassemble_offset == 32);
    submit("u ffff l1"); lib_test_assert(debug->unassemble_offset == 0);
    memory[65535] = 0xb8; memory[0] = 0x34; memory[1] = 0x12;
    submit("u ffff l1"); /* Whole instruction crosses the segment offset. */
    lib_test_assert(lib_text_find_substring(result.text, "1234") && debug->unassemble_offset == 2);
    submit("u ffff:10 l1"); lib_test_assert(lib_text_find_substring(result.text, "<ERROR>") == LIB_NULL);
    submit("d 0"); lib_test_assert(reads == 128);
    submit("u 0"); lib_test_assert(debug->unassemble_offset >= 32);
    code_segment = 0x100u; data_segment = 0x200u;
    lib_test_assert(lib_text_compare_n(submit("u 0 0"), "0100:0000", 9) == 0);
    lib_test_assert(lib_text_compare_n(submit("u 0 l1"), "0100:0000", 9) == 0);
    lib_test_assert(lib_text_compare_n(submit("d 0 l1"), "0200:0000", 9) == 0);
    submit("e DS:10 41"); lib_test_assert(memory[0x2010] == 0x41);
    code_segment = data_segment = 0u;
    submit("f 0 l0 42"); lib_test_assert(writes == 65536);
    submit("m 0 l0 1000:0"); lib_test_assert(reads == 65536 && writes == 65536);
    lib_test_assert(memory[0x10000] == 0x42 && memory[0x1ffff] == 0x42);
    submit("c 0 l0 1000:0"); lib_test_assert(reads == 131072 && lib_text_compare(result.text, "") == 0);
    submit("f 0 l2 41"); submit("m 0 l2 ffff");
    lib_test_assert(memory[0xffff] == 0x41 && memory[0] == 0x41);
    lib_test_assert(lib_text_find_substring(submit("s ffff l1 41"), "0000:FFFF"));
    lib_test_assert(reads == 1);
    lib_test_assert(lib_text_compare(submit(""), "") == 0 && reads == 0 && writes == 0);
