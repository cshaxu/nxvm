...
static lib_i32 vm_dos_keyboard_submit_key(vm_machine *session,
...
static lib_i32 vm_dos_keyboard_has_text(const vm_machine *session,
    const char *text)
{
...
static lib_i32 vm_dos_keyboard_has_prompt(const vm_machine *session)
{
    core_machine_guest_display_frame frame;
    lib_size cell;
...
static lib_i32 vm_dos_keyboard_has_date_prompt(const vm_machine *session)
{
    return vm_dos_keyboard_has_text(session, "Enter new date (mm-dd-yy):");
}

static lib_i32 vm_dos_keyboard_has_time_prompt(const vm_machine *session)
{
    return vm_dos_keyboard_has_text(session, "Enter new time:");
}

static lib_i32 vm_dos_keyboard_has_edit_menu(const vm_machine *session)
{
...
static lib_i32 vm_dos_keyboard_verify_text_frame(const vm_machine *session)
{
    ...
    for (cell = 0u; cell < TEXT_VIDEO_CELLS; ++cell) {
        if (frame.characters[cell] != text[cell * 2u] ||
            ...
            printf("edit display mismatch: cell=%u raw=%02x/%02x "
...
static void vm_dos_keyboard_report_failure(const vm_machine *session,
    const core_machine_cpu_state *state)
{
    ...
    (void)core_machine_debug_read_memory(session->core_machine, 0x041au,
        &head, sizeof(head));
    (void)core_machine_debug_read_memory(session->core_machine, 0x041cu,
        &tail, sizeof(tail));
    (void)core_machine_debug_read_memory(session->core_machine, 0x0449u,
    ...
    printf("edit state: mode=%02x BDA head=%04x tail=%04x halt=%u bytes="
        ...
        instructions[7]);
    printf("keyboard controller: scan_set=%u\n", scan_set);
}
...
lib_i32 main(lib_i32 argc, char **argv)
{
    ...
    DWORD elapsed;
    const lib_u8 scan_codes[] = { 0x12u, 0x20u, 0x17u, 0x14u, 0x1cu };
    const lib_u8 virtual_keys[] = { 'E', 'D', 'I', 'T', VK_RETURN };
    lib_size index;
    DWORD prompt_timeout = argc == 4 ? 20000u : 3000u;
    DWORD edit_timeout = argc == 4 ? 20000u : 5000u;
    lib_i32 display_ok = 0;
    ...
    if (elapsed == prompt_timeout) {
        ...
        if (integration_ini_session_pause(&ini_session, 500u) == LIB_STATUS_OK &&
            core_machine_debug_read_cpu(session->core_machine, &state) ==
                LIB_STATUS_OK) {
            printf("prompt pause: %04x:%08x flags=%08x\n", state.cs,
    ...
    if (vm_dos_keyboard_has_date_prompt(session)) {
        ...
        if (elapsed == prompt_timeout) {
            ...
            if (integration_ini_session_pause(&ini_session, 500u) == LIB_STATUS_OK &&
                core_machine_debug_read_cpu(session->core_machine, &state) ==
                    LIB_STATUS_OK) {
                printf("date input pause: %04x:%08x flags=%08x\n", state.cs,
                    state.eip, state.eflags);
                vm_dos_keyboard_report_failure(session, &state);
            }
            goto fail;
    ...
    if (elapsed == edit_timeout) {
        ...
        if (integration_ini_session_pause(&ini_session, 500u) == LIB_STATUS_OK &&
            core_machine_debug_read_cpu(session->core_machine, &state) ==
                LIB_STATUS_OK) {
            printf("edit pause: %04x:%08x flags=%08x\n", state.cs,
                state.eip, state.eflags);
            vm_dos_keyboard_report_failure(session, &state);
        }

        if (vm_machine_fault_get(session, &outcome) == 0 && outcome.valid) {
            printf("edit fault: detail=%08x pc=%08x mask=%08x code=%08x\n",
    ...
    if (elapsed != edit_timeout) {
        if (integration_ini_session_pause(&ini_session, 500u) == LIB_STATUS_OK) {
            ...
            printf("edit display: pause unavailable\n");
        }
    }
