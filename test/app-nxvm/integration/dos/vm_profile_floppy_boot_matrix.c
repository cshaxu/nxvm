    ...
    const boot_trace_probe *trace_probe)
{
    ...
    if (core_machine_get_cpu_state(session->core_machine, &cpu) == LIB_STATUS_OK) {
        ...
        printf("T515:INI-BOOT:%s:PIT:out0=%u/active=%u/reload=%u/rem=%u:out1=%u/active=%u/reload=%u/rem=%u:clock=%u/%u\n",
        ...
        printf("T526:S13:INI-BOOT:%s:PIT0:control=%02X:count=%04X:latch=%04X:latched=%u:read=%u\n",
