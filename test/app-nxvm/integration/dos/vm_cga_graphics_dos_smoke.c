...
static void vm_cga_dos_fat12_set(lib_u8 *fat, lib_u16 cluster, lib_u16 value)
{
    lib_u32 offset = cluster + cluster / 2u;
...
static lib_i32 vm_cga_dos_install_program(lib_u8 *image, DWORD image_size)
{
    ...
    lib_memory_copy(entry, "CGAT228 COM", 11u);
    entry[11u] = 0x20u;
...
static lib_status vm_cga_dos_install_on_overlay(integration_ini_session *session,
    void *opaque)
{
...
static lib_i32 vm_cga_dos_run_until(vm_machine *session, lib_u32 limit,
...
{
    static const lib_u8 command[] = { 0x21u, 0x34u, 0x1cu, 0x2cu, 0x1eu,
        0x1eu, 0x3eu, 0x5au };
    ...
    if (argc != 3 || integration_ini_session_open_with_overlay_transform(argv[1], argv[2],
            vm_cga_dos_install_on_overlay, LIB_NULL, &ini_session) != LIB_STATUS_OK) {
        return 77;
    }
