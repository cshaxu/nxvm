...
static C_INT vm_cga_dos_fat12_get(const lib_u8 *fat, lib_u16 cluster)
{
...
static C_VOID vm_cga_dos_fat12_set(lib_u8 *fat, lib_u16 cluster, lib_u16 value)
{
    lib_u32 offset = cluster + cluster / 2u;
...
static C_INT vm_cga_dos_install_program(lib_u8 *image, DWORD image_size)
{
...
static C_INT vm_cga_dos_run_until(vm_machine *session, lib_u32 limit,
    C_INT want_graphics)
{
...
C_INT main(C_INT argc, C_CHAR **argv)
{
    ...
    if (argc != 3 || integration_ini_session_open_with_overlay_transform(argv[1], argv[2],
            vm_cga_dos_install_on_overlay, LIB_NULL, &ini_session) != TYPE_STATUS_OK) {
        return 77;
    }
