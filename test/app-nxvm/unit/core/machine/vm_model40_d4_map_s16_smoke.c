...
static lib_i32 write_byte(core_machine *machine, lib_u32 physical,
    lib_u8 value, lib_status expected)
{
    return core_machine_memory_write(machine, physical, &value, sizeof(value)) == expected;
}

static lib_i32 write_word(core_machine *machine, lib_u32 physical,
    lib_u16 value)
{
...
static lib_i32 read_word(core_machine *machine, lib_u32 physical,
    lib_u16 expected)
{
    lib_u16 observed = 0u;
...
#define CHECK(expression) do { ++step; if (!(expression)) { \
    ...
    if (!failed) {
        ...
        CHECK(core_machine_set_a20(session->core_machine, LIB_FALSE) == LIB_STATUS_OK);

        /* AT spare DMA page latches are board-visible state, not unknown
         * ports.  DeskPro firmware uses 86h during POST. */
        CHECK(core_machine_bus_write(session->core_machine, 0x0086u, 0x5au) == LIB_STATUS_OK);
        CHECK(core_machine_bus_read(session->core_machine, 0x0086u, &port_value) == LIB_STATUS_OK &&
            port_value == 0x5au);
        CHECK(core_machine_bus_write(session->core_machine, 0x0080u, 0xa5u) == LIB_STATUS_OK);
        CHECK(core_machine_bus_read(session->core_machine, 0x0080u, &port_value) == LIB_STATUS_OK &&
            port_value == 0xa5u);
