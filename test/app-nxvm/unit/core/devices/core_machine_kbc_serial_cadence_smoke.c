...
lib_i32 main(void)
{
    ...
    failed |= core_machine_kbc_submit_native_bytes(&kbc, bytes, sizeof(bytes)) != LIB_STATUS_OK || kbc.data.fifo_count != 0u || kbc.data.keyboard_serial_count != sizeof(bytes);
    core_machine_kbc_advance(&kbc, 1u);
    failed |= kbc.data.fifo_count != 0u;
    core_machine_kbc_advance(&kbc, 1u);
    failed |= (lib_u8)core_machine_port_read(&port, 0x0060u) != 0x1eu || kbc.data.keyboard_serial_count != 1u;
    core_machine_kbc_advance(&kbc, 1u);
    failed |= kbc.data.fifo_count != 0u;
    core_machine_kbc_advance(&kbc, 1u);
    failed |= (lib_u8)core_machine_port_read(&port, 0x0060u) != 0x30u || kbc.data.keyboard_serial_count != 0u;
