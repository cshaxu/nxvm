...
static C_INT shift_test_forms(C_VOID)
{
    ...
    for(count_index=1u;count_index<=(width?31u:16u);++count_index) {
        ...
        const lib_u8 operand32=width;
        const lib_u8 address32=memory&&width;
        ...
        const lib_u32 expected=shift_result(destination,source,count,width?32u:16u,direction);
        const lib_u32 expected_flags=shift_flags(expected,destination,count,width?32u:16u,direction);
        const lib_u32 flag_mask=VCPU_EFLAGS_CF|VCPU_EFLAGS_PF|VCPU_EFLAGS_ZF|
...
static C_INT shift_test_count_zero(C_VOID)
{
    ...
    const lib_u32 flags=VCPU_EFLAGS_CF|VCPU_EFLAGS_AF|VCPU_EFLAGS_ZF|VCPU_EFLAGS_OF;
    lib_u8 index;
