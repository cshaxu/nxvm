...
static void gpr_mov_reset(void *opaque)
{
...
static void gpr_mov_seed(gpr_mov_machine *state)
{
...
static lib_i32 gpr_mov_run(gpr_mov_machine *state, const lib_u8 *code, lib_u8 bytes,
...
static lib_i32 gpr_mov_test_defaults(void)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]);
        ...
        for (form = 0u; form != sizeof(memory_codes) / sizeof(memory_codes[0]);
            ...
            lib_i32 failed = !gpr_mov_prepare(profiles[profile], &state);
            if (!failed) {
                gpr_mov_seed(&state);
                ...
                failed |= !gpr_mov_run(&state, memory_codes[form], 4u, &after, &diagnostic, &status) ||
                    status != LIB_STATUS_OK || diagnostic.first_fault.valid || after.data.eip != 4u ||
                ...
                if (form == 1u) failed |= core_machine_memory_read_physical(&state.machine->executor_memory,
                    0x1000u, CORE_MACHINE_REFERENCE_OF(image), 2u) != LIB_STATUS_OK || image != 0x5566u;
                if (form == 2u)
...
static lib_i32 gpr_mov_test_immediate_and_reject(void)
{
    ...
    for (form = 0u; form != 2u; ++form) {
        ...
        lib_i32 failed = !gpr_mov_prepare(profiles[profile],
            &state);
        ...
        if (!failed) {
            ...
            failed |= !gpr_mov_run(&state, valid[form], bytes, &after,
                ...
                core_machine_memory_read_physical(&state.machine->executor_memory,
                    0x1000u, CORE_MACHINE_REFERENCE_OF(image), 2u) != LIB_STATUS_OK ||
        ...
        for (extension = 1u; extension != 8u; ++extension) {
            ...
            if (!failed) {
                ...
                failed |= !gpr_mov_run(&state, code, form ? 6u : 5u,
                    ...
                    !gpr_mov_nonparticipants(&before, &after, 8u) ||
                    core_machine_memory_read_physical(&state.machine->executor_memory,
                        0x1000u, CORE_MACHINE_REFERENCE_OF(image), sizeof(image)) !=
        ...
        if (!failed) {
            ...
            if (form == 0u)
                failed |= core_machine_memory_read_physical(
                    ...
                    CORE_MACHINE_REFERENCE_OF(image), 4u) != LIB_STATUS_OK ||
                    image != 0x11225566u;
            if (form == 1u)
...
static lib_i32 gpr_mov_test_segments_and_irq(void)
{
        ...
        if (!failed) {
            ...
            if (form == 2u || form == 4u) {
                ...
                failed |= core_machine_memory_read_physical(
                    ...
                    CORE_MACHINE_REFERENCE_OF(image), 1u) != LIB_STATUS_OK ||
                    image != 0x44u;
            }
        }
        core_machine_destroy(state.machine);
        ...
        if (!failed) {
            failed |= core_machine_memory_write(state.machine, 0x100u,
                ...
                core_machine_memory_write(state.machine, 0x2000u, halt, 1u) !=
                    LIB_STATUS_OK || core_machine_run(state.machine,
        ...
        if (!failed) {
            ...
            failed |= core_machine_memory_write(state.machine, 0x3010u,
                ...
                core_machine_memory_write(state.machine, 0x2000u,
                    codes[form], bytes[form]) != LIB_STATUS_OK;
            before = test_core_machine_fixture_capture_cpu_after_run(state.machine);
            ...
            failed |= !diagnostic.first_fault.valid || !CORE_MACHINE_BIT_IS_SET(
                ...
                CORE_MACHINE_REFERENCE_OF(image), 4u) != LIB_STATUS_OK || image !=
                0x11223344u;
        }
        core_machine_destroy(state.machine);
        if (failed) return 0;
        ...
        if (!failed) {
            failed |= core_machine_memory_write(state.machine, 0x1000u,
                ...
                LIB_STATUS_OK || core_machine_memory_write(state.machine,
                    0x80u, &offset, 2u) != LIB_STATUS_OK ||
                core_machine_memory_write(state.machine, 0x82u, &segment, 2u) !=
                LIB_STATUS_OK || core_machine_memory_write(state.machine,
                    0x100u, &hlt, 1u) != LIB_STATUS_OK;
        }
        if (!failed) {
            gpr_mov_seed(&state);
            ...
            failed |= core_machine_memory_read_physical(
                ...
                CORE_MACHINE_REFERENCE_OF(frame), 2u) != LIB_STATUS_OK ||
                after.data.eip != 0x101u || frame != 4u || !CORE_MACHINE_BIT_IS_SET(
