...
static lib_i32 pda_test_default_access(core_machine_cpu_profile profile)
{
    ...
    static const lib_u8 ss_read[] = { 0x8au,0x46u,0,0xf4u };
    static const lib_u8 stos[] = { 0xaau,0xf4u };
    pda_machine state;
    t_cpu before;
    t_cpu after;
    lib_u8 byte = 0x5au;
    ...
    if (!failed) {
        ...
        failed |= !pda_write(&state, PDA_DATA_ADDRESS + 0x20u, &byte, sizeof(byte)) ||
            ...
            !pda_nonresult_state_same(&before, &after, LIB_TRUE, LIB_FALSE);
    }
    if (!failed) {
        byte = 0u;
        ...
        failed |= !pda_write(&state, PDA_DATA_ADDRESS + 0x21u, &byte, sizeof(byte)) ||
            ...
            !pda_nonresult_state_same(&before, &after, LIB_FALSE, LIB_FALSE);
    }
    if (!failed) {
        byte = 0x6bu;
    ...
    if (!failed) {
        ...
        failed |= !pda_write(&state, PDA_ES_ADDRESS + 0x40u, &byte, sizeof(byte)) ||
            ...
            !pda_nonresult_state_same(&before, &after, LIB_FALSE, LIB_TRUE);
    }
    core_machine_destroy(state.machine);
    return !failed;
}
...
static lib_i32 pda_test_fault_atomicity(void)
{
    ...
    static const lib_u8 read_ss[] = { 0x8au,0x46u,0,0 };
