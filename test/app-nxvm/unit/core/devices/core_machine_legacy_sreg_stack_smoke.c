...
static C_INT legacy_sreg_stack_test_protected_ss_null(C_VOID)
{
    ...
    lib_u16 selector=0u; C_INT failed=!legacy_sreg_stack_prepare(CORE_MACHINE_CPU_PROFILE_80386,&state);
    ...
    if(!failed) {
        failed|=core_machine_memory_write(state.machine,0xc000u,&selector,2u)!=TYPE_STATUS_OK ||
            core_machine_memory_write(state.machine,0x2000u,(lib_u8[]){0x17u},1u)!=TYPE_STATUS_OK;
        before=test_core_machine_fixture_capture_cpu_after_run(state.machine);
        test_core_machine_fixture_resume_after_halt_at(state.machine,0u);
        status=core_machine_run(state.machine,(core_machine_run_budget){1u,0u},&result);
        after=test_core_machine_fixture_capture_cpu_after_run(state.machine);
    ...
    for(form=0u;form!=4u;++form) {
        ...
        if(!failed) {
            ...
            failed|=core_machine_run(state.machine,(core_machine_run_budget){3u,0u},&result)!=TYPE_STATUS_OK ||
                result.reason!=CORE_MACHINE_STOP_WAITING_FOR_INTERRUPT;
            after=test_core_machine_fixture_capture_cpu_after_run(state.machine);
            failed|=core_machine_memory_read_physical(&state.machine->executor_memory,after.data.ss.base+(lib_u16)after.data.esp,TYPE_REFERENCE_OF(ip),2u)!=TYPE_STATUS_OK ||
