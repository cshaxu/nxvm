...
static C_INT lods_case(core_machine_cpu_profile profile, const lib_u8 *code,
    ...
    lib_u32 address, lib_u32 source)
{
    ...
    core_machine_run_result result; type_status status; lib_u32 source_after;
...
static C_INT lods_rep_case(core_machine_cpu_profile profile,
    ...
    C_INT decrement, lib_u16 count, const lib_u32 *source)
{
    lods_machine state;
    t_cpu before;
    t_cpu after;
...
static C_VOID lods_seed_protected(lods_machine *state, lib_u32 ds_limit)
{
    lods_seed(state);
...
static C_INT lods_test_irq_rep_restart(C_VOID)
{
    ...
    if (!failed) {
        ...
        failed |= core_machine_memory_read_physical(&state.machine->executor_memory,
            ...
            TYPE_STATUS_OK || lib_memory_compare(source_after, source, sizeof(source)) != 0;
    }
    core_machine_destroy(state.machine);
    return !failed;
}
