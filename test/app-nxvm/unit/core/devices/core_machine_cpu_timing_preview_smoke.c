...
static C_INT preview_test_layouts(C_VOID)
{
    ...
    static const lib_u8 mov_sib_disp[] = {
        0x67u, 0x8bu, 0x84u, 0x88u, 0x78u, 0x56u, 0x34u, 0x12u
    };
    static const lib_u8 near_jcc[] = {
        0x0fu, 0x84u, 0x78u, 0x56u, 0x34u, 0x12u
    };
    static const lib_u8 group1_alias[] = { 0x82u, 0xc0u, 0xffu };
    static const lib_u8 clts[] = { 0x0fu, 0x06u };
    static const lib_u8 shld_imm8[] = {
        0x0fu, 0xa4u, 0xc1u, 0x05u
    };
    static const lib_u8 rep_movsb[] = { 0xf3u, 0xa4u };
    static const lib_u8 repeated_operand_prefix[] = {
        0x66u, 0x66u, 0xb8u, 0x78u, 0x56u, 0x34u, 0x12u
    };
    static const lib_u8 repeated_address_prefix[] = {
    ...
    return preview_expect(nop, sizeof(nop), CORE_MACHINE_CPU_PROFILE_8086,
        ...
        preview_expect(mov_imm32, sizeof(mov_imm32),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 6u, 3u) &&
        ...
        preview_expect(near_jcc, sizeof(near_jcc),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 6u, 3u) &&
        preview_expect(group1_alias, sizeof(group1_alias),
            CORE_MACHINE_CPU_PROFILE_8086, LIB_FALSE, 3u, 3u) &&
        preview_expect(group1_alias, sizeof(group1_alias),
            CORE_MACHINE_CPU_PROFILE_80186, LIB_FALSE, 3u, 3u) &&
        preview_expect(group1_alias, sizeof(group1_alias),
            CORE_MACHINE_CPU_PROFILE_80286, LIB_FALSE, 3u, 3u) &&
        preview_expect(group1_alias, sizeof(group1_alias),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 3u, 3u) &&
        preview_expect(clts, sizeof(clts), CORE_MACHINE_CPU_PROFILE_80386,
            LIB_TRUE, 2u, 2u) &&
        preview_expect(shld_imm8, sizeof(shld_imm8),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 4u, 4u) &&
        preview_expect(rep_movsb, sizeof(rep_movsb),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 2u, 2u) &&
...
static C_INT preview_test_accumulator_xchg_profiles(C_VOID)
{
    ...
    for (opcode = 0x90u; opcode != 0x98u; ++opcode) {
        const lib_u8 primary[] = {opcode};
        if (!preview_expect(primary, sizeof(primary), profiles[profile], LIB_FALSE,
            1u, 1u)) return 0;
    }
    return preview_expect((const lib_u8[]){0x66u,0x91u}, 2u,
...
static C_INT preview_test_primary_inc_dec_profiles(C_VOID)
{
    ...
    return preview_expect((const lib_u8[]){0x66u,0x40u}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
...
static C_INT preview_test_immediate_register_mov_profiles(C_VOID)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile) {
        for (opcode = 0xb0u; opcode != 0xb8u; ++opcode) {
            const lib_u8 byte_immediate[] = {opcode, 0xa5u};
            if (!preview_expect(byte_immediate, sizeof(byte_immediate),
                profiles[profile], LIB_FALSE, 2u, 2u)) return 0;
        }
        for (opcode = 0xb8u; opcode != 0xc0u; ++opcode) {
            const lib_u8 word_immediate[] = {opcode, 0x34u, 0x12u};
            if (!preview_expect(word_immediate, sizeof(word_immediate),
                profiles[profile], LIB_FALSE, 3u, 2u)) return 0;
        }
    }
    for (opcode = 0xb0u; opcode != 0xb8u; ++opcode) {
        const lib_u8 byte_immediate[] = {0x66u, opcode, 0xa5u};
        if (!preview_expect(byte_immediate, sizeof(byte_immediate),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u)) return 0;
    }
    for (opcode = 0xb8u; opcode != 0xc0u; ++opcode) {
        const lib_u8 dword_immediate[] = {0x66u, opcode,
            0x78u, 0x56u, 0x34u, 0x12u};
        if (!preview_expect(dword_immediate, sizeof(dword_immediate),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 6u, 3u)) return 0;
    }
    return 1;
}
...
static C_INT preview_test_accumulator_test_profiles(C_VOID)
{
    ...
    static const lib_u8 byte[] = {0xa8u, 0x80u};
    static const lib_u8 word[] = {0xa9u, 0u, 0x80u};
    static const lib_u8 dword[] = {0x66u, 0xa9u, 0u, 0u, 0u, 0x80u};
    lib_u8 profile;

    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(byte, sizeof(byte), profiles[profile], LIB_FALSE,
            2u, 2u) || !preview_expect(word, sizeof(word), profiles[profile],
            LIB_FALSE, 3u, 2u)) return 0;
...
static C_INT preview_test_scas_profiles(C_VOID)
{
    ...
    for (opcode = 0xaeu; opcode != 0xb0u; ++opcode) {
        const lib_u8 primary[] = {opcode};
        const lib_u8 repe[] = {0xf3u, opcode};
        const lib_u8 repne[] = {0xf2u, opcode};
        if (!preview_expect(primary, sizeof(primary), profiles[profile],
            LIB_FALSE, 1u, 1u) || !preview_expect(repe, sizeof(repe),
            profiles[profile], LIB_FALSE, 2u, 2u) ||
            !preview_expect(repne, sizeof(repne), profiles[profile],
            LIB_FALSE, 2u, 2u)) return 0;
    }
    for (opcode = 0xaeu; opcode != 0xb0u; ++opcode) {
        const lib_u8 operand[] = {0x66u, opcode};
        const lib_u8 address[] = {0x67u, opcode};
        const lib_u8 combined[] = {0x66u, 0x67u, opcode};
        const lib_u8 repeated[] = {0xf2u, 0x66u, 0x67u, opcode};
        if (!preview_expect(operand, sizeof(operand),
            ...
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u)) return 0;
    }
    return 1;
}
static C_INT preview_test_lods_profiles(C_VOID)
{
    ...
    for (opcode = 0xacu; opcode != 0xaeu; ++opcode) {
        const lib_u8 primary[] = {opcode};
        const lib_u8 repeated[] = {0xf3u, opcode};
        if (!preview_expect(primary, sizeof(primary), profiles[profile],
            LIB_FALSE, 1u, 1u) || !preview_expect(repeated, sizeof(repeated),
            profiles[profile], LIB_FALSE, 2u, 2u)) return 0;
    }
    for (opcode = 0xacu; opcode != 0xaeu; ++opcode) {
        const lib_u8 operand[] = {0x66u, opcode};
        const lib_u8 address[] = {0x67u, opcode};
        const lib_u8 combined[] = {0x66u, 0x67u, opcode};
        const lib_u8 repeated[] = {0xf3u, 0x66u, 0x67u, opcode};
        if (!preview_expect(operand, sizeof(operand),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) ||
            !preview_expect(address, sizeof(address),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) ||
...
static C_INT preview_test_moffs_mov_profiles(C_VOID)
{
    ...
    for (opcode = 0xa0u; opcode != 0xa4u; ++opcode) {
        const lib_u8 moffs16[] = {opcode, 0x34u, 0x12u};
        if (!preview_expect(moffs16, sizeof(moffs16), profiles[profile],
            LIB_FALSE, 3u, 2u)) return 0;
    }
    for (opcode = 0xa0u; opcode != 0xa4u; ++opcode) {
        const lib_u8 operand16[] = {0x66u, opcode, 0x34u, 0x12u};
        const lib_u8 address32[] = {0x67u, opcode,
            0x78u, 0x56u, 0x34u, 0x12u};
        const lib_u8 combined[] = {0x66u, 0x67u, opcode,
            0x78u, 0x56u, 0x34u, 0x12u};
        if (!preview_expect(operand16, sizeof(operand16),
            ...
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 6u, 3u) ||
            !preview_expect(combined, sizeof(combined),
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 7u, 4u)) return 0;
    }
    return 1;
}
...
static C_INT preview_test_enter_leave_profiles(C_VOID)
{
    ...
    static const lib_u8 enter[] = {0xc8u, 4u, 0u, 2u};
    static const lib_u8 leave[] = {0xc9u};
   lib_u8 profile;


    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(enter, sizeof(enter), profiles[profile], LIB_FALSE,
            4u, 2u) || !preview_expect(leave, sizeof(leave), profiles[profile],
            LIB_FALSE, 1u, 1u)) return 0;
    return preview_expect((const lib_u8[]){0x66u,0xc8u,4u,0u,1u},
        5u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 5u, 3u) &&
...
static C_INT preview_test_far_return_profiles(C_VOID)
{
    ...
    static const lib_u8 ret[] = {0xcbu};
    static const lib_u8 cleanup[] = {0xcau, 4u, 0u};
    lib_u8 profile;

    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(ret, sizeof(ret), profiles[profile], LIB_FALSE,
            1u, 1u) || !preview_expect(cleanup, sizeof(cleanup),
            profiles[profile], LIB_FALSE, 3u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0xcbu}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
...
static C_INT preview_test_near_return_profiles(C_VOID)
{
    ...
    static const lib_u8 cleanup[] = {0xc2u, 4u, 0u};
    lib_u8 profile;
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(ret, sizeof(ret), profiles[profile], LIB_FALSE,
            1u, 1u) || !preview_expect(cleanup, sizeof(cleanup),
            profiles[profile], LIB_FALSE, 3u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0xc3u}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
...
static C_INT preview_test_les_lds_profiles(C_VOID)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(les,4u,profiles[profile],LIB_FALSE,4u,3u) || !preview_expect(lds,4u,profiles[profile],LIB_FALSE,4u,3u)) return 0;
...
static C_INT preview_test_lea_profiles(C_VOID)
{
    ...
    lib_u8 profile;
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(lea, sizeof(lea), profiles[profile], LIB_FALSE, 3u, 3u)) return 0;
...
static C_INT preview_test_loop_jcxz_profiles(C_VOID)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        for (opcode = 0u; opcode != sizeof(opcodes); ++opcode)
            if (!preview_expect((const lib_u8[]){opcodes[opcode], 0u},
                2u, profiles[profile], LIB_FALSE, 2u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0xe2u, 0u}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        for (opcode = 0u; opcode != sizeof(opcodes); ++opcode)
            if (!preview_expect(&opcodes[opcode], 1u, profiles[profile],
                LIB_FALSE, 1u, 1u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0xf8u}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
...
static C_INT preview_test_modrm_data_move_profiles(C_VOID)
{
    ...
    static const lib_u8 xchg[] = { 0x86u, 0xc1u };
    static const lib_u8 xchg_memory[] = { 0x87u, 0x06u, 0u, 0x20u };
    static const lib_u8 mov[] = { 0x88u, 0xc1u, 0x89u, 0xc1u,
        0x8au, 0xc1u, 0x8bu, 0xc1u };
    static const lib_u8 mov_memory[] = { 0x8bu, 0x06u, 0u, 0x20u };
    static const lib_u8 sreg_store[] = { 0x8cu, 0xc1u };
    static const lib_u8 sreg_load[] = { 0x8eu, 0xd9u };
    lib_u8 profile;
    lib_u8 opcode;

    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile) {
        if (!preview_expect(xchg, sizeof(xchg), profiles[profile], LIB_FALSE,
                2u, 2u) ||
        ...
        for (opcode = 0u; opcode != sizeof(mov); opcode += 2u)
            if (!preview_expect(&mov[opcode], 2u, profiles[profile], LIB_FALSE,
                    2u, 2u)) return 0;
    }
    return preview_expect((const lib_u8[]){0x66u, 0x87u, 0xc1u},
        3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x67u, 0x87u, 0x46u, 0x10u},
        4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u) &&
        preview_expect((const lib_u8[]){0x66u, 0x8bu, 0xc1u},
        3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x67u, 0x8bu, 0x46u, 0x10u},
        4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u) &&
        preview_expect((const lib_u8[]){0x66u, 0x8cu, 0xc1u},
        3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x67u, 0x8eu, 0x5eu, 0x10u},
        4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u) &&
        preview_expect((const lib_u8[]){0x8cu, 0xe1u}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
...
static C_INT preview_test_rm_immediate_mov_profiles(C_VOID)
{
    ...
    static const lib_u8 mov8_memory[] = {0xc6u, 0x06u, 0u, 0x20u, 0x5au};
    static const lib_u8 mov16_memory[] = {0xc7u, 0x06u, 0u, 0x20u, 0x34u, 0x12u};
    core_machine_cpu_instruction_lexeme lexeme;
    lib_u8 profile;
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile) {
        if (!preview_expect(mov8_memory, sizeof(mov8_memory), profiles[profile],
            ...
            !preview_expect(mov16_memory, sizeof(mov16_memory), profiles[profile],
                LIB_FALSE, 6u, 4u) ||
            !preview_expect((const lib_u8[]){0xc6u, 0xc0u, 0x5au}, 3u,
                profiles[profile], LIB_FALSE, 3u, 3u) ||
            !preview_expect((const lib_u8[]){0xc7u, 0xc0u, 0x34u, 0x12u},
                4u, profiles[profile], LIB_FALSE, 4u, 3u) ||
            core_machine_cpu_instruction_lexeme_scan(
                (const lib_u8[]){0xc6u, 0xc8u, 0x5au}, 3u,
                profiles[profile], LIB_FALSE, &lexeme) || lexeme.available) return 0;
    }
    return preview_expect((const lib_u8[]){0x66u, 0xc7u, 0x06u, 0u,
        0x20u, 0x78u, 0x56u, 0x34u, 0x12u}, 9u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 9u, 5u) &&
...
static C_INT preview_test_shared_prefix_profiles(C_VOID)
{
    ...
    static const lib_u8 segment_prefixes[] = {0x26u, 0x2eu, 0x36u, 0x3eu};
    static const lib_u8 repeat_prefixes[] = {0xf2u, 0xf3u};
    static const lib_u8 size_prefixes[] = {0x66u, 0x67u};
    static const lib_u8 locked_add[] = {0xf0u, 0x01u, 0x06u, 0u, 0x20u};
    core_machine_cpu_instruction_lexeme lexeme;
    lib_u8 profile;
    lib_u8 prefix;
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile) {
        for (prefix = 0u; prefix != sizeof(segment_prefixes); ++prefix)
            if (!preview_expect((const lib_u8[]){segment_prefixes[prefix],
                    0x8au, 0x06u, 0u, 0x20u}, 5u, profiles[profile],
                    LIB_FALSE, 5u, 4u)) return 0;
        for (prefix = 0u; prefix != sizeof(repeat_prefixes); ++prefix)
            if (!preview_expect((const lib_u8[]){repeat_prefixes[prefix],
                    0xa4u}, 2u, profiles[profile], LIB_FALSE, 2u, 2u)) return 0;
    }
    for (profile = 0u; profile != 3u; ++profile) {
        for (prefix = 0u; prefix != sizeof(size_prefixes); ++prefix)
            if (core_machine_cpu_instruction_lexeme_scan(
                    (const lib_u8[]){size_prefixes[prefix], 0x8bu, 0xc1u},
                    3u, profiles[profile], LIB_FALSE, &lexeme) || lexeme.available)
                return 0;
        if (core_machine_cpu_instruction_lexeme_scan(locked_add, sizeof(locked_add),
                profiles[profile], LIB_FALSE, &lexeme) || lexeme.available) return 0;
    }
    return preview_expect((const lib_u8[]){0x64u, 0x8au, 0x06u, 0u, 0x20u},
        5u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 5u, 4u) &&
        preview_expect((const lib_u8[]){0x65u, 0x8au, 0x06u, 0u, 0x20u},
        5u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 5u, 4u) &&
        preview_expect((const lib_u8[]){0x66u, 0x8bu, 0xc1u}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x67u, 0x8bu, 0x46u, 0x10u}, 4u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u) &&
...
static C_INT preview_test_primary_alu_profiles(C_VOID)
{
    ...
    for (operation = 0u; operation != sizeof(base); ++operation) {
        const lib_u8 opcode = base[operation];
        if (!preview_expect((const lib_u8[]){opcode, 0xc1u}, 2u,
                profiles[profile], LIB_FALSE, 2u, 2u) ||
            !preview_expect((const lib_u8[]){(lib_u8)(opcode + 1u),
                0xc1u}, 2u, profiles[profile], LIB_FALSE, 2u, 2u) ||
            !preview_expect((const lib_u8[]){(lib_u8)(opcode + 2u),
                0xc1u}, 2u, profiles[profile], LIB_FALSE, 2u, 2u) ||
            !preview_expect((const lib_u8[]){(lib_u8)(opcode + 3u),
                0xc1u}, 2u, profiles[profile], LIB_FALSE, 2u, 2u) ||
            !preview_expect((const lib_u8[]){(lib_u8)(opcode + 4u),
                0x5au}, 2u, profiles[profile], LIB_FALSE, 2u, 2u) ||
            !preview_expect((const lib_u8[]){(lib_u8)(opcode + 5u),
                0x5au, 0u}, 3u, profiles[profile], LIB_FALSE, 3u, 2u)) return 0;
    }
    return preview_expect((const lib_u8[]){0x66u, 0x01u, 0xc8u}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x67u, 0x01u, 0x46u, 0x10u},
        4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u) &&
        preview_expect((const lib_u8[]){0x66u, 0x05u, 0x5au, 0u,
        0u, 0u}, 6u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 6u, 3u) &&
...
static C_INT preview_test_direct_far_control_profiles(C_VOID)
{
    ...
    static const lib_u8 call_far[] = {0x9au, 0x34u, 0x12u, 0x78u, 0x56u};
    static const lib_u8 jump_far[] = {0xeau, 0x34u, 0x12u, 0x78u, 0x56u};
    lib_u8 profile;
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(call_far, sizeof(call_far), profiles[profile],
            ...
            !preview_expect(jump_far, sizeof(jump_far), profiles[profile],
                LIB_FALSE, 5u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x9au, 0x78u, 0x56u,
            0x34u, 0x12u, 0xbcu, 0x9au}, 7u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 7u, 2u) &&
        preview_expect((const lib_u8[]){0xeau, 0x78u, 0x56u,
            0x34u, 0x12u, 0xbcu, 0x9au}, 7u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 7u, 2u) &&
        preview_expect((const lib_u8[]){0x66u, 0x9au, 0x34u,
            0x12u, 0x78u, 0x56u}, 6u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 6u, 3u) &&
        preview_expect((const lib_u8[]){0x66u, 0xeau, 0x78u,
            0x56u, 0x34u, 0x12u, 0xbcu, 0x9au}, 8u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 8u, 3u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0x9au,
...
static C_INT preview_test_direct_near_control_profiles(C_VOID)
{
    ...
    static const lib_u8 call_near[] = {0xe8u, 0x34u, 0x12u};
    static const lib_u8 jump_near[] = {0xe9u, 0x34u, 0x12u};
    static const lib_u8 jump_short[] = {0xebu, 0x80u};
    lib_u8 profile;
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(call_near, sizeof(call_near), profiles[profile],
            ...
            !preview_expect(jump_short, sizeof(jump_short), profiles[profile],
                LIB_FALSE, 2u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0xe8u,
            0x78u, 0x56u, 0x34u, 0x12u}, 6u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 6u, 3u) &&
        preview_expect((const lib_u8[]){0x66u, 0xe9u,
            0x78u, 0x56u, 0x34u, 0x12u}, 6u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 6u, 3u) &&
        preview_expect((const lib_u8[]){0x67u, 0xe9u, 0x34u, 0x12u},
            4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 3u) &&
        preview_expect((const lib_u8[]){0x66u, 0xebu, 0x80u}, 3u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0xe8u,
...
static C_INT preview_test_near_jcc_profiles(C_VOID)
{
    ...
    for (opcode = 0x80u; opcode != 0x90u; ++opcode)
        if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){
                0x0fu, opcode, 0u, 0u }, 4u, unavailable_profiles[profile],
                LIB_FALSE, &lexeme) || lexeme.available) return 0;
    for (opcode = 0x80u; opcode != 0x90u; ++opcode) {
        if (!preview_expect((const lib_u8[]){0x0fu, opcode, 0x34u, 0x12u},
                4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 3u) ||
            !preview_expect((const lib_u8[]){0x0fu, opcode,
                0x78u, 0x56u, 0x34u, 0x12u}, 6u,
                CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 6u, 3u)) return 0;
    }
    return preview_expect((const lib_u8[]){0x66u, 0x0fu, 0x84u,
            0x78u, 0x56u, 0x34u, 0x12u}, 7u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 7u, 4u) &&
        preview_expect((const lib_u8[]){0x67u, 0x0fu, 0x85u,
            0x34u, 0x12u}, 5u, CORE_MACHINE_CPU_PROFILE_80386,
            LIB_FALSE, 5u, 4u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0x0fu,
...
static C_INT preview_test_setcc_profiles(C_VOID)
{
    ...
    for (opcode = 0x90u; opcode != 0xa0u; ++opcode)
        if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){
                ...
                LIB_FALSE, &lexeme) || lexeme.available) return 0;
    for (opcode = 0x90u; opcode != 0xa0u; ++opcode)
        if (!preview_expect((const lib_u8[]){0x0fu, opcode, 0xc0u},
                3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) ||
            !preview_expect((const lib_u8[]){0x0fu, opcode, 0x06u,
                0x34u, 0x12u}, 5u, CORE_MACHINE_CPU_PROFILE_80386,
                LIB_FALSE, 5u, 4u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0x0fu, 0x94u, 0xc0u},
            4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u) &&
        preview_expect((const lib_u8[]){0x67u, 0x0fu, 0x95u, 0x05u,
            0x78u, 0x56u, 0x34u, 0x12u}, 8u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 8u, 5u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0x0fu, 0x9cu,
...
static C_INT preview_test_bit_test_profiles(C_VOID)
{
    ...
    for (index = 0u; index < sizeof(opcodes); ++index)
        if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){
                ...
                LIB_FALSE, &lexeme) || lexeme.available) return 0;
    for (index = 0u; index < sizeof(opcodes); ++index)
        if (!preview_expect((const lib_u8[]){0x0fu, opcodes[index], 0xc8u},
                3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) ||
            !preview_expect((const lib_u8[]){0x0fu, opcodes[index], 0x0eu,
                0x34u, 0x12u}, 5u, CORE_MACHINE_CPU_PROFILE_80386,
                LIB_FALSE, 5u, 4u)) return 0;
    for (extension = 0u; extension != 8u; ++extension) {
        ...
        if (valid && !preview_expect(ba, sizeof(ba), CORE_MACHINE_CPU_PROFILE_80386,
            LIB_FALSE, 4u, 4u)) return 0;
    }
    return preview_expect((const lib_u8[]){0x0fu, 0xbau, 0x2eu,
            0x34u, 0x12u, 0x1fu}, 6u, CORE_MACHINE_CPU_PROFILE_80386,
            LIB_FALSE, 6u, 5u) &&
        preview_expect((const lib_u8[]){0x66u, 0x0fu, 0xabu, 0xc8u},
            4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u) &&
        preview_expect((const lib_u8[]){0x67u, 0x0fu, 0xbau, 0x2du,
            0x78u, 0x56u, 0x34u, 0x12u, 0x1fu}, 9u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 9u, 6u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0x0fu, 0xbbu,
            0x05u, 0x78u, 0x56u, 0x34u, 0x12u}, 9u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 9u, 6u);
}
static C_INT preview_test_double_shift_profiles(C_VOID)
{
    ...
    static const lib_u8 immediate[] = {0xa4u, 0xacu};
    static const lib_u8 cl[] = {0xa5u, 0xadu};
    core_machine_cpu_instruction_lexeme lexeme;
    lib_u8 profile;
    lib_u8 index;
    ...
    for (index = 0u; index < sizeof(immediate); ++index)
        if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){
                ...
                LIB_FALSE, &lexeme) || lexeme.available) return 0;
    for (index = 0u; index < sizeof(immediate); ++index)
        if (!preview_expect((const lib_u8[]){0x0fu, immediate[index],
                0xc8u, 1u}, 4u, CORE_MACHINE_CPU_PROFILE_80386,
                LIB_FALSE, 4u, 4u) || !preview_expect((const lib_u8[]){
                0x0fu, immediate[index], 0x0eu, 0x34u, 0x12u, 1u}, 6u,
                CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 6u, 5u) ||
            !preview_expect((const lib_u8[]){0x0fu, cl[index], 0xc8u},
                3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u)) return 0;
    return preview_expect((const lib_u8[]){0x66u,0x0fu,0xa4u,0xc8u,1u},
            5u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 5u, 5u) &&
        preview_expect((const lib_u8[]){0x67u,0x0fu,0xadu,0x05u,
            0x78u,0x56u,0x34u,0x12u}, 8u, CORE_MACHINE_CPU_PROFILE_80386,
            LIB_FALSE, 8u, 5u) && preview_expect((const lib_u8[]){
    ...
    for (index = 0u; index < sizeof(opcodes); ++index)
        if (!preview_expect((const lib_u8[]){0x0fu, opcodes[index], 0xc8u},
            ...
            !preview_expect((const lib_u8[]){0x0fu, opcodes[index], 0x0eu,
                0x34u, 0x12u}, 5u, CORE_MACHINE_CPU_PROFILE_80386,
                LIB_FALSE, 5u, 4u)) return 0;
    return preview_expect((const lib_u8[]){0x66u,0x0fu,0xbcu,0xc8u},
            4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 4u, 4u) &&
        preview_expect((const lib_u8[]){0x67u,0x0fu,0xbdu,0x05u,
            0x78u,0x56u,0x34u,0x12u}, 8u, CORE_MACHINE_CPU_PROFILE_80386,
            LIB_FALSE, 8u, 5u) && preview_expect((const lib_u8[]){
...
static C_INT preview_test_movx_profiles(C_VOID)
{
    ...
    core_machine_cpu_instruction_lexeme lexeme; lib_u8 profile,index;
    for(profile=0u;profile<sizeof(unavailable)/sizeof(unavailable[0]);++profile)
    for(index=0u;index<sizeof(opcodes);++index)
        if(core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,opcodes[index],0xc8u},3u,unavailable[profile],LIB_FALSE,&lexeme)||lexeme.available)return 0;
    for(index=0u;index<sizeof(opcodes);++index)
...
static C_INT preview_test_system_selector_group_profiles(C_VOID)
{
    ...
    for (profile = 0u; profile < sizeof(profiles) / sizeof(profiles[0]); ++profile) {
        for (extension = 0u; extension < 6u; ++extension)
            if (!preview_expect((const lib_u8[]){0x0fu, 0x00u,
                    (lib_u8)(0xc0u | (extension << 3u))}, 3u,
                    profiles[profile], LIB_FALSE, 3u, 3u)) return 0;
        for (extension = 6u; extension < 8u; ++extension)
            if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
                    0x00u, (lib_u8)(extension << 3u)}, 3u,
                    profiles[profile], LIB_FALSE, &lexeme) || lexeme.available ||
                core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
                    0x00u, (lib_u8)((extension << 3u) | 6u), 0u, 0u}, 5u,
                    profiles[profile], LIB_FALSE, &lexeme) || lexeme.available) return 0;
    }
    return core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
...
static C_INT preview_test_system_group_profiles(C_VOID)
{
    ...
    static const lib_u8 legal_memory[] = {0x0fu, 0x01u, 0x06u, 0u, 0x20u};
    static const lib_u8 legal_register[] = {0x0fu, 0x01u, 0xe0u};
    core_machine_cpu_instruction_lexeme lexeme;
    lib_u8 profile;
    lib_u8 extension;

    if (!preview_expect((const lib_u8[]){0x0fu}, 1u,
            CORE_MACHINE_CPU_PROFILE_8086, LIB_FALSE, 1u, 1u) ||
        core_machine_cpu_instruction_lexeme_scan(legal_memory, sizeof(legal_memory),
            CORE_MACHINE_CPU_PROFILE_80186, LIB_FALSE, &lexeme) || lexeme.available)
        return 0;
    for (profile = 0u; profile < sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(legal_memory, sizeof(legal_memory), profiles[profile],
                LIB_FALSE, 5u, 4u) || !preview_expect(legal_register,
                sizeof(legal_register), profiles[profile], LIB_FALSE, 3u, 3u) ||
                !preview_expect((const lib_u8[]){0x0fu, 0x01u, 0xf0u},
                3u, profiles[profile], LIB_FALSE, 3u, 3u)) return 0;
    for (extension = 5u; extension < 8u; extension += 2u)
        for (profile = 0u; profile < sizeof(profiles) / sizeof(profiles[0]); ++profile)
            if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
                    0x01u, (lib_u8)(extension << 3u)}, 3u,
                    profiles[profile], LIB_FALSE, &lexeme) || lexeme.available ||
                core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
                    0x01u, (lib_u8)((extension << 3u) | 6u), 0u, 0u}, 5u,
                    profiles[profile], LIB_FALSE, &lexeme) || lexeme.available) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0x0fu, 0x01u,
            0x06u, 0u, 0x20u}, 6u, CORE_MACHINE_CPU_PROFILE_80386,
            LIB_FALSE, 6u, 5u) && preview_expect((const lib_u8[]){0x67u,
...
static C_INT preview_test_test_mov_profiles(C_VOID)
{
    ...
    static const lib_u8 opcodes[] = {0x24u, 0x26u};
    core_machine_cpu_instruction_lexeme lexeme;
    lib_u8 profile;
    lib_u8 opcode;
    lib_u8 index;
    ...
    for (profile = 0u; profile < sizeof(unavailable) / sizeof(unavailable[0]); ++profile)
        for (opcode = 0u; opcode < sizeof(opcodes); ++opcode)
            if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
                    opcodes[opcode], 0xf0u}, 3u, unavailable[profile], LIB_TRUE,
                    &lexeme) || lexeme.available) return 0;
    for (opcode = 0u; opcode < sizeof(opcodes); ++opcode) {
        for (index = 6u; index < 8u; ++index)
            if (!preview_expect((const lib_u8[]){0x0fu, opcodes[opcode],
                    (lib_u8)(0xc0u | (index << 3u))}, 3u,
                    CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 3u, 3u)) return 0;
        for (index = 0u; index < 6u; ++index)
            if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
                    opcodes[opcode], (lib_u8)(0xc0u | (index << 3u))},
                    3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, &lexeme) ||
                    lexeme.available) return 0;
        if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
                opcodes[opcode], 0x06u, 0u, 0x20u}, 5u,
                CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, &lexeme) || lexeme.available)
            return 0;
    }
    return preview_expect((const lib_u8[]){0x66u, 0x0fu, 0x24u,
            0xf0u}, 4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 4u, 4u) &&
        preview_expect((const lib_u8[]){0x67u, 0x0fu, 0x26u,
            0xf8u}, 4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE, 4u, 4u) &&
...
static C_INT preview_test_lss_lfs_lgs_profiles(C_VOID)
{
    ...
    for (profile = 0u; profile < sizeof(unavailable) / sizeof(unavailable[0]); ++profile)
        for (opcode = 0u; opcode < sizeof(opcodes); ++opcode)
            if (core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,
                    opcodes[opcode], 0x06u, 0u, 0x20u}, 5u,
                    unavailable[profile], LIB_FALSE, &lexeme) || lexeme.available)
                return 0;
    for (opcode = 0u; opcode < sizeof(opcodes); ++opcode)
        if (!preview_expect((const lib_u8[]){0x0fu, opcodes[opcode],
            ...
            return 0;
    return preview_expect((const lib_u8[]){0x66u, 0x0fu, 0xb2u,
            0x06u, 0u, 0x20u}, 6u, CORE_MACHINE_CPU_PROFILE_80386,
            LIB_FALSE, 6u, 5u) &&
        preview_expect((const lib_u8[]){0x67u, 0x0fu, 0xb4u,
            0x05u, 0x78u, 0x56u, 0x34u, 0x12u}, 8u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 8u, 5u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0x0fu, 0xb5u,
            0x05u, 0x78u, 0x56u, 0x34u, 0x12u}, 9u,
            CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 9u, 6u);
}
static C_INT preview_test_debug_mov_profiles(C_VOID)
{
    ...
    core_machine_cpu_instruction_lexeme lexeme;lib_u8 p,o,i;
    for(p=0u;p<sizeof(unavailable)/sizeof(unavailable[0]);++p)for(o=0u;o<sizeof(opcodes);++o)if(core_machine_cpu_instruction_lexeme_scan((const lib_u8[]){0x0fu,opcodes[o],0xc0u},3u,unavailable[p],LIB_FALSE,&lexeme)||lexeme.available)return 0;
...
static C_INT preview_test_short_jcc_profiles(C_VOID)
{
    ...
    for (opcode = 0x70u; opcode != 0x80u; ++opcode)
        if (!preview_expect((const lib_u8[]){opcode, 0x80u}, 2u,
                profiles[profile], LIB_FALSE, 2u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0x74u, 0x80u}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x67u, 0x75u, 0x7fu}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
...
static C_INT preview_test_scalar_io_profiles(C_VOID)
{
    ...
    static const lib_u8 immediate[] = { 0xe4u, 0xe5u, 0xe6u, 0xe7u };
    static const lib_u8 dx[] = { 0xecu, 0xedu, 0xeeu, 0xefu };
    lib_u8 profile;
    lib_u8 opcode;

    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile) {
        for (opcode = 0u; opcode != sizeof(immediate); ++opcode)
            if (!preview_expect((const lib_u8[]){immediate[opcode], 0x5au},
                    2u, profiles[profile], LIB_FALSE, 2u, 2u)) return 0;
        for (opcode = 0u; opcode != sizeof(dx); ++opcode)
            if (!preview_expect(&dx[opcode], 1u, profiles[profile], LIB_FALSE,
                    1u, 1u)) return 0;
    }
    return preview_expect((const lib_u8[]){0x66u, 0xe5u, 0x5au},
        3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x66u, 0xe7u, 0x5au}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x66u, 0xedu}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
...
static C_INT preview_test_string_io_profiles(C_VOID)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
    for (opcode = 0x6cu; opcode != 0x70u; ++opcode)
        if (!preview_expect(&opcode, 1u, profiles[profile], LIB_FALSE,
                1u, 1u) || !preview_expect((const lib_u8[]){0xf3u,
                opcode}, 2u, profiles[profile], LIB_FALSE, 2u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0x6du}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
        preview_expect((const lib_u8[]){0x67u, 0x6cu}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0x6fu}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
...
static C_INT preview_test_push_immediate_profiles(C_VOID)
{
    ...
    if (core_machine_cpu_instruction_lexeme_scan(push_iw, sizeof(push_iw),
        ...
        lexeme.available || core_machine_cpu_instruction_lexeme_scan(push_ib,
            sizeof(push_ib), CORE_MACHINE_CPU_PROFILE_8086, LIB_FALSE,
            &lexeme) || lexeme.available) return 0;
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(push_iw, sizeof(push_iw), profiles[profile],
                LIB_FALSE, 3u, 2u) || !preview_expect(push_ib,
                sizeof(push_ib), profiles[profile], LIB_FALSE, 2u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0x68u, 0x78u,
        0x56u, 0x34u, 0x12u}, 6u, CORE_MACHINE_CPU_PROFILE_80386,
        LIB_FALSE, 6u, 3u) && preview_expect((const lib_u8[]){0x66u,
        0x6au, 0x80u}, 3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE,
        3u, 3u) && preview_expect((const lib_u8[]){0x67u, 0x68u,
        0x34u, 0x12u}, 4u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE,
        4u, 3u) && preview_expect((const lib_u8[]){0x66u, 0x67u,
...
static C_INT preview_test_lahf_sahf_profiles(C_VOID)
{
    ...
    lib_u8 profile;
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect((const lib_u8[]){0x9fu},1u,profiles[profile],LIB_FALSE,1u,1u) || !preview_expect((const lib_u8[]){0x9eu},1u,profiles[profile],LIB_FALSE,1u,1u)) return 0;
...
static C_INT preview_test_xlat_profiles(C_VOID)
{
    ...
    lib_u8 profile;
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect((const lib_u8[]){0xd7u},1u,profiles[profile],LIB_FALSE,1u,1u)) return 0;
...
static C_INT preview_test_decimal_adjust_profiles(C_VOID)
{
    ...
    static const core_machine_cpu_profile profiles[] = {CORE_MACHINE_CPU_PROFILE_8086, CORE_MACHINE_CPU_PROFILE_80186, CORE_MACHINE_CPU_PROFILE_80286, CORE_MACHINE_CPU_PROFILE_80386};
    lib_u8 profile; lib_u8 opcode;
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile) {
        for (opcode = 0u; opcode != sizeof(simple); ++opcode)
            if (!preview_expect(&simple[opcode],1u,profiles[profile],LIB_FALSE,1u,1u)) return 0;
        if (!preview_expect((const lib_u8[]){0xd4u,10u},2u,profiles[profile],LIB_FALSE,2u,2u) || !preview_expect((const lib_u8[]){0xd5u,10u},2u,profiles[profile],LIB_FALSE,2u,2u)) return 0;
    }
...
static C_INT preview_test_lar_lsl_profiles(C_VOID)
{
    ...
    if (core_machine_cpu_instruction_lexeme_scan(lar,3u,
        CORE_MACHINE_CPU_PROFILE_80186,LIB_FALSE,&lexeme) || lexeme.available)
        return 0;
    return preview_expect(lar,3u,CORE_MACHINE_CPU_PROFILE_80286,LIB_FALSE,3u,3u) &&
        preview_expect(lsl,3u,CORE_MACHINE_CPU_PROFILE_80286,LIB_FALSE,3u,3u) &&
...
static C_INT preview_test_pusha_popa_profiles(C_VOID)
{
    ...
    if (core_machine_cpu_instruction_lexeme_scan(pusha,1u,CORE_MACHINE_CPU_PROFILE_8086,LIB_FALSE,&lexeme)||lexeme.available||core_machine_cpu_instruction_lexeme_scan(popa,1u,CORE_MACHINE_CPU_PROFILE_8086,LIB_FALSE,&lexeme)||lexeme.available) return 0;
...
static C_INT preview_test_imul_immediate_profiles(C_VOID)
{
    ...
    if (core_machine_cpu_instruction_lexeme_scan(iw,sizeof(iw),CORE_MACHINE_CPU_PROFILE_8086,LIB_FALSE,&lexeme)||lexeme.available||core_machine_cpu_instruction_lexeme_scan(ib,sizeof(ib),CORE_MACHINE_CPU_PROFILE_8086,LIB_FALSE,&lexeme)||lexeme.available) return 0;
...
static C_INT preview_test_bound_profiles(C_VOID)
{
    ...
    return preview_expect(bound, sizeof(bound), CORE_MACHINE_CPU_PROFILE_80186,
        LIB_FALSE, 4u, 3u) && preview_expect(bound, sizeof(bound),
        CORE_MACHINE_CPU_PROFILE_80286, LIB_FALSE, 4u, 3u) &&
        preview_expect(bound, sizeof(bound), CORE_MACHINE_CPU_PROFILE_80386,
        LIB_FALSE, 4u, 3u) && preview_expect((const lib_u8[]){0x66u,0x62u,0x06u,0u,0x20u},5u,
...
static C_INT preview_test_arpl_profiles(C_VOID)
{
    ...
    return preview_expect(arpl, sizeof(arpl), CORE_MACHINE_CPU_PROFILE_80286,
        LIB_FALSE, 2u, 2u) && preview_expect(arpl, sizeof(arpl),
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u);
}
...
static C_INT preview_test_iret_profiles(C_VOID)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(iret, sizeof(iret), profiles[profile],
            LIB_FALSE, 1u, 1u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0xcfu}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0xcfu}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u);
}
...
static C_INT preview_test_int3_into_profiles(C_VOID)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(int3, sizeof(int3), profiles[profile],
            LIB_FALSE, 1u, 1u) || !preview_expect(into, sizeof(into),
            profiles[profile], LIB_FALSE, 1u, 1u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0xccu}, 2u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 2u, 2u) &&
        preview_expect((const lib_u8[]){0x66u, 0x67u, 0xceu}, 3u,
        CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u);
}
...
static C_INT preview_test_int_immediate_profiles(C_VOID)
{
    ...
    static const lib_u8 interrupt[] = {0xcdu, 0x31u};
    lib_u8 profile;

    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile)
        if (!preview_expect(interrupt, sizeof(interrupt), profiles[profile],
            LIB_FALSE, 2u, 2u)) return 0;
    return preview_expect((const lib_u8[]){0x66u, 0xcdu, 0x31u},
        3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
        preview_expect((const lib_u8[]){0x67u, 0xcdu, 0x31u},
        3u, CORE_MACHINE_CPU_PROFILE_80386, LIB_FALSE, 3u, 3u) &&
...
static C_INT preview_test_group3_profiles(C_VOID)
{
    ...
    for (extension = 0u; extension != 8u; ++extension) {
        lib_u8 f6[] = { 0xf6u,
            (lib_u8)((extension << 3u) | 0xc0u), 0xa5u };
        lib_u8 f7[] = { 0xf7u,
...
static C_INT preview_test_group45_profiles(C_VOID)
{
    ...
    for (profile_index = 0u; profile_index != sizeof(profiles) / sizeof(profiles[0]);
        ++profile_index)
    for (extension = 0u; extension != 8u; ++extension) {
        lib_u8 fe[] = { 0xfeu, (lib_u8)((extension << 3u) | 0xc0u) };
        lib_u8 ff_register[] = { 0xffu,
            (lib_u8)((extension << 3u) | 0xc0u) };
        ...
        if (fe_valid != (core_machine_cpu_instruction_lexeme_scan(fe, sizeof(fe),
            ...
            sizeof(ff_register), profiles[profile_index], LIB_FALSE, &lexeme) &&
            lexeme.available) || ff_memory_valid !=
            (core_machine_cpu_instruction_lexeme_scan(ff_memory, sizeof(ff_memory),
            profiles[profile_index], LIB_FALSE, &lexeme) && lexeme.available)) return 0;
        ...
        if (ff_register_valid && !preview_expect(ff_register, sizeof(ff_register),
            profiles[profile_index], LIB_FALSE, 2u, 2u)) return 0;
        if (ff_memory_valid && !preview_expect(ff_memory, sizeof(ff_memory),
            profiles[profile_index], LIB_FALSE, 4u, 3u)) return 0;
    }
    return 1;
}
static C_INT preview_test_unavailable(C_VOID)
{
    ...
    static const lib_u8 invalid_opcode[] = { 0xd6u };
    static const lib_u8 locked_nop[] = { 0xf0u, 0x90u };
    core_machine_cpu_instruction_lexeme lexeme;

    return !core_machine_cpu_instruction_lexeme_scan(truncated,
            sizeof(truncated), CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE,
