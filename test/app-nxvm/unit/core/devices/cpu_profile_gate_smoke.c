...
static void cpu_profile_machine_reset(void *opaque)
{
    ...
    lib_size program_size, lib_i32 expect_ud)
{
    ...
    lib_i32 failed = prepare_machine(profile, &state);
...
static lib_i32 run_pop_cs_8086(void)
{
    ...
    core_machine_run_result result;
    core_machine_cpu_diagnostic diagnostic;
    lib_i32 failed = prepare_machine(CORE_MACHINE_CPU_PROFILE_8086, &state);
...
static lib_i32 run_8088_prefetch_capacity(void)
{
    ...
    lib_i32 failed = prepare_machine(CORE_MACHINE_CPU_PROFILE_8088, &state);
...
static lib_i32 run_8088_prefetch_control_and_self_modify(void)
{
    ...
    if (!failed) {
        ...
        failed |= core_machine_memory_write(state.machine, 3u, &replacement,
            sizeof(replacement)) != LIB_STATUS_OK;
...
lib_i32 main(void)
{
    ...
    printf("M5:T484:S3:XT-8088-QUEUE:OK\n");
    return 0;
}
