...
static C_INT atomic_install(atomic_machine *state)
{
    ...
    static const lib_u8 initial_halt[] = { 0xf4 };
    lib_u8 fault_handlers[0x21u] = {0};
    ...
    const atomic_return_case *test)
{
    ...
    if (use_iret) {
        lib_memory_copy(bytes, iret, sizeof(iret));
        bytes[1u] = (lib_u8)test->ss;
        bytes[2u] = (lib_u8)(test->ss >> 8u);
        bytes[13u] = (lib_u8)test->cs;
        bytes[14u] = (lib_u8)(test->cs >> 8u);
        return sizeof(iret);
    }
    lib_memory_copy(bytes, retf, sizeof(retf));
    bytes[1u] = (lib_u8)test->ss;
    bytes[2u] = (lib_u8)(test->ss >> 8u);
    bytes[9u] = (lib_u8)test->cs;
    bytes[10u] = (lib_u8)(test->cs >> 8u);
    return sizeof(retf);
}

static lib_u16 atomic_fault_stop_ip(lib_u8 vector)
{
    switch (vector) {
...
static C_INT atomic_test_outer_return(const atomic_return_case *test, C_INT use_iret)
{
    ...
    static const lib_u8 expected_access[] = { 0xfau, 0x7au, 0x72u, 0xf2u, 0xfeu };
    ...
    t_cpu cpu;
    lib_u8 code[21u];
    lib_u8 access[sizeof(access_addresses) / sizeof(access_addresses[0])];
    lib_u16 frame[4u] = {0};
    ...
    if (!failed) {
        ...
        if (test->delivered) {
            ...
            state_failed |= cpu.data.eip != atomic_fault_stop_ip(test->vector) ||
                ...
                lib_memory_compare(&cpu.data.cs, &before.cs, sizeof(before.cs)) != 0 ||
                lib_memory_compare(&cpu.data.ss, &before.ss, sizeof(before.ss)) != 0 ||
        ...
        if (failed) {
            STD_FPRINTF(STD_STDERR,
                ...
                lib_memory_compare(&cpu.data.cs, &before.cs, sizeof(before.cs)),
                lib_memory_compare(&cpu.data.ss, &before.ss, sizeof(before.ss)),
    ...
    C_INT wide_new_stack)
{
    ...
    static const lib_u8 retf16[] = { 0x66u,0xcau,0x04u,0x00u };
    static const lib_u32 frame32[] = { 0x0010u,0x001bu,0u,0x1000u,0x0023u };
    static const lib_u16 frame16[] = { 0x0010u,0x001bu,0u,0u,0x1000u,0x0023u };
    const core_machine_run_budget boot_budget = { 128u, 0u };
    const core_machine_run_budget budget = { 1u, 0u };
    const lib_u8 stack_flags = 0x40u;
    const lib_u8 *program = operand16 ? retf16 :
        (address_prefix ? retf32_address : retf32);
    const lib_size program_bytes = operand16 ? sizeof(retf16) :
    ...
    C_INT wide_new_stack, C_INT restricted_flags)
{
    ...
    static const lib_u8 iret16[] = { 0x66u,0xcfu };
    lib_u32 frame32[] = { 0x0010u,0x001bu,0u,0x1000u,0x0023u };
    lib_u16 frame16[] = { 0x0010u,0x001bu,0u,0x1000u,0x0023u };
    const core_machine_run_budget boot_budget = { 128u, 0u };
    const core_machine_run_budget budget = { 1u, 0u };
    const lib_u8 stack_flags = 0x40u;
    const lib_u8 *program = operand16 ? iret16 :
        (address_prefix ? iret32_address : iret32);
    const lib_size program_bytes = operand16 ? sizeof(iret16) :
        (address_prefix ? sizeof(iret32_address) : sizeof(iret32));
    const lib_u32 return_flags = restricted_flags ? 0x00013003u :
        (operand16 ? 0x00003203u : 0x00013203u);
    const lib_u32 expected_flags = restricted_flags ? 0x00010203u :
        return_flags;
    atomic_machine state;
    core_machine_run_result result = {0};
    core_machine_cpu_diagnostic diagnostic;
    ...
    if (!failed) {
        ...
        frame16[2u] = (lib_u16)return_flags;
        if (!failed && wide_new_stack) {
            failed |= !atomic_write(&state, ATOMIC_GDT_BASE + 38u, &stack_flags,
                sizeof(stack_flags));
        }
...
static C_INT atomic_test_outer_iret32_failure(C_INT target_limit)
{
    ...
    const core_machine_run_budget boot_budget = { 128u, 0u };
    const core_machine_run_budget budget = { 128u, 0u };
    const lib_u8 zero = 0u;
    const lib_u8 code_access = 0xf8u;
    atomic_machine state;
    core_machine_run_result result = {0};
    core_machine_cpu_diagnostic diagnostic;
    t_cpu before;
