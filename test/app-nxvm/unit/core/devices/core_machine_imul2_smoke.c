...
static type_status imul_read(C_VOID *owner,lib_u32 physical,type_virtual_address dst,type_native_unsigned bytes)
{ imul_provider *p=(imul_provider *)owner;(C_VOID)dst;if(p==LIB_NULL||physical!=IMUL_PROVIDER_ADDRESS||(bytes!=2u&&bytes!=4u))return TYPE_STATUS_INVALID_ARGUMENT;++p->reads;return TYPE_STATUS_OK; }
static type_status imul_write(C_VOID *o,lib_u32 p,type_virtual_address s,type_native_unsigned b)
{(C_VOID)o;(C_VOID)p;(C_VOID)s;(C_VOID)b;return TYPE_STATUS_UNSUPPORTED;}
static type_status imul_query(C_VOID *o,lib_u32 p,type_native_unsigned b,core_machine_memory_access a)
...
static C_VOID imul_reset(C_VOID *o){imul_machine *s=(imul_machine *)o;if(s!=LIB_NULL)(C_VOID)test_core_machine_fixture_reset_real_mode(s->machine);}
...
static C_INT imul_prepare(core_machine_cpu_profile p,imul_provider *provider,imul_machine *s)
{
 ...
 if(core_machine_create(&c,&s->machine)!=TYPE_STATUS_OK||(provider!=LIB_NULL&&test_core_machine_fixture_register_memory_device_provider(s->machine,IMUL_PROVIDER_ADDRESS,4u,imul_read,imul_write,imul_query,provider)!=TYPE_STATUS_OK)||!test_core_machine_fixture_bind_freeze_reset(s->machine,&imul_execution,s)){core_machine_destroy(s->machine);s->machine=LIB_NULL;return 0;}return 1;
}
static C_INT imul_run(imul_machine *s,const lib_u8 *code,lib_size bytes,C_INT fault,t_cpu *out,core_machine_cpu_diagnostic *d)
{core_machine_run_result r;type_status status;if(s==LIB_NULL||s->machine==LIB_NULL||!test_core_machine_fixture_prepare_real_mode_execution(s->machine,0u)||core_machine_memory_write(s->machine,0u,code,bytes)!=TYPE_STATUS_OK)return 0;if(fault&&!test_core_machine_fixture_preflight_real_ud_terminal(s->machine))return 0;status=core_machine_run(s->machine,(core_machine_run_budget){1u,0u},&r);if(status!=(fault?TYPE_STATUS_FAULT:TYPE_STATUS_OK)||r.reason!=(fault?CORE_MACHINE_STOP_FAULT:CORE_MACHINE_STOP_BUDGET)||core_machine_get_cpu_diagnostic(s->machine,d)!=TYPE_STATUS_OK)return 0;*out=test_core_machine_fixture_capture_cpu_after_run(s->machine);return 1;}
...
{
 static const lib_u8 code[]={0x0f,0xafu,0x0eu,0x10u,0};imul_machine s;t_cpu a;core_machine_cpu_diagnostic d;core_machine_run_result r;const lib_u32 flags=VCPU_EFLAGS_CF|VCPU_EFLAGS_OF;C_INT f=!imul_limit(&s);
