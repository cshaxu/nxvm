...
static void msw_s63_reset(void *opaque)
{
...
static lib_i32 msw_s63_prepare(msw_s63_machine *state, core_machine_cpu_profile profile)
{
...
static void msw_s63_seed(msw_s63_machine *state)
{
...
static lib_i32 msw_s63_sregs_same(const t_cpu *before, const t_cpu *after)
{
...
static lib_i32 msw_s63_gprs_same(const t_cpu *before, const t_cpu *after)
{
...
static lib_i32 msw_s63_run(msw_s63_machine *state, const lib_u8 *code,
...
static lib_i32 msw_s63_expect_ud(core_machine_cpu_profile profile, const lib_u8 *code,
    lib_u8 bytes)
{
    ...
    lib_i32 failed = !msw_s63_prepare(&state, profile);
    if (!failed) {
        ...
        failed |= !msw_s63_run(&state, code, bytes, 1u, &status, &result, &diagnostic) ||
            status != LIB_STATUS_INTERNAL_ERROR || !diagnostic.first_fault.valid || !CORE_MACHINE_BIT_IS_SET(
...
static void msw_s63_enter_protected(msw_s63_machine *state, lib_u8 cpl,
    lib_i32 vm86)
{
    t_cpu *cpu = &state->machine->executor_cpu;
    ...
    if (vm86) CORE_MACHINE_BIT_SET(cpu->data.eflags, VCPU_EFLAGS_VM);
}

static lib_i32 msw_s63_test_success(void)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]); ++profile) {
        msw_s63_machine state; t_cpu before; t_cpu after; core_machine_run_result result;
        core_machine_cpu_diagnostic diagnostic; lib_status status = LIB_STATUS_INVALID_STATE;
        lib_i32 failed = !msw_s63_prepare(&state, profiles[profile]);
        if (!failed) {
            msw_s63_seed(&state); state.machine->executor_cpu.data.cr0 = 0x00a5000du;
            before = test_core_machine_fixture_capture_cpu_after_run(state.machine);
            failed |= !msw_s63_run(&state, smsw, sizeof(smsw), 1u, &status, &result, &diagnostic);
            after = test_core_machine_fixture_capture_cpu_after_run(state.machine);
            failed |= status != LIB_STATUS_OK || diagnostic.first_fault.valid || after.data.eip != 3u ||
...
static lib_i32 msw_s63_test_memory_and_protected(void)
{
    ...
    if (!failed) {
        ...
        failed |= core_machine_memory_read_physical(&state.machine->executor_memory,
            0x0400u, CORE_MACHINE_REFERENCE_OF(image), sizeof(image)) != LIB_STATUS_OK ||
...
static lib_i32 msw_s63_test_attributes(void)
{
    ...
    for (i=0u; i != sizeof(forms)/sizeof(forms[0]); ++i) {
        msw_s63_machine state; t_cpu before; t_cpu after; core_machine_run_result result;
        core_machine_cpu_diagnostic diagnostic; lib_status status; lib_i32 failed = !msw_s63_prepare(&state, CORE_MACHINE_CPU_PROFILE_80386);
        if (!failed) {
            msw_s63_seed(&state); state.machine->executor_cpu.data.eax = 0xdead000du;
            state.machine->executor_cpu.data.cr0 = 0x00a50000u; before = test_core_machine_fixture_capture_cpu_after_run(state.machine);
            failed |= !msw_s63_run(&state, forms[i], lengths[i], 1u, &status, &result, &diagnostic) || status != LIB_STATUS_OK || diagnostic.first_fault.valid;
            after = test_core_machine_fixture_capture_cpu_after_run(state.machine);
...
static lib_i32 msw_s63_test_privilege_and_faults(void)
{
    ...
    for (mode=0u; mode != 3u; ++mode) {
        msw_s63_machine state; t_cpu before; t_cpu after; core_machine_run_result result;
        core_machine_cpu_diagnostic diagnostic; lib_status status = LIB_STATUS_INVALID_STATE; const lib_u8 *code = mode == 0u ? smsw : lmsw;
        lib_i32 failed = !msw_s63_prepare(&state, CORE_MACHINE_CPU_PROFILE_80386);
        if (!failed) {
            ...
            after = test_core_machine_fixture_capture_cpu_after_run(state.machine);
            if (mode == 0u) failed |= status != LIB_STATUS_OK || diagnostic.first_fault.valid || after.data.eax != 0xdead0009u || after.data.cr0 != before.data.cr0;
            else failed |= status != LIB_STATUS_INTERNAL_ERROR || !diagnostic.first_fault.valid || !CORE_MACHINE_BIT_IS_SET(diagnostic.first_fault.exception_mask,VCPUINS_EXCEPT_DF) || after.data.cr0 != before.data.cr0 || after.data.eip != 0u;
    ...
    for (mode=0u; mode != 2u; ++mode) {
        msw_s63_machine state; t_cpu before; t_cpu after; core_machine_run_result result;
        core_machine_cpu_diagnostic diagnostic; lib_status status = LIB_STATUS_INVALID_STATE; const lib_u8 *code = mode == 0u ? smsw_memory : lmsw_memory;
        lib_i32 failed = !msw_s63_prepare(&state, CORE_MACHINE_CPU_PROFILE_80386);
        if (!failed) {
            ...
            after = test_core_machine_fixture_capture_cpu_after_run(state.machine);
            failed |= status != LIB_STATUS_INTERNAL_ERROR || !diagnostic.first_fault.valid || !CORE_MACHINE_BIT_IS_SET(diagnostic.first_fault.exception_mask,VCPUINS_EXCEPT_DF) || after.data.eip != 0u || after.data.cr0 != before.data.cr0 || after.data.eflags != before.data.eflags || !msw_s63_sregs_same(&before,&after);
        }
        core_machine_destroy(state.machine); if (failed) return 0;
    }
    return 1;
...
static lib_i32 msw_s63_test_lock_irq(void)
{
...
lib_i32 main(void)
{
    if(!msw_s63_test_success()){printf("MSW stage=success\n");return 1;}
    if(!msw_s63_test_memory_and_protected()){printf("MSW stage=memory-protected\n");return 1;}
    if(!msw_s63_test_attributes()){printf("MSW stage=attributes\n");return 1;}
    if(!msw_s63_test_privilege_and_faults()){printf("MSW stage=privilege-faults\n");return 1;}
    if(!msw_s63_test_lock_irq()){printf("MSW stage=lock-irq\n");return 1;}
    printf("M5:T316:S63:MSW:OK\n"); return 0;
}
