...
static void lar_lsl_s57_reset(void *opaque)
{
...
static lib_i32 lar_lsl_s57_prepare(lar_lsl_s57_machine *state,
...
static lib_i32 lar_lsl_s57_install_gdt(core_machine *machine)
{
...
static lib_i32 lar_lsl_s57_run_protected(lar_lsl_s57_machine *state,
    const lib_u8 *code, lib_size code_size, t_cpu *out_cpu)
{
    ...
    if (state == LIB_NULL || state->machine == LIB_NULL ||
        code == LIB_NULL || out_cpu == LIB_NULL ||
...
static lib_i32 lar_lsl_s57_boot_protected(lar_lsl_s57_machine *state,
    t_cpu *out_cpu)
{
...
static lib_i32 lar_lsl_s57_resume(lar_lsl_s57_machine *state,
    const lib_u8 *code, lib_size code_size, const core_machine_run_budget *budget,
    core_machine_run_result *result)
{
    if (state == LIB_NULL || state->machine == LIB_NULL || code == LIB_NULL ||
        budget == LIB_NULL || result == LIB_NULL ||
        core_machine_memory_write(state->machine, LAR_LSL_S57_CODE_ADDRESS,
            code, code_size) != LIB_STATUS_OK) {
        return 0;
    }
...
static lib_i32 lar_lsl_s57_sregs_same(const t_cpu *before, const t_cpu *after)
{
...
static lib_i32 lar_lsl_s57_nonstack_gprs_same(const t_cpu *before,
    const t_cpu *after, lib_i32 allow_eax, lib_i32 allow_ecx)
{
...
static lib_i32 lar_lsl_s57_gprs_same_except_eax_ecx(const t_cpu *before,
    const t_cpu *after)
{
...
static lib_i32 lar_lsl_s57_pic_frame_matches(const lib_u32 *frame,
    lib_u32 expected_ip, const t_cpu *before, lib_u8 expected_zf)
{
    return frame[0u] == expected_ip &&
        ((frame[2u] & ~VCPU_EFLAGS_ZF) ==
        (before->data.eflags & ~VCPU_EFLAGS_ZF)) &&
        !!CORE_MACHINE_BIT_IS_SET(frame[2u], VCPU_EFLAGS_ZF) == expected_zf;
}

static lib_i32 lar_lsl_s57_test_default(void)
{
    static const lib_u8 lar[] = { 0x0fu, 0x02u, 0xc1u, 0xf4u };
    static const lib_u8 lsl[] = { 0x0fu, 0x03u, 0xc1u, 0xf4u };
    const lib_u32 expected[] = { 0xa1a19300u, 0xa1afffffu };
    const lib_u8 *codes[] = { lar, lsl };
    lib_u8 profile;
    lib_u8 form;
    ...
    for (profile = 0u; profile != 2u; ++profile) {
        for (form = 0u; form != 2u; ++form) {
            ...
            t_cpu before;
            t_cpu after;
            lib_i32 failed = !lar_lsl_s57_prepare(&state,
            ...
            if (!failed) {
                ...
                failed |= !CORE_MACHINE_BIT_IS_SET(after.data.eflags, VCPU_EFLAGS_ZF);
...
static lib_i32 lar_lsl_s57_test_invalid_selector(void)
{
    static const lib_u8 lar[] = { 0x0fu, 0x02u, 0xc1u, 0xf4u };
    static const lib_u8 lsl[] = { 0x0fu, 0x03u, 0xc1u, 0xf4u };
    const lib_u8 *codes[] = { lar, lsl };
...
static lib_i32 lar_lsl_s57_test_attributes(void)
{
    static const lib_u8 lar_66[] = { 0x66u, 0x0fu, 0x02u, 0xc1u, 0xf4u };
    static const lib_u8 lar_67[] = { 0x67u, 0x0fu, 0x02u, 0xc1u, 0xf4u };
    static const lib_u8 lar_6667[] = {
        0x66u, 0x67u, 0x0fu, 0x02u, 0xc1u, 0xf4u
    };
    static const lib_u8 lsl_66[] = { 0x66u, 0x0fu, 0x03u, 0xc1u, 0xf4u };
    static const lib_u8 lsl_67[] = { 0x67u, 0x0fu, 0x03u, 0xc1u, 0xf4u };
    static const lib_u8 lsl_6667[] = {
        0x66u, 0x67u, 0x0fu, 0x03u, 0xc1u, 0xf4u
    };
    const lib_u8 *codes[] = {
        lar_66, lar_67, lar_6667, lsl_66, lsl_67, lsl_6667
    };
    const lib_u8 lengths[] = { 5u, 5u, 6u, 5u, 5u, 6u };
...
static lib_i32 lar_lsl_s57_test_profile_and_lock_rejects(void)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]);
        ...
        for (form = 0u; form != 8u; ++form) {
            ...
            lib_i32 failed = !lar_lsl_s57_prepare(&state, profiles[profile]);

            if (!failed) {
                failed |= core_machine_memory_write(state.machine, 0u,
                    forms[form], lengths[form]) != LIB_STATUS_OK;
...
static lib_i32 lar_lsl_s57_test_real_mode_rejects(void)
{
    ...
    for (profile = 0u; profile != sizeof(profiles) / sizeof(profiles[0]);
        ...
        for (form = 0u; form != sizeof(forms) / sizeof(forms[0]); ++form) {
            ...
            if (!failed) {
                failed |= core_machine_memory_write(state.machine, 0u,
                    forms[form], sizeof(forms[form])) != LIB_STATUS_OK;
...
static lib_i32 lar_lsl_s57_test_descriptor_matrix(void)
{
    static const lib_u8 nonpresent_lar[] = {
        0xb8u, 0x18u, 0x00u, 0xb9u, 0x34u, 0x12u,
        0x0fu, 0x02u, 0xc8u, 0xf4u
    };
    static const lib_u8 privilege_lsl[] = {
        0xb8u, 0x23u, 0x00u, 0xb9u, 0x34u, 0x12u,
        0x0fu, 0x03u, 0xc8u, 0xf4u
    };
    static const lib_u8 granular_lsl[] = {
    ...
    const lib_u32 expected_ecx[] = {
        0x00001234u, 0x00001234u, 0x00000fffu
    };
    const lib_u8 expected_zf[] = { 0u, 0u, 1u };
    lib_u8 form;
...
static lib_i32 lar_lsl_s57_test_memory_source(void)
{
    static const lib_u8 lar[] = {
        0xb9u, 0x34u, 0x12u,
        0x0fu, 0x02u, 0x0eu, 0x00u, 0x01u,
        0xf4u
    };
    static const lib_u8 lsl[] = {
        0xb9u, 0x34u, 0x12u,
    ...
    const lib_u32 expected[] = { 0x00009300u, 0x0000ffffu };
    lib_u8 form;

    for (form = 0u; form != sizeof(codes) / sizeof(codes[0]); ++form) {
        lar_lsl_s57_machine state;
        t_cpu after = { 0 };
        lib_u16 source = selector;
        lib_i32 failed = !lar_lsl_s57_prepare(&state,
            CORE_MACHINE_CPU_PROFILE_80386);
...
static lib_i32 lar_lsl_s57_test_67_sib_ss_source(void)
{
...
static lib_i32 lar_lsl_s57_test_bp_ss_source(void)
{
    static const lib_u8 code[] = {
...
static lib_i32 lar_lsl_s57_test_override_sources(void)
{
    ...
    const lib_u32 bases[] = { 0x2000u, 0x4000u, 0x5000u, 0x6000u };
    const lib_u32 expected[] = {
...
static lib_i32 lar_lsl_s57_test_source_limit(void)
{
    ...
    for (form = 0u; form != 2u; ++form) {
        ...
        if (!failed) {
            ...
            failed |= core_machine_memory_read_physical(&state.machine->executor_memory,
                0x3010u, (lib_uptr)&source, sizeof(source)) !=
                LIB_STATUS_OK;
...
static lib_i32 lar_lsl_s57_test_vm86(void)
{
...
static lib_i32 lar_lsl_s57_test_pic_lar(void)
{
    static const lib_u8 code[] = { 0xfbu, 0x0fu, 0x02u, 0xc1u, 0x90u };
    lar_lsl_s57_machine state;
...
static lib_i32 lar_lsl_s57_test_80386_timing_granularity(void)
{
    static const lib_u8 lsl[] = { 0x0fu, 0x03u, 0xc8u };
    static const lib_u8 lsl_memory[] = {
        0x0fu, 0x03u, 0x0eu, 0x00u, 0x10u
    };
    const lib_u8 *codes[] = { lsl, lsl, lsl_memory, lsl_memory };
    const core_machine_run_budget budget = { 1u, 0u };
    const lib_u16 selectors[] = { 0x0010u, 0x0028u, 0x0010u, 0x0028u };
    const lib_u64 expected[] = { 21u, 25u, 22u, 26u };
    lib_u8 index;
    ...
    for (index = 0u; index < 4u; ++index) {
        ...
        core_machine_run_result result;
        t_cpu after;
        lib_i32 failed = !lar_lsl_s57_prepare(&state, CORE_MACHINE_CPU_PROFILE_80386) ||
        ...
        if (!failed) {
            ...
            if (index >= 2u) failed |= core_machine_memory_write(state.machine,
                0x4000u, &selectors[index], sizeof(selectors[index])) != LIB_STATUS_OK;
...
static lib_i32 lar_lsl_s57_test_pic_lsl_and_invalid(void)
{
    static const lib_u8 codes[][8] = {
        { 0xfbu, 0x0fu, 0x03u, 0xc1u, 0x90u },
        { 0xfbu, 0x0fu, 0x02u, 0xc1u, 0x90u }
    };
    const lib_u8 lengths[] = { 5u, 5u };
    const lib_u32 expected[] = { 0xa1a1ffffu, 0xa1a10000u };
    const lib_u8 expected_zf[] = { 1u, 0u };
    const lib_u32 expected_ip[] = { 4u, 4u };
    lib_u8 form;
...
static lib_i32 lar_lsl_s57_test_ldt_selector(void)
{
    ...
    const lib_u8 *codes[] = { lar, lsl };
    const lib_u32 expected[] = { 0x00009200u, 0x0000ffffu };
    lib_u8 form;
...
lib_i32 main(void)
{
    ...
    {
        fprintf(stderr, "S57 default failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_invalid_selector())
    {
        fprintf(stderr, "S57 invalid failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_attributes())
    {
        fprintf(stderr, "S57 attributes failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_profile_and_lock_rejects())
    {
        fprintf(stderr, "S57 rejects failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_real_mode_rejects())
    {
        fprintf(stderr, "S57 real-mode rejects failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_descriptor_matrix())
    {
        fprintf(stderr, "S57 descriptor matrix failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_memory_source())
    {
        fprintf(stderr, "S57 memory source failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_67_sib_ss_source())
    {
        fprintf(stderr, "S57 67 SIB source failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_bp_ss_source())
    {
        fprintf(stderr, "S57 BP SS source failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_override_sources())
    {
        fprintf(stderr, "S57 override sources failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_source_limit())
    {
        fprintf(stderr, "S57 source limit failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_vm86())
    {
        fprintf(stderr, "S57 VM86 failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_pic_lar())
    {
        fprintf(stderr, "S57 PIC LAR failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_80386_timing_granularity())
    {
        fprintf(stderr, "S57 80386 LSL timing granularity failed\n");
        return 12;
    }
    if (!lar_lsl_s57_test_pic_lsl_and_invalid())
    {
        fprintf(stderr, "S57 PIC LSL/invalid failed\n");
        return 1;
    }
    if (!lar_lsl_s57_test_ldt_selector())
    {
        fprintf(stderr, "S57 LDT failed\n");
        return 1;
    }
    printf("M5:T316:S57:LAR-LSL:OK\n");
    printf("M5:T401:S32:LAR-LSL-PROFILES:OK\n");
    return 0;
}
