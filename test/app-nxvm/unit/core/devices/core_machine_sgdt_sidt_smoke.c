...
static lib_i32 sgdt_sidt_test_profiles_modes_and_attributes(void)
{
    ...
    static const lib_u8 lengths[] = {5u,6u,8u,9u};
    lib_u8 profile, protected_mode, table, form;
    ...
    for (table = 0u; table != 2u; ++table) for (form = 0u; form != 3u; ++form) {
        ...
        if (!failed) {
            ...
            if (form == 0u) {
                ...
                code[2] = (lib_u8)(0x06u | (table << 3u));
                code[3] = 0x00u; code[4] = 0x02u;
            } else if (form == 1u) {
                code[0] = 0x0fu; code[1] = 0x01u;
                code[2] = (lib_u8)(0x46u | (table << 3u));
                code[3] = 0x10u;
            } else {
                code[0] = 0x26u; code[1] = 0x0fu; code[2] = 0x01u;
                code[3] = (lib_u8)(0x06u | (table << 3u));
...
static lib_i32 sgdt_sidt_test_rejections_and_atomicity(void)
{
    ...
    static const lib_u8 sgdt[] = {0x0fu,0x01u,0x06u,0x0eu,0x00u};
    lib_u8 table;
    for (table = 0u; table != 2u; ++table) {
        lib_u8 low_profile_code[sizeof(sgdt)];

        lib_memory_copy(low_profile_code, sgdt, sizeof(low_profile_code));
    ...
    for (table = 0u; table != 2u; ++table) {
        lib_u8 register_code[sizeof(register_form)];
        lib_u8 lock_code[sizeof(lock_form)];

        lib_memory_copy(register_code, register_form, sizeof(register_code));
        lib_memory_copy(lock_code, lock_form, sizeof(lock_code));
    ...
    for (table = 0u; table != 2u; ++table) {
        ...
        if (!failed) {
            ...
            state.machine->executor_cpu.data.idtr.limit = 0xabceu;
            state.machine->executor_cpu.data.idtr.base = 0x00c0ffeeu;
            lib_memory_copy(code, sgdt, sizeof(code));
            code[2] |= table << 3u;
