    ...
    core_machine_cpu_profile profile, lib_u8 tss32)
{
    ...
    if (!s3_gate_prepare(state, profile, LIB_TRUE,
    ...
    tss_descriptor[5] = (lib_u8)(0x80u | (tss32 ?
    ...
    lib_u8 dpl, lib_u8 parameter_count, lib_u8 present)
{
    ...
    descriptor[5] = (lib_u8)((present ? 0x80u : 0u) | (dpl << 5u) |
...
static lib_i32 s7_outer_success(core_machine_cpu_profile profile, lib_u8 tss32)
{
    ...
    if (!failed) {
        ...
        failed |= !s7_install_call_gate(&state, 3u, S7_PARAMETER_COUNT,
            LIB_TRUE) || !s7_write_call(&state, S7_CALL_GATE_SELECTOR) || !s3_gate_write(&state,
    ...
    if (!failed) {
        ...
        failed |= core_machine_run(state.machine,
            ...
            state.machine->executor_cpu.data.sp != S7_KERNEL_STACK_TOP -
                (lib_u16)sizeof(frame) || state.machine->executor_cpu.data.eflags !=
                before.data.eflags || !s3_gate_gprs_same(&before,
                &state.machine->executor_cpu) || lib_memory_compare(&before.data.es,
                &state.machine->executor_cpu.data.es, sizeof(before.data.es)) != 0 ||
            lib_memory_compare(&before.data.ds, &state.machine->executor_cpu.data.ds,
                sizeof(before.data.ds)) != 0 || lib_memory_compare(&before.data.fs,
                &state.machine->executor_cpu.data.fs, sizeof(before.data.fs)) != 0 ||
            lib_memory_compare(&before.data.gs, &state.machine->executor_cpu.data.gs,
...
static lib_i32 s7_same_cpl(void)
{
    ...
    if (!failed) {
        ...
        failed |= !s7_install_call_gate(&state, 0u, S7_PARAMETER_COUNT,
            LIB_TRUE) || !s7_write_call(&state, S7_CALL_GATE_SELECTOR & 0xfffcu);
    }
    if (!failed) {
        before = test_core_machine_fixture_capture_cpu_after_run(state.machine);
        failed |= core_machine_run(state.machine,
...
static lib_i32 s7_reject(lib_u8 condition)
{
    ...
    if (!failed) {
        failed |= !s7_install_call_gate(&state,
            ...
            LIB_TRUE) || !s7_write_call(&state, S7_CALL_GATE_SELECTOR);
...
static lib_i32 s7_outer_preflight_priority(void)
{
    ...
    if (!failed) {
        failed |= !s7_install_call_gate(&state, 3u, 1u, LIB_TRUE) ||
...
static lib_i32 s7_irq_no_shadow(void)
{
    ...
    core_machine_run_result result;
    lib_u16 parameters[S7_PARAMETER_COUNT] = { 0x1234u,0xabcdu };
    lib_u16 call_frame[4u + S7_PARAMETER_COUNT] = { 0u };
    lib_u16 irq_frame[3u] = { 0u,0u,0u };
    ...
    if (!failed) {
        ...
        failed |= !s7_install_call_gate(&state, 3u, S7_PARAMETER_COUNT,
            LIB_TRUE) || !s3_gate_install(&state, S3_VECTOR, 0x0008u,
