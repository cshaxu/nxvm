    ...
    type_bool address_prefix)
{
    ...
    if (!s3_gate_prepare(state, profile, LIB_FALSE,
...
static C_INT s6_same_iret(C_VOID)
{
    ...
    if (!failed) {
        ...
        after.data.esp != 0x12348006u || after.data.eflags !=
            ...
            !s3_gate_gprs_same(&before, &after) || lib_memory_compare(&before.data.es,
...
static C_INT s6_same_retf(C_VOID)
{
    ...
    if (!failed) {
        ...
        failed |= after.data.eip != 0x0010u || !s6_code_cache(&after.data.cs,
            ...
            sizeof(before.data.es)) != 0 || lib_memory_compare(&before.data.ds,
...
static C_INT s6_outer_retf_immediate(C_VOID)
{
    ...
    if (!failed) {
        ...
        failed |= after.data.eip != 0x0010u || !s6_code_cache(&after.data.cs,
            ...
            lib_memory_compare(&before.data.gs, &after.data.gs,
            sizeof(before.data.gs)) != 0 || !s3_gate_read(&state, S3_STACK_TOP,
            frame_after, sizeof(frame_after)) || lib_memory_compare(frame, frame_after,
            sizeof(frame)) != 0;
    }
    core_machine_destroy(state.machine);
    return !failed;
    ...
    type_bool short_frame)
{
    ...
    if (!failed) {
        ...
        failed |= !s3_gate_cpu_same(&before, &after) || !s3_gate_read(&state,
            S3_STACK_TOP, frame_after, sizeof(frame_after)) || lib_memory_compare(
...
C_INT main(C_VOID)
{
    C_INT failed = !s6_same_iret() || !s6_same_retf() || !s6_outer_retf() ||
        ...
        !s6_success(CORE_MACHINE_CPU_PROFILE_80386, LIB_TRUE) ||
        !s6_failure(0u, 0x0023u, LIB_FALSE) ||
        !s6_failure(0x001bu, 0u, LIB_FALSE) ||
