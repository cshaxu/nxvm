...
int main(void)
{
    ...
    assert(common_session_queue_push_monitor_line(queue, &start, 0));
    assert(common_session_queue_push_broker_completed(queue, 1, 7u));
    assert(common_session_queue_push_kvm_for_run(queue, &raw_key, 7u));
    assert(common_session_queue_push_broker_completed(queue, 0, 7u));
    assert(common_session_queue_push_monitor_line(queue, &pause, 0));
    take(queue, &event); assert(event.kind == COMMON_SESSION_EVENT_MONITOR_LINE);
    take(queue, &event); assert(event.kind == COMMON_SESSION_EVENT_BROKER_COMPLETED &&
        event.value.broker_vm_console_current);
    take(queue, &event); assert(event.kind == COMMON_SESSION_EVENT_KVM_INPUT &&
    ...
    assert(common_session_queue_push_kvm_for_run(queue, &raw_key, 6u));
    take(queue, &event);
    assert(!common_session_accept_kvm_event(&event, 7u, COMMON_SESSION_MACHINE_RUNNING));
    ...
    assert(common_session_queue_push_kvm_for_run(queue, &retired, 7u));
    take(queue, &event); assert(common_session_accept_kvm_event(&event, 7u,
        COMMON_SESSION_MACHINE_RUNNING));
    assert(common_session_accept_kvm_event(&event, 8u, COMMON_SESSION_MACHINE_STOPPED));
    assert(common_session_dispatch_input(queue, &event.value.kvm,
        COMMON_SESSION_MACHINE_STOPPED, deliver_input, NULL));
    ...
    assert(common_session_dispatch_input(queue, &raw_mouse,
        COMMON_SESSION_MACHINE_PAUSED, deliver_input, NULL));
    ...
    assert(common_session_queue_push_kvm_delivery_failed(queue, 41u,
