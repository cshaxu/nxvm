...
C_INT main(C_VOID)
{
    ...
    if (vm_session_request_transport_dequeue_ingress(session->request_transport,
    ...
    if (vm_session_submit_host_input(session, &event) != TYPE_STATUS_OK) goto fail;
    while (vm_session_request_transport_dequeue_ingress(session->request_transport,
            &request) == TYPE_STATUS_OK) {
    ...
    if (vm_session_submit_host_input(session, &event) != TYPE_STATUS_INVALID_ARGUMENT ||
        vm_session_request_transport_dequeue_ingress(session->request_transport,
