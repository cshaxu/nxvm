...
C_INT main(C_VOID)
{
    ...
    failed |= vm_session_create(&invalid_config, &session) != TYPE_STATUS_INVALID_ARGUMENT ||
        session != STD_NULL;
    if (!failed) failed |= vm_model40_fixture_create_bytes("t386-s14-even.bin", even,
        "f7eb6af712d2cbc0ee03468e664d089fa67bfc48591b071c3bcaf8ab830653af",
        "t386-s14-odd.bin", odd,
    ...
    vm_model40_fixture_remove("t386-s14-even.bin", "t386-s14-odd.bin");
    return failed ? 1 : 0;
}
