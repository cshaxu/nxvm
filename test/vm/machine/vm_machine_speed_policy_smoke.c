...
static C_INT verify_wait_speed(vm_machine *session, vm_machine_speed speed)
{
    ...
    if (vm_machine_set_speed(session, speed) != TYPE_STATUS_OK ||
    ...
    if (vm_machine_waiting_advance(session, &waiting, &advanced) != TYPE_STATUS_OK ||
    ...
    if (vm_machine_set_speed(session, VM_MACHINE_SPEED_STANDARD) != TYPE_STATUS_INVALID_STATE) {
        vm_machine_executor_state_stop(session->control.state);
        return 1;
    }
    vm_machine_executor_state_stop(session->control.state);
...
int main(void)
{
    ...
    failed |= vm_machine_get_speed(default_session, &speed) != TYPE_STATUS_OK ||
