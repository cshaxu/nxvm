...
static C_VOID vm_model40_cmos_write(vm_machine *session, type_unsigned_8 index,
...
{
    STD_MEMSET(bytes, 0, VM_MACHINE_CMOS_SEED_BYTES);
...
static type_status vm_model40_cmos_seed_session_create(
    const type_unsigned_8 seed[VM_MACHINE_CMOS_SEED_BYTES], vm_machine **out_session)
{
    ...
    type_unsigned_8 default_seed[VM_MACHINE_CMOS_SEED_BYTES];
    ...
    assets.cmos_seed = (vm_machine_asset_bytes) { seed == STD_NULL ? default_seed : seed,
