...
C_INT main(C_VOID)
{
    ...
    if (
        ...
        STD_STRCMP(request.floppy_format, "720k") || !request.console_control ||
    ...
    if (!catalog_parse(model40, &request) || request.console_control ||
        request.bios_count != 2u || !catalog_parse(xt, &request) ||
        request.console_control || request.bios_count != 1u) {
