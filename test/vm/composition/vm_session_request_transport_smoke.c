...
C_INT main(C_VOID)
{
    vm_session_request_transport *transport = STD_NULL;
    vm_session_request request;
    vm_session_request copy;
    keyboard_event_observer observer = {0};
    STD_SIZE_T index;

    if (vm_session_request_transport_create(&transport) != TYPE_STATUS_OK) return 1;
    ...
    if (vm_session_request_transport_enqueue_ingress(transport,
                                                            &request) !=
        TYPE_STATUS_OK ||
        vm_session_request_transport_dequeue_ingress(transport,
                                                             &copy) !=
        TYPE_STATUS_OK ||
        copy.data.key_event.scan_code != 0x1eu || !copy.data.key_event.pressed) return 1;

    vm_session_request_transport_bind_consumer(
    ...
    if (vm_session_request_transport_enqueue_ingress(transport,
                                                            &request) !=
        TYPE_STATUS_OK) return 1;
    vm_session_request_transport_observe_execution_boundary(transport);
    vm_session_request_transport_observe_execution_boundary(transport);
    vm_session_request_transport_observe_execution_boundary(transport);
    if (observer.count != 3u ||
        observer.requests[0].kind != VM_SESSION_REQUEST_KEY_EVENT ||
        ...
        vm_session_request_transport_execution_boundary_count(transport) != 3u ||
        vm_session_request_transport_dequeue_ingress(transport, &copy) !=
    ...
    vm_session_request_transport_close(transport);
    if (vm_session_request_transport_enqueue_ingress(transport,
                                                            &request) !=
    ...
    if (vm_session_request_transport_dequeue_ingress(transport,
                                                            &copy) !=
        TYPE_STATUS_UNSUPPORTED) return 1;

    vm_session_request_transport_destroy(transport);
    return 0;
}
