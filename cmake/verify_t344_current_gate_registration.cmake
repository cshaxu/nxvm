...
if(NOT DEFINED PROJECT_T344_CURRENT_AUXILIARY_TESTS_FILE OR
        NOT EXISTS "${PROJECT_T344_CURRENT_AUXILIARY_TESTS_FILE}")
    message(FATAL_ERROR "T344 current-gate auxiliary-test inventory is required.")
endif()
...
file(STRINGS "${PROJECT_T344_CURRENT_AUXILIARY_TESTS_FILE}"
    project_t344_auxiliary_tests)
...
set(project_t344_seen_auxiliary_tests)
foreach(project_t344_auxiliary_test IN LISTS project_t344_auxiliary_tests)
    list(FIND project_t344_targets "${project_t344_auxiliary_test}"
        project_t344_auxiliary_target_index)
    if(NOT project_t344_auxiliary_target_index EQUAL -1)
        message(FATAL_ERROR "T344 current-gate auxiliary test duplicates a target: ${project_t344_auxiliary_test}")
    endif()
    list(FIND project_t344_seen_auxiliary_tests "${project_t344_auxiliary_test}"
        project_t344_auxiliary_seen_index)
    if(NOT project_t344_auxiliary_seen_index EQUAL -1)
        message(FATAL_ERROR "T344 current-gate auxiliary test inventory is duplicated: ${project_t344_auxiliary_test}")
    endif()
    list(APPEND project_t344_seen_auxiliary_tests "${project_t344_auxiliary_test}")
endforeach()
...
endforeach()
foreach(project_t344_auxiliary_test IN LISTS project_t344_auxiliary_tests)
    list(FIND project_t344_registered_targets "${project_t344_auxiliary_test}"
        project_t344_auxiliary_registration_index)
    if(project_t344_auxiliary_registration_index EQUAL -1)
        message(FATAL_ERROR "T344 current-gate auxiliary test lacks CTest registration: ${project_t344_auxiliary_test}")
    endif()
    list(FIND project_t344_labeled_targets "${project_t344_auxiliary_test}"
        project_t344_auxiliary_label_index)
    if(project_t344_auxiliary_label_index EQUAL -1)
        message(FATAL_ERROR "T344 current-gate auxiliary test lacks current-gate label: ${project_t344_auxiliary_test}")
    endif()
endforeach()
foreach(project_t344_target IN LISTS project_t344_registered_targets)
    list(FIND project_t344_targets "${project_t344_target}"
        project_t344_target_index)
    list(FIND project_t344_auxiliary_tests "${project_t344_target}"
        project_t344_auxiliary_index)
    if(project_t344_target_index EQUAL -1 AND project_t344_auxiliary_index EQUAL -1)
        message(FATAL_ERROR
...
list(LENGTH project_t344_auxiliary_tests project_t344_auxiliary_count)
...
math(EXPR project_t344_expected_count
    "${project_t344_target_count} + ${project_t344_auxiliary_count}")
if(NOT project_t344_expected_count EQUAL project_t344_registration_count OR
        NOT project_t344_expected_count EQUAL project_t344_label_count)
