...
set(cmos "${PROJECT_SOURCE_DIR}/src/vm/machine/cmos.c")
...
foreach(source IN ITEMS "${cmos}" "${qdcga}" "${machine_devices}" "${cmake_source}")
...
file(READ "${cmos}" cmos_source)
foreach(forbidden IN ITEMS "t_cpu" "flagMaskNMI" "connect.cpu")
    string(FIND "${cmos_source}" "${forbidden}" position)
    if(NOT position EQUAL -1)
        message(FATAL_ERROR "T264 CMOS retains raw CPU ownership: ${forbidden}")
    endif()
endforeach()
string(FIND "${cmos_source}" "set_nmi_mask" position)
if(position EQUAL -1)
    message(FATAL_ERROR "T264 CMOS has no narrow NMI-mask boundary")
endif()
