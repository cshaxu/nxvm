...
set(machine_board "${PROJECT_SOURCE_DIR}/src/core/machine/machine_board.c")
...
foreach(source IN ITEMS "${rtc}" "${machine}" "${machine_board}" "${qdcga}" "${machine_devices}" "${cmake_source}")
...
file(READ "${machine_board}" machine_board_source)
...
foreach(required IN ITEMS "core_machine_set_nmi_mask"
    ...
    string(FIND "${machine_board_source}" "${required}" position)
    if(position EQUAL -1)
