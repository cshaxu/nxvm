param(
    [string]$ManifestPath = "docs/nxvm/etc/cpu-timing/t435-s2-80186-timing-manifest.json",
    [string]$InventoryPath = "docs/nxvm/etc/cpu-timing/t435-s6-80186-decoder-inventory.json",
    [string]$LedgerPath = "docs/nxvm/etc/cpu-timing/t435-s1-80186-ledger.md"
)
...
if ($canonical.Count -ne 616) { throw "Canonical S2 key count mismatch: $($canonical.Count)" }
...
if ([int]$statusCounts.conforming -ne 0 -or [int]$statusCounts.'wrong-value' -ne 0 -or [int]$statusCounts.unallocated -ne 0 -or [int]$statusCounts.'missing-test' -ne 279) { throw "S9 base status counts are not reconciled: conforming=$($statusCounts.conforming) wrong=$($statusCounts.'wrong-value') unallocated=$($statusCounts.unallocated) missing-test=$($statusCounts.'missing-test')" }
...
"M5:T435:S6:I186-S2-CANONICAL-STATUS-RECONCILED:PASS:616:$($canonicalStatusCounts.'wrong-value'):$($canonicalStatusCounts.unallocated):$($canonicalStatusCounts.'missing-input'):$($canonicalStatusCounts.'missing-test')"
