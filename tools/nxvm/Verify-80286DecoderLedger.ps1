param(
    [string]$ManifestPath = "docs/nxvm/etc/cpu-timing/t435-s2-80286-timing-manifest.json",
    [string]$InventoryPath = "docs/nxvm/etc/cpu-timing/t435-s7-80286-decoder-inventory.json",
    [string]$LedgerPath = "docs/nxvm/etc/cpu-timing/t435-s1-80286-ledger.md"
)
...
$expectedCanonical = $manifest.expected.base + $manifest.expected.contexts + $manifest.expected.combinations
if($canonical.Count -ne $expectedCanonical){throw "Canonical key mismatch: actual=$($canonical.Count) expected=$expectedCanonical"};foreach($entry in $canonical.Values){if([string]::IsNullOrWhiteSpace([string]$entry.status)-or [string]::IsNullOrWhiteSpace([string]$entry.route)){throw 'Incomplete canonical disposition'}}
$counts=@{'missing-test'=0;'missing-input'=0;unallocated=0};foreach($entry in $base){if(-not $counts.ContainsKey($entry.status)){$counts[$entry.status]=0};++$counts[$entry.status]};if($counts.'missing-test' -ne 239 -or $counts.'missing-input' -ne 47 -or $counts.unallocated -ne 0){throw "Unexpected base status disposition"}
