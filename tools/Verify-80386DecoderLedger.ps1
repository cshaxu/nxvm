...
function Recipe($key,$a){
  ...
  elseif($key -match '^I386-IMUL[23]-'){$r.opcode=if($a.op -eq 'IMUL2'){0x0f}else{0x69};if($a.op -eq 'IMUL2'){$r.escaped=0xaf;$r.opcode=$null};$r.modrm=if($a.form -match '^R'){0xc0}else{0}}
  ...
  elseif($key -match '^I386-MOV-'){$f=$a.form;$r.opcode=@{'RR'=0x8a;'RM'=0x8a;'MR'=0x88;'RI'=0xb0;'MI'=0xc6;'MOFFS-R'=0xa0;'MOFFS-W'=0xa2;SREG=0x8c;MOVSX=0x0f;MOVZX=0x0f;LEA=0x8d;LDS=0xc5;LES=0xc4;LFS=0x0f;LGS=0x0f;LSS=0x0f}[$f];if($f -in 'MOVSX','MOVZX','LFS','LGS','LSS'){$r.escaped=@{MOVSX=0xbe;MOVZX=0xb6;LFS=0xb4;LGS=0xb5;LSS=0xb2}[$f];$r.opcode=$null};if($f -match '^(R32-CR|CR|R32-DR|DR|R32-TR|TR)'){$r.escaped=if($f -match 'CR'){if($f -match '^R'){0x20}else{0x22}}elseif($f -match 'DR'){if($f -match '^R'){0x21}else{0x23}}elseif($f -match '^R'){0x24}else{0x26};$r.opcode=$null;$r.modrm=if($f -match 'CR2'){0xd0}elseif($f -match 'CR3'){0xd8}elseif($f -match 'DR6|TR6'){0xf0}else{0xc0}}elseif($f -in 'RM','MR','MI','LEA','LDS','LES','LFS','LGS','LSS'){$r.modrm=0}}
...
if($base.Count -ne 454 -or ($base.key_id|Sort-Object -Unique).Count -ne 454){throw 'Base key mismatch'}
...
$c=@{};foreach($x in $base){$c[$x.key_id]=$true};foreach($s in $m.context_key_sets){$suffix=@($s.id_suffix);if($null -ne $s.phase){$suffix=@($s.phase|ForEach-Object{$s.id_suffix.Replace('{phase}',[string]$_)})};if($null -ne $s.kind){$suffix=@($s.kind|ForEach-Object{$s.id_suffix.Replace('{kind}',[string]$_)})};$rx=[regex]$s.base_selector;foreach($x in $base){if($rx.IsMatch($x.key_id)){foreach($q in $suffix){$c["$($x.key_id)-$q"]=$true}}}}
if($c.Count -ne 1431){throw "Canonical mismatch $($c.Count)"}
$status=@{};foreach($t in $m.base_templates){foreach($x in Expand-Template $t){$status[$t.status]=1+$status[$t.status]}};if($status['missing-test'] -ne 261 -or $status['missing-input'] -ne 192 -or $status.unallocated -ne 1){throw 'Base status accounting drifted'}
