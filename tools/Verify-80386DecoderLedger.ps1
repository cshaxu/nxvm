...
if($base.Count -ne 450 -or ($base.key_id|Sort-Object -Unique).Count -ne 450){throw 'Base key mismatch'}
...
if($c.Count -ne 1410){throw "Canonical mismatch $($c.Count)"}
$canonicalStatus=$c.Values|Group-Object -AsHashTable -AsString;if($canonicalStatus['missing-test'].Count -ne 338 -or $canonicalStatus['missing-input'].Count -ne 1052 -or $canonicalStatus.unallocated.Count -ne 20){throw 'Canonical status accounting drifted'}
$status=@{};foreach($t in $m.base_templates){foreach($x in Expand-Template $t){$status[$t.status]=1+$status[$t.status]}};if($status['missing-test'] -ne 257 -or $status['missing-input'] -ne 192 -or $status.unallocated -ne 1){throw 'Base status accounting drifted'}
