...
if($c.Count -ne 1411){throw "Canonical mismatch $($c.Count)"}
$canonicalStatus=$c.Values|Group-Object -AsHashTable -AsString;if($canonicalStatus['missing-test'].Count -ne 338 -or $canonicalStatus['missing-input'].Count -ne 1053 -or $canonicalStatus.unallocated.Count -ne 20){throw 'Canonical status accounting drifted'}
