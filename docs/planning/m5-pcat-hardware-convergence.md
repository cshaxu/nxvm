...
| T243 | T242 baseline; owner-approved core-boundary work order | **Complete.** Checked physical read/write/query shares one frozen provider/RAM route resolver. Query admits only READ/WRITE and returns only ordinary-RAM/provider classification without invoking data callbacks. |
| T244 | T243 | **Complete.** Registered 1--15 byte undefined-instruction transition patterns use a real-mode physical fetch window. Registration is `INITIALIZED` only, rejects identical/prefix conflicts, freezes with providers, and atomically applies only the specified copied-state patch; NXVM registers none. The 34-check/82-smoke GCC gate and `0.5.0244` artifact passed. |
| T245 | T243 | **Complete.** Immutable byte-image provider uses the frozen checked device-memory route, rejects overlap, reads/fetches normally, faults writes, and preserves image bytes across reset. Firmware generation and image contents remain VM profile policy. The 34-check/83-smoke GCC gate and `0.5.0245` artifact passed. |
| T246 | T243, T245 | **Complete.** Two-phase validated entry plan: topology/image registration only in `INITIALIZED`; then T243 prechecks every preload as writable ordinary RAM before atomically applying real-mode state and copies at a clean `STOPPED` reset boundary. Any validation failure preserves reset CPU/RAM/mapping state. The 34-check/84-smoke GCC gate and `0.5.0246` artifact passed. |
| T247 | T246 | **Complete.** Retained Win32 E0 scan-code markers, bounded Console/Linux execution-flip waits with cleanup, rejected overlapping entry-plan preloads, and made the current GCC artifact use `-Werror`. Focused no-flip/entry/input probes and 85/85 current CTest cases passed; see [T247](m5-t247-correctness-closure.md). |
| T248 | T246 | Narrow shared host-capability design/admission after the shared machine-start boundary is validated; each provider requires policy-free reuse evidence. Before T246, evidence collection only: no platform contract or provider implementation. |
| T249 | T213, T233 | Extend ATA/IDE only through a declared feature matrix and failing corpus: selected channel/device topology, timing/error behavior, or other admitted PIO features. IDE DMA, ATAPI, LBA48, and cache remain separate admissions. |
| T250 | T219, T220, T228 | Complete digital CGA through bounded `640x200x2`, remaining text modes, and 6845-visible start/page/cursor/scanline behavior. Composite artifact color remains an optional renderer/profile capability. |
| T251 | M5 profile boundary | Define machine-profile admission: immutable topology/capability declarations, optional user-provided ROM manifests, clock/port/IRQ/DMA/media contracts, fixture corpus, and provenance policy before PC110, DeskPro, Award, or Phoenix profiles. |
| T252 | T217--T225 | Admit the next timing-fidelity level only when a real corpus requires it: profile-bound rational frequency/phase/rounding/event ordering for CPU, PIT, CGA, DMA, and RTC. Instruction, bus, and cycle timing remain separate work. |
| T253 | Adequate real-mode device baseline | Add 286 descriptors, exceptions, and protected-mode control transfer with instruction probes and bounded differential evidence. |
| T254 | T253 | Add 386 paging, CRx, CPL/IOPL, and TSS I/O map with focused probes and bounded differential evidence. |
| T255 | T254 | Add task switching and remaining admitted 286/386 instruction families. |
| T256 | T253--T255 as applicable | Add present FPU state, operations, exceptions, and `FWAIT`. Existing FPU-none ESC consumption is not present-FPU support. |
