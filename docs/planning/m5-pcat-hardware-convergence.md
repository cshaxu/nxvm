...
| T241 | T229, T226, T227 | **Complete.** Owner-built `MOUSE241.COM` boots through DOS, installs IVT `74h`, consumes AUX initialization and one mapped packet through IRQ12, and publishes a guest application result. Exact packet bytes remain locked by the T229 port probe; host input never writes DOS APIs or guest memory. 78/78 current CTest cases passed; see [T241 record](m5-t241-guest-mouse-driver.md). |
