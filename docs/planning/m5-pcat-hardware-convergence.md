...
| T243 | T242 baseline; owner-approved core-boundary work order | **S1 complete; S2 active.** Define and implement checked physical read/write/query: zero length invalid, overflow/out-of-route fault, and query over the same frozen provider/RAM route using only READ/WRITE access plus neutral ordinary-RAM/provider classification. This owner-prioritized boundary task runs before the deferred hardware corpus; it must preserve the full T242 NXVM baseline. |
| T244 | T243 | Registered 1--15 byte undefined-instruction transition patterns over core-captured physical fetch bytes. Register only while `INITIALIZED`, reject identical/prefix conflicts, freeze with providers, and atomically validate/commit only the specified copied-state patch; no default NXVM registration. |
| T245 | T243 | Immutable byte-image provider on the existing frozen checked device-memory route; reject overlap at registration, use normal read/fetch routing, fault writes, and preserve image bytes across reset. Firmware generation and image contents remain VM profile policy. |
