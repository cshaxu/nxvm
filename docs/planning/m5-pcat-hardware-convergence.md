...
| T245 | T243 | **S1 active.** Immutable byte-image provider on the existing frozen checked device-memory route; reject overlap at registration, use normal read/fetch routing, fault writes, and preserve image bytes across reset. Firmware generation and image contents remain VM profile policy. |
