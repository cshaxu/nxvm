...
**M5 T267 S2 active: Standard PS/2 AUX Configuration And Status Protocol.**
...
| --- | --- | --- |
| AUX defaults | `core/machine/kbc` owns per-machine stream mode, reporting disabled, scaling 1:1, resolution `2`, sample rate `100`, device ID `00h`, button state, and no pending AUX parameter. Accepted `F6h`/`FFh` commands and cold reset restore all defaults; a pending parameter is always cleared when it is consumed or by cold reset. | Core port probe observes defaults and reset. Stop if another module must own AUX configuration. |
...
| IRQ / serialization | AUX responses and stream packets are serialized by the existing KBC FIFO. A delayed reply reserves its full sequence; once admitted, an `E9h` reply drains contiguously before any subsequently queued packet. Every byte gets OBF/AUX and IRQ12, and `60h` read plus PIC EOI allows the successor edge. | Probe covers every byte, IRQ12, EOI/deassert, and reply-before-packet ordering. |
...
**S1 complete:** `core-machine-kbc-aux-port-smoke` now emits
`M5:T267:S1:AUX:PORT:OK` after covering defaults, `F3h`/`E8h` valid and invalid
parameters, `E9h` byte order, AUX/OBF/IRQ12 lifecycle, retained button state,
and `F6h` reset. The probe exposed that `D4h` routing state cannot also own an
AUX parameter wait; S2 separates controller routing from the per-AUX pending
parameter, without adding another device or queue.

**S2 implementation scope:** only `src/core/machine/kbc.*` and the core AUX
port probe. Static ownership gates and the real DOS guest fixture are S3
requirements. Stop for any VM/platform/firmware AUX state or host shortcut.
