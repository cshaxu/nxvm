...
| T146 | Implement the generic `core_product_session_manager`; bind the VM composition provider, create/select session `0`, and replace the old target while retaining all old commands for session `0`. | Existing Console lifecycle and boot gates; `SESSION LIST` proves `* 0`; no global/TLS/current selector. |
