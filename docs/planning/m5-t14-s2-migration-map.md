...
| `core/platform/presentation.h -> core/machine/{presentation,status}.h` | Remove the core-platform text-snapshot wrapper. A product machine owns its private snapshot and may embed core text; its product platform owns an independent copied frame. Only product root composition may include both and translate snapshot to frame at the execution boundary. `core/platform` must not expose a typed text snapshot or include `core/machine`. | `core-platform`, `vm-composition`, `vdm-composition` | Core presentation + VDM shell + injected platform-sink spy. Stop if copied text/frame fields or generation timing change, or if platform code requires a machine include. |
...
| `nxvm-machine-core-executor` | Split its current mixed core and VM source list into `core-machine` and `vm-machine`; `vvadp.c`, `vcmos.c`, and `vdebug.c` must not hide in a core target. |
