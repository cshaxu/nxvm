...
| VM product narrow targets | In progress: all pure `vm/product/*.c` sources now compile in `nxvm-vm-product`; historical product smoke executables link that module target directly. Composition/session/profile source targets still require the following consolidation slice. |
