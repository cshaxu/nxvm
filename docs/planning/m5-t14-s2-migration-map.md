...
| `nxvm-core-machine` | Complete for the retained build: it compiles only `core/machine` sources and carries the final core-machine target identity. |
...
| VM product/profile narrow targets | In progress: all pure `vm/product/*.c` sources compile in `nxvm-vm-product`, while profile metadata and default-profile firmware compile in `nxvm-vm-profile`; historical product/profile smoke executables link those module targets directly. Composition/session source targets still require the following consolidation slice. |
