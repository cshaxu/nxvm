...
| `vm/product/debug.c -> vm/machine/device.h` | Move the shared debugger command language and presentation to `core/product/debug` only after it uses a core-owned typed debug target. VM and VDM composition bind their machines; product forms add only capability-specific commands. Console is complete: it uses its product command target and has no machine/platform include. | `core-product`, `vm-composition`, `vdm-composition` | Console + debugger + VM composition. Stop for any changed prompt, command acceptance, media behavior, display toggle, debug entry behavior, or debugger text. |
