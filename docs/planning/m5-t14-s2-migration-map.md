...
| `vm/platform/linux/linuxcon.c -> vm/machine/device.h` | Linux retains a temporary machine adapter for its legacy lifecycle and keyboard ingress. Windows uses copied display frames plus composition-bound input and execution sinks; its renderers and thread loops do not include VM machine state. | `vm-platform`, `vm-composition` | Transport + retained Console/debugger. Stop if key mapping, F9 stop, window close, display paint, or input timing changes. |
| `vm/product/debug.c -> vm/machine/device.h` | Keep exact debugger grammar and presentation in `vm/product`, but route machine effects through an abstract VM target. Console is complete: it uses its product command target and has no machine/platform include. | `vm-product`, `vm-composition` | Console + debugger + VM composition. Stop for any changed prompt, command acceptance, media behavior, display toggle, or debug entry behavior. |
