...
| EGA output disabled | IBM EGA Attribute Controller output-enable state is a guest-visible display gate: Manual L3. | Attribute address phase changes the sole VADP output state; a recognized planar geometry emits a same-geometry black snapshot, while the retained text fallback emits blank text. | 86Box stores Attribute bit 5 and selects `ega_render_blank` unless palette output is enabled; Bochs also gates its generic VGA display on Attribute video enable. NXVM previously returned no snapshot, leaving a stale presentation; this S repairs both selected routes in VADP. |
...
2. An EGA output-disable write could leave the VM with the prior nonblank
   frame. VADP now returns the source-known planar geometry as a zeroed
   snapshot, or its retained blank-text fallback where no planar geometry is
   admitted, and marks the existing dirty generation when Attribute display
   enable changes. VM's existing copy path therefore publishes the blank frame
   without any VM workaround or second truth.

The complete S10 implementation delta is 51 added and 7 removed source/test
lines (before this evidence and state record). It retains one VADP state owner, one memory
...
and Model-40 CECG mapping. The mode-10 contract now proves a rendered frame,
a display-disable planar transition with `buffer_changed` and black pixels,
and the no-geometry blank-text fallback.
