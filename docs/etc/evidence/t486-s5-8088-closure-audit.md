...
`M5:T486:S5:8088-CLOSURE-AUDIT:ACCEPTED`
...
| Group-3 MUL/IMUL/DIV/IDIV ranges | Intel ranges are retained as Manual-L3 source facts, but no exact runtime value is selected. | Source-unallocated/L2 final disposition in T486; a later independently admitted CPU task may only refine it with a source-qualified selection rule. |
