...
| --- | --- |
| L3 | A direct manual fact, a separately labelled external-model fact, or an existing external/board input copied to the sole Core owner.  An input receiver is L3 even when a present profile selects an estimated value. |
| L2 | Only an internal proportional or fixed numerical estimate remains.  The estimate is L2; its existing input receiver is not. |
...
| L0 | No Core-owned logical order is defined.  Electrical waveform/AC/power/monitor facts with no discrete Core consumer are L0 exclusions, not hidden timing debt. |
...
L2.  No L4 category is used by this audit.
...
| PIC-R5, PIC-F3, PIC-F6, PIC-T3 | L0 electrical exclusion | Bus, INTA and CAS waveform facts have no Core discrete consumer. |
...
| DMA-R2, DMA-F4 waveform part | L0 electrical exclusion | Program/bus pulse values have no discrete owner input. |
| DMA-R3--R4, DMA-F3 semantic part, DMA-T1--T2 | L3 only | Direct register, mode and AT topology facts. |
| DMA-F1, DMA-F5, DMA-T4 digital DREQ/DACK/EOP part | Input L3 | Existing DMA/device and FDC bindings deliver the named digital event to the sole DMA owner; pulse width remains L0. |
...
| PIT-R2, PIT-R4, PIT-F1, PIT-F8, PIT-T4 | L0 electrical exclusion | Bus/edge/range/AC values have no Core discrete consumer. |
...
| PIT-T3 IRQ/refresh output relation | Input L3 | PIT output reaches the already-owned PIC/DMA digital endpoints.  Host speaker presentation is outside guest controller time (L0), not L2. |
...
| RTC-R4, RTC-F6, RTC-T4 | L0 electrical exclusion | Bus/oscillator/reset pulse limits have no discrete Core consumer. |
...
| KBC-R3, KBC-F5 | L0 pulse duration; L1 delivery order | Command/reset/A20 effects are Manual L3.  The 6-us waveform has no duration receiver; downstream reset/A20 order is L1. |
...
| KBC-F7 | L0 electrical exclusion | UPI oscillator, bus and voltage tables are not a Core logical schedule. |
...
| FDC-R4 execution duration, FDC-F3 deadline conversion | L2 | `CORE_MACHINE_FDC_SEEK_TRACK_TICKS` is the existing internal fixed estimate.  No FDC construction-time timing receiver exists. |
| FDC-F2 DMA/IRQ route | Input L3 | FDC uses the existing DMA-request binding and PIC endpoint; ISA service duration remains L1. |
| FDC-F3 AC/electrical tables, FDC-R5 later-device extensions | L0 | Neither has a selected Core logical consumer or current receiver. |
...
| VADP-F5 monitor/composite/host presentation, VADP-T3 monitor electrical model | L0 | These physical/host presentation facts have no Core video-state consumer. |
...
| CGA-T4 | L0 physical presentation | Named digital interface facts remain Manual L3; waveform/phosphor/composite and host presentation lack a Core logical consumer. |
...
| ATA-F1, ATA-F5 | L0 electrical exclusion | Cable/controller electrical tables have no Core discrete consumer. |
...
3. Every residual that has only causal order is L1; every waveform or physical
   fact with no discrete Core consumer is L0.

The audit also finds one unrelated code-quality defect while inspecting the
RTC construction path: `src/vm/composition/session/session.c` contains a
duplicated `rtc_clock` conditional at lines 384--396.  It is behaviourally
idempotent, does not change this classification, and needs a separately
admitted minimal cleanup rather than being hidden in this documentation task.
