...
| --- | --- | --- | --- | --- | --- |
| D1 | The selected configuration has two 5.25-inch 1.2-MB mechanisms, with media mounted only in A:. | The available original material establishes the 1.2-MB option but not the fitted-drive count. | PCjs's selected DeskPro configuration declares two 1.2-MB types and mounts only A:. | External-L2 configuration fact | Both mechanical selects are wired; only A: has a media binding.  This is not a manual-L3 electrical READY assertion. |
...
| D5 | FDC drive selection and ST3 READY/TRACK-0 are controller-owned observations. | Retained uPD765/8272A List 1; this board source does not override it. | PCjs has an empty but fitted B: type; generic FDCs remain distinct implementations. | L3 chip relation | Media absence is distinct from fitted-drive selection; no synthetic media or board READY override exists. |
...
| D11 | Exact POST test order and the electrical relation from FDC/drive-change to `601` are not specified by available original material. | Explicit primary-source gap. | External implementations disagree in machine scope and cannot close it. | Source-gated | No board READY value, timing value, or F1 shortcut may be invented. |
