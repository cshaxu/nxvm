...
| Interrupts and time | T346 supplies one allocation-free due-event timeline ordered by `(due_tick, sequence)`, copied CPU/port/external-memory checkpoints, and reset-safe same-tick PC/AT ordering `DMA -> PIT -> PIC -> FDC -> HDC -> RTC -> KBC -> VADP`; T347 closes the selected FDC/ATA pending-service readiness lifecycle on that timeline. | This is deterministic L3 event-and-bus ordering, not cycle-exact execution: instruction costs, general wait states, unselected controller-service durations, and pin/prefetch timing remain separately deferred. |
...
| Storage | Bounded ATA PIO and FDD boot paths through declared ROM/device owners; ATA `nIEN` IRQ14 behavior; neutral media/controller mechanisms in core with VM backing/topology policy; core-owned 8237 DMA through frozen FDC binding; T347 closes selected FDC/ATA pending command/data readiness and T354 verifies its deterministic visibility. | Generic bus wait states, broad DMA behavior, extended IDE, unselected controller behavior, and error/timing compatibility remain deferred admissions. |
