...
| Register/memory/immediate data movement: `MOV`, `XCHG`, `LEA`, `LES/LDS`, moffs, `PUSH/POP`, `PUSHA/POPA`, `PUSH imm`, `ENTER/LEAVE`, `CBW/CWD` (including 32-bit attribute variants) | Primary table routes to `MOV_*`, `XCHG_*`, `LEA_R32_M32`, `LES_R32_M16_32`/`LDS_R32_M16_32`, stack helpers `_kec_push/_kec_pop`, and `ENTER/LEAVE`; primary defaults do not by themselves certify 32-bit forms. | T302 stack/address focused probe; T301 prefix/profile baseline; S25 LES/LDS, S26 LEA, `core_machine_xchg_smoke` (`M5:T316:S27:XCHG:OK`, `M5:T316:S28:XCHG-ACC:OK`), and `core_machine_sign_extend_smoke` (`M5:T316:S29:SIGN-EXTEND:OK`). | **Partial**: S25 completes LES/LDS, S26 LEA, S27 XCHG `86`/`87` r/m forms, S28 accumulator XCHG `90`--`97`, and S29 CBW/CWD `98`/`99` default plus 80386 `66h` forms. MOV/moffs, stack breadth, and general segment-load families remain partial; this does not claim the wider data-movement family complete. |
