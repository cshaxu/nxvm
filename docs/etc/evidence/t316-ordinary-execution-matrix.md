...
| Register/memory/immediate data movement: `MOV`, `XCHG`, `LEA`, `LES/LDS`, moffs, `PUSH/POP`, `PUSHA/POPA`, `PUSH imm`, `ENTER/LEAVE`, `CBW/CWD` (including 32-bit attribute variants) | Primary table routes to `MOV_*`, `XCHG_*`, `LEA_R32_M32`, `LES_R32_M16_32`/`LDS_R32_M16_32`, stack helpers `_kec_push/_kec_pop`, and `ENTER/LEAVE`; primary defaults do not by themselves certify 32-bit forms. | T302 stack/address focused probe; T301 prefix/profile baseline; S25 LES/LDS, S26 LEA, `core_machine_xchg_smoke` (`M5:T316:S27:XCHG:OK`, `M5:T316:S28:XCHG-ACC:OK`), `core_machine_sign_extend_smoke` (`M5:T316:S29:SIGN-EXTEND:OK`), `core_machine_moffs_smoke` (`M5:T316:S30:MOFFS:OK`), `core_machine_gpr_mov_smoke` (`M5:T316:S31:GPR-MOV:OK`), `core_machine_sreg_mov_smoke` (`M5:T316:S32:SREG-MOV:OK`), `core_machine_movs_smoke` (`M5:T316:S33:MOVS:OK`), `core_machine_stos_smoke` (`M5:T316:S34:STOS:OK`), and `core_machine_lods_smoke` (`M5:T316:S35:LODS:OK`). | **Partial**: S25 completes LES/LDS, S26 LEA, S27 XCHG `86`/`87` r/m forms, S28 accumulator XCHG `90`--`97`, S29 CBW/CWD `98`/`99` default plus 80386 `66h` forms, S30 only bounded moffs MOV `A0`--`A3`, S31 primary non-segment GPR MOV `88`/`89`/`8A`/`8B`, `C6`/`C7`, and `B0`--`BF`, S32 only segment-register MOV `8C`/`8E`, S33 only MOVS `A4`/`A5`, S34 only STOS `AA`/`AB`, and S35 only LODS `AC`/`AD`. S35 is not a wider string-family claim: CMPS, SCAS, INS/OUTS, broader strings, and stack breadth remain partial or outside this slice. |
...
| EFLAGS transfers and direct flag control: `PUSHF/POPF`, `LAHF/SAHF`, `CMC/CLC/STC/CLI/STI/CLD/STD` | `PUSHF`, `POPF`, `SAHF`, `LAHF`, and primary flag handlers; protected-mode masking is local to those routes. | T302 checks selected PUSHF/POPF; `core_machine_lahf_sahf_smoke` (`M5:T316:S39:LAHF-SAHF:OK`) proves LAHF/SAHF; retained string/product tests incidentally use DF/IF paths. | **Partial**: S39 closes only LAHF `9F` and SAHF `9E`: all profiles, fixed `66`/`67` prefix classification, LOCK rejection, protected and VM86 fixture-expressible preservation, and PIC no-shadow. PUSHF/POPF, CMC/CLC/STC/CLI/STI/CLD/STD, broader privilege architecture, and other FLAGS/control behavior remain partial or outside this slice. |
