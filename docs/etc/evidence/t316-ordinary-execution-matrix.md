...
| Decimal/ASCII adjust and conversion: `DAA/DAS/AAA/AAS/AAM/AAD`, `XLAT` | Primary handlers `DAA`, `DAS`, `AAA`, `AAS`, `AAM`, `AAD`, `XLAT`; DAA/DAS/AAM/AAD call `_kaf_set_flags`. | `core_machine_inc_dec_smoke` (`M5:T316:S16:DECIMAL-ADJUST:OK`) proves the admitted adjust slice. | **Complete only for T316 S16's six named adjust forms**: DAA/DAS AL+CF/AF boundaries, AAA/AAS AL/AH with only AF/CF asserted, AAM/AAD nondefault base, and AAM `00h` #DE atomicity/profile behavior. `XLAT` and the wider ordinary arithmetic/FLAGS family remain **Partial**. |
