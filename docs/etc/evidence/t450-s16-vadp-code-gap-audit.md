...
| VADP-F2 | Core captures current CGA and four-plane snapshots, but chooses 320x200 or 640x350 solely from CRTC Offset 13h. That cannot distinguish IBM mode E (640x200x16) from mode 10h. The current generated BIOS advertises mode 10h in BDA, while its focused VM replay never observes the expected VADP geometry/plane frame; reverting the rejected geometry experiment reproduces the same failure, so it is a baseline gap rather than a new regression. | Direct CRTC/mode-10 and CGA smokes pass; `vm-ega-mode10-boot-smoke` fails reproducibly with `mode10=5010` and no observed frame. | Manual L3 for D/E/10/F layout facts. One next VADP S must repair the existing generated-firmware port route and make the sole VADP snapshot derive D/E/10 only from complete controller state; no BDA read, second renderer, second VRAM route or host-mode selection is eligible. F chained odd/even and alpha character generation remain explicit later batches. |
