...
| VADP-F2 | The generic BIOS now performs its real `3C2` color-CRTC selection before boot. One VADP output predicate keeps the reset/test-condition boundary distinct from the A000 planar-map predicate: the latter selects four-plane capture, while a non-planar EGA map returns through the existing text capture rather than suppressing all output. Mode 10's existing CRTC Offset `28h` now reaches the 640x350 frame and returns through mode 03 text. Snapshot geometry still chooses 320x200 or 640x350 solely from Offset `13h`, so it cannot distinguish IBM mode E (640x200x16) from mode 10h. | External-port, planar-port, direct mode-10, `vm-ega-planar-system-smoke`, `vm-ega-mode10-boot-smoke` and `vm-rom-ega-int10-system-smoke` pass. | Manual L3 for D/E/10/F layout facts and the completed selected mode-10 route. One later VADP S must derive D/E/10 only from complete controller state; no BDA read, second renderer, second VRAM route or host-mode selection is eligible. F chained odd/even and alpha character generation remain explicit later batches. |
