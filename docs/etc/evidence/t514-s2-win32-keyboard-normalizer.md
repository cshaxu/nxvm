...
The Core-platform Win32 capability is the single recovery boundary for
character-only and virtual-key-only host packets.  It converts host-layout
Unicode characters to a complete physical make/break sequence, resolves a
missing scan from a virtual key, and validates UTF-16 pairs without writing
guest text, BIOS state, or KBC state directly.
