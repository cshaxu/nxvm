...
| Task, LDT, debug, and VM86 breadth | Task/LDT state, remaining VM86 instructions, debug/test-register behavior, and their transfer/fault boundaries compose. | Partial: T329 S1 closes bounded 16-bit-TSS direct far JMP. T329 S2 closes the null-LDTR/paging-off 32-bit direct-JMP state-machine base. Active T329 S3 now mechanically binds the full 32-bit direct image and installed-handler fault ordering; it does not close task gates/CALL/NT, non-null LDT, task paging, or debug breadth. | [T329 S1 direct-TSS evidence](t329-s1-tss16-direct-jump.md), [S2 evidence](t329-s2-tss32-direct-jump.md), and [T329 task-switch state-machine record](t329-task-switch-state-machine.md); S4 transfers task gates/CALL/NT entry, S5 nested-task return/fault chains, S6 non-null LDT, and S7 task paging/debug composition. |
