...
| Task, LDT, debug, and VM86 breadth | Task/LDT state, remaining VM86 instructions, debug/test-register behavior, and their transfer/fault boundaries compose. | Partial: T329 S1 now proves the bounded 80286/80386 protected direct far-JMP transition between valid 16-bit TSS images, including state/busy/TS publication, attributes, rejection, and the local pending-IRQ boundary. | [T329 S1 direct-TSS evidence](t329-s1-tss16-direct-jump.md); the ordered task/LDT/debug/VM86 package retains 32-bit TSS, task gates/CALL/NT, non-null LDT task state, task paging, and broader VM86/debug behavior. |
