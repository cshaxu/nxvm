...
| Task, LDT, debug, and VM86 breadth | Task/LDT state, remaining VM86 instructions, debug/test-register behavior, and their transfer/fault boundaries compose. | Partial: T329 S1--S3 close 16/32-bit direct far-JMP images and fault order. Active S4 adds direct far-CALL and GDT task-gate entry with backlink/NT; it does not close nested return, IDT task gates, non-null LDT, task paging, or debug breadth. | [T329 S1 direct-TSS evidence](t329-s1-tss16-direct-jump.md), [S2 evidence](t329-s2-tss32-direct-jump.md), [S4 entry evidence](t329-s4-task-gate-call-entry.md), and [the state-machine record](t329-task-switch-state-machine.md); S5 transfers nested return/IDT task-gate/double-fault chains, S6 non-null LDT, and S7 task paging/debug composition. |
