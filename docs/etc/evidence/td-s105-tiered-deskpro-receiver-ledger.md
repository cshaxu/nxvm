...
| --- | --- | --- | --- | --- |
| CPU-to-board conversion / BWAIT | Core transaction, CPU execution and machine timeline | `generic-at` | A future bridge may add only a transaction-owned wait request, consumed at the existing Core retirement/timeline boundary; reset and cancellation must clear it. Regression: transaction completion, reset, and no VM-visible ABI change. | Retained: no Compaq wait-state scalar or observable bus cycle is available. |
