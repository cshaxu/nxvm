...
This S4 matrix inventories production libraries and the current NXVM artifact
...
| Target | Source list | Provenance | Effective GCC policy before S4 | S4 disposition and admission trigger |
| --- | --- | --- | --- | --- |
...
| `core-platform` | `backing_resource_interface.c`, `input.c`, `presentation_mailbox.c`, `wait.c`, and platform `sleep.c`. | Mixed: moved/migrated platform units include inherited host code. | Strict. | Deferred and prior strict option removed: admit only with an inherited-unit diagnostic baseline or safely separated target. |
...
| `vm-profile` | `pc_at_profile.c`, `keyboard_mapper.c`, `mouse_mapper.c`, `firmware/bios.c`, `firmware/qdcga.c`. | Mixed: retained NXVM firmware/profile material. | Strict. | Deferred and prior strict option removed: admit only with a firmware/profile diagnostic baseline and owned change. |
...
| `vdm-machine` | `src/vdm/machine/dos_minimal.c`. | NXVM: moved non-runnable VDM scaffold. | Strict. | Deferred and prior strict option removed: admit only after a separate VDM source/provenance review. |
| `vdm-composition` | `src/vdm/composition/session.c`, `presentation.c`. | NXVM: moved non-runnable VDM scaffold composition. | Strict. | Deferred and prior strict option removed: admit only after a separate VDM source/provenance review. |
| `vm-0-5-0313` | `src/vm/main.c`, `src/vm/composition/session/machine_info.c`; links the libraries above. | Mixed: retained NXVM entry and mixed product graph. | Strict on artifact sources only. | Deferred and prior strict option removed: a target-local flag cannot claim strict coverage of the linked mixed graph; require the inherited NXVM strict-compilation admission. |
