...
| CQ-9 | Machine removable/fixed media facade and FDC diagnostic dump | A public HDD insertion API is an unconditional failure stub; floppy eject accepts and ignores a path; FDC exposes an uncalled stdout dump | Retain only supported media transitions with truthful parameters; remove the dead dump path | S10 must remove the unsupported/stale declarations, callers and test normalization. |
