# NXVDM Runtime Detail Record

> Detailed M8 interaction, containment, debugger, and acceptance material.
> [design/UI.md](../../design/UI.md) is the current high-level UX authority;
> this record refines it but cannot establish a competing product surface.

This record preserves the exact user-visible contract for the non-invasive
`nxvdm run` product path. It does not claim that the options are implemented
before their scheduled milestone.
