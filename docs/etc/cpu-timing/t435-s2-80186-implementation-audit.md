...
| calls, direct jumps, near returns, loops, `INT`, `IRET`, `INTO`, push/pop register/memory/flags, 80186 `PUSHA`/`POPA`/push immediate/`ENTER`/`LEAVE` | control-stack | source rows exist for selected paths; far return, segment-stack and all form distinctions remain individually open until manifest-key results |
| ports, string primitives and repeat forms including `INS`/`OUTS` | string/I/O selector and 80186 repeat ledger | selected formula tables exist, but first/continue/zero phase, width, termination and source-register contexts lack complete key results |
