...
| Debug copied operations (`debug_read_cpu`, `debug_read_memory`, `debug_step`, `debug_continue`) | VM debug target; CPU diagnostic and NXVM/DOS/video/timer/Windows probes | Core owns execution/RAM; results are copied or bounded run results. | **Keep.** T298 additionally admits copied instruction observation, field-scoped register/segment patch, checked linear/real memory and port I/O, code metadata, watchpoint, and named presentation operations. | Retain debug smoke and debugger/product acceptance corpus. |
| Debug raw borrows: CPU, instructions, execution, RAM, port (`debug_interface.h`; `debug.c`) | Historical production helpers were `vm_debug_{cpu,instructions,execution,memory,port}` and `vm_session_control_initialize`'s CPU/instruction initialization borrow. | Core owns all targets; no VM debugger path may retain their storage. | **T298 active:** VM receives copied instruction observation only at paused/STOPPED boundaries and calls named core operations. Public raw declarations remain for T299's public-surface/test migration, but T298's static gate rejects their debugger-path use. | Preserve debugger adapter behavior, boundary rejection, and session isolation; T299 retains final public-export removal. |
...
| `src/vm/composition/session/control.c`, debugger initialization | Historical CPU/instruction borrows initialized the VM recorder. | Core owns CPU/instructions; VM owns recorder policy only. | T298 initializes recorder without storage and provides copied instruction observation at the synchronized boundary. |
...
| `src/vm/composition/session/debug_target.c`, debugger target operations | Historical `vm_debug_{cpu,instructions,execution,memory,port}` helpers called all five raw borrows. | Core owns targets; VM composition owns only a session-bound adapter. | T298 maps retained target operations to named copied/checking core calls; T299 removes raw debug exports. |
