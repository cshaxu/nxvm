...
[Core specification-driven timing design](../../architecture/specification-driven-l3-timing.md)
through the plan boundary and consumes the earliest CPU-program batch:
`CPU-EXEC`, `CPU-EXCEPT`, and `CPU-FPU` from the
[T433 S6 Core L3 admission ledger](../../evidence/t433-s6-core-l3-admission-feasibility-ledger.md).
The [T433 S7 source-sufficiency ledger](../../evidence/t433-s7-core-source-sufficiency-ledger.md)
...
This candidate follows the binding [Td S121 CPU timing governance record](../../evidence/td-s121-cpu-timing-evidence-first-governance.md). It is a three-phase task, in this order:
...
| S6 / 80186 re-audit | Re-read the 80186 manual and reconcile its S1/S2 register with the successful decoder before implementation | accepted: 23 primary-manual locators, 279 decoder-backed base keys and 603 explicit current dispositions; no runtime key is claimed conforming |
| S7 / 80286 re-audit | Re-read the 80286 Appendix-B corpus and reconcile its S1/S2 register with the successful decoder, including real/PM, privilege/gate/task, EA/odd word, next-byte and repeat contexts | every admitted manual row has a locator; every base key has a decoder recipe; every legal context has a finite S2 disposition; no runtime key is claimed conforming by the audit |
| S8 / 80386DX re-audit | Re-read the 80386DX manual corpus and reconcile its S1/S2 register with the successful decoder, including `0F` extensions, operand/address-size, real/PM/VM86, privilege/path and repeat contexts | every admitted manual row has a locator; every base key has a decoder recipe; every legal context has a finite S2 disposition; no runtime key is claimed conforming by the audit |
| S9 / 80186 | Implement exact/formula keys and declared midpoint L2 keys, including 80186-only forms and legal contexts | every 80186 S2 key is conforming; no endpoint/constrained legacy arithmetic path remains |
| S10 / 80286 | Implement all L3 forms including real/PM, privilege/gate/task, EA/odd word, next-byte and repeat inputs | every 80286 S2 key is conforming; no successful fallback remains |
| S11 / 80386DX | Implement all L3 forms including size, VM86/PM, system forms, early-out multiply and r/m/repeat contexts | every 80386DX S2 key is conforming; formula/path inputs are published and checked |
| S12 / B4 | Cross-profile result consumption, obsolete-selector removal and closure audit | zero nonconforming key across four manifests; focused and cross-profile regressions pass |
