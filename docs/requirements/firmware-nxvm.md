...
| `vpic`, `vpit`, `vdma`, `vkbc`, `qdx` | `core` optional devices | Machine-neutral PC-compatible interrupt, timer, DMA, keyboard-controller and text/CGA device behavior. They expose ports, IRQ/DRQ, and immutable snapshots only. | M5 T1 establishes the device contracts; M1 FDD/HDD boot and presentation regressions remain required. |
