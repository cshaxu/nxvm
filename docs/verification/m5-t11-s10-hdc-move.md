...
`src/machine/vm/vhdc.c`; its header is `src/machine/vm/vhdc.h`. The old header
forwards retained BIOS callers to the new owner. `vmachine.c` includes HDC
...
and debugger-prompt regressions passed. No BIOS disk registration, HDD media,
or user-visible NXVM behavior changed.
