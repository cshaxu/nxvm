...
- The copied source has CRLF and pre-existing space/tab indentation. A
  path-scoped Git text/whitespace exemption preserves byte identity for this
  baseline only; all project-owned paths remain subject to `git diff --check`.
