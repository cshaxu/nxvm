...
`src/platform/vm/platform.c`; its header is `src/platform/vm/platform.h`. The
old header forwards remaining baseline callers to the new owner. Direct machine,
Console, and adapter callers include the new header, while Win32 and Linux leaf
providers remain in their existing locations.
