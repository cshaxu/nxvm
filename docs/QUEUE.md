...
The first candidates are an evidence-led M5 80386 capability package. A
...
1. **Package A: 80386 admission map and checkpoint replay.** Inventory every
...
2. **Package B: 32-bit protected execution baseline.** Starting only from a
...
3. **Package C: 32-bit exception, interrupt, and return delivery.** From a
...
4. **Package D: CPL3 paging and user/kernel boundary.** Starting from a real
...
5. **Package E: trace-driven 80386 system extensions.** Admit 32-bit TSS,
...
6. **Pre-decode transition gateway and mantle probe, conditional.** Admit only
   ...
   enters core.
7. **Ordinary-RAM transaction for staged runtime input.** Add a
...
8. **Transition-local combined commit, conditional.** Combine a checked staged
...
9. **Minimal mantle single-session closure.** Implement one policy-free mantle
