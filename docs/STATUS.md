...
| --- | --- |
| Identifier Mode | Corrective; M5 T306 S4 outer protected `RETF` frame implementation; Coordinated Dual-Session Mode. S2/S3 are accepted. |
...
| Objective | Implement and prove the admitted protected outer `RETF` 16/32-bit frame paths, including immediate stack adjustment, selector/cache and new SS/ESP checks, fault precedence, and all-or-nothing commit boundaries. |
...
| Files And ABI Surface | S4 may change only core CPU outer-return/stack/selector helpers, focused tests, CMake registrations, and task records. It must not alter public interfaces or cross-module ownership. |
...
| Expected Markers | A focused T306 outer-return marker plus retained T293/T303/T305 markers. S4 creates no artifact. |
...
| Similar-Issue Sweep | Sweep outer `RETF` decoding, frame peek/pop helpers, stack-width and immediate-adjust paths, selector/cache validators, and retained outer-return probes. Classify every hit; outer `IRET`, task, V86, and paging paths remain deferred. |
...
| Exit Criteria | S4 passes focused outer-return and retained T293/T303/T305 probes, documentation governance, and diff check. It remains active pending coordinator acceptance; it does not alter Queue, create an artifact, or start outer `IRET` work. |
