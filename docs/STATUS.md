...
| --- | --- |
| Identifier Mode | Corrective; M5 T303 S4 implementation batch C; Coordinated Dual-Session Mode. S1--S3 established the matrix plus relative and near-transfer evidence; S4 implements real-mode and protected same-CPL far `CALL`/`JMP`/`RETF` only. |
...
| Exit Criteria | Real-mode and protected same-CPL far `CALL`/`JMP`/`RETF` follow the frozen pointer-width, selector/cache, descriptor/type/present/privilege/limit, return-frame, and failure-before-visible-commit matrix. Focused probes and required gates pass without touching outer return, gates, tasks, delivery, or later families. |
