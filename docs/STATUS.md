...
| --- | --- |
| Identifier Mode | Corrective; M5 T303 S3 implementation batch B; Coordinated Dual-Session Mode. S1/S2 established the committed control-transfer matrix and relative-transfer evidence; S3 implements near `CALL`/`JMP` and near `RET` only. |
...
| Exit Criteria | Near relative/indirect `CALL` and `JMP`, plus near `RET`, follow the frozen 16/32 form/profile/mode, stack-address, target-validation, and failure-before-visible-commit matrix. Focused probes and required gates pass without touching far selectors, privilege, delivery, or later families. |
