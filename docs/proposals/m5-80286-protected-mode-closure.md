...
its remaining implementation in the descriptor/transfer candidate; accepted
T328 retains the pre-386 `LOCK` policy matrix. It is not itself an admission
contract or a numeric task.
...
The closure is complete only when the assigned ledger has no in-scope partial,
missing, or unclassified row and its pre-386 `LOCK` disposition is reconciled
to T328 rather than duplicated.
