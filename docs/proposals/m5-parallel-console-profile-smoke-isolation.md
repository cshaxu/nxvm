...
affected tests.  Route each test to a unique owned working/profile directory
through existing test setup, prove both smokes retain their expected selection,
and run repeated parallel current-gate replays.  A complete result includes a
control serial run and no shared source-working-directory profile residue.
