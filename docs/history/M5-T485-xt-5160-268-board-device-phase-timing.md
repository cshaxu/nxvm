...
| S15 | Accepted: one retained control/stack evaluator reads published branch outcome for all 8088 Jcc/JCXZ/LOOP forms and assigns the generic source form; no decoder, state owner or physical-time behavior is added. The 299/299 current gate passes. [Evidence](../etc/evidence/t485-s15-8088-branch-source-evaluator.md). |
| S16 | Accepted: rendered Table-2-21 rows establish complete HLT/software-delivery input facts for one existing control/stack receiver; INTR/NMI remain non-instruction exclusions. [Evidence](../etc/evidence/t485-s16-8088-delivery-halting-source-ledger.md). |
| S17 | Accepted: the retained control/stack evaluator source-classifies all finite 8088 HLT/software-delivery rows, including the `CD 03` type-3 distinction; INTR/NMI remain excluded. The 299/299 current gate passes. [Evidence](../etc/evidence/t485-s17-8088-delivery-halting-source-evaluator.md). |
| S18 | Accepted: every 8088 Group-3 range form has an explicit source-unallocated/L2 disposition; no external timing calibration is promoted to Manual-L3. [Evidence](../etc/evidence/t485-s18-8088-group3-range-disposition.md). |
| S19 | Accepted: the complete Table-2-21 universe is partitioned into accepted 8088 receivers, range boundaries, finite next batches and explicit non-instruction exclusions; no residual is hidden by compatibility timing. [Evidence](../etc/evidence/t485-s19-8088-table-2-21-coverage-inventory.md). |
| S20 | Accepted: one retained primary receiver classifies every unprefixed 8088 fixed-scalar row; memory, prefix and FPU batches remain separate. [Evidence](../etc/evidence/t485-s20-8088-fixed-scalar-source-evaluator.md). |
