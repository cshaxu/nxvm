...
T6 artifacts may use the retained adapter while their replacement is incomplete;
T7 must use the first-class composed path. The Console banner follows the
task-level pre-cutover form `0.4.015d.m5t<T>` until version policy changes in an
approved task.

## M5 Exit Review

M5 closes only when the first-class `nxvm.exe` path owns evidence-backed CPU
capability claims, the profile/provider registry, built-in PC/AT firmware, PC/AT
device composition, media policy, Console, and presentation/debug adapters
through the M4 contracts; both M1 fixture paths retain bounded checkpoint
evidence; and the verified `main` commit receives an immutable M5 snapshot
before M6 starts.
