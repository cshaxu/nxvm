...
| T70 | VM machine providers | Context-bind VM-only CMOS/FDD/FDC/HDD/debug state and callbacks; remove `vcmos`, `vfdd`, `vfdc`, `vhdd`, and `vdebug`. Its artifact must boot the FDD fixture to a guest DOS prompt within three seconds. | `nxvm_0_5_0070.exe` |
