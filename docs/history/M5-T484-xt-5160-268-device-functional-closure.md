...
| S5 | Active: resolved profiles now copy one board topology and session passes it directly to Core; S5 continues with the remaining selected B2 low-memory/ROM and XT reset/NMI facts, without an XT runtime path. [Evidence](../etc/evidence/t484-s5-xt-b2-core-topology.md). |
