...
- [ ] **Compaq CECG physical, firmware and board behavior (`TODO(Medium)`).** T386 S6/S9/S10 establish the selected Compaq Enhanced Color Graphics digital personality, 64-colour capture mapping, static configuration reads, source-backed `3C6h` state/reset and Feature-Control-to-Environment state, plus selected no-light-pen latch/status. Before DeskPro board/device timing or final Model-40 acceptance, admit the remaining source-backed effects: `3C2h` miscellaneous-output state, monitor blanking/selection and alternate I/O-base routing, CPU video-memory gating, Input Status 0, special-interface and multiplexed live-video status, option-ROM/BIOS mode programming, physical monitor signalling, raster/service duration, ISA availability and waits through the selected DeskPro composition/firmware and board receiver. Do not infer these from generic/IBM EGA, expose an incomplete profile, or call the captured palette an analog-monitor model.
