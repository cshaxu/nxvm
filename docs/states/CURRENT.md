...
| Task | Compact result |
| --- | --- |
| T470 S4 | Accepted: the [controller/transaction settlement](../etc/evidence/t470-s4-controller-transaction-settlement.md) replaces the ambiguous publication boolean with private origins, admits only the existing qualified deadline path beside CPU retirement, and retains every other origin as a physical blocker; all current profiles remain unavailable. |
