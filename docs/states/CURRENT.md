...
**Active.** T353 remains open between accepted subtasks. S3 will determine whether
an approved corpus exists for a selected system-control/PPI/speaker receiver;
without it, the existing bounded transfers remain authoritative.
...
| Task | Compact result |
| --- | --- |
| T353 S2 | Accepted the single selected PC/AT port-leaf and named-route topology contract: 75 exact directional leaves cover dual DMA, `92h`, sparse KBC/VADP/FDC, and ATA; profile validation precedes core publication; routing proves IRQ0/1/6/8/12/14 and FDC DMA2 against actual bindings. 230 current-gate tests passed. [Evidence](../etc/evidence/t353-s2-profile-topology-contract.md). |
