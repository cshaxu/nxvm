...
| --- | --- |
| Identifier Mode | Continuation; M5 T401 S20. |
| Admission And Approval | Owner approved continuing the four-profile CPU audit. S19 is accepted after complete SCAS execution/preview evidence. |
| Objective | Audit primary TEST accumulator-immediate (`A8h`/`A9h`) across 8086, 80186, 80286 and 80386: byte/word/dword forms, flags, prefix/profile gates, preview and retirement. |
...
| Expected Markers | New T401 S20 markers and any bounded defect marker. |
...
| Asset Needs | None. |
| Exit Criteria | Every selected `A8h`/`A9h` profile/form row has focused proof or an explicit transfer; bounded repairs have full-gate proof. |
...
| Similar-Issue Sweep | For any mismatch, sweep TEST width/flags/immediate forms, arithmetic siblings, profile gates, prefixes, preview callers and flag/register consumers, profile gates, prefixes, preview callers and address/segment/register/flag consumers. |
...
| Task | Compact result |
| --- | --- |
| T401 S19 | Accepted: SCAS audit closes AEh/AFh four-profile execution/preview, flags, REPE/REPNE, DF/ES, attributes and prefix gates. S20 owns accumulator TEST. [Ledger](../etc/evidence/t401-s1-four-profile-cpu-audit-ledger.md). |
