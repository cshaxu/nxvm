...
**M5 T339 S5 - active.** Close the 80286 protected 16-bit return and frame
consumption matrix before TSS16 task-transition work proceeds.
...
| Admission And Approval | The owner approved Queue-ordered single-agent implementation through four-profile cross-closure verification on 2026-08-13. Accepted T339 S4 (`022f89ab`) closes the preceding protected 16-bit entry and gate-transfer matrix. |
| Objective | Reconcile and, only where a reproducer requires it, implement the complete 80286 protected 16-bit return and frame-consumption matrix: `IRET`, `RETF`, and matching same- and outer-CPL stack/cache publication after S4 entry. Prove source-frame preflight, CS/SS/cache selection, permitted IOPL/IF flag restoration, 16-bit SP high-half discipline, restored-IF IRQ ordering, restart, and no-publication rejection under one return-plan ledger. |
| Non-goals | No 80286 `LOCK` legality matrix; no entry/gate creation, task switch/backlink/busy transition, 80386 32-bit frames or return-to-VM86, VM86, paging, debug, VME/PVI, x87, generic fixture framework, generic delivery rewrite, or selector-cache reimplementation. |
| Reference Baseline | `022f89ab` (`M5 T339 S4 P1: close protected entry gate matrix`), the accepted [S1 allocation](../etc/evidence/t339-s1-80286-descriptor-transfer-allocation.md), retained T323 protected-return records, T337 delivery foundations, and the 80286 descriptor-transfer proposal. |
...
| Files And ABI Surface | Expected: local 16-bit return handlers only if a reproducer requires it; owner-bound smokes, CMake registration, T339 evidence/history/Current, and required artifact identity. No public ABI, generic descriptor/table/decoder/exception/fixture interface, device model, or product boundary change. |
...
| Verification | Audit `_e_iret`, `_ser_iret_protected_same`, `_ser_iret_protected_outer`, far-return routes, stack peek/pop helpers, code/stack preparation helpers, all frame readers and cache/register writers, and all return callers. For every admitted return form prove 80286 real/protected and privilege disposition, genuine 16-bit frame and stack-address behavior, CS/SS/type/DPL/RPL/present/limit validation, permitted EFLAGS restoration, source-frame/cache nonpublication on failure, and restored-IF PIC ordering. Run focused owners, exact registrations, static inventories, artifact verifier, documentation governance, diff check, and full current gate. |
| Expected Markers | A complete S5 matrix has one current-gate owner per allocated same- or outer-return form; no 80286 return/frame row is represented only by a 80386 32-bit, VM86, task-return, or entry-path vector. New marker, if necessary, uses `M5:T339:S5:`. |
...
| Stop Conditions | Stop if correctness requires `66/67`, 32-bit return/frame/stack layouts, return-to-VM86, paging/CR0 32-bit state, task mechanics, generic descriptor-table/decoder/delivery redesign, or Intel behavior cannot be classified. Transfer the row rather than fitting a local symptom. |
| Exit Criteria | Every S5 return/frame form has an 80286 real/protected/privilege disposition; table/cache/frame/stack state, fault/restart/nonpublication, and restored-IF PIC boundaries are proved or exactly transferred; any mechanism defect is fixed with its complete caller sweep; required gates pass; the P is committed and pushed. |
...
| Similar-Issue Sweep | Search `IRET`, `RETF`, `_e_iret`, `_ser_iret_protected_{same,outer}`, far-return helpers, stack peek/pop helpers, code/stack preparation helpers, all frame readers and cache/register writers, profile metadata, current owner smokes, CMake registrations, and T323/T329/T337 evidence. |
