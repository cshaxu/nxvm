...
| Task | Compact result |
| --- | --- |
| T366 S20 | Accepted at `25d3a773`: successful unprefixed legal real-mode `MOV Sreg,r/m16` publishes source-backed `2` or `5 + EA + odd-word` through the existing retirement owner. Protected `8E`, invalid encoding, prefix, fault/delivery, bus and physical-time timing remain transferred. [Timing evidence](../etc/evidence/t366-s20-80286-sreg-load-real-timing.md). |
