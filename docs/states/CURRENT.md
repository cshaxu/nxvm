...
| Verification | P1 proves the 15-sector non-DMA port path, reset/IRQ and geometry only. Corrective P2 must prove an actual DMA2 transfer and a Model-40 ROM/BIOS-visible selected-storage consumer, then repeat strict-owner/static gates, serial current gate, documentation gate, diff check and actual-change review. |
