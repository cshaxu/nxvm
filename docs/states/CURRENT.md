...
| Objective | Close T528 only if its complete proposal exit criteria remain proven, with current task identity `0.5.0528` stripped Release artifacts for both Windows architectures; repair only the demonstrated x86 product-console copied-payload stack overflow needed for usable artifact parity. |
| Non-goals | Do not change the established Common-protocol/NXVM-bounded-runner boundary; do not create a generic executor worker, alter Core behavior, profile topology, media, firmware, YAML grammar, public ABI, host synchronization or Common payload contracts. |
...
| Files And ABI Surface | Current-artifact CMake declaration; the heap-owned NXVM product console context and its private transient copied-payload fields; task/history/status/evidence records; and generated ignored artifacts. No public API or behavior change. |
...
| Verification | Focused Console unit/regression and x86 debugger launch proof; reconfigure/build current Release x64/x86 artifacts; full repository-only unit suite; full owner-managed external integration suite; specialized gates; documentation governance; PE architecture/version/SHA-256 records; actual-diff and final-exit audit. |
...
| Stop Conditions | Stop and revise if the x86 console repair needs a Common API change, full integration exposes an unrelated runtime defect, either required architecture cannot build, artifacts do not meet identity/strip/architecture criteria, or final review finds a duplicate owner. |
