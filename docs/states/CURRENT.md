...
| --- | --- |
| Identifier Mode | Continuation; M5 T401 S22. |
| Admission And Approval | Owner approved continuing the four-profile CPU audit. S21 is accepted after complete immediate-count group-2 execution/preview evidence. |
| Objective | Audit near return `C2h`/`C3h` across 8086, 80186, 80286 and 80386: stack/IP behavior, immediate cleanup, prefix/profile gates, preview and retirement. |
...
| Expected Markers | New T401 S22 markers and any bounded defect marker. |
...
| Asset Needs | None. |
| Exit Criteria | Every selected `C2h`/`C3h` profile/form row has focused proof or an explicit transfer; bounded repairs have full-gate proof. |
...
| Similar-Issue Sweep | For any mismatch, sweep near-return stack/IP/cleanup forms, control-transfer siblings, profile gates, prefixes, preview callers and stack consumers, profile gates, prefixes, preview callers and address/segment/register/flag consumers. |
...
| Task | Compact result |
| --- | --- |
| T401 S21 | Accepted: C0/C1 audit closes immediate-count group-2 execution/preview, profile availability, extensions, widths and prefix gates. S22 owns near RET. [Ledger](../etc/evidence/t401-s1-four-profile-cpu-audit-ledger.md). |
