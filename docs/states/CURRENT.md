...
| --- | --- |
| Identifier Mode | Continuation; M5 T401 S14. |
| Admission And Approval | Owner approved continuing the four-profile CPU audit. S13 is accepted after complete immediate-register-MOV execution/preview evidence. |
| Objective | Audit primary moffs MOV (`A0h`--`A3h`) across 8086, 80186, 80286 and 80386: byte/word/dword forms, accumulator publication, address-size and segment selection, prefix/profile gates, FLAGS preservation, preview and retirement. |
| Non-goals | No broad MOV, address-generation or memory-transaction rewrite, no device/board work, no physical timing or DeskPro L3 claim. |
...
| Verification | Four-profile all-opcode execution/preview matrix, byte/word/dword memory size, 80386 address/operand prefixes, pre-80386 #UD atomicity, segment and FLAGS/retirement; focused tests, full gate after repair, governance/diff checks. |
| Expected Markers | New T401 S14 markers and any bounded defect marker. |
| Reporting Requirements | Record CPU/profile, opcode, memory direction/width, address and segment attribute state, manual/source tier, current owner, focused proof or transfer and retained nonphysical timing boundary. |
...
| Asset Needs | None. |
| Exit Criteria | Every `A0h`--`A3h` profile/form row has focused proof or an explicit transfer; bounded repairs have full-gate proof. |
...
| Similar-Issue Sweep | For any mismatch, sweep all four A0h--A3h forms, ModR/M and immediate MOV siblings, profile gates, prefixes, preview callers and address/segment/register/flag consumers. |
...
| Task | Compact result |
| --- | --- |
| T401 S13 | Accepted: immediate-register MOV audit closes B0h--BFh four-profile execution/preview, 66h and pre-386 prefix gates. S14 owns moffs MOV. [Ledger](../etc/evidence/t401-s1-four-profile-cpu-audit-ledger.md). |
