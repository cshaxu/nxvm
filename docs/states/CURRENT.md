...
| Objective | Add one Core-owned, configurable DMA-cycle ready gate. A false BUSRDY input defers the pending DMA grant without a transfer; when ready resumes, the existing one transaction route performs it. Model-40 selects the capability with its reset-default-ready binding. |
...
| Reference Baseline | `master` at `9afae9d7`; T419 S2 evidence; Compaq D3PE processor description pp. 19-21 in the approved Compaq DeskPro 386/16 research corpus, which states BUSRDY can extend a DMA cycle. PCjs DeskPro configuration is inspected only as `reference-derived` topology context and is not a timing source. |
...
| Verification | Focused Core smoke proves a false BUSRDY input blocks both existing grant wait completion and DMA memory write, a true input releases exactly one existing DMA transfer, reset restores ready and no stale wait, and default profiles retain current behavior. Model-40 smoke proves default-ready gate selection. Run `cmake --build build/mingw-gcc-x64 --target run-current-smokes --parallel 4` and documentation governance gate. |
| Expected Markers | `M5:T419:S3:D4-DMA-BUSRDY:OK`; existing S1/S2 and competition markers remain. |
| Asset Needs | Read-only approved primary research corpus; no ROM, guest media, external source text, or machine-local path committed. |
