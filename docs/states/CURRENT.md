...
| Objective | Close the fixed-product conversion: prove all four canonical optimized builds, resolve the Model 40 Debug-timeout versus Release-product qualification boundary, remove any remaining duplicate deployment/configuration path, and produce the required dual-architecture artifacts. |
...
| Files And ABI Surface | Fixed CMake product/build/deployment boundary, Profile-to-Machine composition, App INI entry and integration fixtures. No Core repair is assumed unless an optimized-product replay shows a real shared device failure. |
...
| Verification | Execute the ledger: configure each product against its manifest; run all repository-only units; run every product's registered integrations using optimized product builds; verify no YAML/catalog/deployment duplication; create and inspect all required stripped x64/x86 artifacts with matching INI companions. |
