...
| Task | Compact result |
| --- | --- |
| T366 S19 | Accepted at `ad1ed794`: successful unprefixed legal memory `MOV Sreg,m16` publishes its source-backed `3 + EA + odd-word` 80286 cost through the existing retirement owner. `8E`, invalid encoding, prefix, fault/delivery, bus and physical-time timing remain transferred. [Timing evidence](../etc/evidence/t366-s19-80286-sreg-store-memory-timing.md). |
