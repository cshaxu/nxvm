...
| --- | --- |
| Identifier Mode | Continuation; M5 T401 S36. |
| Admission And Approval | Owner approved continuing the four-profile CPU audit. S35 is accepted after complete XLAT execution/preview evidence. |
| Objective | Audit LAHF/SAHF `9Fh`/`9Eh` across 8086, 80186, 80286 and 80386: defined FLAGS transfer, reserved-bit handling, attributes and preview behavior. |
| Non-goals | No broad flags-engine or privilege-model rewrite, no device/board work, no physical timing or DeskPro L3 claim. |
| Reference Baseline | Intel 8086/80386 instruction references; retained flags/prefix/timing ledgers; current decoder/execution/preview. Secondary emulators only corroborate Intel-underspecified behavior and must be labelled reference-derived. |
...
| Files And ABI Surface | Audit ledger, packet, LAHF/SAHF fixture and preview smoke only unless a bounded shared defect is reproduced; no public ABI. |
...
| Expected Markers | New T401 S36 markers and any bounded defect marker. |
| Reporting Requirements | Record CPU/profile, opcode, FLAGS/AH state, attribute and retirement behavior, manual/source tier, current owner, focused proof or transfer and retained nonphysical timing boundary. |
...
| Asset Needs | None. |
| Exit Criteria | Every selected LAHF/SAHF profile/form row has focused proof or an explicit transfer; bounded repairs have full-gate proof. |
...
| Similar-Issue Sweep | For any mismatch, sweep direct-FLAGS siblings, profile gates, prefixes, preview callers and FLAGS/AH consumers. |
...
| Task | Compact result |
| --- | --- |
| T401 S35 | Accepted: XLAT audit closes four-profile table/segment/address/limit behavior and lexical coverage. S36 owns LAHF/SAHF. [Ledger](../etc/evidence/t401-s1-four-profile-cpu-audit-ledger.md). |
