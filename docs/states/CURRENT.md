...
| --- | --- |
| Identifier Mode | Continuation; M5 T401 S15. |
| Admission And Approval | Owner approved continuing the four-profile CPU audit. S14 is accepted after complete moffs-MOV execution/preview evidence. |
| Objective | Audit primary MOVS (`A4h`/`A5h`) across 8086, 80186, 80286 and 80386: byte/word/dword forms, DF and segment routing, repeat, prefix/profile gates, flags, preview and retirement. |
| Non-goals | No broad string-engine, address-generation or memory-transaction rewrite, no device/board work, no physical timing or DeskPro L3 claim. |
...
| Verification | Four-profile execution/preview matrix, byte/word/dword payload, DF/REP progression, 80386 address/operand prefixes, pre-80386 #UD atomicity, segment and FLAGS/retirement; focused tests, full gate after repair, governance/diff checks. |
| Expected Markers | New T401 S15 markers and any bounded defect marker. |
| Reporting Requirements | Record CPU/profile, opcode, payload/address state, repeat/DF/segment attributes, manual/source tier, current owner, focused proof or transfer and retained nonphysical timing boundary. |
...
| Asset Needs | None. |
| Exit Criteria | Every selected `A4h`/`A5h` profile/form row has focused proof or an explicit transfer; bounded repairs have full-gate proof. |
...
| Similar-Issue Sweep | For any mismatch, sweep MOVS width/repeat/DF forms, string siblings, profile gates, prefixes, preview callers and address/segment/register/flag consumers. |
...
| Task | Compact result |
| --- | --- |
| T401 S14 | Accepted: moffs MOV audit closes A0h--A3h four-profile execution/preview, attribute and prefix gates. S15 owns MOVS. [Ledger](../etc/evidence/t401-s1-four-profile-cpu-audit-ledger.md). |
