...
| --- | --- |
| Identifier Mode | Continuation; M5 T401 S21. |
| Admission And Approval | Owner approved continuing the four-profile CPU audit. S20 is accepted after complete accumulator TEST execution/preview evidence. |
| Objective | Audit immediate-count shift/rotate groups `C0h`/`C1h` across 8086, 80186, 80286 and 80386: byte/word/dword forms, flags, count rules, prefix/profile gates, preview and retirement. |
...
| Expected Markers | New T401 S21 markers and any bounded defect marker. |
...
| Asset Needs | None. |
| Exit Criteria | Every selected `C0h`/`C1h` profile/form row has focused proof or an explicit transfer; bounded repairs have full-gate proof. |
...
| Similar-Issue Sweep | For any mismatch, sweep immediate-count shift/rotate width/count/flag forms, group-2 siblings, profile gates, prefixes, preview callers and register/memory consumers, profile gates, prefixes, preview callers and address/segment/register/flag consumers. |
...
| Task | Compact result |
| --- | --- |
| T401 S20 | Accepted: accumulator TEST audit closes A8h/A9h four-profile execution/preview, flags, 80386 dword and prefix-gate coverage. S21 owns C0/C1 shifts. [Ledger](../etc/evidence/t401-s1-four-profile-cpu-audit-ledger.md). |
