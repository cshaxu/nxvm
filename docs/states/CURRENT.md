...
**M5 T339 S4 - active.** Close the 80286 protected 16-bit entry and gate
transfer matrix before return or task work proceeds.
...
| Admission And Approval | The owner approved Queue-ordered single-agent implementation through four-profile cross-closure verification on 2026-08-13. Accepted T339 S3 (`7d4235f4`) closes the preceding selector-query and cache-materialization matrix. |
| Objective | Reconcile and, only where a reproducer requires it, implement the complete 80286 protected 16-bit entry and gate-transfer matrix: direct far `CALL/JMP`; software/external/NMI entry; 16-bit interrupt, trap, and call gates; and the TSS16 outer-stack selection that entry alone requires. Prove gate/table/code/stack validation, same/outer-CPL frame construction, IF/TF/error-code rules, origin ordering, prepublication failure, and PIC/NMI boundaries under one entry-plan ledger. |
| Non-goals | No 80286 `LOCK` legality matrix; no return/IRET/RETF frame consumption, task switch/backlink/busy transition, 80386 32-bit gates/frames, VM86, paging, debug, VME/PVI, x87, generic fixture framework, generic delivery rewrite, or selector-cache reimplementation. |
| Reference Baseline | `7d4235f4` (`M5 T339 S3 P1: close selector cache matrix`), the accepted [S1 allocation](../etc/evidence/t339-s1-80286-descriptor-transfer-allocation.md), retained T323 protected-entry records, T337 delivery foundations, and the 80286 descriptor-transfer proposal. |
...
| Files And ABI Surface | Expected: local 16-bit entry/gate handlers only if a reproducer requires it; owner-bound smokes, CMake registration, T339 evidence/history/Current, and required artifact identity. No public ABI, generic descriptor/table/decoder/exception/fixture interface, device model, or product boundary change. |
...
| Verification | Audit `_ser_int_protected_16`, `ExecInt`, direct far transfer and call-gate routes, gate/table readers, `_ksa_prepare_code_sreg`, `_ksa_prepare_stack_sreg`, TSS16 stack reads, all frame writes, and all callers/origins. For every admitted entry form prove 80286 real/protected disposition, gate/code/stack DPL/type/present/limit validation, same/outer-CPL 16-bit frame image, IF/TF/error-code publication, PIC/NMI ordering, and no partial frame/cache publication on failure. Run focused owners, exact registrations, static inventories, artifact verifier, documentation governance, diff check, and full current gate. |
| Expected Markers | A complete S4 matrix has one current-gate owner per allocated entry/origin form; no 80286 gate/frame row is represented only by a 80386 32-bit, VM86, or return-path vector. New marker, if necessary, uses `M5:T339:S4:`. |
...
| Stop Conditions | Stop if correctness requires `66/67`, 32-bit gate/frame/stack layouts, VM86, paging/CR0 32-bit state, return/task mechanics, generic descriptor-table/decoder/delivery redesign, or Intel behavior cannot be classified. Transfer the row rather than fitting a local symptom. |
| Exit Criteria | Every S4 entry/origin and gate extension has an 80286 real/protected/privilege disposition; table/cache/frame/stack state, fault/restart/nonpublication, and PIC/NMI boundaries are proved or exactly transferred; any mechanism defect is fixed with its complete caller sweep; required gates pass; the P is committed and pushed. |
...
| Similar-Issue Sweep | Search direct far transfer, `ExecInt`, `_ser_int_protected_16`, gate/table readers, code/stack preparation helpers, all TSS16 stack and frame writers, interrupt/call origins, profile metadata, current owner smokes, CMake registrations, and T323/T329/T337 evidence. |
