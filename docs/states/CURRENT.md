...
| Admission And Approval | T481 S3 implementation is `84d2deb2`; this one-session coordinator transition accepts its bounded controller audit before S4 begins. The owner-approved ordered M5 queue remains the task authority. |
| Objective | Reconcile Core guest-time ownership, copied plan/observation publication, immutable profile inputs and Standard/Turbo VM consumer policy. |
| Non-goals | No runtime code, public ABI, source import, controller reimplementation, profile claim, or L4 reconstruction. |
| Reference Baseline | T474 Core-time closure, T471/T472 pacing policy, T475 profile resolver and T481 S1--S3 audit records. |
...
| Files And ABI Surface | Core/VM/profile source inspection plus T481 history/evidence/index and Current only; no ABI change. |
| Applicable Rules | `docs/rules/EXECUTION.md`, `docs/rules/DOCUMENT.md`, source policy, and the proposal's L0--L3 meanings. |
| Verification | Core time writer/reader and plan/observation caller sweep, Standard/Turbo policy review and documentation governance. |
| Expected Markers | `M5:T481:S4:TIME-OWNER:OK`; `M5:T481:S4:PACING-BOUNDARY:OK`; `M5:T481:S4:CORE-VM-AUDIT:OK`. |
| Asset Needs | No acquisition or import; accepted sources and repositories remain evidence only. |
| Reporting Requirements | Record time writer/reader, plan/observation owner, profile input boundary, Standard/Turbo policy, test proof and concrete transfer. |
...
| Stop Conditions | Stop and record an unavailable boundary if a physical timebase or profile input lacks evidence. |
| Exit Criteria | Core is the only guest-time writer; profiles provide immutable copied inputs; VM only observes and paces completed Core progress. |
| Similar-Issue Sweep | Core scheduler/time APIs, VM waiting, profile resolver, controller deadline inputs and speed-policy tests. |
