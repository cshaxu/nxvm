...
| --- | --- |
| Identifier Mode | `Continuation`: M5 T435 S10, the next unused subtask of the latest open numeric task. |
| Admission And Approval | Owner approved in this thread on 2026-08-22: continue with the 80286 closure task after the accepted 80186 S9 result closure, using the same single-person dual-role lifecycle. No exception is approved. |
| Objective | Make every canonical 80286 successful-retirement key in the S2 manifest conforming at its Appendix-B L3 result, while retaining T401-approved instruction semantics and one Core timing/publication path. |
| Non-goals | Do not reopen 8086, 80186 or 80386 implementation; add a CPU profile; assign board/READY/HOLD/DMA/prefetch/device timing; create an ABI; import third-party source; or change task/rule authorities. |
| Reference Baseline | Accepted S7/S1/S2 80286 evidence: [manual ledger](../etc/cpu-timing/t435-s1-80286-ledger.md), [manifest](../etc/cpu-timing/t435-s2-80286-timing-manifest.json), [tracker](../etc/cpu-timing/t435-s2-80286-implementation-tracker.md), [audit](../etc/cpu-timing/t435-s2-80286-implementation-audit.md), [decoder inventory](../etc/cpu-timing/t435-s7-80286-decoder-inventory.json) and both 80286 verifiers. S9 is accepted predecessor progress. |
| Candidate Proposal | [M5 Core CPU instruction timing programs](../proposals/m5-core-cpu-instruction-timing-program.md), S10 row; [specification-driven timing design](../etc/architecture/specification-driven-l3-timing.md); T433 S6/S7 source-sufficiency ledgers. |
| Files And ABI Surface | Core-private `src/core/machine/cpu_timing.c/.h`, retained execution coordination in `src/core/machine/machine.c`, decoder-owned inputs in `cpu_instructions.c`, 80286 manifest/verifiers/results and focused tests/CMake registration. No public header or external ABI change is allowed. |
...
| Verification | Before each tracker-changing P run `Verify-80286TimingManifest.ps1` and `Verify-80286DecoderLedger.ps1`. Every implementation P builds and runs its focused 80286 timing/function regression. S closure additionally runs the full generated-key result verifier, decoder/manifest verifiers, relevant cross-profile regression, documentation governance, actual diff/code review, and records the required T435 developer artifact if runnable code changed. |
| Expected Markers | Retain S1/S2 markers; add `M5:T435:S10:I286-B0-SOLE-PUBLISHER:PASS`, batch-specific result markers, and final `M5:T435:S10:I286-ALL-KEYS-CONFORMING:PASS` with base/context counts and zero `wrong-value`, `unallocated`, `missing-input`, and `missing-test`. |
...
| Stop Conditions | Stop for manual/manifest conflict, missing protected-mode or next-byte input that cannot be represented without changing the approved boundary, required public ABI/board timing, source/license issue, verifier contradiction, or a detected defect outside 80286 scope. Report evidence for coordinator decision; do not guess or silently downgrade an L3 row. |
| Exit Criteria | All 286 base plus 521 legal context/combined keys have source, L3 target, one Core-private selection path, focused function/timing result with origin and `source_timing_unallocated=false`, and `conforming` status. No successful 80286 form uses terminal/unallocated, a parallel selector, or a non-Appendix-B fallback; required behavior/fault regressions pass, obsolete code is removed/dispositioned, and one dual-role closure audit accepts the actual changes. |
| Original Owner Request | Owner request, faithfully translated: continue T435 by implementing and closing the complete 80286 instruction function and timing universe against the authoritative-manual rules and the S1/S2 80286 audit tables. |
| Similar-Issue Sweep | Defect class: any successful 80286 decoded form bypassing the sole timing publisher, retaining legacy selection, or returning unallocated/wrong timing. Scope: all tracked Core CPU timing/execution source, 80286 tests, CMake targets, manifest/results/verifiers and S10 evidence. Per P, search `rg -n "80286|source_timing_unallocated|source_timing|cpu_timing" src/core tests CMakeLists.txt tools docs/etc/cpu-timing`; every production hit is fixed, justified not applicable, or recorded as a bounded in-S10 item. Final generated-key verifier is the static closure gate. |
