...
| --- | --- |
| Identifier Mode | Continuation; M5 T401 S39. |
| Admission And Approval | Owner approved continuing the four-profile CPU audit. S38 is accepted after complete HLT execution/preview evidence. |
| Objective | Audit PUSHF/POPF `9Ch`/`9Dh` across 8086, 80186, 80286 and 80386: flag-image, privilege masks, stack widths and preview behavior. |
| Non-goals | No broad stack, flags or privilege-model rewrite, no device/board work, no physical timing or DeskPro L3 claim. |
| Reference Baseline | Intel 8086/80386 instruction references; retained flags/stack/prefix/timing ledgers; current decoder/execution/preview. Secondary emulators only corroborate Intel-underspecified behavior and must be labelled reference-derived. |
...
| Files And ABI Surface | Audit ledger, packet, PUSHF/POPF fixture and preview smoke only unless a bounded shared defect is reproduced; no public ABI. |
...
| Expected Markers | New T401 S39 markers and any bounded defect marker. |
| Reporting Requirements | Record CPU/profile, opcode, FLAGS/stack/privilege state, attribute and retirement behavior, manual/source tier, current owner, focused proof or transfer and retained nonphysical timing boundary. |
...
| Asset Needs | None. |
| Exit Criteria | Every selected PUSHF/POPF profile/form row has focused proof or an explicit transfer; bounded repairs have full-gate proof. |
...
| Similar-Issue Sweep | For any mismatch, sweep FLAGS-stack siblings, profile gates, prefixes, preview callers and stack/privilege consumers. |
...
| Task | Compact result |
| --- | --- |
| T401 S38 | Accepted: HLT audit closes four-profile halted/privilege/IRQ behavior and lexical coverage. S39 owns PUSHF/POPF. [Ledger](../etc/evidence/t401-s1-four-profile-cpu-audit-ledger.md). |
