...
- A device admission follows the
  [hardware-device verification template](etc/requirements/hardware-device-verification.md):
...
- [ ] **M5 core second-consumer closure (`TODO(High)`).** T296
  ...
  port ownership, failure propagation, and session-init atomicity. The first
  three queue candidates remain conditional on recorded first-party consumer
  evidence and their stated dependencies. This proves a core-only
  second-consumer boundary but does not implement `mantle`,
  DOS, VDM, an external ABI, or host policy.
- [ ] **M6 mantle session envelope (`TODO(High)`).** After M5 proves the
