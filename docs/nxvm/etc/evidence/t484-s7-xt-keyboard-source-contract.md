...
| --- | --- | --- |
| Selected keyboard and byte form | `5160-268` selects the enhanced keyboard (T483 S1).  The 101/102-key keyboard sends a serial 9-bit frame: start bit `1`, then data bits 0 through 7 least-to-most significant (4-33), and retains a 16-byte keyboard-side buffer (4-25). | The XT PPI attachment alone deserializes each completed frame into one byte and owns the matching bounded pending state.  VM supplies host input only at the existing typed host-input boundary; it does not own a guest FIFO, frame, or port state. |
| Wire ownership and inhibition | Clock and data are open-collector; either side can force inactive/low.  Host-forced clock-low inhibits transmission (4-32).  The rendered system-board PPI map assigns port `61h`/PPI-B bit 6 to hold keyboard clock low and bit 7 to enable keyboard or clear keyboard (1-28). | The PPI-B latch and mode state own these two selected line controls.  No `64h` command/status port, 8042 command byte, A20/reset output port, AUX origin, or 8042 FIFO is permitted. |
...
| Guest byte delivery and IRQ | The system board exposes keyboard scan code at `60h`; the selected XT interrupt relation is keyboard IRQ1 (T483 S1; IBM system-board/interrupt material).  The IBM XT BIOS reads port A and pulses PPI-B bit 7 around its IRQ1 service. | The attachment owns byte-ready and asserts/deasserts the existing Core PIC's IRQ1 source.  Reading `60h` observes the held byte; the documented PPI-B clear pulse releases it.  Profile construction provides immutable ports/IRQ selection and never holds a second guest-visible byte. |
