...
| VADP-F2 | The generic BIOS emits one real `3C2` selection and D/E/10 CRTC tuples through its sole generated INT 10h program. That program occupies its fixed ROM slot, separate from the FDC service, rather than overflowing the sequential BIOS region. One VADP tuple classifier derives D (`320x200x16`), E (`640x200x16`) and 10h (`640x350x16`) only from Horizontal Display End, Overflow, Vertical Display End and Offset port state; unknown tuples have no snapshot. | EGA external/sequencer/controller/CRTC/planar/mode-10/Compaq direct smokes; `vm-ega-planar-system-smoke`, `vm-rom-ega-int10-system-smoke`, D/E/10 `vm-ega-mode10-boot-smoke`, and `vm-default-pc-at-rom-materialization-smoke` pass. | Manual L3 for D/E/10 layout facts and their selected port-driven route. No BDA classifier, renderer, VRAM route or compatibility state exists. F chained odd/even and alpha character generation remain explicit later batches. |
