...
| KBC-R2 | `core_machine_kbc_read_status` supplies OBF/IBF/system/command-data/inhibit/AUX bits at 0064h and `register_ports` owns 0060h/0064h. The input-port owner supplies the selected inhibit switch. Parity and serial timeout flags still require a frame/error owner. | Controller smoke now covers data/status/command selection and inhibit; serial/host-ingress smokes retain byte delivery. | Partial Manual L3: selected ports, principal flags and inhibit conform; sourced serial error latches remain fallback to L2. |
| KBC-R3 | `core_machine_kbc_write_command` implements 20h, 60h, AAh, ABh, ADh/AEh, C0h, D0h/D1h and E0h; input/test state has one KBC owner and a bounded board-signal setter. It adds PS/2 AUX commands A7h--A9h/D4h. AC is not simulated because its 60 scan-code bytes require the selected 8042 RAM/PSW/ROM state; F0h--FFh retains only reset selection. | Controller smoke covers each retained selected command and board input/test values. | Manual L3 for implemented commands. AC is fallback to L2; six-us width is L4 and consumer delivery is board L3 or fallback to L2. |
| KBC-R4 | `command_byte`, `output_port`, `input_port`, `test_inputs`, `pending_write`, `keyboard_enabled` and callbacks are one controller owner. Reserved AT command-byte bits are cleared for the selected non-AUX controller; PC mode suppresses translation and inhibit override gates keyboard admission. | Controller smoke covers command-byte, interface enable, output port, input port and test inputs. | Manual L3 for selected AT fields; AUX remains Other L3 only under its explicit profile. |
...
| KBC-R3 | Manual L3 selected commands; AC is fallback to L2, pulse width L4 and consumer phase board L3 or fallback to L2. | C0/E0 closed; retain only AC/pulse boundary. |
| KBC-R4 | Manual L3 command/input/test-port fields. | Closed. |
