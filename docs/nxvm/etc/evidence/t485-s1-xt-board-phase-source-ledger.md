...
| PIT/refresh | IBM supplies 1.193182 MHz PIT input, OUT0 to IRQ0 and OUT1 to refresh/DMA. Intel 8254 defines counter/output phase. **Manual L3 rational clock and phase.** | Core timing plan can syntactically accept `pit_clock = SOURCE_RATIONAL_CLOCK`, but XT `elapsed_ticks` is instruction/deterministic time rather than the 4.77 MHz board-clock axis. | Blocked until CPU/board physical axis exists; `1/4` on current ticks would be false L3. |
...
The current Core plan exposes typed rational-clock and DMA-phase fields, but
they require an already qualified common axis. XT has none: one tick per
instruction is deterministic scheduling, not a 4.77 MHz board clock. PIC
visibility remains L2; CPU retirement and external bus timing must establish
the common axis before any controller ratio becomes L3.
...
compatibility workarounds create no XT timing fact. No controller clock is
currently implementable without first qualifying the CPU/board common axis;
CPU/bus, PIT, DMA service, PIC, PPI, FDC, CGA and Xebec remain bounded.
