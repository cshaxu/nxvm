...
| --- | --- | --- | --- |
| Prefixes `26/2E/36/3E`, `64/65`, `66/67`, `F0`, `F2/F3`; repeated-prefix resolution | Prefix loop and `_GetOperandSize`/address decode in `cpu_instructions.c`; FS/GS and `66/67` explicitly gate at 80386. | `core_machine_prefix_attributes_s64_smoke` (`M5:T316:S64:PREFIX-ATTRIBUTES:OK`) plus retained operand/address and string smokes; T328 legacy LOCK evidence. | **Complete only for T316 S64's semantic-class closure and T328's retained historical LOCK boundary**: representative memory read/write, fixed-segment and REP strings, register/attribute decode, legal and illegal 80386 LOCK, repeated segment precedence, rejection, and IRQ ordering prove the shared ordinary prefix rules; T328 adds the 8086/80186/80286 legality matrix. This is not an every-opcode Cartesian-product claim. |
...
| Unary, decimal, and ordinary FLAGS breadth | No additional Intel 80386 primary arithmetic, conversion, or local FLAGS opcode form after the accepted S2--S20 and S39--S49 consolidation. | **T322 S1 reconciliation:** ordinary forms complete; system-state boundaries retain their own destinations. T328 separately closes the historical legacy LOCK boundary. |
...
| Memory and port strings | No additional Intel 80386 MOVS/CMPS/STOS/LODS/SCAS/INS/OUTS opcode form after the accepted S33--S38 consolidation; generic I/O permissions and device semantics are distinct. | **T322 S1 reconciliation:** ordinary string forms complete; generic I/O privilege architecture remains the **80386DX protection and privilege-transfer closure** Queue package. |
...
No production or shared-helper change was required. Other `0F 00` forms,
...
/6`. It proves fixed r/m16 register and memory transfer width on 80286 and
80386, including 80386 `66h`, `67h`, and combined prefix behavior. SMSW is
unprivileged in protected mode and retains CR0; LMSW clears/sets only the
low MSW bits, retains non-MSW CR0 state, and cannot clear PE once PE is set.
LMSW is rejected at protected CPL3 and ordinary VM86; protected source and
destination access-limit paths retain instruction state at the established
no-IDT terminal boundary. 80186 and every pre-386 attribute case reject;
all 80386 LOCK combinations reject before publication. Pending IRQ0 is
delivered immediately after either successful form with no added shadow.
No production or shared-helper change was required. SGDT/SIDT/LGDT/LIDT,
CLTS, MOV control registers, paging policy, task switching, VME/PVI and
