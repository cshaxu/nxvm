# M5 80386DX Candidate Admission Policy

> Retained historical proposal; superseded by the fixed Standard/PC110 product
> direction. Not an active candidate or authority. See [disposition](README.md).

Before M6 admission, complete the remaining M5 candidates. The first CPU
candidate establishes the authoritative four-profile ledger and refines the
later CPU candidates from actual evidence; the Queue order expresses required
dependency, not a pre-allocation of numeric tasks or implementation subtasks.

Every 80386DX candidate is matrix-driven rather than Windows-demand-driven.
Before admitting a task, audit its relevant Intel 80386 PRM instruction and
behavior forms and record each form as complete, partial, missing, outside
80386, or an explicit external-coprocessor boundary. A task may close only its
declared bounded matrix slice; it cannot claim its family complete while an
in-scope form is partial, missing, or unclassified. A family candidate advances
only when its whole assigned matrix is resolved by implementation and focused
evidence, or by an approved boundary classification.

The [80386DX closure map](../../evidence/80386-closure-map.md) is the supporting
crosswalk between horizontal form matrices and vertical architecture-state
closures. Every admitted CPU candidate identifies the map rows it advances and
any residual row it transfers. A completed smoke count or opcode slice is never
a substitute for a closed state transition.
