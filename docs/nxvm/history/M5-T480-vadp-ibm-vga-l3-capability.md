...
| S4 | Accepted: one VADP owner now contains source-backed DAC, chain-4 and Mode-13 copied-frame behavior; profile/card binding and physical timing remain explicit S5 boundaries. |
| S5 | Accepted: no current profile/card may select VGA; the Core capability remains unselected, one-owner VADP behavior is preserved, contract-aligned regressions and the 296-target gate pass, and stripped Release 0480 is recorded in the [closure audit](../etc/evidence/t480-s5-profile-closure-audit.md). |
